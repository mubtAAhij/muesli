    ...
    * When transcribing numbers, write the digits, i.e. write 1.7 and not one point seven, and write 3 instead of three.
    ...
    static func ensureModelDownloaded(
        ...
        if localOverrideURL(environment: environment) != nil {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 1, userInfo: [
                NSLocalizedDescriptionKey: String(format: String(localized: "gemma4_litert.error.model_missing_at_path", defaultValue: "Gemma 4 LiteRT-LM model is missing at %@.", bundle: .module, comment: "Error when Gemma model file is missing at expected path."), "\(modelURL.path)"),
            ])
        }
        ...
        guard isAvailableLocally(environment: environment, fileManager: fileManager) else {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "gemma4_litert.error.download_failed", defaultValue: "Gemma 4 LiteRT-LM did not download successfully.", bundle: .module, comment: "Error when Gemma model download fails."),
            ])
        }
    ...
    private static func downloadManagedModel(
        ...
        try await ModelDownloadCoordinator.shared.download(manifest, to: directory) { snapshot in
            ...
            progress?(fraction, String(localized: "gemma4_litert.download.downloading_model", defaultValue: "Downloading Gemma 4 E2B", bundle: .module, comment: "Status while downloading Gemma model.") + speed)
    ...
    static func validateDownloadedLiteRTLMFile(at url: URL, fileManager: FileManager) throws {
        ...
        guard attributes[.type] as? FileAttributeType == .typeRegular else {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 4, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "gemma4_litert.error.downloaded_model_not_regular_file", defaultValue: "Downloaded Gemma 4 LiteRT-LM model is not a regular file.", bundle: .module, comment: "Error when downloaded Gemma model path is not a regular file."),
            ])
        }
        let size = (attributes[.size] as? NSNumber)?.int64Value ?? 0
        guard size >= minimumDownloadedModelSizeBytes else {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 4, userInfo: [
                NSLocalizedDescriptionKey: String(format: String(localized: "gemma4_litert.error.model_file_too_small", defaultValue: "Downloaded Gemma 4 LiteRT-LM model is too small (%d bytes).", bundle: .module, comment: "Error when downloaded Gemma model file size is below minimum."), size),
            ])
        }
    }
}
...
actor Gemma4LiteRTTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            ...
            case .modelMissing(let path):
                return String(format: String(localized: "gemma4_litert.error.model_missing_with_download_hint", defaultValue: "Gemma 4 LiteRT-LM model is missing at %@. Download it from the Models tab or set %@.", bundle: .module, comment: "Error with instructions when Gemma model is missing."), "\(path)", "\(Gemma4LiteRTModelStore.modelPathEnvVar)")
            case .failedToCreateSettings:
                return String(localized: "gemma4_litert.error.create_engine_settings_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create engine settings.", bundle: .module, comment: "Error when backend cannot create engine settings.")
            case .failedToCreateEngine:
                return String(localized: "gemma4_litert.error.create_engine_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create the engine.", bundle: .module, comment: "Error when backend cannot create engine.")
            case .failedToCreateSessionConfig:
                return String(localized: "gemma4_litert.error.create_session_config_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create session config.", bundle: .module, comment: "Error when backend cannot create session config.")
            case .failedToCreateConversationConfig:
                return String(localized: "gemma4_litert.error.create_conversation_config_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create conversation config.", bundle: .module, comment: "Error when backend cannot create conversation config.")
            case .failedToCreateConversation:
                return String(localized: "gemma4_litert.error.create_conversation_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create a conversation.", bundle: .module, comment: "Error when backend cannot create conversation.")
            case .failedToCreateOptionalArgs:
                return String(localized: "gemma4_litert.error.create_optional_conversation_args_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create optional conversation arguments.", bundle: .module, comment: "Error when backend cannot create optional conversation arguments.")
            case .failedToCreateMessage:
                return String(localized: "gemma4_litert.error.create_conversation_message_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create a conversation message.", bundle: .module, comment: "Error when backend cannot create conversation message.")
            case .audioTooLong(let seconds, let maxSeconds):
                return String(format: String(localized: "gemma4_litert.error.audio_clip_too_long", defaultValue: "Gemma 4 supports audio clips up to %d seconds; this clip is %.1f seconds.", bundle: .module, comment: "Error when provided audio clip exceeds Gemma maximum duration."), Int(maxSeconds), String(format: "%.1f", seconds))
            case .invalidResponse:
                return String(localized: "gemma4_litert.error.invalid_response", defaultValue: "Gemma 4 LiteRT-LM returned an invalid response.", bundle: .module, comment: "Error when backend returns malformed response.")
            case .notLoaded:
                return String(localized: "gemma4_litert.error.not_loaded_call_prepare", defaultValue: "Gemma 4 LiteRT-LM is not loaded. Call prepare() first.", bundle: .module, comment: "Error when generate is called before preparing backend.")
            }
        }
    }
    ...
    private func loadEngine(
        ...
        progressSnapshot?(ModelDownloadProgress.preparing(modelID: Gemma4LiteRTModelStore.repoID, message: String(localized: "gemma4_litert.status.preparing", defaultValue: "Preparing Gemma 4 LiteRT-LM...", bundle: .module, comment: "Status while preparing Gemma backend.")))
        ...
        progress?(0.9, String(localized: "gemma4_litert.status.loading_model", defaultValue: "Loading Gemma 4 E2B...", bundle: .module, comment: "Status while loading Gemma model into runtime."))
