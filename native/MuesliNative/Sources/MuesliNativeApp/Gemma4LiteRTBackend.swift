    ...
    * When transcribing numbers, write the digits, i.e. write 1.7 and not one point seven, and write 3 instead of three.
    ...
    static func ensureModelDownloaded(
        ...
        if localOverrideURL(environment: environment) != nil {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 1, userInfo: [
                NSLocalizedDescriptionKey: String(format: String(localized: "gemma4_litert.error.model_missing_at_path", defaultValue: "Gemma 4 LiteRT-LM model is missing at %@.", bundle: .module, comment: "Error shown when Gemma 4 model file is missing at expected path."), "\(modelURL.path)"),
            ])
        }
        ...
        guard isAvailableLocally(environment: environment, fileManager: fileManager) else {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "gemma4_litert.error.download_failed", defaultValue: "Gemma 4 LiteRT-LM did not download successfully.", bundle: .module, comment: "Error shown when Gemma 4 model download fails."),
            ])
        }
    ...
    private static func downloadManagedModel(
        ...
        try await ModelDownloadCoordinator.shared.download(manifest, to: directory) { snapshot in
            ...
            progress?(fraction, String(localized: "gemma4_litert.download.downloading_model", defaultValue: "Downloading Gemma 4 E2B", bundle: .module, comment: "Status shown while downloading Gemma 4 E2B model.") + speed)
...
actor Gemma4LiteRTTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            ...
            case .modelMissing(let path):
                return String(format: String(localized: "gemma4_litert.error.model_missing_with_download_hint", defaultValue: "Gemma 4 LiteRT-LM model is missing at %@. Download it from the Models tab or set %@.", bundle: .module, comment: "Error shown when model is missing and includes remediation guidance."), "\(path)", "\(Gemma4LiteRTModelStore.modelPathEnvVar)")
            case .failedToCreateSettings:
                return String(localized: "gemma4_litert.error.create_engine_settings_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create engine settings.", bundle: .module, comment: "Error shown when engine settings creation fails.")
            case .failedToCreateEngine:
                return String(localized: "gemma4_litert.error.create_engine_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create the engine.", bundle: .module, comment: "Error shown when inference engine creation fails.")
            case .failedToCreateSessionConfig:
                return String(localized: "gemma4_litert.error.create_session_config_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create session config.", bundle: .module, comment: "Error shown when session config creation fails.")
            case .failedToCreateConversationConfig:
                return String(localized: "gemma4_litert.error.create_conversation_config_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create conversation config.", bundle: .module, comment: "Error shown when conversation config creation fails.")
            case .failedToCreateConversation:
            ...
            case .failedToCreateOptionalArgs:
                return String(localized: "gemma4_litert.error.create_optional_conversation_args_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create optional conversation arguments.", bundle: .module, comment: "Error shown when optional conversation arguments creation fails.")
            case .failedToCreateMessage:
            ...
            case .invalidResponse:
                return String(localized: "gemma4_litert.error.invalid_response", defaultValue: "Gemma 4 LiteRT-LM returned an invalid response.", bundle: .module, comment: "Error shown when model returns invalid response payload.")
            case .notLoaded:
                return String(localized: "gemma4_litert.error.not_loaded_call_prepare", defaultValue: "Gemma 4 LiteRT-LM is not loaded. Call prepare() first.", bundle: .module, comment: "Error shown when backend is used before prepare() is called.")
            }
        }
    }
    ...
    private func loadEngine(
        ...
        progressSnapshot?(ModelDownloadProgress.preparing(modelID: Gemma4LiteRTModelStore.repoID, message: String(localized: "gemma4_litert.status.preparing", defaultValue: "Preparing Gemma 4 LiteRT-LM...", bundle: .module, comment: "Status shown while preparing Gemma 4 LiteRT backend.")))
        ...
        progress?(0.9, String(localized: "gemma4_litert.status.loading_model", defaultValue: "Loading Gemma 4 E2B...", bundle: .module, comment: "Status shown while loading Gemma 4 E2B model."))
