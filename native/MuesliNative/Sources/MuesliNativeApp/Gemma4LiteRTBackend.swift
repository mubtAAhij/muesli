    ...
    * When transcribing numbers, write the digits, i.e. write 1.7 and not one point seven, and write 3 instead of three.
    ...
    static func ensureModelDownloaded(
        ...
        if localOverrideURL(environment: environment) != nil {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 1, userInfo: [
                NSLocalizedDescriptionKey: String(format: String(localized: "gemma4_litert.error.model_missing_at_path", defaultValue: "Gemma 4 LiteRT-LM model is missing at %@.", comment: ""), "\(modelURL.path)"),
            ])
        }
        ...
        guard isAvailableLocally(environment: environment, fileManager: fileManager) else {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "gemma4_litert.error.download_failed", defaultValue: "Gemma 4 LiteRT-LM did not download successfully.", comment: ""),
            ])
        }
    ...
    private static func downloadManagedModel(
        ...
        try await ModelDownloadCoordinator.shared.download(manifest, to: directory) { snapshot in
            ...
            progress?(fraction, String(localized: "gemma4_litert.download.downloading_model", defaultValue: "Downloading Gemma 4 E2B", comment: "") + speed)
    ...
    static func validateDownloadedLiteRTLMFile(at url: URL, fileManager: FileManager) throws {
        ...
        guard attributes[.type] as? FileAttributeType == .typeRegular else {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 4, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "gemma4_litert.error.downloaded_model_not_regular_file", defaultValue: "Downloaded Gemma 4 LiteRT-LM model is not a regular file.", comment: ""),
            ])
        }
        let size = (attributes[.size] as? NSNumber)?.int64Value ?? 0
        guard size >= minimumDownloadedModelSizeBytes else {
            throw NSError(domain: "Gemma4LiteRTModelStore", code: 4, userInfo: [
                NSLocalizedDescriptionKey: String(format: String(localized: "gemma4_litert.error.model_file_too_small", defaultValue: "Downloaded Gemma 4 LiteRT-LM model is too small (%d bytes).", comment: ""), size),
            ])
        }
    }
}
...
actor Gemma4LiteRTTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            ...
            case .modelMissing(let path):
                return String(format: String(localized: "gemma4_litert.error.model_missing_with_download_hint", defaultValue: "Gemma 4 LiteRT-LM model is missing at %@. Download it from the Models tab or set %@.", comment: ""), "\(path)", "\(Gemma4LiteRTModelStore.modelPathEnvVar)")
            case .failedToCreateSettings:
                return String(localized: "gemma4_litert.error.create_engine_settings_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create engine settings.", comment: "")
            case .failedToCreateEngine:
                return String(localized: "gemma4_litert.error.create_engine_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create the engine.", comment: "")
            case .failedToCreateSessionConfig:
                return String(localized: "gemma4_litert.error.create_session_config_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create session config.", comment: "")
            case .failedToCreateConversationConfig:
                return String(localized: "gemma4_litert.error.create_conversation_config_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create conversation config.", comment: "")
            case .failedToCreateConversation:
                return String(localized: "gemma4_litert.error.create_conversation_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create a conversation.", comment: "")
            case .failedToCreateOptionalArgs:
                return String(localized: "gemma4_litert.error.create_optional_conversation_args_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create optional conversation arguments.", comment: "")
            case .failedToCreateMessage:
                return String(localized: "gemma4_litert.error.create_conversation_message_failed", defaultValue: "Gemma 4 LiteRT-LM failed to create a conversation message.", comment: "")
            case .audioTooLong(let seconds, let maxSeconds):
                return String(format: String(localized: "gemma4_litert.error.audio_clip_too_long", defaultValue: "Gemma 4 supports audio clips up to %d seconds; this clip is %.1f seconds.", comment: ""), Int(maxSeconds), String(format: "%.1f", seconds))
            case .invalidResponse:
                return String(localized: "gemma4_litert.error.invalid_response", defaultValue: "Gemma 4 LiteRT-LM returned an invalid response.", comment: "")
            case .notLoaded:
                return String(localized: "gemma4_litert.error.not_loaded_call_prepare", defaultValue: "Gemma 4 LiteRT-LM is not loaded. Call prepare() first.", comment: "")
            }
        }
    }
    ...
    private func loadEngine(
        ...
        progressSnapshot?(ModelDownloadProgress.preparing(modelID: Gemma4LiteRTModelStore.repoID, message: String(localized: "gemma4_litert.status.preparing", defaultValue: "Preparing Gemma 4 LiteRT-LM...", comment: "")))
        ...
        progress?(0.9, String(localized: "gemma4_litert.status.loading_model", defaultValue: "Loading Gemma 4 E2B...", comment: ""))
