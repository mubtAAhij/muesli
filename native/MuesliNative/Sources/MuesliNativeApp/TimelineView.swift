...
struct TimelineView: View {
    ...
    private var emptyStateTitle: String {
        ...
        case .all: return String(localized: "timeline.empty_state.no_activity_yet", defaultValue: "No activity yet", bundle: .module, comment: "Empty state title when no timeline activity exists.")
        case .thisMac: return String(localized: "timeline.empty_state.no_activity_this_mac", defaultValue: "No activity from this Mac", bundle: .module, comment: "Empty state title when no activity exists from this Mac.")
        case .fromIPhone: return String(localized: "timeline.empty_state.no_activity_iphone", defaultValue: "No activity from iPhone", bundle: .module, comment: "Empty state title when no activity exists from iPhone.")
        }
    }
...
private struct TimelineMeetingRow: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: MuesliTheme.spacing8) {
                HStack(spacing: MuesliTheme.spacing8) {
                    Image(systemName: "person.2.fill")
                        ...
                        .accessibilityLabel(String(localized: "timeline.meeting.accessibility_label", defaultValue: "Meeting", bundle: .module, comment: "Accessibility label for meeting entry in timeline."))
                    ...
                    if let label = SyncOriginDisplay.badgeLabel(forMeetingSource: record.source) {
                        ...
                        SyncOriginBadge(label: String(localized: "timeline.sync_origin.mac.title", defaultValue: "Mac", bundle: .module, comment: "Sync origin title for entries recorded on this Mac."), help: String(localized: "timeline.sync_origin.mac.subtitle", defaultValue: "Recorded on this Mac", bundle: .module, comment: "Sync origin subtitle for entries recorded on this Mac."))
                    }
        ...
        .accessibilityHint(String(localized: "timeline.meeting.accessibility_hint.open", defaultValue: "Open meeting", bundle: .module, comment: "Accessibility hint for opening a meeting entry."))
    }
    ...
    private var previewText: String {
        ...
        return preview.isEmpty ? String(localized: "timeline.preview.no_transcript_or_notes", defaultValue: "No transcript or notes yet", bundle: .module, comment: "Preview text shown when no transcript or notes are available.") : preview
    }

    private static func formatDuration(_ seconds: Double) -> String {
        let rounded = max(0, Int(seconds.rounded()))
        if rounded >= 3600 {
            return String(format: String(localized: "timeline.duration.hours_minutes", defaultValue: "%dh %dm", bundle: .module, comment: "Compact duration format showing hours and minutes."), rounded / 3600, (rounded % 3600) / 60)
        }
        if rounded >= 60 {
            let minutes = rounded / 60
            let remainingSeconds = rounded % 60
            return remainingSeconds == 0 ? String(format: String(localized: "timeline.duration.minutes_only", defaultValue: "%dm", bundle: .module, comment: "Compact duration format showing minutes only."), minutes) : String(format: String(localized: "timeline.duration.minutes_seconds", defaultValue: "%dm %ds", bundle: .module, comment: "Compact duration format showing minutes and seconds."), minutes, remainingSeconds)
        }
        return String(format: String(localized: "timeline.duration.seconds_only", defaultValue: "%ds", bundle: .module, comment: "Compact duration format showing seconds only."), rounded)
    }
