...
enum MeetingLiveCaptionModelStore {
    ...
    static let sizeLabel = String(localized: "meeting_live_captions.model.size_label", defaultValue: "~430 MB", bundle: Bundle.module, comment: "Displayed approximate size for live captions model download.")
    static let label = String(localized: "meeting_live_captions.model.label", defaultValue: "Parakeet Live Captions", bundle: Bundle.module, comment: "Display name for the Parakeet live captions model.")
    ...
    static func makeEngines(
        ...
        case .parakeetRealtimeEOU:
            ...
            do {
                return (mic, try await makeEngine(label: String(localized: "meeting_live_captions.speaker.others", defaultValue: "Others", bundle: Bundle.module, comment: "Speaker label for other participants in live captions.")))
            } catch {
                await mic.shutdown()
                throw error
            }
        case .nemotron35:
            guard #available(macOS 15, *) else {
                throw NSError(
                    ...
                    code: 2,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.nemotron_requires_macos_15", defaultValue: "Nemotron 3.5 requires macOS 15 or later.", bundle: Bundle.module, comment: "Error shown when Nemotron 3.5 is unavailable on older macOS versions.")]
                )
            }
            ...
            let system = Nemotron35MeetingPartialEngine(transcriber: transcriber, label: String(localized: "meeting_live_captions.speaker.others", defaultValue: "Others", bundle: Bundle.module, comment: "Speaker label for other participants in live captions."))
            do {
    ...
    func process(samples: [Float]) async throws {
        ...
        guard let format = AVAudioFormat(
            ...
            throw NSError(
                domain: "MeetingLiveCaptions",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.buffer_allocation_failed", defaultValue: "Could not allocate a 16 kHz live-caption buffer.", bundle: Bundle.module, comment: "Error when live caption audio buffer allocation fails.")]
            )
        }
        ...
        while state.withLock({ $0.isDraining || !$0.chunkQueue.isEmpty }) {
            guard DispatchTime.now().uptimeNanoseconds < drainDeadline else {
                goDormant(error: NSError(
                    ...
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.finalization_timed_out", defaultValue: "Timed out finalizing live transcript audio.", bundle: Bundle.module, comment: "Error when finalizing live transcript audio exceeds timeout.")]
                ))
                return nil
            }
