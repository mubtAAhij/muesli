...
enum MeetingLiveCaptionModelStore {
    ...
    static let sizeLabel = String(localized: "meeting_live_captions.model.size_label", defaultValue: "~430 MB", comment: "Approximate model size label for live captions model.")
    ...
    static func makeEngines(
        ...
        case .parakeetRealtimeEOU:
            ...
            do {
                return (mic, try await makeEngine(label: String(localized: "meeting_live_captions.speaker.others", defaultValue: "Others", comment: "Speaker label for other participants in live captions.")))
            } catch {
                await mic.shutdown()
                throw error
            }
        case .nemotron35:
            guard #available(macOS 15, *) else {
                throw NSError(
                    ...
                    code: 2,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.nemotron_requires_macos_15", defaultValue: "Nemotron 3.5 requires macOS 15 or later.", comment: "Error shown when Nemotron backend is used on unsupported macOS version.")]
                )
            }
            ...
            let system = Nemotron35MeetingPartialEngine(transcriber: transcriber, label: String(localized: "meeting_partials.speaker.others", defaultValue: "Others", comment: "Speaker label for other participants in meeting partials."))
            do {
    ...
    func process(samples: [Float]) async throws {
        ...
        guard let format = AVAudioFormat(
            ...
            throw NSError(
                domain: "MeetingLiveCaptions",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.buffer_allocation_failed", defaultValue: "Could not allocate a 16 kHz live-caption buffer.", comment: "Error shown when audio buffer allocation for live captions fails.")]
            )
        }
