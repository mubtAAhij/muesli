...
enum MeetingLiveCaptionModelStore {
    ...
    static let sizeLabel = String(localized: "meeting_live_captions.model.size_label", defaultValue: "~430 MB", bundle: .module, comment: "Approximate download size label for live captions model.")
    static let label = String(localized: "meeting_live_captions.model.label", defaultValue: "Parakeet Live Captions", bundle: .module, comment: "Display label for the Parakeet live captions model.")
    ...
    static func makeEngines(
        ...
        case .parakeetRealtimeEOU:
            ...
            do {
                return (mic, try await makeEngine(label: String(localized: "meeting_live_captions.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label representing other participants in live captions.")))
            } catch {
                await mic.shutdown()
                throw error
            }
        case .nemotron35:
            guard #available(macOS 15, *) else {
                throw NSError(
                    ...
                    code: 2,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.nemotron_requires_macos_15", defaultValue: "Nemotron 3.5 requires macOS 15 or later.", bundle: .module, comment: "Error shown when Nemotron model requires newer macOS version.")]
                )
            }
            ...
            let system = Nemotron35MeetingPartialEngine(transcriber: transcriber, label: String(localized: "meeting_live_captions.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label representing other participants in live captions."))
            do {
    ...
    func process(samples: [Float]) async throws {
        ...
        guard let format = AVAudioFormat(
            ...
            throw NSError(
                domain: "MeetingLiveCaptions",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.buffer_allocation_failed", defaultValue: "Could not allocate a 16 kHz live-caption buffer.", bundle: .module, comment: "Error shown when allocating live-caption audio buffer fails.")]
            )
        }
        ...
        while state.withLock({ $0.isDraining || !$0.chunkQueue.isEmpty }) {
            guard DispatchTime.now().uptimeNanoseconds < drainDeadline else {
                goDormant(error: NSError(
                    ...
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_live_captions.error.finalization_timed_out", defaultValue: "Timed out finalizing live transcript audio.", bundle: .module, comment: "Error shown when finalizing live transcript audio exceeds timeout.")]
                ))
                return nil
            }
