...
enum MeetingBrowserFilter: Hashable {
    ...
    var label: String {
        switch self {
        case .all: return String(localized: "meetings.filter.range.all_time", defaultValue: "All time", bundle: .module, comment: "Filter option for all meetings regardless of time range.")
        case .last2Days: return String(localized: "meetings.filter.range.last_2_days", defaultValue: "Last 2 days", bundle: .module, comment: "Filter option for meetings from last 2 days.")
        case .lastWeek: return String(localized: "meetings.filter.range.last_week", defaultValue: "Last week", bundle: .module, comment: "Filter option for meetings from last week.")
        case .last2Weeks: return String(localized: "meetings.filter.range.last_2_weeks", defaultValue: "Last 2 weeks", bundle: .module, comment: "Filter option for meetings from last 2 weeks.")
        case .lastMonth: return String(localized: "meetings.filter.range.last_month", defaultValue: "Last month", bundle: .module, comment: "Filter option for meetings from last month.")
        case .last3Months: return String(localized: "meetings.filter.range.last_3_months", defaultValue: "Last 3 months", bundle: .module, comment: "Filter option for meetings from last 3 months.")
        }
    }
}
...
struct MeetingsView: View {
    ...
    private var currentFolderName: String {
        guard let folderID = appState.selectedFolderID else { return String(localized: "meetings.navigation.all_meetings", defaultValue: "All Meetings", bundle: .module, comment: "Navigation title for the all meetings screen.") }
        return appState.folders.first(where: { $0.id == folderID })?.name ?? String(localized: "meetings.navigation.all_meetings", defaultValue: "All Meetings", bundle: .module, comment: "Navigation button/title text for all meetings.")
    }
    ...
    private var groupedUpcomingEvents: [UpcomingEventGroup] {
        ...
        for date in sortedDates {
            ...
            let dayLabel: String
            if isToday {
                dayLabel = String(localized: "meetings.group.today", defaultValue: "Today", bundle: .module, comment: "Group header label for meetings happening today.")
            } else if isTomorrow {
                dayLabel = String(localized: "meetings.group.tomorrow", defaultValue: "Tomorrow", bundle: .module, comment: "Group header label for meetings happening tomorrow.")
            } else {
    ...
    private var comingUpSection: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            VStack(alignment: .leading, spacing: 4) {
                Text(String(localized: "meetings.coming_up.title", defaultValue: "Coming Up", bundle: .module, comment: "Title for the upcoming meetings section."))
                    ...
                    Button {
                        ...
                        HStack(spacing: 4) {
                            Image(systemName: "arrow.triangle.2.circlepath")
                                .font(.system(size: 9))
                            Text(String(localized: "meetings.coming_up.add_google_calendar_realtime_sync", defaultValue: "Add Google to macOS Calendar for real-time sync", bundle: .module, comment: "Prompt encouraging connecting Google calendar for real-time sync."))
                                .font(.system(size: 11))
                        }
                        ...
                        ForEach(group.events) { event in
                            HStack(spacing: 8) {
                                    ...
                                    Button {
                                        ...
                                        HStack(spacing: 4) {
                                            Image(systemName: "video.fill")
                                                .font(.system(size: 9))
                                            Text(String(localized: "meetings.coming_up.join_and_record", defaultValue: "Join & Record", bundle: .module, comment: "Action button label to join and record a meeting."))
                                ...
                                Menu {
                                    ...
                                    Text(String(localized: "meetings.coming_up.add_to_folder", defaultValue: "Add to folder", bundle: .module, comment: "Action label to add a meeting to a folder."))
                                        .font(.system(size: 10, weight: .medium))
                                        .foregroundStyle(MuesliTheme.textSecondary)
                                        .padding(.horizontal, 8)
                                        .padding(.vertical, 3)
    ...
    private func hideEventButton(_ event: UnifiedCalendarEvent) -> some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "meetings.coming_up.hide_event_help", defaultValue: "Hide from Coming Up", bundle: .module, comment: "Help/action text to hide an event from coming up list."))
    }
    ...
    private func browserHeaderMeta(meetingCount: Int) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            Text(String(localized: "meetings.browser.header.review_notes_transcript_summaries", defaultValue: "Open a meeting to review notes, transcript, and template-driven summaries", bundle: .module, comment: "Instructional header in meetings browser when no meeting is selected."))
                .font(MuesliTheme.callout())
                .foregroundStyle(MuesliTheme.textTertiary)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        ...
        HStack(spacing: MuesliTheme.spacing8) {
            Button {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(String(localized: "meetings.browser.quick_note", defaultValue: "Quick Note", bundle: .module, comment: "Button label to start a quick meeting note."))
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "square.and.arrow.down")
                        .font(.system(size: 11, weight: .semibold))
                    Text(String(localized: "meetings.browser.import_audio", defaultValue: "Import Audio", bundle: .module, comment: "Button label to import audio into meetings browser."))
                        .font(.system(size: 12, weight: .semibold))
                        .lineLimit(1)
                }
                .foregroundStyle(MuesliTheme.textPrimary)
            ...
            .buttonStyle(.plain)
            .disabled(appState.isMeetingRecording || appState.isMeetingStarting)
            .help(String(localized: "meetings.browser.import_audio_help", defaultValue: "Import an audio file for offline transcription", bundle: .module, comment: "Help text for import audio action."))
            .fixedSize()
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "square.and.pencil")
                        .font(.system(size: 11, weight: .medium))
                    Text(String(localized: "meetings.browser.manage_templates", defaultValue: "Manage Templates", bundle: .module, comment: "Button label to manage meeting templates."))
                        .font(.system(size: 12, weight: .semibold))
                        .lineLimit(1)
                }
                .foregroundStyle(MuesliTheme.textPrimary)
        ...
        HStack(spacing: MuesliTheme.spacing12) {
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "square.and.pencil")
                        .font(.system(size: 11, weight: .semibold))
                    Text(String(localized: "meetings.active_banner.open_notes", defaultValue: "Open Notes", bundle: .module, comment: "Button label to open notes from active meeting banner."))
                        .font(.system(size: 12, weight: .semibold))
                }
                .foregroundStyle(MuesliTheme.textPrimary)
                .padding(.horizontal, MuesliTheme.spacing12)
    ...
    private func activeMeetingStatusText(for meeting: MeetingRecord) -> String {
        guard meeting.status == .recording else { return String(localized: "meetings.active_status.finalizing_notes", defaultValue: "Finalizing notes", bundle: .module, comment: "Status text while meeting notes are being finalized.") }
        return appState.isMeetingRecordingPaused ? String(localized: "meetings.active_status.recording_paused", defaultValue: "Recording paused", bundle: .module, comment: "Status text when recording is paused.") : String(localized: "meetings.active_status.recording_now", defaultValue: "Recording now", bundle: .module, comment: "Status text when recording is active.")
    }
    ...
    private var emptyStateTitle: String {
        ...
        case .thisMac:
            return String(localized: "meetings.empty_state.title.no_meetings_from_mac", defaultValue: "No meetings from this Mac", bundle: .module, comment: "Empty state title when filtering to meetings from current Mac and none exist.")
        case .fromIPhone:
            return String(localized: "meetings.empty_state.title.no_meetings_from_iphone", defaultValue: "No meetings from iPhone", bundle: .module, comment: "Empty state title when filtering to iPhone meetings and none exist.")
        case .all:
            return appState.selectedFolderID == nil ? String(localized: "meetings.empty_state.title.no_meetings_yet", defaultValue: "No meetings yet", bundle: .module, comment: "General empty state title when no meetings exist.") : String(localized: "meetings.empty_state.title.no_meetings_in_folder", defaultValue: "No meetings in this folder", bundle: .module, comment: "Empty state title when selected folder has no meetings.")
        }
    }

    private var emptyStateInstruction: String {
        if appState.meetingOriginFilter != .all || selectedFilter != .all {
            return String(localized: "meetings.empty_state.instruction.try_another_source_time_range_or_folder", defaultValue: "Try another source, time range, or folder.", bundle: .module, comment: "Empty state guidance to change filters or folder.")
        }
        return appState.selectedFolderID == nil
            ? String(localized: "meetings.empty_state.instruction.start_recording_from_menu_bar", defaultValue: "Start a recording from the menu bar to create your first meeting note.", bundle: .module, comment: "Empty state guidance to start first recording from menu bar.")
            : String(localized: "meetings.empty_state.instruction.choose_another_folder_or_move_meeting", defaultValue: "Choose another folder or move a meeting here from the browser.", bundle: .module, comment: "Empty state guidance to choose a different folder or move a meeting.")
    }
}
