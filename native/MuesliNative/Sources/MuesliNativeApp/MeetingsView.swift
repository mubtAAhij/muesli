...
enum MeetingBrowserFilter: Hashable {
    ...
    var label: String {
        switch self {
        case .all: return String(localized: "meetings.filter.range.all_time", defaultValue: "All time", comment: "")
        case .last2Days: return String(localized: "meetings.filter.range.last_2_days", defaultValue: "Last 2 days", comment: "")
        case .lastWeek: return String(localized: "meetings.filter.range.last_week", defaultValue: "Last week", comment: "")
        case .last2Weeks: return String(localized: "meetings.filter.range.last_2_weeks", defaultValue: "Last 2 weeks", comment: "")
        case .lastMonth: return String(localized: "meetings.filter.range.last_month", defaultValue: "Last month", comment: "")
        case .last3Months: return String(localized: "meetings.filter.range.last_3_months", defaultValue: "Last 3 months", comment: "")
        }
    }
}
...
enum MeetingBrowserSort: Hashable {
    ...
    var label: String {
        switch self {
        case .newestFirst: return String(localized: "meetings.sort.newest_first", defaultValue: "Newest first", comment: "")
        case .oldestFirst: return String(localized: "meetings.sort.oldest_first", defaultValue: "Oldest first", comment: "")
        }
    }
}
...
struct MeetingsView: View {
    ...
    private var currentFolderName: String {
        guard let folderID = appState.selectedFolderID else { return String(localized: "meetings.current_folder.all_meetings", defaultValue: "All Meetings", comment: "") }
        return appState.folders.first(where: { $0.id == folderID })?.name ?? String(localized: "meetings.current_folder.all_meetings", defaultValue: "All Meetings", comment: "")
    }
        ...
        Group {
            if let meeting = currentDocumentMeeting {
                MeetingDetailView(
                    ...
                    backLabel: String(localized: "meetings.navigation.back_to_meetings", defaultValue: "Back to Meetings", comment: "")
                )
                .id(meeting.id)
            } else {
                browserView
    ...
    private var groupedUpcomingEvents: [UpcomingEventGroup] {
        ...
        for date in sortedDates {
            ...
            let dayLabel: String
            if isToday {
                dayLabel = String(localized: "meetings.group.today", defaultValue: "Today", comment: "")
            } else if isTomorrow {
                dayLabel = String(localized: "meetings.group.tomorrow", defaultValue: "Tomorrow", comment: "")
            } else {
    ...
    private var comingUpSection: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            VStack(alignment: .leading, spacing: 4) {
                Text(String(localized: "meetings.coming_up.title", defaultValue: "Coming Up", comment: ""))
                    ...
                    Button {
                        ...
                        HStack(spacing: 4) {
                            Image(systemName: "arrow.triangle.2.circlepath")
                                .font(.system(size: 9))
                            Text(String(localized: "meetings.coming_up.add_google_calendar_realtime_sync", defaultValue: "Add Google to macOS Calendar for real-time sync", comment: ""))
                                .font(.system(size: 11))
                        }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ForEach(group.events) { event in
                            HStack(spacing: 8) {
                                    ...
                                    Button {
                                        ...
                                        HStack(spacing: 4) {
                                            Image(systemName: "video.fill")
                                                .font(.system(size: 9))
                                            Text(String(localized: "meetings.coming_up.join_and_record", defaultValue: "Join & Record", comment: ""))
                                ...
                                Menu {
                                    Button(String(localized: "meetings.coming_up.all_meetings_button", defaultValue: "All Meetings", comment: "")) {
                                    ...
                                    Text(String(localized: "meetings.coming_up.add_to_folder", defaultValue: "Add to folder", comment: ""))
                                        .font(.system(size: 10, weight: .medium))
                                        .foregroundStyle(MuesliTheme.textSecondary)
                                        .padding(.horizontal, 8)
                                        .padding(.vertical, 3)
    ...
    private func hideEventButton(_ event: UnifiedCalendarEvent) -> some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "meetings.coming_up.hide_event_help", defaultValue: "Hide from Coming Up", comment: ""))
    }
    ...
    private func browserHeaderMeta(meetingCount: Int) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            Text(String(localized: "meetings.browser.header.review_notes_transcript_summaries", defaultValue: "Open a meeting to review notes, transcript, and template-driven summaries", comment: ""))
                .font(MuesliTheme.callout())
                .foregroundStyle(MuesliTheme.textTertiary)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        ...
        HStack(spacing: MuesliTheme.spacing8) {
            Button {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(String(localized: "meetings.browser.quick_note", defaultValue: "Quick Note", comment: ""))
            ...
            .help(String(localized: "meetings.browser.quick_note_help", defaultValue: "Start a quick meeting note", comment: ""))
            .fixedSize()
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "square.and.arrow.down")
                        .font(.system(size: 11, weight: .semibold))
                    Text(String(localized: "meetings.browser.import_audio", defaultValue: "Import Audio", comment: ""))
                        .font(.system(size: 12, weight: .semibold))
                        .lineLimit(1)
                }
                .foregroundStyle(MuesliTheme.textPrimary)
            ...
            .buttonStyle(.plain)
            .disabled(appState.isMeetingRecording || appState.isMeetingStarting)
            .help(String(localized: "meetings.browser.import_audio_help", defaultValue: "Import an audio file for offline transcription", comment: ""))
            .fixedSize()
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "square.and.pencil")
                        .font(.system(size: 11, weight: .medium))
                    Text(String(localized: "meetings.browser.manage_templates", defaultValue: "Manage Templates", comment: ""))
                        .font(.system(size: 12, weight: .semibold))
                        .lineLimit(1)
                }
                .foregroundStyle(MuesliTheme.textPrimary)
        ...
        HStack(spacing: MuesliTheme.spacing12) {
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "square.and.pencil")
                        .font(.system(size: 11, weight: .semibold))
                    Text(String(localized: "meetings.active_banner.open_notes", defaultValue: "Open Notes", comment: ""))
                        .font(.system(size: 12, weight: .semibold))
                }
                .foregroundStyle(MuesliTheme.textPrimary)
                .padding(.horizontal, MuesliTheme.spacing12)
            ...
            if meeting.status == .recording {
                Button {
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(appState.isMeetingRecordingPaused ? String(localized: "meetings.active_banner.resume", defaultValue: "Resume", comment: "") : String(localized: "meetings.active_banner.pause", defaultValue: "Pause", comment: ""))
                            .font(.system(size: 12, weight: .semibold))
                    }
                ...
                Button {
                    ...
                    HStack(spacing: 6) {
                        Image(systemName: "stop.fill")
                            .font(.system(size: 10, weight: .semibold))
                        Text(String(localized: "meetings.active_banner.stop", defaultValue: "Stop", comment: ""))
                            .font(.system(size: 12, weight: .semibold))
                    }
                    .foregroundStyle(.white)
                    .padding(.horizontal, MuesliTheme.spacing12)
    ...
    private func activeMeetingStatusText(for meeting: MeetingRecord) -> String {
        guard meeting.status == .recording else { return String(localized: "meetings.active_status.finalizing_notes", defaultValue: "Finalizing notes", comment: "") }
        return appState.isMeetingRecordingPaused ? String(localized: "meetings.active_status.recording_paused", defaultValue: "Recording paused", comment: "") : String(localized: "meetings.active_status.recording_now", defaultValue: "Recording now", comment: "")
    }
    ...
    private var emptyStateTitle: String {
        ...
        case .thisMac:
            return String(localized: "meetings.empty_state.title.no_meetings_from_mac", defaultValue: "No meetings from this Mac", comment: "")
        case .fromIPhone:
            return String(localized: "meetings.empty_state.title.no_meetings_from_iphone", defaultValue: "No meetings from iPhone", comment: "")
        case .all:
            return appState.selectedFolderID == nil ? String(localized: "meetings.empty_state.title.no_meetings_yet", defaultValue: "No meetings yet", comment: "") : String(localized: "meetings.empty_state.title.no_meetings_in_folder", defaultValue: "No meetings in this folder", comment: "")
        }
    }

    private var emptyStateInstruction: String {
        if appState.meetingOriginFilter != .all || selectedFilter != .all {
            return String(localized: "meetings.empty_state.instruction.try_another_source_time_range_or_folder", defaultValue: "Try another source, time range, or folder.", comment: "")
        }
        return appState.selectedFolderID == nil
            ? String(localized: "meetings.empty_state.instruction.start_recording_from_menu_bar", defaultValue: "Start a recording from the menu bar to create your first meeting note.", comment: "")
            : String(localized: "meetings.empty_state.instruction.choose_another_folder_or_move_meeting", defaultValue: "Choose another folder or move a meeting here from the browser.", comment: "")
    }
}
