...
struct DictionaryView: View {
    ...
    var body: some View {
        ...
        .alert(String(localized: "dictionary.accessibility_alert.title", defaultValue: "Enable Accessibility?", comment: ""), isPresented: $isShowingAccessibilityPrompt) {
            Button(String(localized: "common.button.cancel", defaultValue: "Cancel", comment: ""), role: .cancel) {
                controller.cancelDictionaryCorrectionAccessibilityEnableRequest()
            }
            Button(String(localized: "dictionary.accessibility_alert.enable", defaultValue: "Enable", comment: "")) {
                controller.requestDictionaryCorrectionAccessibilityEnable()
            }
        } message: {
            Text(String(localized: "dictionary.accessibility_alert.message", defaultValue: "Dictionary suggestions briefly read focused app text via Accessibility after dictation. Grant access, then relaunch Muesli to turn suggestions on.", comment: ""))
        }
        .alert(
            String(localized: "dictionary.accessibility_alert.source", defaultValue: "Dictionary", comment: ""),
            isPresented: Binding(
                get: { dictionaryAlertMessage != nil },
                set: { if !$0 { dictionaryAlertMessage = nil } }
            )
        ) {
            Button(String(localized: "common.button.ok", defaultValue: "OK", comment: ""), role: .cancel) { dictionaryAlertMessage = nil }
        } message: {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing8) {
            HStack {
                Text(String(localized: "dictionary.header.title", defaultValue: "Dictionary", comment: ""))
                    .font(MuesliTheme.title1())
                    .foregroundStyle(MuesliTheme.textPrimary)
                Spacer()
                Toggle(
                    String(localized: "dictionary.header.suggestions_toggle", defaultValue: "Dictionary suggestions", comment: ""),
                    isOn: Binding(
                ...
                .help(String(localized: "dictionary.header.help.accessibility_summary", defaultValue: "Briefly reads focused app text after dictation to detect corrections.", comment: ""))
                .featureTourTarget(.dictionarySuggestions)
                Button {
                    importDictionary()
                } label: {
                    Label(String(localized: "dictionary.header.import", defaultValue: "Import", comment: ""), systemImage: "square.and.arrow.down")
                ...
                .buttonStyle(.plain)
                .help(String(localized: "dictionary.header.import.help", defaultValue: "Import dictionary entries from a JSON file", comment: ""))
                Button {
                    exportDictionary()
                } label: {
                    Label(String(localized: "dictionary.header.export", defaultValue: "Export", comment: ""), systemImage: "square.and.arrow.up")
                        .font(.system(size: 12, weight: .medium))
                        .foregroundStyle(MuesliTheme.textPrimary)
                        .padding(.horizontal, MuesliTheme.spacing12)
                        .padding(.vertical, MuesliTheme.spacing8)
                ...
                .buttonStyle(.plain)
                .help(String(localized: "dictionary.header.export.help", defaultValue: "Export the current dictionary as JSON", comment: ""))
                Button {
                    isAdding = true
                    newWord = ""
                    newReplacement = ""
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(String(localized: "dictionary.header.add_new", defaultValue: "Add new", comment: ""))
            ...
            Text(String(localized: "dictionary.header.description", defaultValue: "Add custom words for names, brands, and domain terms, and tune how aggressively each entry should fuzzy-match transcription errors.", comment: ""))
    ...
    private func importDictionary() {
        ...
        panel.title = String(localized: "dictionary.import.panel_title", defaultValue: "Import Muesli Dictionary", comment: "")
        panel.message = String(localized: "dictionary.import.panel_message", defaultValue: "Choose a JSON dictionary file", comment: "")
        panel.prompt = String(localized: "dictionary.import.panel_confirm", defaultValue: "Import", comment: "")
            ...
            do {
                ...
                if totalChanged == 0 {
                    dictionaryAlertMessage = imported.isEmpty
                        ? String(localized: "dictionary.import.result.no_entries", defaultValue: "The selected dictionary did not contain any entries.", comment: "")
                        : String(localized: "dictionary.import.result.already_present", defaultValue: "All dictionary entries were already present.", comment: "")
                } else {
                    var details = [String(format: String(localized: "dictionary.import.result.added_fragment", defaultValue: "Imported %d new", comment: ""), result.addedCount), String(format: String(localized: "dictionary.import.result.updated_fragment", defaultValue: "updated %d", comment: ""), result.updatedCount)]
                    if result.skippedCount > 0 {
                        details.append(String(format: String(localized: "dictionary.import.result.skipped_fragment", defaultValue: "skipped %d", comment: ""), result.skippedCount))
                    }
                    dictionaryAlertMessage = details.joined(separator: ", ") + " " + String(localized: "dictionary.import.result.entries_suffix", defaultValue: "dictionary entries.", comment: "")
                }
            } catch {
                dictionaryAlertMessage = String(format: String(localized: "dictionary.import.error", defaultValue: "Could not import the dictionary. %@", comment: ""), "\(error.localizedDescription)")
            }
        }
    }
    ...
    private func exportDictionary() {
        ...
        panel.title = String(localized: "dictionary.export.panel_title", defaultValue: "Export Muesli Dictionary", comment: "")
        panel.prompt = String(localized: "dictionary.export.panel_confirm", defaultValue: "Export", comment: "")
        ...
        presentFilePanel(panel) { url in
            do {
                ...
                dictionaryAlertMessage = String(format: String(localized: "dictionary.export.success", defaultValue: "Exported %d dictionary entries.", comment: ""), appState.config.customWords.count)
            } catch {
                dictionaryAlertMessage = String(format: String(localized: "dictionary.export.error", defaultValue: "Could not export the dictionary. %@", comment: ""), "\(error.localizedDescription)")
            }
        }
    }
        ...
        VStack(spacing: 0) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text(String(localized: "dictionary.suggestions.title", defaultValue: "Suggested Corrections", comment: ""))
                        .font(MuesliTheme.headline())
                        .foregroundStyle(MuesliTheme.textPrimary)
                    Text(String(localized: "dictionary.suggestions.description", defaultValue: "Corrections Muesli noticed by briefly reading focused app text after dictation.", comment: ""))
                        .font(MuesliTheme.caption())
    ...
    private var suggestionRangeText: String {
        ...
        return String(format: String(localized: "dictionary.suggestions.range", defaultValue: "%d-%d of %d", comment: ""), start, end, count)
    }
    ...
    private var suggestionPaginationControls: some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            DictionaryIconButton(
                ...
                label: String(localized: "dictionary.suggestions.pagination.previous", defaultValue: "Previous suggestions", comment: ""),
            ...
            DictionaryIconButton(
                ...
                label: String(localized: "dictionary.suggestions.pagination.next", defaultValue: "Next suggestions", comment: ""),
                tint: MuesliTheme.textSecondary,
    ...
    private var emptyState: some View {
        VStack(spacing: MuesliTheme.spacing8) {
            Image(systemName: "character.book.closed")
                .font(.system(size: 28))
                .foregroundStyle(MuesliTheme.textTertiary)
            Text(String(localized: "dictionary.empty_state.title", defaultValue: "No custom words yet", comment: ""))
                .font(MuesliTheme.body())
                .foregroundStyle(MuesliTheme.textSecondary)
            Text(String(localized: "dictionary.empty_state.subtitle", defaultValue: "Add words that transcription frequently gets wrong", comment: ""))
                .font(MuesliTheme.caption())
                .foregroundStyle(MuesliTheme.textTertiary)
        }
    ...
    private var columnHeader: some View {
        HStack(spacing: MuesliTheme.spacing8) {
            Text(String(localized: "dictionary.table.column.match", defaultValue: "Match", comment: ""))
                .frame(maxWidth: .infinity, alignment: .leading)
            Color.clear
                .frame(width: DictionaryRowMetrics.arrowWidth)
            Text(String(localized: "dictionary.table.column.replace", defaultValue: "Replace", comment: ""))
                .frame(maxWidth: .infinity, alignment: .leading)
            Text(String(localized: "dictionary.table.column.threshold", defaultValue: "Threshold", comment: ""))
    ...
    private var addWordRow: some View {
        HStack(spacing: MuesliTheme.spacing8) {
            TextField(String(localized: "dictionary.add_word.field_label", defaultValue: "Word", comment: ""), text: $newWord)
            ...
            Image(systemName: "arrow.right")
                ...
                .foregroundStyle(MuesliTheme.textTertiary)
                .frame(width: DictionaryRowMetrics.arrowWidth)
            TextField(String(localized: "dictionary.add_word.replace_with.prompt", defaultValue: "Replace with", comment: ""), text: $newReplacement)
                .textFieldStyle(.roundedBorder)
                .frame(maxWidth: .infinity)
            ThresholdEditor(value: $newThreshold)
            DictionaryIconButton(
                systemName: "checkmark",
                label: String(localized: "dictionary.add_word.button", defaultValue: "Add word", comment: ""),
            ...
            DictionaryIconButton(
                systemName: "xmark",
                label: String(localized: "dictionary.add_word.cancel", defaultValue: "Cancel", comment: ""),
...
private struct DictionarySuggestionRow: View {
    ...
    var body: some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            DictionaryIconButton(
                systemName: "checkmark",
                label: String(localized: "dictionary.suggestion.add_correction", defaultValue: "Add correction", comment: ""),
                tint: MuesliTheme.accent
            ) {
            ...
            DictionaryIconButton(
                systemName: "xmark",
                label: String(localized: "dictionary.suggestion.dismiss_correction", defaultValue: "Dismiss correction", comment: ""),
                tint: MuesliTheme.textTertiary
            ) {
    ...
    private var detailText: String {
        var parts = [String(format: String(localized: "dictionary.suggestion.seen_count", defaultValue: "Seen %dx", comment: ""), suggestion.occurrenceCount)]
...
private struct DictionaryWordEditorRow: View {
    ...
    var body: some View {
        HStack(spacing: MuesliTheme.spacing8) {
            TextField(String(localized: "dictionary.add_word.prompt", defaultValue: "Word", comment: ""), text: $draftWord)
                .textFieldStyle(.roundedBorder)
                .frame(maxWidth: .infinity)
            Image(systemName: "arrow.right")
                .font(.system(size: 10, weight: .semibold))
                .foregroundStyle(MuesliTheme.textTertiary)
                .frame(width: DictionaryRowMetrics.arrowWidth)
            TextField(String(localized: "dictionary.edit_word.replace_with.prompt", defaultValue: "Replace with", comment: ""), text: $draftReplacement)
                .textFieldStyle(.roundedBorder)
                .frame(maxWidth: .infinity)
            ThresholdEditor(value: $draftThreshold)
            DictionaryIconButton(
                systemName: "checkmark",
                label: String(localized: "dictionary.edit_word.save", defaultValue: "Save word", comment: ""),
            ...
            DictionaryIconButton(
                systemName: "trash",
                label: String(localized: "dictionary.edit_word.delete", defaultValue: "Delete word", comment: ""),
...
private struct ThresholdEditor: View {
    ...
    var body: some View {
        ...
        .help(String(localized: "dictionary.threshold.help.title", defaultValue: "Matching threshold", comment: ""))
        .accessibilityLabel(String(localized: "dictionary.threshold.accessibility_label", defaultValue: "Matching threshold", comment: ""))
    ...
    private var thresholdPopover: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing12) {
            HStack {
                Text(String(localized: "dictionary.threshold_popover.title", defaultValue: "Threshold", comment: ""))
                    .font(MuesliTheme.caption())
                    .foregroundStyle(MuesliTheme.textTertiary)
                Spacer()
                HStack(spacing: 4) {
