...
enum ChatGPTAuthError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthenticated: return String(localized: "chatgpt_auth.error.not_signed_in", defaultValue: "Not signed in to ChatGPT", comment: "Error shown when ChatGPT account is not authenticated")
        case .callbackTimeout: return String(localized: "chatgpt_auth.error.signin_timed_out", defaultValue: "Sign-in timed out — no response from browser", comment: "Error shown when ChatGPT browser sign-in times out")
        ...
        case .portInUse: return String(localized: "chatgpt_auth.error.callback_port_in_use", defaultValue: "Callback port 1455 is already in use", comment: "Error shown when OAuth callback port is already occupied")
        }
    }
}
