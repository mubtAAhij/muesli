...
enum MuesliBridgeDeviceIdentity {
    ...
    private static func currentDeviceName() -> String {
        ...
        return hostName.isEmpty ? String(localized: "sync.device.this_mac", defaultValue: "This Mac", comment: "Label representing the current device in iCloud sync metadata") : hostName
    }
    ...
    private static func appVersion() -> String {
        ...
        return (info?["CFBundleShortVersionString"] as? String)
            ...
            ?? String(localized: "sync.app_version.unknown", defaultValue: "unknown", comment: "Fallback app version label when sync source app version is unavailable")
    }
}
...
private enum ICloudSyncAccountError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noAccount:
            return String(localized: "sync.error.account_unavailable", defaultValue: "Sign in to iCloud on this Mac to sync text records.", comment: "Error shown when iCloud account is unavailable for sync")
        case .restricted:
            return String(localized: "sync.error.restricted", defaultValue: "iCloud is restricted for this Mac.", comment: "Error shown when iCloud access is restricted on the device")
        case .temporarilyUnavailable:
...
enum ICloudSyncDeadlineError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        String(localized: "sync.error.timeout_retry", defaultValue: "iCloud did not respond in time. Your text is safe and will be retried.", comment: "Error shown when iCloud sync request times out and will retry")
    }
}
