...
enum DictationContextCapture {
    ...
    static func capture() -> DictationContext {
        ...
        let appName = app?.localizedName ?? String(localized: "screen_context_capture.value.unknown", defaultValue: "Unknown", bundle: .module, comment: "Fallback app or context value when source is unknown.")
    ...
    static func formatForPrompt(_ ctx: DictationContext) -> String {
        var parts = String(format: String(localized: "screen_context_capture.prompt.app_label", defaultValue: "App: %@", bundle: .module, comment: "Prompt section showing active app name in captured screen context."), "\(ctx.appName)")
        if let url = ctx.url {
            parts += " (\(url))"
        }
        if !ctx.documentContext.isEmpty {
            parts += "\n" + String(format: String(localized: "screen_context_capture.prompt.document_context", defaultValue: "Document context: %@", bundle: .module, comment: "Prompt section label for captured document context text."), ctx.documentContext)
        }
        if !ctx.selectedText.isEmpty {
            parts += "\n" + String(format: String(localized: "screen_context_capture.prompt.selected_text", defaultValue: "Selected text: %@", bundle: .module, comment: "Prompt section label for captured selected text."), ctx.selectedText)
        }
        if !ctx.ocrText.isEmpty {
            parts += "\n" + String(format: String(localized: "screen_context_capture.prompt.ocr_screen_text", defaultValue: "OCR screen text: %@", bundle: .module, comment: "Prompt section label for captured OCR text."), ctx.ocrText)
        }
        return parts
    }
