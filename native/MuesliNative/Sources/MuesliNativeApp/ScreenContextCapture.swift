...
enum DictationContextCapture {
    ...
    static func capture() -> DictationContext {
        ...
        let appName = app?.localizedName ?? String(localized: "screen_context_capture.value.unknown", defaultValue: "Unknown", bundle: .module, comment: "Fallback value when screen context field is unavailable")
    ...
    static func formatForPrompt(_ ctx: DictationContext) -> String {
        var parts = String(format: String(localized: "screen_context_capture.prompt.app_label", defaultValue: "App: %@", bundle: .module, comment: "Prompt line showing current app name in captured screen context"), "\(ctx.appName)")
        if let url = ctx.url {
            parts += " (\(url))"
        }
        if !ctx.documentContext.isEmpty {
            parts += "\n" + String(format: String(localized: "screen_context_capture.prompt.document_context", defaultValue: "Document context: %@", bundle: .module, comment: "Label and value for captured document context in screen context prompt"), ctx.documentContext)
        }
        if !ctx.selectedText.isEmpty {
            parts += "\n" + String(format: String(localized: "screen_context_capture.prompt.selected_text", defaultValue: "Selected text: %@", bundle: .module, comment: "Label and value for selected text in screen context prompt"), ctx.selectedText)
        }
        if !ctx.ocrText.isEmpty {
            parts += "\n" + String(format: String(localized: "screen_context_capture.prompt.ocr_screen_text", defaultValue: "OCR screen text: %@", bundle: .module, comment: "Label and value for OCR text in screen context prompt"), ctx.ocrText)
        }
        return parts
    }
