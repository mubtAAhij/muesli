...
enum GoogleCalendarClientError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .requestFailed(let message):
            return String(
                format: String(
                    localized: "google_calendar.error.request_failed",
                    defaultValue: "Google Calendar request failed: %@",
                    comment: "Error shown when Google Calendar request fails"
                ),
                message
            )
        case .staleRequest:
            return String(
                localized: "google_calendar.error.request_superseded",
                defaultValue: "Google Calendar request was superseded",
                comment: "Error shown when a Google Calendar request is superseded by a newer request"
            )
        }
    }
}
        ...
        do {
            ...
            if cachedCalendarList.isEmpty {
                ...
                cachedCalendarList = [GoogleCalendarSummary(
                    ...
                    summary: String(
                        localized: "google_calendar.summary.primary",
                        defaultValue: "Primary",
                        comment: "Fallback display name for the user's primary calendar"
                    ),
                    isPrimary: true,
                    colorHex: nil
                )]
