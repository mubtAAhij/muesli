...
enum GoogleCalendarClientError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .requestFailed(let message):
            return String(format: String(localized: "google_calendar.error.request_failed", defaultValue: "Google Calendar request failed: %@", bundle: .module, comment: "Error message when Google Calendar request fails with backend message"), "\(message)")
        case .staleRequest:
            return String(localized: "google_calendar.error.request_superseded", defaultValue: "Google Calendar request was superseded", bundle: .module, comment: "Error message when a Google Calendar request is replaced by a newer one")
        }
    }
}
        ...
        do {
            ...
            if cachedCalendarList.isEmpty {
                ...
                cachedCalendarList = [GoogleCalendarSummary(
                    ...
                    summary: String(localized: "google_calendar.summary.primary", defaultValue: "Primary", bundle: .module, comment: "Display name fallback for the user's primary Google Calendar"),
                    isPrimary: true,
                    colorHex: nil
                )]
            }
