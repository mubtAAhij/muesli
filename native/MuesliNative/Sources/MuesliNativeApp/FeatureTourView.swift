    ...
    private var callout: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            HStack(alignment: .top, spacing: MuesliTheme.spacing12) {
                ...
                .help(String(localized: "feature_tour.action.end_walkthrough", defaultValue: "End walkthrough", bundle: .module, comment: "Button title to end the feature walkthrough."))
                .accessibilityLabel(String(localized: "feature_tour.action.end_walkthrough", defaultValue: "End walkthrough", bundle: .module, comment: "Confirmation action to end the feature walkthrough."))
            }
            ...
            HStack(spacing: MuesliTheme.spacing12) {
                Text(String(format: String(localized: "feature_tour.progress.step_of_total", defaultValue: "%d of %d", bundle: .module, comment: "Progress indicator showing current walkthrough step and total steps."), stepIndex + 1, tour.steps.count))
                    .font(MuesliTheme.caption())
                    .monospacedDigit()
                    .foregroundStyle(MuesliTheme.textTertiary)

                Button(String(localized: "feature_tour.action.skip", defaultValue: "Skip", bundle: .module, comment: "Action label to skip the feature walkthrough."), action: onDismiss)
                    .buttonStyle(.plain)
                    .foregroundStyle(MuesliTheme.textSecondary)
                ...
                if stepIndex > 0 {
                    Button(action: onBack) {
                        Label(String(localized: "feature_tour.back", defaultValue: "Back", bundle: .module, comment: "Navigation button to return to previous tour step."), systemImage: "chevron.left")
                    }
                ...
                Button(action: onNext) {
                    Label(
                        stepIndex == tour.steps.count - 1 ? String(localized: "feature_tour.done", defaultValue: "Done", bundle: .module, comment: "Primary button title to finish the tour.") : String(localized: "feature_tour.next", defaultValue: "Next", bundle: .module, comment: "Primary button title to advance to next tour step."),
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: MuesliTheme.spacing20) {
                HStack(alignment: .top, spacing: MuesliTheme.spacing12) {
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        Text(String(format: String(localized: "feature_tour.header.version_title", defaultValue: "MUESLI %@", bundle: .module, comment: "Feature tour header showing app name and display version."), "\(tour.displayVersion)"))
                            .font(.system(size: 10, weight: .bold))
                            .foregroundStyle(MuesliTheme.accent)
                        Text(String(localized: "feature_tour.prompt.whats_new", defaultValue: "Want a quick tour of what’s new?", bundle: .module, comment: "Prompt inviting user to start what's new tour."))
                    ...
                    .buttonStyle(.plain)
                    .foregroundStyle(MuesliTheme.textSecondary)
                    .help(String(localized: "feature_tour.action.skip_walkthrough", defaultValue: "Skip walkthrough", bundle: .module, comment: "Action title to skip walkthrough from intro dialog."))
                    .accessibilityLabel(String(localized: "feature_tour.action.skip_walkthrough", defaultValue: "Skip walkthrough", bundle: .module, comment: "Confirmation button title to skip walkthrough."))
                }

                Text(String(format: String(localized: "feature_tour.prompt.summary_with_count", defaultValue: "See %d additions in the places where you’ll actually use them. You can replay this later from What’s New in Muesli.", bundle: .module, comment: "Intro dialog summary showing number of additions covered by walkthrough."), tour.steps.count))
                    .font(MuesliTheme.body())
                    .foregroundStyle(MuesliTheme.textSecondary)
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)
                ...
                HStack(spacing: MuesliTheme.spacing12) {
                    Spacer()
                    Button(String(localized: "feature_tour.action.skip", defaultValue: "Skip", bundle: .module, comment: "Secondary action to skip the feature tour prompt."), action: onSkip)
                        .buttonStyle(.bordered)
                    Button(action: onAccept) {
                        Label(String(localized: "feature_tour.take_the_tour", defaultValue: "Take the Tour", bundle: .module, comment: "Primary action to begin the feature walkthrough."), systemImage: "arrow.right")
                    }
                    .buttonStyle(.borderedProminent)
                }
            }
