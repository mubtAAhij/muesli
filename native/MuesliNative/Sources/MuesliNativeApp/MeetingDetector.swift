...
final class MeetingDetector {
    ...
    func currentDetection(_ signals: MeetingSignals, now: Date = Date()) -> MeetingDetection? {
        ...
        if let cal = signals.calendarEvent {
            let (appName, _, sourceID) = bestSnapshot(signals.activitySnapshot) ?? bestApp(from: signals.runningApps)
            return MeetingDetection(appName: appName ?? String(localized: "meeting_detector.default_title.meeting", defaultValue: "Meeting", bundle: .module, comment: "Fallback title used when a detected meeting has no explicit title"), meetingTitle: cal.title, sourceID: sourceID)
        }
    ...
    func evaluate(_ signals: MeetingSignals, now: Date = Date()) -> MeetingDetection? {
        ...
        if signals.cameraActive, signals.micActive, !detectedKeys.contains("camera") {
            ...
            if appName != nil {
                ...
                let title = signals.calendarEvent?.title
                return MeetingDetection(appName: appName ?? String(localized: "meeting_detector.default_title.meeting", defaultValue: "Meeting", bundle: .module, comment: "Fallback title used when a detected meeting has no explicit title"), meetingTitle: title, sourceID: sourceID)
            }
        }
