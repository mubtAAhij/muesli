...
final class MeetingDetector {
    ...
    static let dedicatedApps: [String: String] = [
        ...
        "com.microsoft.teams2": String(localized: "meeting_detector.platform.teams", defaultValue: "Teams", bundle: .module, comment: "Meeting platform name for Microsoft Teams."),
        "com.microsoft.teams": String(localized: "meeting_detector.platform.teams", defaultValue: "Teams", bundle: .module, comment: "Meeting platform name for Microsoft Teams."),
        "com.tinyspeck.slackmacgap": "Slack",
        "com.webex.meetingmanager": String(localized: "meeting_detector.platform.webex", defaultValue: "Webex", bundle: .module, comment: "Meeting platform name for Cisco Webex."),
        "com.cisco.webexmeetingsapp": String(localized: "meeting_detector.platform.webex", defaultValue: "Webex", bundle: .module, comment: "Meeting platform name for Cisco Webex."),
    ...
    func currentDetection(_ signals: MeetingSignals, now: Date = Date()) -> MeetingDetection? {
        ...
        if let cal = signals.calendarEvent {
            let (appName, _, sourceID) = bestSnapshot(signals.activitySnapshot) ?? bestApp(from: signals.runningApps)
            return MeetingDetection(appName: appName ?? String(localized: "meeting_detector.default_title.meeting", defaultValue: "Meeting", bundle: .module, comment: "Default meeting title when no specific title is available."), meetingTitle: cal.title, sourceID: sourceID)
        }
    ...
    func evaluate(_ signals: MeetingSignals, now: Date = Date()) -> MeetingDetection? {
        ...
        if signals.cameraActive, signals.micActive, !detectedKeys.contains("camera") {
            ...
            if appName != nil {
                ...
                let title = signals.calendarEvent?.title
                return MeetingDetection(appName: appName ?? String(localized: "meeting_detector.default_title.meeting", defaultValue: "Meeting", bundle: .module, comment: "Default meeting title when no specific title is available."), meetingTitle: title, sourceID: sourceID)
            }
        }
