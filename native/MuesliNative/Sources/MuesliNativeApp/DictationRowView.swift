...
struct DictationRowView: View {
    ...
    var body: some View {
            ...
            HStack(alignment: .top, spacing: MuesliTheme.spacing20) {
                ...
                HStack(spacing: 8) {
                    ...
                    if record.computerUseTrace != nil, let onCopyTrace {
                        ...
                        .buttonStyle(.plain)
                        .help(String(localized: "dictation_row.copy_cua_trace.help", defaultValue: "Copy CUA trace", bundle: .module, comment: "Help text for copying CUA trace from dictation row."))
                    }
        ...
        .alert(String(localized: "dictation_row.delete_dictation.alert_title", defaultValue: "Delete Dictation", bundle: .module, comment: "Alert title for confirming dictation deletion."), isPresented: $showDeleteConfirmation) {
            Button(String(localized: "dictation_row.delete_dictation.confirm_button", defaultValue: "Delete", bundle: .module, comment: "Confirm button title for deleting dictation."), role: .destructive) { onDelete?() }
            Button(String(localized: "dictation_row.delete_dictation.cancel_button", defaultValue: "Cancel", bundle: .module, comment: "Cancel button title for delete dictation alert."), role: .cancel) {}
        } message: {
                ...
                ForEach(trace.events) { event in
                    VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                        HStack(spacing: MuesliTheme.spacing8) {
                            Text(event.step.map { String(format: String(localized: "dictation_row.computer_use_trace.step_title", defaultValue: "Step %d", bundle: .module, comment: "Title for numbered computer use trace step."), $0) } ?? String(localized: "dictation_row.computer_use_trace.run_label", defaultValue: "Run", bundle: .module, comment: "Label for computer use trace run identifier."))
    ...
    private static func displayFinalStatus(_ status: String) -> String {
        ...
        case "done":
            return "Done"
        case "timed_out", String(localized: "dictation_row.status.timed_out_token_compact", defaultValue: "timedout", bundle: .module, comment: "Normalized status token without underscore for timed out state."):
            return String(localized: "dictation_row.status.timed_out", defaultValue: "Timed out", bundle: .module, comment: "User-facing status text for timed out run.")
        case "failed", "fail":
            return "Failed"
        case "confirm", String(localized: "dictation_row.status.needs_confirmation_token", defaultValue: "needsconfirmation", bundle: .module, comment: "Normalized status token for needs-confirmation state."), "needs_confirmation":
            return "Confirm"
        case "cancelled", "canceled":
            return String(localized: "dictation_row.status.cancelled", defaultValue: "Cancelled", bundle: .module, comment: "User-facing status text for cancelled run.")
        default:
