...
struct DictationRowView: View {
    ...
    var body: some View {
            ...
            HStack(alignment: .top, spacing: MuesliTheme.spacing20) {
                ...
                HStack(spacing: 8) {
                    ...
                    if record.computerUseTrace != nil, let onCopyTrace {
                        ...
                        .buttonStyle(.plain)
                        .help(String(localized: "dictation_row.copy_cua_trace.help", defaultValue: "Copy CUA trace", comment: "Context menu/help label to copy Computer Use Agent trace."))
                    }
        ...
        .alert(String(localized: "dictation_row.delete_dictation.alert_title", defaultValue: "Delete Dictation", comment: "Alert title for deleting a dictation."), isPresented: $showDeleteConfirmation) {
            Button(String(localized: "dictation_row.delete_dictation.confirm_button", defaultValue: "Delete", comment: "Destructive confirmation button for deleting a dictation."), role: .destructive) { onDelete?() }
            Button(String(localized: "dictation_row.delete_dictation.cancel_button", defaultValue: "Cancel", comment: "Cancel button in delete dictation confirmation alert."), role: .cancel) {}
        } message: {
            Text(String(localized: "dictation_row.delete_dictation.alert_message", defaultValue: "Are you sure you want to delete this dictation? This cannot be undone.", comment: "Warning message in delete dictation confirmation alert."))
        }
    }
                ...
                ForEach(trace.events) { event in
                    VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                        HStack(spacing: MuesliTheme.spacing8) {
                            Text(event.step.map { String(format: String(localized: "dictation_row.computer_use_trace.step_title", defaultValue: "Step %@", comment: "Title for a numbered CUA trace step."), "\($0)") } ?? String(localized: "dictation_row.computer_use_trace.run_label", defaultValue: "Run", comment: "Label for run identifier in CUA trace step title."))
    ...
    private static func displayFinalStatus(_ status: String) -> String {
        ...
        case "done":
            return "Done"
        case "timed_out", "timedout":
            return String(localized: "dictation_row.status.timed_out", defaultValue: "Timed out", comment: "Status label for dictation action timeout.")
        case "failed", "fail":
            return "Failed"
        case "confirm", "needsconfirmation", "needs_confirmation":
            return "Confirm"
        case "cancelled", "canceled":
            return String(localized: "dictation_row.status.cancelled", defaultValue: "Cancelled", comment: "Status label for cancelled dictation action.")
        default:
