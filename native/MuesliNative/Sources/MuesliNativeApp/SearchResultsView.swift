...
struct SearchResultsView: View {
    ...
    private var header: some View {
        HStack(spacing: 0) {
            ...
            Button {
                ...
                Text(String(localized: "search_results.clear", defaultValue: "Clear", bundle: .module, comment: "Button label to clear current search filters or query."))
    ...
    private var tabContent: some View {
        ...
        case .dictations:
            if appState.searchResultDictations.isEmpty {
                noResultsForTab(String(localized: "search_results.no_results.subject.dictations", defaultValue: "dictations", bundle: .module, comment: "No-results subject noun for dictations."))
            } else {
                ScrollView {
        ...
        case .meetings:
            if appState.searchResultMeetings.isEmpty {
                noResultsForTab(String(localized: "search_results.no_results.subject.meetings", defaultValue: "meetings", bundle: .module, comment: "No-results subject noun for meetings."))
            } else {
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 0) {
    ...
    private var emptyState: some View {
        VStack(spacing: MuesliTheme.spacing12) {
            ...
            Text(String(format: String(localized: "search_results.no_results.for_query", defaultValue: "No results for \"%@\"", bundle: .module, comment: "No-results message including the search query."), "\(appState.searchQuery)"))
    ...
    private func noResultsForTab(_ name: String) -> some View {
        VStack(spacing: MuesliTheme.spacing8) {
            Text(String(format: String(localized: "search_results.no_results.no_matching_name", defaultValue: "No matching %@", bundle: .module, comment: "No-results message including selected subject name."), "\(name)"))
                .font(MuesliTheme.body())
                .foregroundStyle(MuesliTheme.textSecondary)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        ...
        HStack(alignment: .top, spacing: MuesliTheme.spacing12) {
            ...
            HStack(spacing: 8) {
                if record.computerUseTrace != nil, let onCopyTrace {
                    ...
                    .buttonStyle(.plain)
                    .help(String(localized: "search_results.copy_cua_trace.help", defaultValue: "Copy CUA trace", bundle: .module, comment: "Help text for action that copies CUA trace."))
                }
...
private struct SearchMeetingRow: View {
    ...
    private func formatDuration(_ seconds: Double) -> String {
        ...
        if rounded < 60 { return String(format: String(localized: "search_results.duration.seconds", defaultValue: "%@s", bundle: .module, comment: "Duration label in seconds."), "\(rounded)") }
        let m = rounded / 60
        let s = rounded % 60
        if m < 60 { return s > 0 ? String(format: String(localized: "search_results.duration.minutes_seconds", defaultValue: "%@m %@s", bundle: .module, comment: "Duration label in minutes and seconds."), "\(m)", "\(s)") : String(format: String(localized: "search_results.duration.minutes", defaultValue: "%@m", bundle: .module, comment: "Duration label in minutes only."), "\(m)") }
        let h = m / 60
        let rm = m % 60
        return rm > 0 ? String(format: String(localized: "search_results.duration.hours_minutes", defaultValue: "%@h %@m", bundle: .module, comment: "Duration label in hours and minutes."), "\(h)", "\(rm)") : String(format: String(localized: "search_results.duration.hours", defaultValue: "%@h", bundle: .module, comment: "Duration label in hours only."), "\(h)")
    }
}
