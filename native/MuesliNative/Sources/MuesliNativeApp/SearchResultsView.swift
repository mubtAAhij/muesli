...
private enum SearchTab: String, CaseIterable {
    case dictations = String(localized: "search_results.tab.dictations", defaultValue: "Dictations", comment: "")
    case meetings = String(localized: "search_results.tab.meetings", defaultValue: "Meetings", comment: "")
}
...
struct SearchResultsView: View {
    ...
    private var header: some View {
        HStack(spacing: 0) {
            ...
            Button {
                ...
                Text(String(localized: "search_results.clear", defaultValue: "Clear", comment: ""))
    ...
    private var tabContent: some View {
        ...
        case .dictations:
            if appState.searchResultDictations.isEmpty {
                noResultsForTab(String(localized: "search_results.no_results.subject.dictations", defaultValue: "dictations", comment: ""))
            } else {
                ScrollView {
        ...
        case .meetings:
            if appState.searchResultMeetings.isEmpty {
                noResultsForTab(String(localized: "search_results.no_results.subject.meetings", defaultValue: "meetings", comment: ""))
            } else {
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 0) {
    ...
    private var emptyState: some View {
        VStack(spacing: MuesliTheme.spacing12) {
            ...
            Text(String(format: String(localized: "search_results.no_results.for_query", defaultValue: "No results for \"%@\"", comment: ""), "\(appState.searchQuery)"))
    ...
    private func noResultsForTab(_ name: String) -> some View {
        VStack(spacing: MuesliTheme.spacing8) {
            Text(String(format: String(localized: "search_results.no_matching_subject", defaultValue: "No matching %@", comment: ""), "\(name)"))
                .font(MuesliTheme.body())
                .foregroundStyle(MuesliTheme.textSecondary)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        ...
        HStack(alignment: .top, spacing: MuesliTheme.spacing12) {
            ...
            HStack(spacing: 8) {
                if record.computerUseTrace != nil, let onCopyTrace {
                    ...
                    .buttonStyle(.plain)
                    .help(String(localized: "search_results.copy_cua_trace.help", defaultValue: "Copy CUA trace", comment: ""))
                }
...
private struct SearchMeetingRow: View {
    ...
    private func formatDuration(_ seconds: Double) -> String {
        ...
        if rounded < 60 { return String(format: String(localized: "search_results.duration.seconds", defaultValue: "%ds", comment: ""), rounded) }
        let m = rounded / 60
        let s = rounded % 60
        if m < 60 { return s > 0 ? String(format: String(localized: "search_results.duration.minutes_seconds", defaultValue: "%dm %ds", comment: ""), m, s) : String(format: String(localized: "search_results.duration.minutes", defaultValue: "%dm", comment: ""), m) }
        let h = m / 60
        let rm = m % 60
        return rm > 0 ? String(format: String(localized: "search_results.duration.hours_minutes", defaultValue: "%dh %dm", comment: ""), h, rm) : String(format: String(localized: "search_results.duration.hours", defaultValue: "%dh", comment: ""), h)
    }
}
