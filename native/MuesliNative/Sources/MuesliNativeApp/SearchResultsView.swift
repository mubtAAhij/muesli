...
struct SearchResultsView: View {
    ...
    private var header: some View {
        HStack(spacing: 0) {
            ...
            Button {
                ...
                Text(String(localized: "search_results.clear", defaultValue: "Clear", bundle: .module, comment: "Button title to clear search query."))
    ...
    private var tabContent: some View {
        ...
        case .dictations:
            if appState.searchResultDictations.isEmpty {
                noResultsForTab(String(localized: "search_results.no_results.subject.dictations", defaultValue: "dictations", bundle: .module, comment: "No-results subject label for dictations."))
            } else {
                ScrollView {
        ...
        case .meetings:
            if appState.searchResultMeetings.isEmpty {
                noResultsForTab(String(localized: "search_results.no_results.subject.meetings", defaultValue: "meetings", bundle: .module, comment: "No-results subject label for meetings."))
            } else {
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 0) {
    ...
    private var emptyState: some View {
        VStack(spacing: MuesliTheme.spacing12) {
            ...
            Text(String(format: String(localized: "search_results.no_results.for_query", defaultValue: "No results for \"%@\"", bundle: .module, comment: "No-results message showing the current search query."), "\(appState.searchQuery)"))
    ...
    private func noResultsForTab(_ name: String) -> some View {
        VStack(spacing: MuesliTheme.spacing8) {
            Text(String(format: String(localized: "search_results.no_results.no_matching_name", defaultValue: "No matching %@", bundle: .module, comment: "No-results message for missing matching subject name."), "\(name)"))
                .font(MuesliTheme.body())
                .foregroundStyle(MuesliTheme.textSecondary)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
...
private struct SearchMeetingRow: View {
    ...
    private func formatDuration(_ seconds: Double) -> String {
        ...
        if rounded < 60 { return String(format: String(localized: "search_results.duration.seconds", defaultValue: "%ds", bundle: .module, comment: "Compact duration format for seconds only."), rounded) }
        let m = rounded / 60
        let s = rounded % 60
        if m < 60 { return s > 0 ? String(format: String(localized: "search_results.duration.minutes_seconds", defaultValue: "%dm %ds", bundle: .module, comment: "Compact duration format for minutes and seconds."), m, s) : String(format: String(localized: "search_results.duration.minutes", defaultValue: "%dm", bundle: .module, comment: "Compact duration format for minutes only."), m) }
        let h = m / 60
        let rm = m % 60
        return rm > 0 ? String(format: String(localized: "search_results.duration.hours_minutes", defaultValue: "%dh %dm", bundle: .module, comment: "Compact duration format for hours and remaining minutes."), h, rm) : String(format: String(localized: "search_results.duration.hours", defaultValue: "%dh", bundle: .module, comment: "Compact duration format for hours only."), h)
    }
}
