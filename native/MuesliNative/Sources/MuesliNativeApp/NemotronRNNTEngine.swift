...
enum NemotronRNNTError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notLoaded: return String(localized: "nemotron_rnnt.error.models_not_loaded", defaultValue: "Nemotron models not loaded.", comment: "Error when Nemotron RNNT engine is used before models are loaded")
        case .downloadFailed(let m): return String(format: String(localized: "nemotron_rnnt.error.download_failed", defaultValue: "Download failed: %@", comment: "Error when model download fails with details"), "\(m)")
        case .preprocessingFailed(let m): return String(format: String(localized: "nemotron_rnnt.error.preprocessing_failed", defaultValue: "Preprocessing failed: %@", comment: "Error when RNNT preprocessing stage fails with details"), "\(m)")
        case .decodingFailed(let m): return String(format: String(localized: "nemotron_rnnt.error.decoding_failed", defaultValue: "Decoding failed: %@", comment: "Error when RNNT decoding stage fails with details"), "\(m)")
        }
    }
}
...
func nemotronTranscribeChunk(
    ...
    guard let mel = prepOutput.featureValue(for: "mel")?.multiArrayValue,
        ...
        throw NemotronRNNTError.preprocessingFailed(String(localized: "nemotron_rnnt.error.no_mel_output", defaultValue: "No mel output", comment: "Error when mel-spectrogram stage returns no output"))
    }
    ...
    guard let encoded = encOutput.featureValue(for: "encoded")?.multiArrayValue,
        ...
        throw NemotronRNNTError.decodingFailed(String(localized: "nemotron_rnnt.error.no_encoder_output", defaultValue: "No encoder output", comment: "Error when encoder stage returns no output"))
    }
    ...
    for t in 0..<numFrames {
        ...
        while maxSteps > 0 {
            ...
            guard let decoderOut = decOutput.featureValue(for: "decoder_out")?.multiArrayValue else {
                throw NemotronRNNTError.decodingFailed(String(localized: "nemotron_rnnt.error.no_decoder_output", defaultValue: "No decoder output", comment: "Error when decoder stage returns no output"))
            }
            ...
            guard let logits = jointOutput.featureValue(for: "logits")?.multiArrayValue else {
                throw NemotronRNNTError.decodingFailed(String(localized: "nemotron_rnnt.error.no_joint_logits", defaultValue: "No joint logits", comment: "Error when joint network stage returns no logits"))
            }
...
func nemotronLoadWavAsFloats(url: URL) throws -> [Float] {
    ...
    guard data.count > 44 else { throw NemotronRNNTError.decodingFailed(String(localized: "nemotron_rnnt.error.wav_too_small", defaultValue: "WAV too small", comment: "Error when provided WAV input is too short for processing")) }
