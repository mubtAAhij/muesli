...
final class StatusBarController: NSObject, NSMenuDelegate {
    ...
    private func rebuildMenu() {
        ...
        let recentItem = NSMenuItem(title: String(
            localized: "status-bar.menu.recent-dictations.title",
            defaultValue: "Recent Dictations",
            comment: ""
        ...
        if recentRows.isEmpty {
            let empty = NSMenuItem(title: String(
                localized: "status-bar.menu.no-dictations.placeholder",
                defaultValue: "No dictations yet",
                comment: ""
            ), action: nil, keyEquivalent: "")
        ...
        let backendItem = NSMenuItem(title: String(
            localized: "status-bar.menu.transcription-backend.title",
            defaultValue: "Transcription Backend",
            comment: ""
        ), action: nil, keyEquivalent: "")
        let backendMenu = NSMenu()
        for option in BackendOption.downloaded {
            let prefix = controller.selectedBackend == option ? "✓ " : ""
            let item = NSMenuItem(title: String(format: String(
                localized: "status-bar.menu.transcription-backend.option-title",
                defaultValue: "%@%@",
                comment: ""
            ), "\(prefix)", "\(option.label)"), action: #selector(MuesliController.selectBackendFromMenu(_:)), keyEquivalent: "")
            item.target = controller
        ...
        let meetingBackendItem = NSMenuItem(title: String(
            localized: "status-bar.menu.meetings-backend.title",
            defaultValue: "Meetings Backend",
            comment: ""
        ), action: nil, keyEquivalent: "")
        ...
        for option in MeetingSummaryBackendOption.all {
            ...
            let item = NSMenuItem(
                title: String(format: String(
                    localized: "status-bar.menu.meetings-backend.option-title",
                    defaultValue: "%@%@",
                    comment: ""
                ), "\(prefix)", "\(option.label)"),
    ...
    private func addUpcomingEventGroup(
        ...
        for event in displayedEvents {
            ...
            let item = NSMenuItem(
                title: String(format: String(
                    localized: "status-bar.menu.recent-dictations.entry-title",
                    defaultValue: "%@\n%@",
                    comment: ""
                ), "\(event.title)", "\(timeStr)"),
    ...
    private func checkForUpdatesItem() -> NSMenuItem {
        let item = NSMenuItem(
            title: String(
                localized: "status-bar.menu.check-for-updates.title",
