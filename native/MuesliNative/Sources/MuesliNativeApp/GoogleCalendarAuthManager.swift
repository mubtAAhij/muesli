...
enum GoogleCalendarAuthError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthenticated: return String(localized: "google_calendar_auth.error.not_signed_in", defaultValue: "Not signed in to Google Calendar", bundle: .module, comment: "Error when calendar action is attempted without signed-in Google account.")
        case .notAvailable: return String(localized: "google_calendar_auth.error.credentials_not_configured", defaultValue: "Google Calendar credentials not configured", bundle: .module, comment: "Error when required Google Calendar OAuth credentials are missing.")
        case .callbackTimeout: return String(localized: "google_calendar_auth.error.signin_timed_out", defaultValue: "Sign-in timed out — no response from browser", bundle: .module, comment: "Error when OAuth browser sign-in does not complete before timeout.")
        case .callbackMissingCode: return String(localized: "google_calendar_auth.error.missing_authorization_code", defaultValue: "OAuth callback missing authorization code", bundle: .module, comment: "Error when OAuth callback returns without authorization code.")
        case .callbackStateMismatch: return String(localized: "google_calendar_auth.error.state_mismatch", defaultValue: "OAuth state mismatch — possible CSRF attack", bundle: .module, comment: "Security error shown when OAuth state token validation fails.")
        case .tokenExchangeFailed(let msg): return String(format: String(localized: "google_calendar_auth.error.token_exchange_failed", defaultValue: "Token exchange failed: %@", bundle: .module, comment: "Error shown when exchanging authorization code for tokens fails."), "\(msg)")
        case .refreshFailed(let msg): return String(format: String(localized: "google_calendar_auth.error.token_refresh_failed", defaultValue: "Token refresh failed: %@", bundle: .module, comment: "Error shown when refreshing access token fails."), "\(msg)")
        case .portInUse: return String(localized: "google_calendar_auth.error.callback_port_in_use", defaultValue: "Callback port 1456 is already in use", bundle: .module, comment: "Error shown when local OAuth callback port is unavailable.")
        }
    }
}
...
final class GoogleCalendarAuthManager {
    ...
    private func exchangeCodeForTokens(
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let errorBody = String(data: data, encoding: .utf8) ?? String(localized: "google_calendar_auth.error.unknown", defaultValue: "unknown error", bundle: .module, comment: "Fallback error text when calendar auth error details are unavailable.")
            throw GoogleCalendarAuthError.tokenExchangeFailed(errorBody)
        }

        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let accessToken = json["access_token"] as? String else {
            throw GoogleCalendarAuthError.tokenExchangeFailed(String(localized: "google_calendar_auth.error.missing_access_token", defaultValue: "missing access_token in response", bundle: .module, comment: "Error shown when token exchange response lacks access token."))
        }
    ...
    private func refreshAccessToken(
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let errorBody = String(data: data, encoding: .utf8) ?? String(localized: "google_calendar_auth.error.unknown", defaultValue: "unknown error", bundle: .module, comment: "Fallback error text when token refresh error details are unavailable.")
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let accessToken = json["access_token"] as? String else {
            throw GoogleCalendarAuthError.refreshFailed(String(localized: "google_calendar_auth.error.missing_access_token_refresh", defaultValue: "missing access_token in refresh response", bundle: .module, comment: "Error shown when token refresh response lacks access token."))
        }
