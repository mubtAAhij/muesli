...
enum GoogleCalendarAuthError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthenticated: return String(localized: "google_calendar_auth.error.not_signed_in", defaultValue: "Not signed in to Google Calendar", comment: "Error when no Google Calendar account session is available.")
        case .notAvailable: return String(localized: "google_calendar_auth.error.credentials_not_configured", defaultValue: "Google Calendar credentials not configured", comment: "Error when OAuth client credentials are missing or unset.")
        case .callbackTimeout: return String(localized: "google_calendar_auth.error.signin_timed_out", defaultValue: "Sign-in timed out — no response from browser", comment: "Error when browser OAuth callback does not arrive in time.")
        case .callbackMissingCode: return String(localized: "google_calendar_auth.error.missing_authorization_code", defaultValue: "OAuth callback missing authorization code", comment: "Error when OAuth callback lacks authorization code parameter.")
        case .callbackStateMismatch: return String(localized: "google_calendar_auth.error.state_mismatch", defaultValue: "OAuth state mismatch — possible CSRF attack", comment: "Error when OAuth state parameter does not match expected value.")
        case .tokenExchangeFailed(let msg): return String(format: String(localized: "google_calendar_auth.error.token_exchange_failed", defaultValue: "Token exchange failed: %@", comment: "Error when exchanging authorization code for tokens fails."), "\(msg)")
        case .refreshFailed(let msg): return String(format: String(localized: "google_calendar_auth.error.token_refresh_failed", defaultValue: "Token refresh failed: %@", comment: "Error when refreshing OAuth access token fails."), "\(msg)")
        case .portInUse: return String(localized: "google_calendar_auth.error.callback_port_in_use", defaultValue: "Callback port 1456 is already in use", comment: "Error when local OAuth callback port is unavailable.")
        }
    }
}
...
final class GoogleCalendarAuthManager {
    ...
    private func exchangeCodeForTokens(
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let errorBody = String(data: data, encoding: .utf8) ?? String(localized: "google_calendar_auth.error.unknown", defaultValue: "unknown error", comment: "Fallback error message when no specific Google auth error is available.")
            throw GoogleCalendarAuthError.tokenExchangeFailed(errorBody)
        }

        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let accessToken = json["access_token"] as? String else {
            throw GoogleCalendarAuthError.tokenExchangeFailed(String(localized: "google_calendar_auth.error.missing_access_token", defaultValue: "missing access_token in response", comment: "Error when OAuth token response lacks access_token field."))
        }
        ...
        guard let refreshToken = json["refresh_token"] as? String, !refreshToken.isEmpty else {
            throw GoogleCalendarAuthError.tokenExchangeFailed(
                String(localized: "google_calendar_auth.error.no_refresh_token", defaultValue: "No refresh token received. Ensure access_type=offline and prompt=consent are set, or revoke access at https://myaccount.google.com/permissions and try again.", comment: "Error guidance when OAuth response does not include refresh token.")
            )
        }
    ...
    private func refreshAccessToken(
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let errorBody = String(data: data, encoding: .utf8) ?? String(localized: "google_calendar_auth.error.unknown", defaultValue: "unknown error", comment: "Fallback error message when no specific token refresh error is available.")
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let accessToken = json["access_token"] as? String else {
            throw GoogleCalendarAuthError.refreshFailed(String(localized: "google_calendar_auth.error.missing_access_token_refresh", defaultValue: "missing access_token in refresh response", comment: "Error when refresh-token response lacks access_token field."))
        }
