...
enum ComputerUseToolRegistry {
    ...
    static let definitions: [ComputerUseToolDefinition] = [
        ...
        definition(.getAppState, "Capture fresh app/window state: state_id, app/window identity, screenshot metadata/image for the planner, AX candidates, focused element, selected text, cursor, and app hints.", required: [], properties: [
            "app_bundle_id": .string(String(localized: "computer_use.tool_registry.capture.option.app_bundle_hint", defaultValue: "Optional app bundle to activate before capture.", bundle: .module, comment: "Hint text describing optional app bundle parameter before capture.")),
            "window_id": .integer(String(localized: "computer_use.tool_registry.capture.option.window_id_hint", defaultValue: "Optional window id hint.", bundle: .module, comment: "Hint text describing optional window id parameter for capture.")),
        ], risk: "safe read-only"),
        ...
        definition(.moveCursor, "Move the visible Muesli CUA cursor to a screenshot coordinate without clicking. Use this before uncertain coordinate clicks to show intent.", required: ["screenshot_id", "x", "y"], properties: [
            "screenshot_id": .string(String(localized: "computer_use.tool_registry.option.current_screenshot_id", defaultValue: "Current screenshot id.", bundle: .module, comment: "Hint text for current screenshot identifier parameter.")),
        ...
        definition(.clickPoint, "Click a screenshot coordinate when no AX target exists. Requires screenshot_id plus x/y from the latest state.", required: ["screenshot_id", "x", "y"], properties: [
            ...
            "clicks": .integer("1 for single click, 2 for double click."),
            "button": .string("left or right."),
            "label": .string(String(localized: "computer_use.tool_registry.option.human_target_label_trace_safety", defaultValue: "Human target label for trace and safety.", bundle: .module, comment: "Hint text for human-readable target label used in trace and safety checks.")),
        ...
        definition(.performSecondaryAction, "Perform an advertised AX action other than AXPress on an element from the latest get_app_state. Use only action_name values present on that element's action_names.", required: ["action_name"], properties: [
            "element_index": .integer(String(localized: "computer_use.tool_registry.option.temporary_element_index_latest_state", defaultValue: "Temporary element index from the latest state.", bundle: .module, comment: "Hint text for temporary element index from latest UI state.")),
            "element_id": .string(String(localized: "computer_use.tool_registry.option.temporary_element_id_latest_state", defaultValue: "Temporary element id from the latest state.", bundle: .module, comment: "Hint text for temporary element id from latest UI state.")),
        ...
        definition(.setValue, "Set an AX element value by element_index/element_id from the latest state.", required: ["value"], properties: [
            ...
            "label": .string(String(localized: "computer_use.tool_registry.option.human_target_label_trace", defaultValue: "Human target label for trace.", bundle: .module, comment: "Hint text for human-readable target label used in trace output.")),
        ...
        definition(.typeText, "Type literal text using keyboard input. Prefer app_name/app_bundle_id and, when available, element_index/element_id so Muesli can activate the app and focus the editable target before typing.", required: ["text"], properties: [
            "app_name": .string(String(localized: "computer_use.tool_registry.option.target_app_name_example_notes", defaultValue: "Optional target app name, for example Notes.", bundle: .module, comment: "Hint text for optional target app name parameter.")),
            "app_bundle_id": .string(String(localized: "computer_use.tool_registry.option.target_app_bundle_identifier_example_notes", defaultValue: "Optional target app bundle identifier, for example com.apple.Notes.", bundle: .module, comment: "Hint text for optional target app bundle identifier parameter.")),
            "element_index": .integer(String(localized: "computer_use.tool_registry.option.temporary_editable_element_index_latest_state", defaultValue: "Optional temporary editable element index from the latest state.", bundle: .module, comment: "Hint text for optional temporary editable element index parameter.")),
            "element_id": .string(String(localized: "computer_use.tool_registry.option.temporary_editable_element_id_latest_state", defaultValue: "Optional temporary editable element id from the latest state.", bundle: .module, comment: "Hint text for optional temporary editable element id parameter.")),
        ...
        definition(.pressKey, "Press one key with optional modifiers.", required: ["key"], properties: [
            ...
            "modifiers": .array("Optional modifiers.", item: .string(String(localized: "computer_use.tool_registry.option.modifier", defaultValue: "Modifier", bundle: .module, comment: "Label or hint for keyboard modifier option."), enumValues: ComputerUseKeyModifier.allCases.map(\.rawValue))),
        ...
        definition(.hotkey, "Alias for press_key used for keyboard shortcuts.", required: ["key"], properties: [
            "key": .string("Key name."),
            "modifiers": .array("Required or optional modifiers.", item: .string(String(localized: "computer_use.tool_registry.option.modifier", defaultValue: "Modifier", bundle: .module, comment: "Label or hint for keyboard modifier option."), enumValues: ComputerUseKeyModifier.allCases.map(\.rawValue))),
        ], risk: "confirmation for Cmd-Q and Cmd-W"),
        ...
        definition(.activateBrowserTab, "Activate a browser tab by window_index and tab_index.", required: ["app_bundle_id", "window_index", "tab_index"], properties: [
            "app_bundle_id": .string(String(localized: "computer_use.tool_registry.option.browser_bundle_identifier_currently_chrome", defaultValue: "Browser bundle identifier, currently com.google.Chrome.", bundle: .module, comment: "Hint text for browser bundle identifier option.")),
        ...
        definition(.pageGetText, "Read visible/body text from a Chrome tab using read-only Apple Events JavaScript.", required: ["app_bundle_id"], properties: [
            "app_bundle_id": .string(String(localized: "computer_use.tool_registry.option.browser_bundle_identifier_currently_chrome", defaultValue: "Browser bundle identifier, currently com.google.Chrome.", bundle: .module, comment: "Hint text for browser bundle identifier option.")),
            "window_index": .integer(String(localized: "computer_use.tool_registry.option.browser_window_index_one_based", defaultValue: "Optional 1-based browser window index.", bundle: .module, comment: "Hint text for optional one-based browser window index.")),
            "tab_index": .integer(String(localized: "computer_use.tool_registry.option.tab_index_one_based", defaultValue: "Optional 1-based tab index.", bundle: .module, comment: "Hint text for optional one-based tab index.")),
        ], risk: "safe read-only"),
