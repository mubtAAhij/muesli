...
enum MeetingContactCreatorError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .nameRequired:
            return String(localized: "meeting_contact_creator.error.missing_name", defaultValue: "Add a first or last name before saving this contact.", bundle: .module, comment: "Validation error shown when saving a contact without a name")
        case .accessDenied:
            return String(localized: "meeting_contact_creator.error.contacts_permission_required", defaultValue: "Muesli does not have permission to add contacts. Enable Contacts access in System Settings.", bundle: .module, comment: "Error shown when contacts permission is missing while creating contact")
        case .missingIdentifier:
            return String(localized: "meeting_contact_creator.error.missing_contact_identifier", defaultValue: "Apple Contacts saved the person without returning an identifier. Try choosing them from Contacts instead.", bundle: .module, comment: "Error shown when Contacts framework saves contact but does not return identifier")
        }
    }
}
