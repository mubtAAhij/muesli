...
struct SettingsView: View {
    private enum PendingDataDestruction {
        ...
        var title: String {
            switch self {
            case .dictations:
                return String(localized: "settings.clear_history.dictations.title", defaultValue: "Clear dictation history?", comment: "")
            case .meetings:
                return String(localized: "settings.clear_history.meetings.title", defaultValue: "Clear meeting history?", comment: "")
            }
        }
        ...
        var message: String {
            switch self {
            case .dictations:
                return String(localized: "settings.clear_history.dictations.message", defaultValue: "This will permanently remove all saved dictations. This cannot be undone.", comment: "")
            case .meetings:
                return String(localized: "settings.clear_history.meetings.message", defaultValue: "This will permanently remove all saved meetings, notes, transcripts, and retained audio recordings. This cannot be undone.", comment: "")
            }
        }
        ...
        var confirmLabel: String {
            switch self {
            case .dictations:
                return String(localized: "settings.clear_history.dictations.confirm", defaultValue: "Clear Dictations", comment: "")
            case .meetings:
                return String(localized: "settings.clear_history.meetings.confirm", defaultValue: "Clear Meetings", comment: "")
            }
        }
    }
    ...
    private var meetingLiveTranscriptDescription: String {
        ...
        guard appState.config.enableLiveStreamingPartials,
              downloadedMeetingLiveCaptionBackends.contains(selected) else {
            return String(localized: "settings.meeting_live_transcript.description.completed_only", defaultValue: "Shows completed transcript segments only.", comment: "")
        }
        if usesUnifiedMeetingTranscript {
            return String(localized: "settings.meeting_live_transcript.description.live_and_final", defaultValue: "Creates the live and final transcript.", comment: "")
        }
        return String(localized: "settings.meeting_live_transcript.description.low_latency_preview", defaultValue: "Adds a low-latency preview.", comment: "")
    }
    ...
    private var selectedMeetingBackendLabel: String {
        ...
        return meetingBackendOptions.first?.label ?? String(localized: "settings.meeting_backend.no_downloaded_models", defaultValue: "No downloaded models", comment: "")
    }
    ...
    private var cleanupBackendDescription: String {
        if appState.selectedPostProcessorBackend == .local {
            return downloadedPostProcOptions.isEmpty
                ? String(localized: "settings.cleanup_backend.description.download_model", defaultValue: "Download a cleanup model from Models to refine dictations on this Mac.", comment: "")
                : String(localized: "settings.cleanup_backend.description.refines_on_mac", defaultValue: "Refines dictated text on this Mac.", comment: "")
        }
        if appState.selectedPostProcessorBackend == .gemma4LiteRT {
            return Gemma4LiteRTModelStore.isAvailableLocally()
                ? String(localized: "settings.cleanup_backend.description.gemma4_refinement", defaultValue: "Uses the downloaded Gemma 4 model to refine dictated text on this Mac.", comment: "")
                : String(localized: "settings.cleanup_backend.description.download_gemma4_for_cleanup", defaultValue: "Download Gemma 4 E2B from Models to use it for cleanup.", comment: "")
        }
        return String(format: String(localized: "settings.cleanup_backend.description.sends_to_backend_with_latency", defaultValue: "Sends dictated text to %@ and may add latency.", comment: ""), "\(appState.selectedPostProcessorBackend.label)")
    }
    ...
    private var selectedDictationMicrophoneLabel: String {
        ...
        return dictationMicrophoneOptions.first(where: { $0.uid == selectedUID })?.label ?? String(localized: "settings.dictation_microphone.automatic", defaultValue: "Automatic", comment: "")
    }
    ...
    private var selectedMeetingMicrophoneLabel: String {
        ...
        return meetingMicrophoneOptions.first(where: { $0.uid == selectedUID })?.label ?? String(localized: "settings.meeting_microphone.automatic", defaultValue: "Automatic", comment: "")
    }

    private func microphoneOptions(selectedUID: String?) -> [MicrophoneOption] {
        var options = [MicrophoneOption(uid: nil, label: String(localized: "settings.microphone.option.automatic", defaultValue: "Automatic", comment: ""))]
        ...
        if let selectedUID, !options.contains(where: { $0.uid == selectedUID }) {
            options.append(MicrophoneOption(uid: selectedUID, label: String(localized: "settings.microphone.option.selected_unavailable", defaultValue: "Selected microphone unavailable", comment: "")))
        }
        return options
    }
        ...
        ScrollViewReader { scrollProxy in
            ScrollView {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
                    Text(String(localized: "settings.title", defaultValue: "Settings", comment: ""))
            ...
            .alert(
                pendingDataDestruction?.title ?? String(localized: "settings.alert.confirm_destructive_action.title", defaultValue: "Confirm Destructive Action", comment: ""),
                isPresented: Binding(
                    get: { pendingDataDestruction != nil },
                    set: { if !$0 { pendingDataDestruction = nil } }
                )
            ) {
                Button(String(localized: "settings.action.cancel", defaultValue: "Cancel", comment: ""), role: .cancel) {
                    pendingDataDestruction = nil
                }
                Button(pendingDataDestruction?.confirmLabel ?? String(localized: "settings.action.delete", defaultValue: "Delete", comment: ""), role: .destructive) {
            ...
            .alert(
                String(localized: "settings.alert.enable_accessibility.title", defaultValue: "Enable Accessibility?", comment: ""),
                isPresented: $isShowingDictionaryAccessibilityPrompt
            ) {
                Button(String(localized: "settings.alert.action.cancel", defaultValue: "Cancel", comment: ""), role: .cancel) {
                    controller.cancelDictionaryCorrectionAccessibilityEnableRequest()
                }
                Button(String(localized: "settings.alert.action.enable", defaultValue: "Enable", comment: "")) {
                    controller.requestDictionaryCorrectionAccessibilityEnable()
                }
            } message: {
                Text(String(localized: "settings.alert.enable_accessibility.message", defaultValue: "Dictionary suggestions briefly read focused app text via Accessibility after dictation. Grant access, then relaunch Muesli to turn suggestions on.", comment: ""))
            }
    ...
    private static let accentPresets: [(hex: String, name: String)] = [
        ("2563eb", String(localized: "settings.accent_presets.blue", defaultValue: "Blue", comment: "")),
        ("ef4444", String(localized: "settings.accent_presets.red", defaultValue: "Red", comment: "")),
        ("f59e0b", String(localized: "settings.accent_presets.amber", defaultValue: "Amber", comment: "")),
        ("10b981", String(localized: "settings.accent_presets.green", defaultValue: "Green", comment: "")),
        ("8b5cf6", String(localized: "settings.accent_presets.purple", defaultValue: "Purple", comment: "")),
        ("ec4899", String(localized: "settings.accent_presets.pink", defaultValue: "Pink", comment: "")),
        ("1e1e2e", String(localized: "settings.accent_presets.dark", defaultValue: "Dark", comment: "")),
    ]

    private func screenContextDescription(includesScreenOCR: Bool) -> String {
        if !accessibilityGranted {
            return String(localized: "settings.screen_context.description.grant_accessibility_then_toggle", defaultValue: "Grant Accessibility, then toggle again if needed.", comment: "")
        }
        if includesScreenOCR, !screenRecordingGranted {
            return String(localized: "settings.screen_context.description.app_text_plus_screen_recording_ocr", defaultValue: "Adds nearby app text for post-processing. Screen Recording enables OCR context.", comment: "")
        }
        if includesScreenOCR {
            return String(localized: "settings.screen_context.description.app_text_and_ocr", defaultValue: "Adds nearby app text and OCR context.", comment: "")
        }
        return String(localized: "settings.screen_context.description.app_text_only", defaultValue: "Adds nearby app text for post-processing.", comment: "")
    }

    private var dictationOCRContextDescription: String {
        if !appState.config.enableScreenContext {
            return String(localized: "settings.dictation_ocr_context.description.turn_on_app_context_first", defaultValue: "Turn on App context first.", comment: "")
        }
        if !screenRecordingGranted {
            return String(localized: "settings.dictation_ocr_context.description.grant_screen_recording", defaultValue: "Grant Screen Recording to add frontmost-window OCR text.", comment: "")
        }
        return String(localized: "settings.dictation_ocr_context.description.ocr_text_cloud_cleanup_notice", defaultValue: "Adds frontmost-window OCR text. Cloud cleanup may send this text to the selected provider.", comment: "")
    }
    ...
    private var dictationOCRContextRow: some View {
        ...
        HStack(alignment: .top, spacing: 20) {
            VStack(alignment: .leading, spacing: 6) {
                Text(String(localized: "settings.dictation.screen_ocr_context", defaultValue: "Screen OCR context", comment: ""))
                    .font(MuesliTheme.body())
                    .foregroundStyle(MuesliTheme.textPrimary)
    ...
    private let customIndicatorPositionLabel = String(localized: "settings.indicator_position.custom_drag_to_reposition", defaultValue: "Custom (drag to reposition)", comment: "")
    ...
    private var generalSettingsPane: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
            settingsSection(String(localized: "settings.section.general", defaultValue: "General", comment: "")) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing8) {
                    settingsRow(String(localized: "settings.general.launch_at_login", defaultValue: "Launch at login", comment: "")) {
                ...
                settingsRow(String(localized: "settings.general.open_dashboard_on_launch", defaultValue: "Open dashboard on launch", comment: "")) {
            ...
            settingsSection(String(localized: "settings.section.data", defaultValue: "Data", comment: "")) {
                HStack(spacing: MuesliTheme.spacing12) {
                    actionButton(String(localized: "settings.data.clear_dictation_history", defaultValue: "Clear dictation history", comment: ""), role: .destructive) {
                        pendingDataDestruction = .dictations
                    }
                    actionButton(String(localized: "settings.data.clear_meeting_history", defaultValue: "Clear meeting history", comment: ""), role: .destructive) {
                        pendingDataDestruction = .meetings
                    }
                    .disabled(controller.isMeetingRecording())
                    .help(String(localized: "settings.general.help.stop_recording_before_clear_history", defaultValue: "Stop the current meeting recording before clearing meeting history.", comment: ""))
                }
            }
        }
    }
    ...
    private var launchAtLoginApprovalPrompt: some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            Text(String(localized: "settings.launch_at_login.requires_approval_system_settings", defaultValue: "Requires approval in System Settings", comment: ""))
                .font(MuesliTheme.caption())
                .foregroundStyle(MuesliTheme.textTertiary)
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(String(localized: "settings.launch_at_login.action.open", defaultValue: "Open", comment: ""))
                }
            }
            .buttonStyle(.plain)
            ...
            .help(String(localized: "settings.launch_at_login.help.open_login_items_system_settings", defaultValue: "Open Login Items in System Settings", comment: ""))
        }
    ...
    private var syncSettingsPane: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
            settingsSection(String(localized: "settings.sync.icloud_text_sync.section_title", defaultValue: "iCloud Text Sync", comment: "")) {
                settingsRow(String(localized: "settings.sync.private_icloud_sync", defaultValue: "Private iCloud sync", comment: "")) {
                    settingsSwitch(isOn: appState.config.iCloudSyncEnabled) { newValue in
                        controller.setICloudSyncEnabledFromSettings(newValue)
                    }
                }
                settingsDescription(String(localized: "settings.sync.description.private_icloud_scope", defaultValue: "Sync dictation text, meeting transcripts, notes, summaries, and manual notes with Muesli for iPhone through your private iCloud account. Audio recordings are never synced.", comment: ""))
                ...
                HStack(spacing: MuesliTheme.spacing12) {
                    VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                        ...
                        if let lastSyncedText = syncLastSyncedText {
                            Text(String(format: String(localized: "settings.sync.last_synced", defaultValue: "Last synced: %@", comment: ""), "\(lastSyncedText)"))
                                .font(MuesliTheme.caption())
                                .foregroundStyle(MuesliTheme.textTertiary)
                        }
                    ...
                    actionButton(String(localized: "settings.sync.action.sync_now", defaultValue: "Sync now", comment: ""), systemImage: "arrow.triangle.2.circlepath") {
            ...
            settingsSection(String(localized: "settings.sync.iphone_bridge.section_title", defaultValue: "iPhone Bridge", comment: "")) {
                settingsRow(String(localized: "settings.sync.iphone_bridge.show_ios_companion_prompt", defaultValue: "Show iOS companion prompt", comment: "")) {
                ...
                settingsDescription(String(localized: "settings.sync.iphone_bridge.description.keep_timeline_card", defaultValue: "Keep the timeline bridge card available while users connect Muesli on iPhone.", comment: ""))

                Divider().background(MuesliTheme.surfaceBorder)

                HStack(spacing: MuesliTheme.spacing12) {
                    VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                        Text(String(localized: "settings.sync.muesli_for_iphone", defaultValue: "Muesli for iPhone", comment: ""))
                            .font(MuesliTheme.body())
                            .foregroundStyle(MuesliTheme.textPrimary)
                        Text(String(localized: "settings.sync.iphone_description", defaultValue: "Use iPhone for offline meetings, keyboard dictation, and private iCloud text sync with this Mac.", comment: ""))
                            .font(MuesliTheme.caption())
                            .foregroundStyle(MuesliTheme.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                    Spacer(minLength: MuesliTheme.spacing16)
                    actionButton(String(localized: "settings.sync.action.open_ios_app_page", defaultValue: "Open iOS app page", comment: "")) {
    ...
    private var syncStatusText: String {
        if !appState.config.iCloudSyncEnabled {
            return String(localized: "settings.sync.status.off_turn_on_bridge", defaultValue: "Sync is off. Turn it on to bridge this Mac with Muesli for iPhone.", comment: "")
        }
        return appState.iCloudSyncStatus ?? String(localized: "settings.sync.status.private_icloud_ready", defaultValue: "Private iCloud text sync is ready.", comment: "")
    }
    ...
    private var syncLinkedDeviceText: String? {
        ...
        if let remoteDeviceName = appState.iCloudBridgeCompanionDeviceName {
            if let platform = appState.iCloudBridgeRemoteDevicePlatform {
                return String(format: String(localized: "settings.sync.linked_device.with_platform_and_name", defaultValue: "Linked %@: %@", comment: ""), "\(syncDeviceLabel(for: platform))", "\(remoteDeviceName)")
            }
            return String(format: String(localized: "settings.sync.linked_device.name_only", defaultValue: "Linked device: %@", comment: ""), "\(remoteDeviceName)")
        }
        return String(localized: "settings.sync.linked_device.none_iphone_yet", defaultValue: "No linked iPhone yet.", comment: "")
    }
    ...
    private func syncDeviceLabel(for platform: String) -> String {
        ...
        case "ios":
            return String(localized: "settings.sync.device_label.iphone", defaultValue: "iPhone", comment: "")
        case "ipados":
            return String(localized: "settings.sync.device_label.ipad", defaultValue: "iPad", comment: "")
        default:
            return platform
        }
    }
    ...
    private var dictationModelSettingsSection: some View {
        settingsSection(String(localized: "settings.section.speech_recognition", defaultValue: "Speech Recognition", comment: "")) {
            settingsRow(String(localized: "settings.speech_recognition.dictation_model", defaultValue: "Dictation model", comment: ""), controlWidth: meetingControlWidth) {
                settingsMenu(
            ...
            if !disabledDictationBackendLabels.isEmpty {
                settingsDescription(String(localized: "settings.speech_recognition.description.gemma4_unavailable_with_cleanup_backend", defaultValue: "Gemma 4 dictation is unavailable while Gemma 4 is the cleanup backend.", comment: ""))
            }
            if appState.selectedBackend.backend == BackendOption.cohereTranscribe.backend {
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.speech_recognition.cohere_language", defaultValue: "Cohere language", comment: ""), controlWidth: meetingControlWidth) {
                    cohereLanguageMenu
                }
            }
            if appState.selectedBackend.backend == BackendOption.indicASR.backend {
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.speech_recognition.indic_language", defaultValue: "Indic language", comment: ""), controlWidth: meetingControlWidth) {
                    indicLanguageMenu
                }
            }
            if appState.selectedBackend.supportsWhisperLanguageSelection {
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.speech_recognition.whisper_language", defaultValue: "Whisper language", comment: ""), controlWidth: meetingControlWidth) {
                    whisperLanguageMenu
                }
            }
        }
    ...
    private var meetingTranscriptionSettingsSection: some View {
        settingsSection(String(localized: "settings.section.transcription", defaultValue: "Transcription", comment: "")) {
            settingsRow(
                "Microphone",
                description: String(localized: "settings.transcription.microphone.description.only_affects_muesli", defaultValue: "Only affects Muesli. Changes apply immediately.", comment: ""),
            ...
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(
                String(localized: "settings.transcription.show_transcript_on_hover", defaultValue: "Show transcript on hover", comment: ""),
                description: String(localized: "settings.transcription.show_transcript_on_hover.description", defaultValue: "Show recent transcript beside the waveform.", comment: ""),
                controlWidth: meetingControlWidth
            ) {
            ...
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(
                String(localized: "settings.transcription.live_preview_model", defaultValue: "Live preview model", comment: ""),
                ...
                if !downloadedMeetingLiveCaptionBackends.isEmpty {
                    ...
                    Text(String(localized: "settings.meeting_transcription.download_from_models", defaultValue: "Download from Models", comment: ""))
            ...
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.transcription.final_transcript", defaultValue: "Final transcript", comment: ""), controlWidth: meetingControlWidth) {
                if usesUnifiedMeetingTranscript {
                    Text(String(format: String(localized: "settings.transcription.final_transcript.same_model_suffix", defaultValue: "%@ (same model)", comment: ""), "\(MeetingLiveCaptionBackend.nemotron35.label)"))
                        .font(MuesliTheme.body())
                        .foregroundStyle(MuesliTheme.textSecondary)
                        .frame(width: meetingControlWidth, alignment: .trailing)
                } else if meetingBackendOptions.isEmpty {
                    Text(String(localized: "settings.meeting_transcription.no_downloaded_models", defaultValue: "No downloaded models", comment: ""))
                        .font(MuesliTheme.body())
                        .foregroundStyle(MuesliTheme.textTertiary)
            ...
            if usesUnifiedMeetingTranscript {
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.transcription.language", defaultValue: "Language", comment: ""), controlWidth: meetingControlWidth) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.transcription.cohere_language", defaultValue: "Cohere language", comment: ""), controlWidth: meetingControlWidth) {
                    cohereLanguageMenu
                }
            } else if appState.selectedMeetingTranscriptionBackend.backend == BackendOption.indicASR.backend {
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.transcription.indic_language", defaultValue: "Indic language", comment: ""), controlWidth: meetingControlWidth) {
                    indicLanguageMenu
                }
            } else if appState.selectedMeetingTranscriptionBackend.supportsWhisperLanguageSelection {
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.transcription.whisper_language", defaultValue: "Whisper language", comment: ""), controlWidth: meetingControlWidth) {
                    whisperLanguageMenu
                }
            }
        }
    ...
    private var dictationCleanupSettingsSection: some View {
        settingsSection(String(localized: "settings.section.dictation_cleanup", defaultValue: "Dictation Cleanup", comment: "")) {
            settingsRow(
                String(localized: "settings.dictation_cleanup.cleanup_backend", defaultValue: "Cleanup backend", comment: ""),
            ...
            if !disabledCleanupBackendLabels.isEmpty {
                settingsDescription(String(localized: "settings.dictation_cleanup.description.gemma4_unavailable_with_dictation_model", defaultValue: "Gemma 4 cleanup is unavailable while Gemma 4 is the dictation model.", comment: ""))
            }
            if appState.selectedPostProcessorBackend == .local {
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.dictation_cleanup.cleanup_model", defaultValue: "Cleanup model", comment: ""), controlWidth: meetingControlWidth) {
                    if downloadedPostProcOptions.isEmpty {
                        compactActionButton(String(localized: "settings.dictation_cleanup.action.view_cleanup_models", defaultValue: "View cleanup models", comment: ""), systemImage: "arrow.right") {
                ...
                settingsRow(String(localized: "settings.dictation_cleanup.cleanup_model", defaultValue: "Cleanup model", comment: ""), controlWidth: meetingControlWidth) {
                    if Gemma4LiteRTModelStore.isAvailableLocally() {
                        Text(String(localized: "settings.dictation_cleanup.model.gemma_4_e2b_downloaded", defaultValue: "Gemma 4 E2B (Downloaded)", comment: ""))
                            .font(.system(size: 12, weight: .medium))
                            .foregroundStyle(MuesliTheme.textSecondary)
                            .frame(width: meetingControlWidth, alignment: .trailing)
                    } else {
                        compactActionButton(String(localized: "settings.dictation_cleanup.action.view_gemma_model", defaultValue: "View Gemma model", comment: ""), systemImage: "arrow.right") {
                            controller.showModels(category: .postProcessing)
                        }
                        .frame(width: meetingControlWidth, alignment: .trailing)
                    }
    ...
    private func hostedCleanupSettings(for backend: TranscriptCleanupBackendOption) -> some View {
        ...
        case .some(.chatGPT):
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.hosted_cleanup.account", defaultValue: "Account", comment: ""), controlWidth: meetingControlWidth) {
                chatGPTAccountControl(selectMeetingSummaryBackend: false)
            }
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.hosted_cleanup.cleanup_model", defaultValue: "Cleanup model", comment: ""), controlWidth: meetingControlWidth) {
                settingsModelMenu(
        ...
        case .some(.openAI):
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.hosted_cleanup.api_key", defaultValue: "API Key", comment: ""), controlWidth: meetingControlWidth) {
                PastableSecureField(
        ...
        case .some(.openRouter):
            ...
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.hosted_cleanup.model_preset", defaultValue: "Model preset", comment: ""), controlWidth: meetingControlWidth) {
                settingsModelMenu(
            ...
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.hosted_cleanup.custom_model_id", defaultValue: "Custom model ID", comment: ""), controlWidth: meetingControlWidth) {
        ...
        case .some(.ollama):
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.hosted_cleanup.ollama_url", defaultValue: "Ollama URL", comment: ""), controlWidth: meetingControlWidth) {
                PastableTextField(
        ...
        case .some(.lmStudio):
            Divider().background(MuesliTheme.surfaceBorder)
            settingsRow(String(localized: "settings.hosted_cleanup.lm_studio_url", defaultValue: "LM Studio URL", comment: ""), controlWidth: meetingControlWidth) {
                PastableTextField(
            ...
            settingsRow(String(localized: "settings.hosted_cleanup.cleanup_model", defaultValue: "Cleanup model", comment: ""), controlWidth: meetingControlWidth) {
                settingsModelTextField(
                    ...
                    placeholder: String(localized: "settings.hosted_cleanup.loaded_lm_studio_model", defaultValue: "Loaded LM Studio model", comment: "")
                ) { controller.updatePostProcessorModel($0, for: backend) }
            }
        case .some(.customLLM):
    ...
    private var cleanupPromptSettings: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing12) {
            settingsRow(String(localized: "settings.cleanup_prompt.cleanup_preset", defaultValue: "Cleanup preset", comment: ""), controlWidth: meetingControlWidth) {
                FixedWidthPopUp(
            ...
            HStack {
                Spacer()
                compactActionButton(String(localized: "settings.cleanup_prompt.action.manage_presets", defaultValue: "Manage Presets…", comment: ""), systemImage: "slider.horizontal.3") {
    ...
    private var meetingSummarySettingsSection: some View {
        settingsSection(String(localized: "settings.section.meeting_summaries", defaultValue: "Meeting Summaries", comment: "")) {
            settingsRow(String(localized: "settings.meeting_summaries.summary_backend", defaultValue: "Summary backend", comment: ""), controlWidth: meetingControlWidth) {
                settingsMenu(
            ...
            if appState.selectedMeetingSummaryBackend == .chatGPT {
                settingsRow(String(localized: "settings.meeting_summaries.account", defaultValue: "Account", comment: ""), controlWidth: meetingControlWidth) {
                    chatGPTAccountControl()
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.meeting_summaries.model", defaultValue: "Model", comment: ""), controlWidth: meetingControlWidth) {
                    settingsModelMenu(
                ...
                settingsRow(String(localized: "settings.meeting_summaries.api_key", defaultValue: "API Key", comment: ""), controlWidth: meetingControlWidth) {
                    PastableSecureField(
                        text: appState.config.openAIAPIKey,
                        placeholder: "sk-...",
                        onChange: { val in controller.updateConfig { $0.openAIAPIKey = val } }
                ...
                settingsRow(String(localized: "settings.meeting_summaries.ollama_url", defaultValue: "Ollama URL", comment: ""), controlWidth: meetingControlWidth) {
                    PastableTextField(
                        text: appState.config.ollamaURL,
                        placeholder: "http://localhost:11434",
                        onChange: { val in controller.updateConfig { $0.ollamaURL = val } }
                ...
                settingsRow(String(localized: "settings.meeting_summaries.lm_studio_url", defaultValue: "LM Studio URL", comment: ""), controlWidth: meetingControlWidth) {
                    PastableTextField(
                        text: appState.config.lmStudioURL,
                        placeholder: "http://localhost:1234",
                        onChange: { val in controller.updateConfig { $0.lmStudioURL = val } }
                ...
                settingsRow(String(localized: "settings.meeting_summaries.model", defaultValue: "Model", comment: ""), controlWidth: meetingControlWidth) {
                    settingsModelTextField(
                        ...
                        placeholder: String(localized: "settings.meeting_summaries.select_loaded_lm_studio_model", defaultValue: "Select a loaded LM Studio model", comment: "")
    ...
    private func customLLMSettingsRows(model: String, onModelChange: @escaping (String) -> Void) -> some View {
        Divider().background(MuesliTheme.surfaceBorder)
        settingsRow(String(localized: "settings.custom_llm.api_format", defaultValue: "API Format", comment: ""), controlWidth: meetingControlWidth) {
            settingsMenu(
        ...
        Divider().background(MuesliTheme.surfaceBorder)
        settingsRow(String(localized: "settings.custom_llm.endpoint", defaultValue: "Endpoint", comment: ""), controlWidth: meetingControlWidth) {
            PastableTextField(
        ...
        Divider().background(MuesliTheme.surfaceBorder)
        settingsRow(String(localized: "settings.custom_llm.api_key", defaultValue: "API Key", comment: ""), controlWidth: meetingControlWidth) {
            PastableSecureField(
                text: appState.config.customLLMAPIKey,
                placeholder: appState.config.customLLMFormat == CustomLLMFormat.anthropic.rawValue
                    ? String(localized: "settings.custom_llm.api_key.required_for_anthropic", defaultValue: "Required for Anthropic API", comment: "")
                    : String(localized: "settings.custom_llm.api_key.optional_for_local_servers", defaultValue: "Optional for local servers", comment: ""),
                onChange: { val in controller.updateConfig { $0.customLLMAPIKey = val } }
            )
            .frame(height: 22)
        }
        Divider().background(MuesliTheme.surfaceBorder)
        settingsRow(String(localized: "settings.custom_llm.model", defaultValue: "Model", comment: ""), controlWidth: meetingControlWidth) {
            settingsModelTextField(
                currentModel: model,
                placeholder: appState.config.customLLMFormat == CustomLLMFormat.anthropic.rawValue
    ...
    private var dictationSettingsPane: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
            ...
            settingsSection(String(localized: "settings.section.transcription", defaultValue: "Transcription", comment: "")) {
                settingsRow(
                    "Microphone",
                    description: String(localized: "settings.dictation.microphone.description.automatic_system_or_mac_mic", defaultValue: "Automatic uses system input, or Mac mic with AirPods.", comment: "")
                ) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.dictation.ai_transcript_cleanup", defaultValue: "AI transcript cleanup", comment: "")) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(
                    String(localized: "settings.dictation.dictionary_suggestions", defaultValue: "Dictionary suggestions", comment: ""),
                    description: String(localized: "settings.dictation.dictionary_suggestions.description", defaultValue: "Suggest words after corrections by briefly reading focused app text via Accessibility.", comment: "")
                ) {
                    ...
                    .help(String(localized: "settings.dictation.help.ocr_context_brief_description", defaultValue: "Briefly reads focused app text after dictation to detect corrections.", comment: ""))
                }
            }

            dictationCleanupSettingsSection

            settingsSection(String(localized: "settings.section.advanced", defaultValue: "Advanced", comment: "")) {
                settingsRow(String(localized: "settings.dictation.pause_media_during_dictation", defaultValue: "Pause media during dictation", comment: "")) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.dictation.mute_system_audio_during_dictation", defaultValue: "Mute system audio during dictation", comment: "")) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                screenContextRow(String(localized: "settings.dictation.app_context", defaultValue: "App context", comment: ""))
                Divider().background(MuesliTheme.surfaceBorder)
    ...
    private var computerUseSettingsPane: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
            settingsSection(String(localized: "settings.section.computer_use", defaultValue: "Computer Use", comment: "")) {
                settingsRow(String(localized: "settings.computer_use.enable_planner", defaultValue: "Enable planner", comment: ""), controlWidth: meetingControlWidth) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.computer_use.account", defaultValue: "Account", comment: ""), controlWidth: meetingControlWidth) {
                    chatGPTAccountControl()
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.computer_use.planner_model", defaultValue: "Planner model", comment: ""), controlWidth: meetingControlWidth) {
                    settingsModelMenu(
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.computer_use.timeout", defaultValue: "Timeout", comment: ""), controlWidth: meetingControlWidth) {
                    Stepper(
                        value: Binding(
                        ...
                        Text(String(format: String(localized: "settings.computer_use.timeout.seconds_value", defaultValue: "%d seconds", comment: ""), max(appState.config.computerUseTimeoutSeconds, 1)))
                            .font(MuesliTheme.body())
                            .foregroundStyle(MuesliTheme.textPrimary)
                    }
                }
    ...
    private var meetingsSettingsPane: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
            ...
            settingsSection(String(localized: "settings.section.meeting_context", defaultValue: "Meeting Context", comment: "")) {
                screenContextRow(String(localized: "settings.meetings.meeting_context", defaultValue: "Meeting context", comment: ""), includesScreenOCR: true)
            }

            meetingSummarySettingsSection

            settingsSection(String(localized: "settings.section.meeting_notes", defaultValue: "Meeting Notes", comment: "")) {
                settingsRow(String(localized: "settings.meetings.default_template", defaultValue: "Default template", comment: ""), controlWidth: meetingControlWidth) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.meetings.summary_retries", defaultValue: "Summary retries", comment: ""), controlWidth: meetingControlWidth) {
                    Stepper(
                        value: Binding(
                            get: {
                ...
                settingsDescription(String(localized: "settings.meetings.summary_retries.description", defaultValue: "Retry transient AI summary failures before saving failed notes.", comment: ""))
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.meetings.templates", defaultValue: "Templates", comment: ""), controlWidth: meetingControlWidth) {
                    actionButton(String(localized: "settings.meetings.action.manage_templates", defaultValue: "Manage Templates…", comment: "")) {
                        controller.showMeetingTemplatesManager()
                    }
                }
            }

            settingsSection(String(localized: "settings.section.recording", defaultValue: "Recording", comment: "")) {
                settingsRow(String(localized: "settings.meetings.auto_record_calendar_meetings", defaultValue: "Auto-record calendar meetings", comment: "")) {
                    settingsSwitch(isOn: appState.config.autoRecordMeetings) { newValue in
                        controller.updateConfig { $0.autoRecordMeetings = newValue }
                    }
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.meetings.save_meeting_recording", defaultValue: "Save meeting recording", comment: "")) {
                    settingsMenu(
                ...
                if appState.config.meetingRecordingSavePolicy != .never {
                    Divider().background(MuesliTheme.surfaceBorder)
                    settingsRow(String(localized: "settings.meetings.recording_format", defaultValue: "Recording format", comment: "")) {
                        settingsMenu(
                    ...
                    settingsDescription(String(localized: "settings.meetings.recording_format.description", defaultValue: "M4A is recommended for smaller files. WAV is lossless and uses more storage.", comment: ""))
                }
            }

            settingsSection(String(localized: "settings.section.auto_export", defaultValue: "Auto Export", comment: "")) {
                settingsRow(String(localized: "settings.meetings.auto_export_meetings", defaultValue: "Auto-export meetings", comment: "")) {
                ...
                if appState.config.autoExportMarkdownEnabled {
                    Divider().background(MuesliTheme.surfaceBorder)
                    settingsRow(String(localized: "settings.meetings.destination_folder", defaultValue: "Destination folder", comment: "")) {
                        autoExportFolderPicker
                    }
                    Divider().background(MuesliTheme.surfaceBorder)
                    settingsRow(String(localized: "settings.meetings.content", defaultValue: "Content", comment: "")) {
                        settingsMenu(
                    ...
                    Divider().background(MuesliTheme.surfaceBorder)
                    settingsRow(String(localized: "settings.meetings.file_format", defaultValue: "File format", comment: "")) {
                        settingsMenu(
                ...
                Text(String(localized: "settings.meetings.auto_export.description", defaultValue: "Automatically saves each completed meeting to the chosen folder in the selected format.", comment: ""))
                    .font(MuesliTheme.caption())
                    .foregroundStyle(MuesliTheme.textTertiary)
                    .padding(.horizontal, MuesliTheme.spacing16)
            }

            settingsSection(String(localized: "settings.section.meeting_notifications", defaultValue: "Meeting Notifications", comment: "")) {
                settingsRow(String(localized: "settings.meetings.notifications.scheduled_meetings", defaultValue: "Scheduled meetings", comment: "")) {
                ...
                settingsDescription(String(localized: "settings.meetings.notifications.scheduled_meetings.description", defaultValue: "Show notifications for calendar meetings with a join link.", comment: ""))

                if appState.config.showScheduledMeetingNotifications {
                    Divider().background(MuesliTheme.surfaceBorder)

                    settingsRow(String(localized: "settings.meetings.notifications.reminder_timing", defaultValue: "Reminder timing", comment: "")) {
                        settingsMenu(
                    ...
                    settingsDescription(String(localized: "settings.meetings.notifications.reminder_timing.description", defaultValue: "At start time avoids early calendar-only prompts before you join.", comment: ""))
                }

                Divider().background(MuesliTheme.surfaceBorder)

                settingsRow(String(localized: "settings.meetings.notifications.auto_detected_meetings", defaultValue: "Auto-detected meetings", comment: "")) {
                ...
                settingsDescription(String(localized: "settings.meetings.notifications.auto_detected_meetings.description", defaultValue: "Show notifications when a call is detected from browser, camera, microphone, or app audio activity.", comment: ""))
            ...
            settingsSection(String(localized: "settings.section.calendars", defaultValue: "Calendars", comment: "")) {
                settingsRow(String(localized: "settings.meetings.calendars.upcoming_meetings", defaultValue: "Upcoming meetings", comment: ""), controlWidth: meetingControlWidth) {
                    settingsMenu(
                ...
                settingsDescription(String(localized: "settings.meetings.calendars.upcoming_meetings.description", defaultValue: "Controls how many calendar days appear in Coming Up, the menu bar, and scheduled meeting checks.", comment: ""))
                Divider().background(MuesliTheme.surfaceBorder)
            ...
            if appState.isGoogleCalendarAvailable {
                settingsSection(String(localized: "settings.section.calendar", defaultValue: "Calendar", comment: "")) {
                    settingsRow(String(localized: "settings.meetings.calendar.google_calendar", defaultValue: "Google Calendar", comment: "")) {
            ...
            settingsSection(String(localized: "settings.section.advanced", defaultValue: "Advanced", comment: "")) {
                settingsRow(String(localized: "settings.meetings.advanced.enable_post_meeting_hook", defaultValue: "Enable post-meeting hook", comment: ""), controlWidth: meetingControlWidth) {
                    settingsSwitch(isOn: appState.config.meetingHookEnabled) { newValue in
                        controller.updateConfig { $0.meetingHookEnabled = newValue }
                    }
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.meetings.advanced.hook_script", defaultValue: "Hook script", comment: ""), controlWidth: meetingControlWidth) {
                    meetingHookPathPicker
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.meetings.advanced.timeout", defaultValue: "Timeout", comment: ""), controlWidth: meetingControlWidth) {
                    meetingHookTimeoutControl
                }
                settingsDescription(String(localized: "settings.meetings.advanced.hook_script.description", defaultValue: "Runs a user-supplied executable after each completed meeting. The executable receives JSON on stdin and must already be runnable on its own.", comment: ""))
            }
    ...
    private var appearanceSettingsPane: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
            settingsSection(String(localized: "settings.section.floating_indicator", defaultValue: "Floating Indicator", comment: "")) {
                settingsRow(String(localized: "settings.appearance.show_floating_indicator", defaultValue: "Show floating indicator", comment: "")) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.appearance.show_hotkey_on_floating_indicator", defaultValue: "Show hotkey on floating indicator", comment: "")) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.appearance.indicator_position", defaultValue: "Indicator position", comment: "")) {
            ...
            settingsSection(String(localized: "settings.section.appearance", defaultValue: "Appearance", comment: "")) {
                settingsRow(String(localized: "settings.appearance.dark_mode", defaultValue: "Dark mode", comment: "")) {
                    settingsSwitch(isOn: appState.config.darkMode) { newValue in
                        controller.updateConfig { $0.darkMode = newValue }
                    }
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.appearance.menu_bar_icon", defaultValue: "Menu bar icon", comment: "")) {
                    menuBarIconPicker
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.appearance.show_hotkey_in_menu_bar", defaultValue: "Show hotkey in menu bar", comment: "")) {
                ...
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.appearance.accent_color", defaultValue: "Accent color", comment: "")) {
                    glassTintPicker
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.appearance.play_sound_effects", defaultValue: "Play sound effects", comment: "")) {
                    settingsSwitch(isOn: appState.config.soundEnabled) { newValue in
                        controller.updateConfig { $0.soundEnabled = newValue }
                    }
                }
                Divider().background(MuesliTheme.surfaceBorder)
                settingsRow(String(localized: "settings.appearance.show_next_meeting_in_menu_bar", defaultValue: "Show next meeting in menu bar", comment: "")) {
            ...
            if appState.config.maraudersMapUnlocked {
                settingsSection(String(localized: "settings.section.marauders_map", defaultValue: "Marauder’s Map", comment: "")) {
                    settingsRow(String(localized: "settings.appearance.meeting_countdown_audio", defaultValue: "Meeting countdown audio", comment: "")) {
                        maraudersMapControl
                    }
                    Divider().background(MuesliTheme.surfaceBorder)
                    settingsRow("") {
                        Button {
                            ...
                            Text(String(localized: "settings.appearance.mischief_managed", defaultValue: "Mischief Managed", comment: ""))
                                .font(.system(size: 11))
        ...
        if appState.isChatGPTAuthenticated {
            Button {
                ...
                HStack(spacing: 5) {
                    ...
                    Text(String(localized: "settings.chatgpt_account.signed_in_sign_out", defaultValue: "Signed in · Sign Out", comment: ""))
                        .font(.system(size: 11, weight: .medium))
            ...
            HStack(spacing: 6) {
                ProgressView()
                    .controlSize(.small)
                Text(String(localized: "settings.chatgpt_account.signing_in", defaultValue: "Signing in...", comment: ""))
                    .font(.system(size: 11))
                    .foregroundStyle(MuesliTheme.textSecondary)
            }
        } else {
                ...
                Button {
                    ...
                    HStack(spacing: 5) {
                        OpenAILogoShape()
                            .fill(.white)
                            .frame(width: 10, height: 10)
                        Text(String(localized: "settings.chatgpt_account.sign_in_with_chatgpt", defaultValue: "Sign in with ChatGPT", comment: ""))
                            .font(.system(size: 11, weight: .medium))
                            .foregroundStyle(.white)
                            .lineLimit(1)
                    }
    ...
    private var googleCalendarControl: some View {
        if appState.isGoogleCalendarAuthenticated {
            Button {
                ...
                HStack(spacing: 5) {
                    Image(systemName: "calendar")
                        .font(.system(size: 10))
                        .foregroundStyle(.white)
                    Text(String(localized: "settings.google_calendar.connected_disconnect", defaultValue: "Connected · Disconnect", comment: ""))
                        .font(.system(size: 11, weight: .medium))
                        .foregroundStyle(.white)
                        .lineLimit(1)
                }
            ...
            HStack(spacing: 6) {
                ProgressView()
                    .controlSize(.small)
                Text(String(localized: "settings.google_calendar.connecting", defaultValue: "Connecting...", comment: ""))
                    .font(.system(size: 11))
                    .foregroundStyle(MuesliTheme.textSecondary)
            }
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 5) {
                    ...
                    Text(String(localized: "settings.google_calendar.connect", defaultValue: "Connect Google Calendar", comment: ""))
                        .font(.system(size: 11, weight: .medium))
                        .foregroundStyle(.white.opacity(0.4))
                        .lineLimit(1)
                }
                ...
                Text(String(localized: "settings.google_calendar.oauth_verification_pending", defaultValue: "Google OAuth verification pending", comment: ""))
                    .font(.system(size: 10))
                    .foregroundStyle(MuesliTheme.textTertiary)
            }
        } else {
    ...
    private func pickCustomAudioFile() {
        ...
        panel.title = String(localized: "settings.appearance.audio_picker.choose_clip", defaultValue: "Choose an audio clip", comment: "")
    ...
    private func pickMeetingHookFile() {
        let panel = NSOpenPanel()
        panel.title = String(localized: "settings.meetings.hook_script.choose_prompt", defaultValue: "Choose a hook script", comment: "")
        panel.prompt = String(localized: "settings.meetings.hook_script.choose_button", defaultValue: "Choose Script", comment: "")
        panel.allowsMultipleSelection = false
        panel.canChooseDirectories = false
    ...
    private func pickAutoExportFolder() {
        let panel = NSOpenPanel()
        panel.title = String(localized: "settings.meetings.auto_export.choose_folder_prompt", defaultValue: "Choose a folder for exported notes", comment: "")
        panel.prompt = String(localized: "settings.meetings.auto_export.choose_folder_button", defaultValue: "Choose Folder", comment: "")
        panel.allowsMultipleSelection = false
    ...
    private var permissionsSection: some View {
        settingsSection(String(localized: "settings.permissions.section", defaultValue: "Permissions", comment: "")) {
            permissionStatusRow(
                "Microphone",
                granted: micGranted,
            ...
            Divider().background(MuesliTheme.surfaceBorder)
            permissionStatusRow(
                String(localized: "settings.permissions.screen_recording", defaultValue: "Screen Recording", comment: ""),
            ...
            if appState.config.useCoreAudioTap {
                Divider().background(MuesliTheme.surfaceBorder)
                permissionStatusRow(
                    String(localized: "settings.permissions.system_audio", defaultValue: "System Audio", comment: ""),
    ...
    private func permissionStatusRow(_ name: String, granted: Bool, action: @escaping () -> Void, pane: String) -> some View {
        HStack {
            ...
            Spacer()
            if granted {
                Text(String(localized: "settings.permissions.status.granted", defaultValue: "Granted", comment: ""))
                    .font(.system(size: 11))
                    .foregroundStyle(MuesliTheme.success)
            } else {
                Button(String(localized: "settings.permissions.action.grant", defaultValue: "Grant", comment: "")) {
                    action()
                }
                .buttonStyle(.plain)
                .font(.system(size: 11, weight: .medium))
            ...
            .buttonStyle(.plain)
            .help(String(localized: "settings.permissions.help.open_in_system_settings", defaultValue: "Open in System Settings", comment: ""))
        }
        .frame(minHeight: 32)
    }
    ...
    private func screenContextControl(width: CGFloat? = nil) -> some View {
        if accessibilityGranted {
            ...
            Button {
                ...
                Text(String(localized: "settings.screen_context.action.grant", defaultValue: "Grant", comment: ""))
    ...
    private func dictationOCRContextControl(width: CGFloat? = nil) -> some View {
        if !appState.config.enableScreenContext {
            ...
            Button {
                ...
                Text(String(localized: "settings.dictation_ocr_context.action.grant", defaultValue: "Grant", comment: ""))
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(MuesliTheme.accent)
                    .frame(width: width)
                    .frame(minHeight: 32)
    ...
    private func summaryRetryLabel(_ retryCount: Int) -> String {
        ...
        switch clamped {
        case 0:
            return String(localized: "settings.meetings.summary_retries.none", defaultValue: "No retries", comment: "")
        case 1:
            return String(localized: "settings.meetings.summary_retries.one", defaultValue: "1 retry", comment: "")
        default:
            return String(format: String(localized: "settings.meetings.summary_retries.many", defaultValue: "%d retries", comment: ""), clamped)
        }
    }
    ...
    private var mutedMeetingDetectionAppsControl: some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            Text(String(localized: "settings.meeting_detection.muted_apps.description", defaultValue: "Don't notify me when a call is detected in these apps:", comment: ""))
                .font(MuesliTheme.body())
                .foregroundStyle(MuesliTheme.textPrimary)
    ...
    private var calendarSourceGroups: [CalendarSourceGroup] {
        ...
        if appState.isGoogleCalendarAuthenticated && !appState.availableGoogleCalendars.isEmpty {
            let items = appState.availableGoogleCalendars.map { cal in
                CalendarToggleItem(
                    id: cal.id,
                    title: cal.summary + (cal.isPrimary ? " " + String(localized: "settings.calendars.primary_suffix", defaultValue: "(Primary)", comment: "") : ""),
                    colorHex: cal.colorHex,
                    isEnabled: !disabled.contains(cal.id)
                )
            }
            groups.append(CalendarSourceGroup(
                ...
                subtitle: String(localized: "settings.calendars.google.connected_directly", defaultValue: "Connected directly to Muesli", comment: ""),
    ...
    private var calendarSourcesControl: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            Text(String(localized: "settings.calendars.sources.explanation", defaultValue: "Calendar sources are listed first, with their calendars underneath. Disabled calendars are hidden from Muesli — no notifications, no Coming Up, no meeting detection.", comment: ""))
                .font(MuesliTheme.caption())
                .foregroundStyle(MuesliTheme.textTertiary)
                .fixedSize(horizontal: false, vertical: true)

            if calendarSourceGroups.isEmpty {
                Text(String(localized: "settings.calendars.none_detected_permission_hint", defaultValue: "No calendars detected. Make sure Calendar permission is granted in System Settings > Privacy & Security > Calendars.", comment: ""))
                    .font(MuesliTheme.caption())
                    .foregroundStyle(MuesliTheme.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            } else {
            ...
            if appState.isGoogleCalendarAuthenticated && !appState.availableEventKitCalendars.isEmpty {
                Text(String(localized: "settings.calendars.google_duplicates_explanation", defaultValue: "Google calendars may appear once from macOS Calendar and once from Muesli's Google connection. Turn off both copies to hide that calendar completely.", comment: ""))
                    .font(MuesliTheme.caption())
                    .foregroundStyle(MuesliTheme.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("\(group.subtitle) • \(group.items.count) \(group.items.count == 1 ? String(localized: "settings.calendar_source.count.singular", defaultValue: "calendar", comment: "") : String(localized: "settings.calendar_source.count.plural", defaultValue: "calendars", comment: ""))")
                        .font(.system(size: 11))
                        .foregroundStyle(MuesliTheme.textTertiary)
                        .lineLimit(1)
                }
    ...
    private func calendarSourceSubtitle(for sourceTitle: String) -> String {
        ...
        if normalized == "icloud" {
            return String(localized: "settings.calendars.source_subtitle.icloud_account", defaultValue: "iCloud account in macOS Calendar", comment: "")
        }
        if normalized == "subscribed calendars" {
            return String(localized: "settings.calendars.source_subtitle.subscribed", defaultValue: "Subscribed in macOS Calendar", comment: "")
        }
        if normalized == "other" {
            return String(localized: "settings.calendars.source_subtitle.system_calendars", defaultValue: "System calendars from macOS", comment: "")
        }
        return String(localized: "settings.calendars.source_subtitle.account_macos", defaultValue: "Calendar account in macOS", comment: "")
    }
    ...
    private var googleCalendarListLoadStateView: some View {
        ...
        case .loading:
            Text(String(localized: "settings.google_calendar.loading_calendars", defaultValue: "Loading Google calendars…", comment: ""))
                .font(MuesliTheme.caption())
                .foregroundStyle(MuesliTheme.textTertiary)
        case .failed(let message):
            HStack(spacing: 8) {
                Text(String(format: String(localized: "settings.google_calendar.load_failed_with_message", defaultValue: "Failed to load Google calendars: %@", comment: ""), "\(message)"))
                    .font(MuesliTheme.caption())
                    .foregroundStyle(MuesliTheme.textTertiary)
                Button(String(localized: "settings.google_calendar.retry", defaultValue: "Retry", comment: "")) {
        ...
        HStack(spacing: 8) {
            HStack(spacing: 8) {
                ...
                if appState.config.autoExportMarkdownFolderPath.isEmpty {
                    Text(String(localized: "settings.auto_export.choose_folder", defaultValue: "Choose a folder…", comment: ""))
                        .font(.system(size: 12))
                        .foregroundStyle(MuesliTheme.textTertiary)
                        .lineLimit(1)
                } else {
            ...
            .help(appState.config.autoExportMarkdownFolderPath.isEmpty ? String(localized: "settings.auto_export.no_destination_folder_selected", defaultValue: "No destination folder selected", comment: "") : appState.config.autoExportMarkdownFolderPath)
            ...
            if !appState.config.autoExportMarkdownFolderPath.isEmpty {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(String(localized: "settings.auto_export.clear_destination_folder_accessibility_label", defaultValue: "Clear destination folder", comment: ""))
                .help(String(localized: "settings.auto_export.help.clear_destination_folder", defaultValue: "Clear destination folder", comment: ""))
            }
            ...
            .buttonStyle(.plain)
            .accessibilityLabel(String(localized: "settings.auto_export.choose_destination_folder_accessibility_label", defaultValue: "Choose destination folder", comment: ""))
            .help(String(localized: "settings.auto_export.help.choose_destination_folder", defaultValue: "Choose destination folder", comment: ""))
        }
    }
        ...
        HStack(spacing: 8) {
            HStack(spacing: 8) {
                ...
                if appState.config.meetingHookPath.isEmpty {
                    Text(String(localized: "settings.meeting_hook.choose_script", defaultValue: "Choose a script…", comment: ""))
                        .font(.system(size: 12))
                        .foregroundStyle(MuesliTheme.textTertiary)
                        .lineLimit(1)
                } else {
            ...
            .frame(maxWidth: .infinity)
            .help(appState.config.meetingHookPath.isEmpty ? String(localized: "settings.meeting_hook.no_script_selected", defaultValue: "No hook script selected", comment: "") : appState.config.meetingHookPath)
            ...
            if !appState.config.meetingHookPath.isEmpty {
                ...
                .buttonStyle(.plain)
                .help(String(localized: "settings.meeting_hook.help.clear_script", defaultValue: "Clear hook script", comment: ""))
            }
            ...
            .buttonStyle(.plain)
            .help(String(localized: "settings.meeting_hook.help.choose_script", defaultValue: "Choose hook script", comment: ""))
        }
    ...
    private var meetingHookTimeoutControl: some View {
        Stepper(
            ...
            Text(String(format: String(localized: "settings.meeting_hook.timeout.seconds_value", defaultValue: "%d seconds", comment: ""), max(appState.config.meetingHookTimeoutSeconds, 1)))
                .font(MuesliTheme.body())
                .foregroundStyle(MuesliTheme.textPrimary)
                .monospacedDigit()
    ...
    private func meetingTemplateMenu(selectionID: String, onChange: @escaping (String) -> Void) -> some View {
        ...
        let selectedLabel = allItems.first(where: { $0.id == selectionID })?.label ?? String(localized: "settings.common.auto", defaultValue: "Auto", comment: "")
        FixedWidthPopUp(
    ...
    private var openRouterFreeModelMenu: some View {
        if isLoadingOpenRouterFreeModels {
            HStack(spacing: 8) {
                ProgressView()
                    .controlSize(.small)
                Text(String(localized: "settings.open_router_free_model.loading_models", defaultValue: "Loading models", comment: ""))
                    .font(.system(size: 12, weight: .medium))
                    .foregroundStyle(MuesliTheme.textTertiary)
            }
            .frame(maxWidth: .infinity, alignment: .trailing)
            ...
            HStack(spacing: 8) {
                ...
                Button(String(localized: "settings.open_router_free_model.load", defaultValue: "Load", comment: "")) {
    ...
    private func keyStatusRow(key: String) -> some View {
        HStack(spacing: 6) {
            ...
            Text(key.isEmpty ? String(localized: "settings.api_key.status.not_configured", defaultValue: "No API key configured", comment: "") : String(localized: "settings.api_key.status.configured", defaultValue: "Key configured", comment: ""))
                .font(.system(size: 11))
    ...
    private func recordingSaveLabel(for policy: MeetingRecordingSavePolicy) -> String {
        switch policy {
        case .never:
            return String(localized: "settings.meetings.recording_save.never", defaultValue: "Never", comment: "")
        case .prompt:
            return String(localized: "settings.meetings.recording_save.ask_every_time", defaultValue: "Ask every time", comment: "")
        case .always:
            return String(localized: "settings.meetings.recording_save.always", defaultValue: "Always", comment: "")
        }
    }
    ...
    private func scheduledMeetingLeadTimeLabel(for leadTime: ScheduledMeetingNotificationLeadTime) -> String {
        switch leadTime {
        case .atStart:
            return String(localized: "settings.meetings.reminder_timing.at_start", defaultValue: "At start time", comment: "")
        case .oneMinute:
            return String(localized: "settings.meetings.reminder_timing.one_min_before", defaultValue: "1 min before", comment: "")
        case .threeMinutes:
            return String(localized: "settings.meetings.reminder_timing.three_min_before", defaultValue: "3 min before", comment: "")
        case .fiveMinutes:
            return String(localized: "settings.meetings.reminder_timing.five_min_before", defaultValue: "5 min before", comment: "")
        }
    }
