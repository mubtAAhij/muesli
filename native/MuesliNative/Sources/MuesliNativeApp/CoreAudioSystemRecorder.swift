    ...
    enum RecorderError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .tapCreationFailed(let s):
                return String(format: String(localized: "system_audio.error.process_tap_creation_failed", defaultValue: "Process tap creation failed (status: %d)", bundle: .module, comment: "Error shown when creating CoreAudio process tap fails with status code"), s)
            case .aggregateDeviceCreationFailed(let s):
                return String(format: String(localized: "system_audio.error.aggregate_device_creation_failed", defaultValue: "Aggregate device creation failed (status: %d)", bundle: .module, comment: "Error shown when creating CoreAudio aggregate device fails with status code"), s)
            case .coreAudioSetupFailed(let step, let s):
                return String(format: String(localized: "system_audio.error.coreaudio_setup_failed_at_step", defaultValue: "CoreAudio setup failed at '%@' (status: %d)", bundle: .module, comment: "Error shown when CoreAudio setup fails at a specific step with status code"), "\(step)", s)
            case .deviceIOProcCreationFailed:
                return String(localized: "system_audio.error.could_not_create_aggregate_device_ioproc", defaultValue: "Could not create aggregate device IOProc", bundle: .module, comment: "Error shown when creating aggregate device IOProc fails")
            case .tapFormatUnavailable(let s):
                return String(format: String(localized: "system_audio.error.could_not_read_tap_stream_format", defaultValue: "Could not read tap stream format (status: %d)", bundle: .module, comment: "Error shown when reading tap stream format fails with status code"), s)
            }
        }
    }
