...
final class CoreAudioSystemRecorder: SystemAudioCapturing, SystemAudioDiagnosticsProviding {
    ...
    enum RecorderError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .fileCreationFailed:
                return String(localized: "system_audio.error.could_not_create_output_file", defaultValue: "Could not create output file", comment: "Error when system audio recorder cannot create output file")
            case .noDefaultOutputDevice:
                return String(localized: "system_audio.error.no_default_output_device", defaultValue: "No default audio output device found", comment: "Error when no default audio output device is available")
            case .tapCreationFailed(let s):
                return String(format: String(localized: "system_audio.error.process_tap_creation_failed", defaultValue: "Process tap creation failed (status: %@)", comment: "Error when CoreAudio process tap creation fails with status"), "\(s)")
            case .aggregateDeviceCreationFailed(let s):
                return String(format: String(localized: "system_audio.error.aggregate_device_creation_failed", defaultValue: "Aggregate device creation failed (status: %@)", comment: "Error when aggregate CoreAudio device creation fails with status"), "\(s)")
            case .coreAudioSetupFailed(let step, let s):
                return String(format: String(localized: "system_audio.error.coreaudio_setup_failed_at_step", defaultValue: "CoreAudio setup failed at '%@' (status: %@)", comment: "Error when CoreAudio setup fails at a specific step with status"), "\(step)", "\(s)")
            case .deviceIOProcCreationFailed:
                return String(localized: "system_audio.error.could_not_create_aggregate_device_ioproc", defaultValue: "Could not create aggregate device IOProc", comment: "Error when IOProc cannot be created for aggregate audio device")
            case .tapFormatUnavailable(let s):
                return String(format: String(localized: "system_audio.error.could_not_read_tap_stream_format", defaultValue: "Could not read tap stream format (status: %@)", comment: "Error when tap stream format cannot be read with status"), "\(s)")
            }
        }
    }
