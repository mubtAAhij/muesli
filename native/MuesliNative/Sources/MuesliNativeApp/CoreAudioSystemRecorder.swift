...
final class CoreAudioSystemRecorder: SystemAudioCapturing, SystemAudioDiagnosticsProviding {
    ...
    enum RecorderError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .fileCreationFailed:
                return String(localized: "system_audio.error.could_not_create_output_file", defaultValue: "Could not create output file", bundle: .module, comment: "Error shown when system audio recorder cannot create output file.")
            case .noDefaultOutputDevice:
                return String(localized: "system_audio.error.no_default_output_device", defaultValue: "No default audio output device found", bundle: .module, comment: "Error shown when no default audio output device is available.")
            case .tapCreationFailed(let s):
                return String(format: String(localized: "system_audio.error.process_tap_creation_failed", defaultValue: "Process tap creation failed (status: %d)", bundle: .module, comment: "Error shown when creating CoreAudio process tap fails, including status code."), s)
            case .aggregateDeviceCreationFailed(let s):
                return String(format: String(localized: "system_audio.error.aggregate_device_creation_failed", defaultValue: "Aggregate device creation failed (status: %d)", bundle: .module, comment: "Error shown when creating CoreAudio aggregate device fails, including status code."), s)
            case .coreAudioSetupFailed(let step, let s):
                return String(format: String(localized: "system_audio.error.coreaudio_setup_failed_at_step", defaultValue: "CoreAudio setup failed at '%@' (status: %d)", bundle: .module, comment: "Error shown when CoreAudio setup fails at a named step with status code."), "\(step)", s)
            case .deviceIOProcCreationFailed:
                return String(localized: "system_audio.error.could_not_create_aggregate_device_ioproc", defaultValue: "Could not create aggregate device IOProc", bundle: .module, comment: "Error shown when creating IOProc for aggregate audio device fails.")
            case .tapFormatUnavailable(let s):
                return String(format: String(localized: "system_audio.error.could_not_read_tap_stream_format", defaultValue: "Could not read tap stream format (status: %d)", bundle: .module, comment: "Error shown when tap stream format cannot be read, including status code."), s)
            }
        }
    }
