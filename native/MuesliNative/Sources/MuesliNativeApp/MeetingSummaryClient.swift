...
enum MeetingSummaryError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .backendFailed(backend, statusCode, message):
            let statusText = statusCode.map { String(format: String(localized: "meeting_summary.error.status_suffix", defaultValue: "Status %d.", comment: "Status code suffix appended to backend failure errors"), $0) } ?? ""
            return String(format: String(localized: "meeting_summary.error.backend_failed", defaultValue: "%@ could not generate meeting notes.%@ %@ The selected model may be unavailable or retired.", comment: ""), "\(backend)", "\(statusText)", "\(message)")
        case let .emptyResponse(backend):
            return String(format: String(localized: "meeting_summary.error.empty_response", defaultValue: "%@ returned an empty response while generating meeting notes. The selected model may be unavailable or incompatible.", comment: ""), "\(backend)")
        case let .requestFailed(backend, underlying):
            return String(format: String(localized: "meeting_summary.error.unreachable", defaultValue: "%@ could not be reached while generating meeting notes. %@", comment: ""), "\(backend)", "\(underlying.localizedDescription)")
        }
    }
}
...
enum MeetingSummaryClient {
    ...
    static func summaryFailureNotes(transcript: String, meetingTitle: String, error: Error, manualNotes: String? = nil) -> String {
        ...
        var sections = [String(localized: "meeting_summary.failure.heading", defaultValue: "## Summary failed", comment: "")]
        if !trimmedTitle.isEmpty {
            sections.append(String(format: String(localized: "meeting_summary.failure.meeting_line", defaultValue: "Meeting: %@", comment: ""), "\(trimmedTitle)"))
        }
    ...
    private static func summarizeWithOpenAI(
        ...
        do {
            ...
            try validateHTTPResponse(response, data: data, backend: String(localized: "meeting_summary.backend.openai", defaultValue: "OpenAI", comment: ""))
            guard
                ...
                if let message = extractErrorMessage(from: data) {
                    throw MeetingSummaryError.backendFailed(backend: String(localized: "meeting_summary.backend.openai", defaultValue: "OpenAI", comment: ""), statusCode: nil, message: message)
                }
                throw MeetingSummaryError.emptyResponse(backend: String(localized: "meeting_summary.backend.openai", defaultValue: "OpenAI", comment: ""))
            }
            return text
        } catch {
            throw summaryRequestError(backend: String(localized: "meeting_summary.backend.openai", defaultValue: "OpenAI", comment: ""), error: error)
        }
    }
    ...
    private static func summarizeWithOpenRouter(
        ...
        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            try validateHTTPResponse(response, data: data, backend: String(localized: "meeting_summary.backend.openrouter", defaultValue: "OpenRouter", comment: ""))
            guard
                let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                ...
                if let message = extractErrorMessage(from: data) {
                    throw MeetingSummaryError.backendFailed(backend: String(localized: "meeting_summary.backend.openrouter", defaultValue: "OpenRouter", comment: ""), statusCode: nil, message: message)
                }
                throw MeetingSummaryError.emptyResponse(backend: String(localized: "meeting_summary.backend.openrouter", defaultValue: "OpenRouter", comment: ""))
            }
            return text
        } catch {
            throw summaryRequestError(backend: String(localized: "meeting_summary.backend.openrouter", defaultValue: "OpenRouter", comment: ""), error: error)
        }
    }
    ...
    private static func summarizeWithChatGPT(
        ...
        do {
            ...
            guard !text.isEmpty else {
                throw MeetingSummaryError.emptyResponse(backend: String(localized: "meeting_summary.backend.chatgpt", defaultValue: "ChatGPT", comment: ""))
            }
            return text
        } catch {
            fputs("[summary] ChatGPT summarization failed: \(error)\n", stderr)
            throw summaryRequestError(backend: String(localized: "meeting_summary.backend.chatgpt", defaultValue: "ChatGPT", comment: ""), error: error)
        }
    }
    ...
    private static func summarizeWithOllama(
        ...
        if baseURLString.isEmpty {
            ...
            guard let url = URL(string: baseURLString) else {
                throw MeetingSummaryError.backendFailed(backend: String(localized: "meeting_summary.backend.ollama", defaultValue: "Ollama", comment: ""), statusCode: nil, message: "Invalid Ollama URL: \(baseURLString)")
            }
            baseURL = url
        }
        ...
        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            try validateHTTPResponse(response, data: data, backend: String(localized: "meeting_summary.backend.ollama", defaultValue: "Ollama", comment: ""))
            guard
                let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                ...
                if let message = extractErrorMessage(from: data) {
                    throw MeetingSummaryError.backendFailed(backend: String(localized: "meeting_summary.backend.ollama", defaultValue: "Ollama", comment: ""), statusCode: nil, message: message)
                }
                throw MeetingSummaryError.emptyResponse(backend: String(localized: "meeting_summary.backend.ollama", defaultValue: "Ollama", comment: ""))
            }
            return text
        } catch {
            throw summaryRequestError(backend: String(localized: "meeting_summary.backend.ollama", defaultValue: "Ollama", comment: ""), error: error)
        }
    }
    ...
    private static func summarizeWithLMStudio(
        ...
        guard let requestURL = resolveLMStudioURL(config: config) else {
            throw MeetingSummaryError.backendFailed(backend: String(localized: "meeting_summary.backend.lm_studio", defaultValue: "LM Studio", comment: ""), statusCode: nil, message: "Invalid LM Studio URL: \(config.lmStudioURL)")
        }
        ...
        guard !configuredModel.isEmpty else {
            throw MeetingSummaryError.backendFailed(
                backend: String(localized: "meeting_summary.backend.lm_studio", defaultValue: "LM Studio", comment: ""),
                statusCode: nil,
                message: String(localized: "meeting_summary.error.lm_studio.no_model_selected", defaultValue: "No model selected. Select an LM Studio model in Settings.", comment: "")
            )
        }
    ...
    private static func summarizeWithCustomLLM(
        ...
        guard let requestURL = resolveCustomLLMURL(config: config, format: format) else {
            throw MeetingSummaryError.backendFailed(backend: String(localized: "meeting_summary.backend.custom_llm", defaultValue: "Custom LLM", comment: ""), statusCode: nil, message: "Invalid custom URL: \(config.customLLMURL)")
        }
        let configuredModel = config.customLLMModel.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !configuredModel.isEmpty else {
            throw MeetingSummaryError.backendFailed(
                backend: String(localized: "meeting_summary.backend.custom_llm", defaultValue: "Custom LLM", comment: ""),
                statusCode: nil,
                message: String(localized: "meeting_summary.error.custom_llm.no_model_selected", defaultValue: "No model selected. Enter a model in Settings.", comment: "")
            )
        }
        ...
        if customLLMRequiresAPIKey(config: config) && apiKey.isEmpty {
            throw MeetingSummaryError.backendFailed(
                backend: String(localized: "meeting_summary.backend.custom_llm", defaultValue: "Custom LLM", comment: ""),
                statusCode: nil,
                message: String(localized: "meeting_summary.error.custom_llm.missing_api_key", defaultValue: "Enter an API key for the selected Custom LLM format.", comment: "")
            )
        }
    ...
    private static func callAnthropicMessages(
        ...
        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            try validateHTTPResponse(response, data: data, backend: String(localized: "meeting_summary.backend.custom_llm", defaultValue: "Custom LLM", comment: ""))
            guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
