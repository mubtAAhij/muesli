...
enum MeetingSummaryError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .backendFailed(backend, statusCode, message):
            let statusText = statusCode.map { String(format: String(localized: "meeting_summary.error.status_suffix", defaultValue: "Status %@.", bundle: .module, comment: "Sentence fragment appended with backend status code in meeting summary errors."), "\($0)") } ?? ""
