...
final class AppScopedDictationRecorder: DictationAudioRecording {
    ...
    private final class ExplicitPreparation {
        ...
        private static func cancelledError() -> NSError {
            NSError(domain: "AppScopedDictationRecorder", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "app_scoped_dictation_recorder.error.dictation_microphone_preparation_cancelled", defaultValue: "Dictation microphone preparation was cancelled", bundle: .module, comment: "Error when dictation microphone preparation is cancelled."),
            ])
        }
    }
    ...
    private static func cancelledStartupError() -> NSError {
        NSError(domain: "AppScopedDictationRecorder", code: 1, userInfo: [
            NSLocalizedDescriptionKey: String(localized: "app_scoped_dictation_recorder.error.cancelled_before_microphone_startup_finished", defaultValue: "Dictation recording was cancelled before microphone startup finished", bundle: .module, comment: "Error when dictation recording is cancelled before microphone startup completes."),
        ])
    }
