...
enum TranscriptFormatter {
        ...
        if let diarizationSegments, !diarizationSegments.isEmpty {
            ...
            for seg in diarizationSegments.sorted(by: { $0.startTimeSeconds < $1.startTimeSeconds }) {
                if speakerLabelMap[seg.speakerId] == nil {
                    speakerLabelMap[seg.speakerId] = String(format: String(localized: "transcript_formatter.speaker.numbered", defaultValue: "Speaker %@", comment: "Numbered speaker label in transcript output"), "\(nextSpeakerNumber)")
                    nextSpeakerNumber += 1
                }
            }
            ...
            taggedSystem = systemSegments.map { TaggedSegment(segment: $0, speaker: String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in transcript output")) }
        }
    ...
    private static func findSpeaker(
        ...
        if labelMap.count == 1 {
            return labelMap.values.first ?? String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in transcript output")
        }
        ...
        if let bestSpeakerId, bestOverlap > 0 {
            return labelMap[bestSpeakerId] ?? String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in transcript output")
        }
        ...
        if let nearestSpeakerId = nearestSpeaker(
            ...
            return labelMap[nearestSpeakerId] ?? String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in transcript output")
        }
        return String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in transcript output")
    }
