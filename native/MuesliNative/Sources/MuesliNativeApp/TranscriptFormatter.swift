...
enum TranscriptFormatter {
        ...
        if let diarizationSegments, !diarizationSegments.isEmpty {
            ...
            for seg in diarizationSegments.sorted(by: { $0.startTimeSeconds < $1.startTimeSeconds }) {
                if speakerLabelMap[seg.speakerId] == nil {
                    speakerLabelMap[seg.speakerId] = String(format: String(localized: "transcript_formatter.speaker.numbered", defaultValue: "Speaker %d", bundle: .module, comment: "Transcript speaker label for numbered fallback speaker identities"), nextSpeakerNumber)
                    nextSpeakerNumber += 1
                }
            }
            ...
            taggedSystem = systemSegments.map { TaggedSegment(segment: $0, speaker: String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", bundle: .module, comment: "Transcript speaker label for grouped non-local participants")) }
        }
    ...
    private static func findSpeaker(
        ...
        if labelMap.count == 1 {
            return labelMap.values.first ?? String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", bundle: .module, comment: "Transcript speaker label for grouped non-local participants")
        }
        ...
        if let bestSpeakerId, bestOverlap > 0 {
            return labelMap[bestSpeakerId] ?? String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", bundle: .module, comment: "Transcript speaker label for grouped non-local participants")
        }
        ...
        if let nearestSpeakerId = nearestSpeaker(
            ...
            return labelMap[nearestSpeakerId] ?? String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", bundle: .module, comment: "Transcript speaker label for grouped non-local participants")
        }
        return String(localized: "transcript_formatter.speaker.others", defaultValue: "Others", bundle: .module, comment: "Transcript speaker label for grouped non-local participants")
    }
