    ...
    private static func transcodeWAVToM4AAsync(sourceURL: URL, destinationURL: URL) async throws {
        ...
        guard let exportSession = AVAssetExportSession(
            ...
            throw NSError(
                domain: "MeetingRecordingWriter",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_recording_writer.error.could_not_create_m4a_export_session", defaultValue: "Could not create M4A export session for meeting recording.", bundle: .module, comment: "Error shown when M4A export session cannot be created for meeting recording")]
            )
        }
            ...
            exportSessionBox.session.exportAsynchronously {
                guard exportSessionBox.session.status == .completed else {
                    continuation.resume(throwing: exportSessionBox.session.error ?? NSError(
                        domain: "MeetingRecordingWriter",
                        code: 3,
                        userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_recording_writer.error.could_not_export_as_m4a", defaultValue: "Could not export meeting recording as M4A.", bundle: .module, comment: "Error shown when meeting recording export to M4A fails")]
                    ))
                    return
                }
