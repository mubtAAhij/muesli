    ...
    init() throws {
        ...
        guard let fileHandle = FileHandle(forWritingAtPath: fileURL.path) else {
            throw NSError(
                ...
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_recording_writer.error.could_not_open_retained_file_for_writing", defaultValue: "Could not open retained meeting recording file for writing.", comment: "Error message when opening retained meeting recording file for writing fails")]
            )
        }
    ...
    private static func transcodeWAVToM4AAsync(sourceURL: URL, destinationURL: URL) async throws {
        ...
        guard let exportSession = AVAssetExportSession(
            ...
            throw NSError(
                domain: "MeetingRecordingWriter",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_recording_writer.error.could_not_create_m4a_export_session", defaultValue: "Could not create M4A export session for meeting recording.", comment: "Error message when creating M4A export session fails")]
            )
        }
            ...
            exportSessionBox.session.exportAsynchronously {
                guard exportSessionBox.session.status == .completed else {
                    continuation.resume(throwing: exportSessionBox.session.error ?? NSError(
                        domain: "MeetingRecordingWriter",
                        code: 3,
                        userInfo: [NSLocalizedDescriptionKey: String(localized: "meeting_recording_writer.error.could_not_export_as_m4a", defaultValue: "Could not export meeting recording as M4A.", comment: "Error message when exporting meeting recording as M4A fails")]
                    ))
                    return
                }
