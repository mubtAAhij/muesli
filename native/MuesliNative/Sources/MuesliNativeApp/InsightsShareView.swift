...
struct InsightsShareSheet: View {
        ...
        VStack(spacing: 20) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text(String(localized: "insights_share.title", defaultValue: "Share your activity", comment: "Title for insights sharing sheet"))
                        .font(.system(size: 20, weight: .semibold))
                        .tracking(-0.4)
                    Text(String(localized: "insights_share.subtitle.privacy", defaultValue: "A private snapshot with no transcripts or account details", comment: "Subtitle explaining privacy for shared insights image"))
                        .font(.system(size: 12))
            ...
            .accessibilityLabel(String(localized: "insights_share.preview.accessibility_label", defaultValue: "Preview of your Muesli activity image", comment: "Accessibility label for shared image preview"))
            ...
            if let saveErrorMessage {
                HStack(alignment: .top, spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(String(localized: "insights_share.error.save_failed", defaultValue: "The image couldn’t be saved", comment: "Error message when saving shared image fails"))
                    ...
                    .buttonStyle(.plain)
                    .accessibilityLabel(String(localized: "insights_share.error.dismiss.accessibility_label", defaultValue: "Dismiss save error", comment: "Accessibility label for dismissing save error"))
                }
                .padding(10)
            ...
            HStack(spacing: 10) {
                Button {
                    ...
                    Label(String(localized: "insights_share.action.copy_image", defaultValue: "Copy Image", comment: "Action label to copy generated insights image"), systemImage: "doc.on.doc")
                }
                ...
                Button {
                    ...
                    Label(String(localized: "insights_share.action.save_png", defaultValue: "Save PNG", comment: "Action label to save generated insights image as PNG"), systemImage: "arrow.down.to.line")
                }

                Button {
                    shareImage()
                } label: {
                    Label(String(localized: "insights_share.action.share", defaultValue: "Share…", comment: "Action label to open share sheet"), systemImage: "square.and.arrow.up")
                }
    ...
    private func copyImage() {
        ...
        showConfirmation(String(localized: "insights_share.confirmation.copied", defaultValue: "Copied", comment: "Confirmation text after copying image"))
    }
    ...
    private func saveImage() {
        ...
        panel.begin { response in
            ...
            Task { @MainActor in
                ...
                case .saved:
                    saveErrorMessage = nil
                    showConfirmation(String(localized: "insights_share.confirmation.saved", defaultValue: "Saved", comment: "Confirmation text after saving image"))
...
enum InsightsShareRenderer {
    ...
    static func renderTemplate() -> NSImage? {
        ...
        let renderer = ImageRenderer(
            content: InsightsShareCard(snapshot: snapshot, rangeLabel: String(localized: "insights_share.card.your_range", defaultValue: "YOUR RANGE", comment: "Header text on shared insights card"), showsNumbers: false)
                .frame(width: size.width, height: size.height)
        )
        renderer.scale = 1
        renderer.isOpaque = true
...
private struct InsightsShareCard: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text(String(localized: "insights_share.metric.words_captured", defaultValue: "WORDS CAPTURED", comment: "Metric label for captured words on shared insights card"))
                ...
                HStack(spacing: 0) {
                    shareDatum(value: showsNumbers ? snapshot.selected.meetings.formatted() : "—", label: String(localized: "insights_share.metric.meetings", defaultValue: "MEETINGS", comment: "Metric label for meetings on shared insights card"))
                    shareDivider
                    shareDatum(value: showsNumbers ? "\(Int(snapshot.selected.averageWPM.rounded()))" : "—", label: String(localized: "insights_share.metric.average_wpm", defaultValue: "AVERAGE WPM", comment: "Metric label for average words per minute on shared insights card"))
                    shareDivider
                    shareDatum(value: showsNumbers ? dayCount(snapshot.currentStreakDays) : "—", label: String(localized: "insights_share.metric.current_streak", defaultValue: "CURRENT STREAK", comment: "Metric label for current streak on shared insights card"))
                    shareDivider
                    shareDatum(value: showsNumbers ? dayCount(snapshot.longestStreakDays) : "—", label: String(localized: "insights_share.metric.longest_streak", defaultValue: "LONGEST STREAK", comment: "Metric label for longest streak on shared insights card"))
                }
                .padding(.vertical, 24)
                ...
                HStack {
                    Text(String(localized: "insights_share.footer.privacy_tagline", defaultValue: "Private by design. Made on this Mac.", comment: "Privacy tagline shown in shared insights footer"))
    ...
    private func dayCount(_ value: Int) -> String {
        "\(value) \(value == 1 ? String(localized: "insights_share.day_count.day_singular", defaultValue: "DAY", comment: "Unit label for singular day count on shared insights card") : String(localized: "insights_share.day_count.day_plural", defaultValue: "DAYS", comment: "Unit label for plural day count on shared insights card"))"
    }
}
