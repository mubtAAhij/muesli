...
struct InsightsShareSheet: View {
        ...
        VStack(spacing: 20) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text(String(localized: "insights_share.title", defaultValue: "Share your activity", comment: ""))
                        .font(.system(size: 20, weight: .semibold))
                        .tracking(-0.4)
                    Text(String(localized: "insights_share.subtitle.privacy", defaultValue: "A private snapshot with no transcripts or account details", comment: ""))
                        .font(.system(size: 12))
                        .foregroundStyle(.secondary)
                }
                Spacer()
                Button(String(localized: "common.done", defaultValue: "Done", comment: "")) { dismiss() }
            ...
            .accessibilityLabel(String(localized: "insights_share.preview.accessibility_label", defaultValue: "Preview of your Muesli activity image", comment: ""))
            ...
            if let saveErrorMessage {
                HStack(alignment: .top, spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(String(localized: "insights_share.error.save_failed", defaultValue: "The image couldn’t be saved", comment: ""))
                    ...
                    .buttonStyle(.plain)
                    .accessibilityLabel(String(localized: "insights_share.error.dismiss.accessibility_label", defaultValue: "Dismiss save error", comment: ""))
                }
                .padding(10)
            ...
            HStack(spacing: 10) {
                Button {
                    ...
                    Label(String(localized: "insights_share.action.copy_image", defaultValue: "Copy Image", comment: ""), systemImage: "doc.on.doc")
                }
                ...
                Button {
                    ...
                    Label(String(localized: "insights_share.action.save_png", defaultValue: "Save PNG", comment: ""), systemImage: "arrow.down.to.line")
                }

                Button {
                    shareImage()
                } label: {
                    Label(String(localized: "insights_share.action.share", defaultValue: "Share…", comment: ""), systemImage: "square.and.arrow.up")
                }
    ...
    private func copyImage() {
        ...
        showConfirmation(String(localized: "insights_share.confirmation.copied", defaultValue: "Copied", comment: ""))
    }
    ...
    private func saveImage() {
        ...
        panel.begin { response in
            ...
            Task { @MainActor in
                ...
                case .saved:
                    saveErrorMessage = nil
                    showConfirmation(String(localized: "insights_share.confirmation.saved", defaultValue: "Saved", comment: ""))
...
enum InsightsShareRenderer {
    ...
    static func renderTemplate() -> NSImage? {
        ...
        let renderer = ImageRenderer(
            content: InsightsShareCard(snapshot: snapshot, rangeLabel: String(localized: "insights_share.card.your_range", defaultValue: "YOUR RANGE", comment: ""), showsNumbers: false)
                .frame(width: size.width, height: size.height)
        )
        renderer.scale = 1
        renderer.isOpaque = true
...
private struct InsightsShareCard: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text(String(localized: "insights_share.metric.words_captured", defaultValue: "WORDS CAPTURED", comment: ""))
                ...
                HStack(spacing: 0) {
                    shareDatum(value: showsNumbers ? snapshot.selected.meetings.formatted() : "—", label: String(localized: "insights_share.metric.meetings", defaultValue: "MEETINGS", comment: ""))
                    shareDivider
                    shareDatum(value: showsNumbers ? "\(Int(snapshot.selected.averageWPM.rounded()))" : "—", label: String(localized: "insights_share.metric.average_wpm", defaultValue: "AVERAGE WPM", comment: ""))
                    shareDivider
                    shareDatum(value: showsNumbers ? dayCount(snapshot.currentStreakDays) : "—", label: String(localized: "insights_share.metric.current_streak", defaultValue: "CURRENT STREAK", comment: ""))
                    shareDivider
                    shareDatum(value: showsNumbers ? dayCount(snapshot.longestStreakDays) : "—", label: String(localized: "insights_share.metric.longest_streak", defaultValue: "LONGEST STREAK", comment: ""))
                }
                .padding(.vertical, 24)
                ...
                HStack {
                    Text(String(localized: "insights_share.footer.privacy_tagline", defaultValue: "Private by design. Made on this Mac.", comment: ""))
    ...
    private func dayCount(_ value: Int) -> String {
        "\(value) \(value == 1 ? String(localized: "insights_share.day_count.day_singular", defaultValue: "DAY", comment: "") : String(localized: "insights_share.day_count.day_plural", defaultValue: "DAYS", comment: ""))"
    }
}
