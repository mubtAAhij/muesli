...
enum ComputerUsePlannerError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthenticated:
            return String(localized: "computer_use_planner.error.connect_chatgpt_required", defaultValue: "Connect ChatGPT to use model-driven computer use.", comment: "Error shown when ChatGPT authentication is required for computer-use planner")
        case .invalidResponse(let message):
            return String(format: String(localized: "computer_use_planner.error.invalid_tool_call", defaultValue: "CUA planner returned an invalid tool call. %@", comment: "Error when planner returns invalid tool call with explanation"), "\(message)")
        case .invalidToolCall(let name, let arguments, let message):
            return String(format: String(localized: "computer_use_planner.error.invalid_tool_call_with_raw", defaultValue: "CUA planner returned an invalid tool call. %@ Raw native tool call: %@ %@", comment: "Error when planner returns invalid tool call including raw tool-call details"), "\(message)", "\(name)", String(arguments.prefix(800)))
        case .backendFailed(let statusCode, let message):
            return String(format: String(localized: "computer_use_planner.error.http_status_failed", defaultValue: "CUA planner failed with status %d. %@", comment: "Error when planner HTTP request fails with status and message"), statusCode, "\(message)")
        case .requestFailed(let message):
            return String(format: String(localized: "computer_use_planner.error.unreachable", defaultValue: "CUA planner could not be reached. %@", comment: "Error when planner service is unreachable"), "\(message)")
        }
    }
}
...
enum ComputerUsePlannerClient {
    ...
    private static func callWHAM(
        ...
        throw ComputerUsePlannerError.invalidResponse(
            trimmedText.isEmpty
                ? String(localized: "computer_use_planner.error.no_native_tool_call", defaultValue: "The model did not return a native tool call.", comment: "Error when model response lacks required native tool call")
                : String(format: String(localized: "computer_use_planner.error.text_returned_instead_of_tool_call", defaultValue: "The model returned text instead of a native tool call: %@", comment: "Error when model returns plain text instead of native tool call"), String(trimmedText.prefix(800)))
        )
    }
