...
final class FloatingIndicatorController: NSObject {
    ...
    private var transcribingTitle = String(localized: "floating_indicator.status.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Floating indicator status while speech is being transcribed.")
    ...
    func showComputerUseTranscript(_ transcript: String, config: AppConfig) {
        ...
        transcribingTitle = normalized.isEmpty ? String(localized: "floating_indicator.status.starting_cua", defaultValue: "Starting CUA", bundle: .module, comment: "Floating indicator status while Computer Use automation is starting.") : normalized
        setState(.transcribing, config: config)
    }
    ...
    func setState(_ state: DictationState, config: AppConfig) {
        ...
        if state != .transcribing {
            transcribingTitle = String(localized: "floating_indicator.status.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Floating indicator status while speech is being transcribed.")
            computerUseTranscriptText = nil
        }
        if state != .recording {
            recordingWaveformMode = .level
    ...
    static func idleHoverPillSize(hotkeyLabel: String, screenWidth: CGFloat) -> NSSize {
        let title = String(format: String(localized: "floating_indicator.title.hold_hotkey_to_dictate", defaultValue: "Hold %@ to dictate", bundle: .module, comment: "Floating indicator title showing the current hotkey needed to dictate."), "\(hotkeyLabel)")
