...
final class FloatingIndicatorController: NSObject {
    ...
    private var transcribingTitle = String(localized: "floating_indicator.status.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Floating indicator status text while transcription is running")
    ...
    func showComputerUseTranscript(_ transcript: String, config: AppConfig) {
        ...
        transcribingTitle = normalized.isEmpty ? String(localized: "floating_indicator.status.starting_cua", defaultValue: "Starting CUA", bundle: .module, comment: "Floating indicator status text while computer use automation is starting") : normalized
        setState(.transcribing, config: config)
    }
    ...
    func setState(_ state: DictationState, config: AppConfig) {
        ...
        if state != .transcribing {
            transcribingTitle = String(localized: "floating_indicator.status.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Floating indicator status text while transcription is running")
            computerUseTranscriptText = nil
        }
        if state != .recording {
            recordingWaveformMode = .level
    ...
    private func styleForState(_ state: DictationState, config: AppConfig) -> (background: NSColor, border: NSColor, icon: String, title: String, iconColor: NSColor, textColor: NSColor, alpha: CGFloat) {
        ...
        case .idle:
            return (
                ...
                "",
                isHovered ? String(format: String(localized: "floating_indicator.instruction.hold_hotkey_to_dictate", defaultValue: "Hold %@ to dictate", bundle: .module, comment: "Instruction shown in floating indicator telling user which hotkey to hold for dictation"), "\(config.dictationHotkey.label)") : "",
    ...
    static func idleHoverPillSize(hotkeyLabel: String, screenWidth: CGFloat) -> NSSize {
        let title = String(format: String(localized: "floating_indicator.title.hold_hotkey_to_dictate", defaultValue: "Hold %@ to dictate", bundle: .module, comment: "Floating indicator title instructing user to hold the displayed hotkey to dictate"), "\(hotkeyLabel)")
