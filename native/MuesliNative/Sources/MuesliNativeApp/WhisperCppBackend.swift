...
actor WhisperKitTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notLoaded: return String(localized: "whisper_cpp_backend.error.model_not_loaded", defaultValue: "WhisperKit model not loaded.", bundle: .module, comment: "Error shown when WhisperKit model has not been loaded")
            case .transcriptionFailed(let msg): return String(format: String(localized: "whisper_cpp_backend.error.transcription_failed", defaultValue: "Transcription failed: %@", bundle: .module, comment: "Error shown when transcription fails with backend-provided message"), "\(msg)")
            }
        }
    }
    ...
    func loadModel(
        ...
        let loadedWhisperKit = try await ManagedASRModelDownloader.loadValidated(
            ...
            let preparing = ModelDownloadProgress.preparing(
                ...
                message: String(localized: "whisper_cpp_backend.status.loading_coreml", defaultValue: "Loading WhisperKit into Core ML...", bundle: .module, comment: "Status text shown while loading WhisperKit into Core ML")
            )
