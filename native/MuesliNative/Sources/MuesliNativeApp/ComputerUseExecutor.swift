...
struct ComputerUseExecutionResult: Equatable {
    ...
    static func cancelled(_ message: String = String(localized: "computer_use.status.cancelled", defaultValue: "Cancelled", comment: "")) -> ComputerUseExecutionResult {
...
enum ComputerUseToolExecutor {
    ...
    static func execute(
        ...
        guard !toolCall.requiresConfirmation else {
            return .needsConfirmation(String(format: String(localized: "computer_use.confirmation.prompt", defaultValue: "Confirm: %@", comment: ""), "\(toolCall.summary)"))
        }
        ...
        case .getAppState, .getWindowState:
            ...
            return .executed(String(localized: "computer_use.status.captured_window_state", defaultValue: "Captured window state", comment: ""))
        case .moveCursor:
        ...
        case .fail:
            return .failed(toolCall.reason ?? String(localized: "computer_use.status.failed", defaultValue: "Failed", comment: ""))
        }
    }
    ...
    private static func listApps() -> ComputerUseExecutionResult {
        let apps = NSWorkspace.shared.runningApplications
            ...
            .map { app in
                String(format: String(localized: "computer_use.apps.item_summary", defaultValue: "%@ (%@, pid %d)%@", comment: ""), "\(app.localizedName ?? String(localized: "computer_use.apps.unknown_name", defaultValue: "Unknown", comment: ""))", "\(app.bundleIdentifier ?? String(localized: "computer_use.apps.unknown_bundle_id", defaultValue: "unknown", comment: ""))", app.processIdentifier, "\(app.isActive ? " " + String(localized: "computer_use.apps.active_suffix", defaultValue: "active", comment: "") : "")")
            }
            .prefix(80)
            .joined(separator: "\n")
        return .executed(apps.isEmpty ? String(localized: "computer_use.apps.empty", defaultValue: "No running apps", comment: "") : apps)
    }
    ...
    private static func listWindows(appBundleID: String) -> ComputerUseExecutionResult {
        ...
        guard !windows.isEmpty else {
            return .executed(String(localized: "computer_use.windows.empty", defaultValue: "No visible windows", comment: ""))
        }
    ...
    private static func windowInfos(appBundleID: String) -> [ComputerUseWindowInfo] {
        ...
        return windowList.compactMap { window in
            ...
            let ownerName = window[kCGWindowOwnerName] as? String ?? app?.localizedName ?? String(localized: "computer_use.windows.unknown_owner", defaultValue: "Unknown", comment: "")
    ...
    private static func openApp(named rawName: String) async -> ComputerUseExecutionResult {
        ...
        do {
            if let app = runningApplication(named: name) {
                ...
                return .executed(String(format: String(localized: "computer_use.open_app.opened_already_running", defaultValue: "Opened %@ (already running)", comment: ""), "\(name)"))
            }

            guard let appURL = try await applicationURL(for: name) else {
                return .failed(String(format: String(localized: "computer_use.open_app.could_not_find", defaultValue: "Could not find %@", comment: ""), "\(name)"))
            }
            ...
            app.activate(options: [.activateAllWindows])
            _ = try await waitUntilActive(app: app, timeout: 1.5)
            return .executed(String(format: String(localized: "computer_use.open_app.opened", defaultValue: "Opened %@", comment: ""), "\(name)"))
        } catch is CancellationError {
            return .cancelled(String(format: String(localized: "computer_use.open_app.cancelled", defaultValue: "Cancelled opening %@", comment: ""), "\(name)"))
        } catch {
            return .failed(String(format: String(localized: "computer_use.open_app.could_not_open", defaultValue: "Could not open %@: %@", comment: ""), "\(name)", "\(error.localizedDescription)"))
        }
    }
    ...
    private static func focusApp(named rawName: String) async -> ComputerUseExecutionResult {
        ...
        if let app = runningApplication(named: name) {
            ...
            do {
                ...
                return .cancelled(String(format: String(localized: "computer_use.focus_app.cancelled", defaultValue: "Cancelled focusing %@", comment: ""), "\(name)"))
            } catch {
                return .failed(String(format: String(localized: "computer_use.focus_app.could_not_focus", defaultValue: "Could not focus %@: %@", comment: ""), "\(name)", "\(error.localizedDescription)"))
            }
            return .executed(String(format: String(localized: "computer_use.focus_app.focused", defaultValue: "Focused %@", comment: ""), "\(name)"))
        }
    ...
    private static func pressKey(_ command: ComputerUseKeyCommand) -> ComputerUseExecutionResult {
        guard let keyCode = keyCode(for: command.key),
            ...
            return .unsupported(String(format: String(localized: "computer_use.press_key.unsupported_key", defaultValue: "Unsupported key %@", comment: ""), "\(command.key)"))
        }
        ...
        return .executed(String(localized: "computer_use.press_key.pressed", defaultValue: "Pressed key", comment: ""))
    }
    ...
    private static func scroll(direction: ComputerUseScrollDirection, pages: Double) -> ComputerUseExecutionResult {
        guard let source = CGEventSource(stateID: .combinedSessionState) else {
            return .failed(String(localized: "computer_use.scroll.could_not_create_event", defaultValue: "Could not create scroll event", comment: ""))
        }
        ...
        return .executed(String(format: String(localized: "computer_use.scroll.scrolled_direction", defaultValue: "Scrolled %@", comment: ""), "\(direction.rawValue)"))
    }
    ...
    private static func scrollElement(
        ...
        guard advertisedActions.contains(action) else {
            ...
            return .unsupported(String(format: String(localized: "computer_use.scroll_element.action_not_advertised", defaultValue: "Element does not advertise %@ for element-scoped scroll (actions: %@).", comment: ""), "\(action)", "\(actions)"))
        }
        let count = max(1, min(8, Int(pages.rounded(.up))))
        for _ in 0..<count {
            guard AXUIElementPerformAction(element, action as CFString) == .success else {
                return .failed(String(format: String(localized: "computer_use.scroll_element.could_not_perform_action", defaultValue: "Could not perform %@ on scroll target", comment: ""), "\(action)"))
            }
        }
        if let rect = rect(of: element) {
            ComputerUseCursorOverlay.shared.show(at: CGPoint(x: rect.midX, y: rect.midY), label: label)
        }
        return .executed(String(format: String(localized: "computer_use.scroll_element.scrolled_direction", defaultValue: "Scrolled element %@", comment: ""), "\(direction.rawValue)"))
    }
    ...
    private static func click(_ toolCall: ComputerUseToolCall, registry: ComputerUseElementRegistry?) -> ComputerUseExecutionResult {
        ...
        return .needsConfirmation(String(localized: "computer_use.click.confirm_unknown_target", defaultValue: "Confirm: unknown click target", comment: ""))
    }
    ...
    private static func performSecondaryAction(
        ...
        guard AXUIElementPerformAction(element, actionName as CFString) == .success else {
            return .failed(String(format: String(localized: "computer_use.perform_secondary_action.could_not_perform", defaultValue: "Could not perform %@ on %@", comment: ""), "\(actionName)", "\(elementTargetLabel(toolCall))"))
        }
        return .executed(String(format: String(localized: "computer_use.perform_secondary_action.performed", defaultValue: "Performed %@ on %@", comment: ""), "\(actionName)", "\(elementTargetLabel(toolCall))"))
    }
    ...
    private static func setValue(_ toolCall: ComputerUseToolCall, registry: ComputerUseElementRegistry?) -> ComputerUseExecutionResult {
        guard let elementResult = elementTarget(toolCall, registry: registry) else {
            return .failed(String(localized: "computer_use.set_value.stale_or_unknown_target", defaultValue: "Stale or unknown element target", comment: ""))
        }
        let element: AXUIElement
        switch elementResult {
        case .failure(let message):
        ...
        if result == .success {
            return .executed(String(localized: "computer_use.set_value.success", defaultValue: "Set value", comment: ""))
        }
    ...
    private static func elementTargetLabel(_ toolCall: ComputerUseToolCall) -> String {
        ...
        return String(localized: "computer_use.element.label", defaultValue: "element", comment: "")
    }
    ...
    private enum TextEntryMode {
        ...
        var completedMessage: String {
            switch self {
            case .keyboard: String(localized: "computer_use.enter_text.typed", defaultValue: "Typed text", comment: "")
            case .paste: String(localized: "computer_use.enter_text.pasted", defaultValue: "Pasted text", comment: "")
            }
        }
    }
    ...
    private static func clickElement(labeled rawLabel: String) -> ComputerUseExecutionResult {
        guard AXIsProcessTrusted() else {
            return .failed(String(localized: "computer_use.click_element.accessibility_permission_required", defaultValue: "Accessibility permission required", comment: ""))
        }
        guard let app = NSWorkspace.shared.frontmostApplication else {
            return .failed(String(localized: "computer_use.click_element.no_frontmost_app", defaultValue: "No frontmost app", comment: ""))
        }
        ...
        guard let match = findElement(labeled: label, in: root, maxDepth: 8, visited: []) else {
            return .failed(String(format: String(localized: "computer_use.click_element.could_not_find", defaultValue: "Could not find %@", comment: ""), "\(rawLabel)"))
        }

        if AXUIElementPerformAction(match, kAXPressAction as CFString) == .success {
            return .executed(String(format: String(localized: "computer_use.click_element.clicked", defaultValue: "Clicked %@", comment: ""), "\(rawLabel)"))
        }
        if clickCenter(of: match) {
            return .executed(String(format: String(localized: "computer_use.click_element.clicked_raw_label", defaultValue: "Clicked %@", comment: ""), "\(rawLabel)"))
        }
        return .failed(String(format: String(localized: "computer_use.click_element.could_not_click_raw_label", defaultValue: "Could not click %@", comment: ""), "\(rawLabel)"))
    }
    ...
    private static func clickElement(_ element: AXUIElement, fallbackLabel: String) -> ComputerUseExecutionResult {
        ...
        if axBool(element, kAXEnabledAttribute) == false {
            return .failed(String(format: String(localized: "computer_use.click_element.disabled_noop", defaultValue: "%@ is disabled; click would likely be a no-op", comment: ""), "\(fallbackLabel)"))
        }
        ...
        if AXUIElementPerformAction(element, kAXPressAction as CFString) == .success {
            return .executed(String(format: String(localized: "computer_use.click_element.clicked_fallback_label", defaultValue: "Clicked %@", comment: ""), "\(fallbackLabel)"))
        }
        if clickCenter(of: element) {
            return .executed("Clicked \(fallbackLabel) by coordinates after AXPress failed")
        }
        return .failed(String(format: String(localized: "computer_use.click_element.could_not_click_fallback_label", defaultValue: "Could not click %@", comment: ""), "\(fallbackLabel)"))
    }
    ...
    private static func clickPoint(
        ...
        guard let point = screenPoint(for: toolCall, registry: registry) else {
            return .failed(String(localized: "computer_use.click_point.no_current_screenshot", defaultValue: "No current screenshot for point click", comment: ""))
        }
        guard let source = CGEventSource(stateID: .combinedSessionState) else {
            return .failed(String(localized: "computer_use.click_point.could_not_create_mouse_event", defaultValue: "Could not create mouse event", comment: ""))
        }
        ...
        return .executed(String(format: String(localized: "computer_use.click_point.clicked_label", defaultValue: "Clicked %@", comment: ""), "\(label?.isEmpty == false ? label! : String(localized: "computer_use.click_point.point_fallback", defaultValue: "point", comment: ""))"))
    }
    ...
    private static func moveCursor(
        ...
        guard let point = screenPoint(for: toolCall, registry: registry) else {
            return .failed(String(localized: "computer_use.move_cursor.no_current_screenshot", defaultValue: "No current screenshot for cursor move", comment: ""))
        }
        CGWarpMouseCursorPosition(point)
        ComputerUseCursorOverlay.shared.show(at: point, label: toolCall.label)
        return .executed(String(format: String(localized: "computer_use.move_cursor.moved_to_coordinates", defaultValue: "Moved cursor to %d,%d", comment: ""), Int(point.x.rounded()), Int(point.y.rounded())))
    }
    ...
    private static func drag(
        ...
        guard let start = screenPoint(for: toolCall, registry: registry),
            ...
            return .failed(String(localized: "computer_use.drag.no_current_screenshot", defaultValue: "No current screenshot for drag", comment: ""))
        }
        guard let source = CGEventSource(stateID: .combinedSessionState),
            ...
            return .failed(String(localized: "computer_use.drag.could_not_create_event", defaultValue: "Could not create drag event", comment: ""))
        }
        ...
        return .executed(String(localized: "computer_use.drag.dragged_pointer", defaultValue: "Dragged pointer", comment: ""))
    }
    ...
    private static func textEntryTargetDescription(
        ...
        return appName.isEmpty ? "" : String(format: String(localized: "computer_use.text_entry_target.in_app_suffix", defaultValue: "in %@", comment: "Suffix describing the app target for text entry"), appName)
    }
