...
struct ComputerUseExecutionResult: Equatable {
    ...
    static func cancelled(_ message: String = String(localized: "computer_use.status.cancelled", defaultValue: "Cancelled", comment: "Status shown when computer use action is cancelled.")) -> ComputerUseExecutionResult {
...
enum ComputerUseToolExecutor {
    ...
    static func execute(
        ...
        guard !toolCall.requiresConfirmation else {
            return .needsConfirmation(String(format: String(localized: "computer_use.confirmation.prompt", defaultValue: "Confirm: %@", comment: "Confirmation prompt prefixing tool call summary."), "\(toolCall.summary)"))
        }
        ...
        case .getAppState, .getWindowState:
            ...
            return .executed(String(localized: "computer_use.status.captured_window_state", defaultValue: "Captured window state", comment: "Status indicating window state capture completed."))
        case .moveCursor:
    ...
    private static func listApps() -> ComputerUseExecutionResult {
        let apps = NSWorkspace.shared.runningApplications
            ...
            .map { app in
                String(format: String(localized: "computer_use.apps.item_summary", defaultValue: "%@ (%@, pid %d)%@", comment: "Summary line for a running app with optional active suffix."), "\(app.localizedName ?? String(localized: "computer_use.apps.unknown_name", defaultValue: "Unknown", comment: "Fallback app display name when localized name is unavailable."))", "\(app.bundleIdentifier ?? String(localized: "computer_use.apps.unknown_bundle_id", defaultValue: "unknown", comment: "Fallback bundle identifier text when unavailable."))", app.processIdentifier, "\(app.isActive ? " " + String(localized: "computer_use.apps.active_suffix", defaultValue: "active", comment: "Suffix appended to running app summary when app is active.") : "")")
            }
            .prefix(80)
            .joined(separator: "\n")
        return .executed(apps.isEmpty ? String(localized: "computer_use.apps.empty", defaultValue: "No running apps", comment: "Message when there are no running applications.") : apps)
    }
    ...
    private static func listWindows(appBundleID: String) -> ComputerUseExecutionResult {
        ...
        guard !windows.isEmpty else {
            return .executed(String(localized: "computer_use.windows.empty", defaultValue: "No visible windows", comment: "Message when there are no visible windows."))
        }
    ...
    private static func windowInfos(appBundleID: String) -> [ComputerUseWindowInfo] {
        ...
        return windowList.compactMap { window in
            ...
            let ownerName = window[kCGWindowOwnerName] as? String ?? app?.localizedName ?? String(localized: "computer_use.windows.unknown_owner", defaultValue: "Unknown", comment: "Fallback window owner name when unavailable.")
    ...
    private static func openApp(named rawName: String) async -> ComputerUseExecutionResult {
        ...
        do {
            if let app = runningApplication(named: name) {
                ...
                return .executed(String(format: String(localized: "computer_use.open_app.opened_already_running", defaultValue: "Opened %@ (already running)", comment: "Result when opening app that was already running."), "\(name)"))
            }

            guard let appURL = try await applicationURL(for: name) else {
                return .failed(String(format: String(localized: "computer_use.open_app.could_not_find", defaultValue: "Could not find %@", comment: "Error when requested app cannot be found."), "\(name)"))
            }
            ...
            app.activate(options: [.activateAllWindows])
            _ = try await waitUntilActive(app: app, timeout: 1.5)
            return .executed(String(format: String(localized: "computer_use.open_app.opened", defaultValue: "Opened %@", comment: "Result when app opens successfully."), "\(name)"))
        } catch is CancellationError {
            return .cancelled(String(format: String(localized: "computer_use.open_app.cancelled", defaultValue: "Cancelled opening %@", comment: "Result when opening app is cancelled."), "\(name)"))
        } catch {
            return .failed(String(format: String(localized: "computer_use.open_app.could_not_open", defaultValue: "Could not open %@: %@", comment: "Error when opening app fails with system message."), "\(name)", "\(error.localizedDescription)"))
        }
    }
    ...
    private static func focusApp(named rawName: String) async -> ComputerUseExecutionResult {
        ...
        if let app = runningApplication(named: name) {
            ...
            do {
                ...
                return .cancelled(String(format: String(localized: "computer_use.focus_app.cancelled", defaultValue: "Cancelled focusing %@", comment: "Result when focusing app is cancelled."), "\(name)"))
            } catch {
                return .failed(String(format: String(localized: "computer_use.focus_app.could_not_focus", defaultValue: "Could not focus %@: %@", comment: "Error when focusing app fails with system message."), "\(name)", "\(error.localizedDescription)"))
            }
            return .executed(String(format: String(localized: "computer_use.focus_app.focused", defaultValue: "Focused %@", comment: "Result when app is focused successfully."), "\(name)"))
        }
    ...
    private static func pressKey(_ command: ComputerUseKeyCommand) -> ComputerUseExecutionResult {
        guard let keyCode = keyCode(for: command.key),
            ...
            return .unsupported(String(format: String(localized: "computer_use.press_key.unsupported_key", defaultValue: "Unsupported key %@", comment: "Error when requested key press is not supported."), "\(command.key)"))
        }
        ...
        return .executed(String(localized: "computer_use.press_key.pressed", defaultValue: "Pressed key", comment: "Result after pressing a keyboard key."))
    }
    ...
    private static func scroll(direction: ComputerUseScrollDirection, pages: Double) -> ComputerUseExecutionResult {
        guard let source = CGEventSource(stateID: .combinedSessionState) else {
            return .failed(String(localized: "computer_use.scroll.could_not_create_event", defaultValue: "Could not create scroll event", comment: "Error when synthetic scroll event cannot be created."))
        }
        ...
        return .executed(String(format: String(localized: "computer_use.scroll.scrolled_direction", defaultValue: "Scrolled %@", comment: "Result after scrolling in a direction."), "\(direction.rawValue)"))
    }
    ...
    private static func scrollElement(
        ...
        guard advertisedActions.contains(action) else {
            ...
            return .unsupported(String(format: String(localized: "computer_use.scroll_element.action_not_advertised", defaultValue: "Element does not advertise %@ for element-scoped scroll (actions: %@).", comment: "Error when target element does not expose the requested scroll action."), "\(action)", "\(actions)"))
        }
        let count = max(1, min(8, Int(pages.rounded(.up))))
        for _ in 0..<count {
            guard AXUIElementPerformAction(element, action as CFString) == .success else {
                return .failed(String(format: String(localized: "computer_use.scroll_element.could_not_perform_action", defaultValue: "Could not perform %@ on scroll target", comment: "Error when requested scroll action cannot be performed on element."), "\(action)"))
            }
        }
        if let rect = rect(of: element) {
            ComputerUseCursorOverlay.shared.show(at: CGPoint(x: rect.midX, y: rect.midY), label: label)
        }
        return .executed(String(format: String(localized: "computer_use.scroll_element.scrolled_direction", defaultValue: "Scrolled element %@", comment: "Result after element-scoped scroll in a direction."), "\(direction.rawValue)"))
    }
    ...
    private static func click(_ toolCall: ComputerUseToolCall, registry: ComputerUseElementRegistry?) -> ComputerUseExecutionResult {
        ...
        return .needsConfirmation(String(localized: "computer_use.click.confirm_unknown_target", defaultValue: "Confirm: unknown click target", comment: "Confirmation prompt when click target could not be resolved."))
    }
    ...
    private static func performSecondaryAction(
        ...
        guard let elementResult = elementTarget(toolCall, registry: registry) else {
            return .failed(String(localized: "computer_use.perform_secondary_action.requires_element_target", defaultValue: "perform_secondary_action requires element_index or element_id", comment: "Error when secondary action is missing both element index and element ID."))
        }
        ...
        guard advertisedActions.contains(actionName) else {
            let actions = advertisedActions.isEmpty ? "none" : advertisedActions.joined(separator: ", ")
            return .unsupported(String(format: String(localized: "computer_use.perform_secondary_action.not_advertised", defaultValue: "Element does not advertise %@ (actions: %@). Run get_app_state again if the target changed.", comment: "Error when target element does not advertise requested secondary action."), "\(actionName)", "\(actions)"))
        }
        if let rect = rect(of: element) {
        ...
        guard AXUIElementPerformAction(element, actionName as CFString) == .success else {
            return .failed(String(format: String(localized: "computer_use.perform_secondary_action.could_not_perform", defaultValue: "Could not perform %@ on %@", comment: "Error for failed secondary action on target element."), "\(actionName)", "\(elementTargetLabel(toolCall))"))
        }
        return .executed(String(format: String(localized: "computer_use.perform_secondary_action.performed", defaultValue: "Performed %@ on %@", comment: "Result for successful secondary action on target element."), "\(actionName)", "\(elementTargetLabel(toolCall))"))
    }
    ...
    private static func setValue(_ toolCall: ComputerUseToolCall, registry: ComputerUseElementRegistry?) -> ComputerUseExecutionResult {
        guard let elementResult = elementTarget(toolCall, registry: registry) else {
            return .failed(String(localized: "computer_use.set_value.stale_or_unknown_target", defaultValue: "Stale or unknown element target", comment: "Error when set-value target is stale or unresolved."))
        }
        let element: AXUIElement
        switch elementResult {
        case .failure(let message):
        ...
        if result == .success {
            return .executed(String(localized: "computer_use.set_value.success", defaultValue: "Set value", comment: "Result when value is set successfully."))
        }
        return .unsupported(String(localized: "computer_use.set_value.not_supported", defaultValue: "Element does not support set_value", comment: "Error when target element cannot set AXValue attribute."))
    }
    ...
    private static func elementTargetLabel(_ toolCall: ComputerUseToolCall) -> String {
        ...
        return String(localized: "computer_use.element.label", defaultValue: "element", comment: "Generic fallback label for an accessibility element.")
    }
    ...
    private enum TextEntryMode {
        ...
        var completedMessage: String {
            switch self {
            case .keyboard: String(localized: "computer_use.enter_text.typed", defaultValue: "Typed text", comment: "Result after entering text by typing.")
            case .paste: String(localized: "computer_use.enter_text.pasted", defaultValue: "Pasted text", comment: "Result after entering text by paste action.")
            }
        }
    }
    ...
    private static func clickElement(labeled rawLabel: String) -> ComputerUseExecutionResult {
        guard AXIsProcessTrusted() else {
            return .failed(String(localized: "computer_use.click_element.accessibility_permission_required", defaultValue: "Accessibility permission required", comment: "Error indicating accessibility permission is needed."))
        }
        guard let app = NSWorkspace.shared.frontmostApplication else {
            return .failed(String(localized: "computer_use.click_element.no_frontmost_app", defaultValue: "No frontmost app", comment: "Error when no frontmost app is available for click action."))
        }
        ...
        guard let match = findElement(labeled: label, in: root, maxDepth: 8, visited: []) else {
            return .failed(String(format: String(localized: "computer_use.click_element.could_not_find", defaultValue: "Could not find %@", comment: "Error when labeled click target cannot be found."), "\(rawLabel)"))
        }

        if AXUIElementPerformAction(match, kAXPressAction as CFString) == .success {
            return .executed(String(format: String(localized: "computer_use.click_element.clicked", defaultValue: "Clicked %@", comment: "Result after clicking the resolved label target."), "\(rawLabel)"))
        }
        if clickCenter(of: match) {
            return .executed(String(format: String(localized: "computer_use.click_element.clicked_raw_label", defaultValue: "Clicked %@", comment: "Result after clicking using raw label fallback target."), "\(rawLabel)"))
        }
        return .failed(String(format: String(localized: "computer_use.click_element.could_not_click_raw_label", defaultValue: "Could not click %@", comment: "Error when click action fails for raw label target."), "\(rawLabel)"))
    }
    ...
    private static func clickElement(_ element: AXUIElement, fallbackLabel: String) -> ComputerUseExecutionResult {
        ...
        if axBool(element, kAXEnabledAttribute) == false {
            return .failed(String(format: String(localized: "computer_use.click_element.disabled_noop", defaultValue: "%@ is disabled; click would likely be a no-op", comment: "Warning that fallback click target appears disabled."), "\(fallbackLabel)"))
        }
        ...
        if AXUIElementPerformAction(element, kAXPressAction as CFString) == .success {
            return .executed(String(format: String(localized: "computer_use.click_element.clicked_fallback_label", defaultValue: "Clicked %@", comment: "Result after clicking fallback label target."), "\(fallbackLabel)"))
        }
        if clickCenter(of: element) {
            return .executed("Clicked \(fallbackLabel) by coordinates after AXPress failed")
        }
        return .failed(String(format: String(localized: "computer_use.click_element.could_not_click_fallback_label", defaultValue: "Could not click %@", comment: "Error when clicking fallback label target fails."), "\(fallbackLabel)"))
    }
    ...
    private static func clickPoint(
        ...
        guard let point = screenPoint(for: toolCall, registry: registry) else {
            return .failed(String(localized: "computer_use.click_point.no_current_screenshot", defaultValue: "No current screenshot for point click", comment: "Error when point click is requested without an available screenshot."))
        }
        guard let source = CGEventSource(stateID: .combinedSessionState) else {
            return .failed(String(localized: "computer_use.click_point.could_not_create_mouse_event", defaultValue: "Could not create mouse event", comment: "Error when mouse event creation fails for point click."))
        }
        ...
        return .executed(String(format: String(localized: "computer_use.click_point.clicked_label", defaultValue: "Clicked %@", comment: "Result after clicking a point with optional label fallback."), "\(label?.isEmpty == false ? label! : String(localized: "computer_use.click_point.point_fallback", defaultValue: "point", comment: "Fallback label for unlabeled point click target."))"))
    }
    ...
    private static func moveCursor(
        ...
        guard let point = screenPoint(for: toolCall, registry: registry) else {
            return .failed(String(localized: "computer_use.move_cursor.no_current_screenshot", defaultValue: "No current screenshot for cursor move", comment: "Error when cursor move is requested without an available screenshot."))
        }
        CGWarpMouseCursorPosition(point)
        ComputerUseCursorOverlay.shared.show(at: point, label: toolCall.label)
        return .executed(String(format: String(localized: "computer_use.move_cursor.moved_to_coordinates", defaultValue: "Moved cursor to %d,%d", comment: "Result after moving cursor to x,y coordinates."), Int(point.x.rounded()), Int(point.y.rounded())))
    }
    ...
    private static func drag(
        ...
        guard let start = screenPoint(for: toolCall, registry: registry),
            ...
            return .failed(String(localized: "computer_use.drag.no_current_screenshot", defaultValue: "No current screenshot for drag", comment: "Error when drag is requested without an available screenshot."))
        }
        guard let source = CGEventSource(stateID: .combinedSessionState),
            ...
            return .failed(String(localized: "computer_use.drag.could_not_create_event", defaultValue: "Could not create drag event", comment: "Error when drag event creation fails."))
        }
        ...
        return .executed(String(localized: "computer_use.drag.dragged_pointer", defaultValue: "Dragged pointer", comment: "Result after pointer drag action completes."))
    }
    ...
    private static func textEntryTargetDescription(
        ...
        return appName.isEmpty ? "" : String(format: String(localized: "computer_use.text_entry_target.in_app_suffix", defaultValue: "in %@", comment: "Suffix indicating the target app for text entry actions."), "\(appName)")
    }
