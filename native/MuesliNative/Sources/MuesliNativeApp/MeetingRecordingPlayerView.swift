...
struct MeetingRecordingPlayerView: View {
    ...
    var body: some View {
        HStack(spacing: MuesliTheme.spacing12) {
            ...
            .buttonStyle(.plain)
            .disabled(player == nil)
            .help(isPlaying ? String(localized: "meeting_recording_player.help.pause_recording", defaultValue: "Pause recording", comment: "Help text for control that pauses meeting recording") : String(localized: "meeting_recording_player.help.play_recording", defaultValue: "Play recording", comment: "Help text for control that plays meeting recording"))
            ...
            Group {
                if let waveform {
                    ...
                    Text(String(localized: "meeting_recording_player.status.recording_unavailable", defaultValue: "Recording unavailable", comment: "Status label when meeting recording is unavailable"))
                    ...
                    HStack(spacing: MuesliTheme.spacing8) {
                        ProgressView()
                            .controlSize(.small)
                        Text(String(localized: "meeting_recording_player.status.loading_recording", defaultValue: "Loading recording", comment: "Status label while meeting recording is loading"))
                            .font(MuesliTheme.captionMedium())
                            .foregroundStyle(MuesliTheme.textTertiary)
                    }
                    .frame(maxWidth: .infinity, alignment: .leading)
...
private struct RecordingWaveformView: View {
    ...
    var body: some View {
        ...
        .frame(minHeight: 36)
        .accessibilityLabel(String(localized: "meeting_recording_player.accessibility.waveform_label", defaultValue: "Recording waveform", comment: "Accessibility label for meeting recording waveform visualization"))
    }
