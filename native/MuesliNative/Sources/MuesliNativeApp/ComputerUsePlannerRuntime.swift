...
final class ComputerUsePlannerRuntime {
    ...
    func run(command: String) async -> ComputerUsePlannerRuntimeResult {
        ...
        while true {
            ...
            if Date() >= deadline {
                ...
                return .init(status: .timedOut, message: String(localized: "computer_use.runtime.status.cua_timed_out", defaultValue: "CUA timed out", bundle: .module, comment: "Status label shown when CUA times out"), traceEvents: traceEvents)
            }
            if let maxSteps, step > maxSteps {
                traceEvents.append(traceEvent(kind: "failed", title: "Failed", body: String(localized: "computer_use.runtime.status.cua_step_limit_reached", defaultValue: "CUA reached its step limit", bundle: .module, comment: "Status label shown when CUA reaches its maximum step limit"), status: "failed", step: maxSteps))
                return .init(status: .failed, message: String(localized: "computer_use.runtime.status.cua_step_limit_reached", defaultValue: "CUA reached its step limit", bundle: .module, comment: "Status label shown when CUA reaches its maximum step limit"), traceEvents: traceEvents)
            }
            defer { step += 1 }
            ...
            do {
                ...
                traceEvents.append(traceEvent(kind: "failed", title: String(localized: "computer_use.runtime.status.planner_failed", defaultValue: "Planner failed", bundle: .module, comment: "Status label shown when planner execution fails"), body: repairMessage, status: "failed", step: step))
                return .init(status: .failed, message: repairMessage, traceEvents: traceEvents)
            } catch {
                traceEvents.append(traceEvent(
                    kind: "failed",
                    title: String(localized: "computer_use.runtime.status.planner_failed", defaultValue: "Planner failed", bundle: .module, comment: "Status label shown when planner execution fails"),
            ...
            if toolCall.requiresConfirmation {
                onStatus("Confirm")
                let message = String(format: String(localized: "computer_use.runtime.confirmation.message", defaultValue: "Confirm: %@", bundle: .module, comment: "Confirmation message with tool call summary"), "\(toolCall.summary)")
            ...
            case .getAppState, .getWindowState:
                ...
                if let blocked = feedback.blocked {
                    traceEvents.append(traceEvent(kind: "failed", title: String(localized: "computer_use.runtime.status.repeated_action_stopped", defaultValue: "Repeated action stopped", bundle: .module, comment: "Status label shown when repeated action detection stops execution"), body: blocked, status: "failed", step: step))
    ...
    private func cancelledResult(traceEvents: [ComputerUseTraceEvent], step: Int) -> ComputerUsePlannerRuntimeResult {
        ...
        return .init(status: .cancelled, message: String(localized: "computer_use.runtime.status.cua_cancelled", defaultValue: "CUA cancelled", bundle: .module, comment: "Status label shown when CUA execution is cancelled"), traceEvents: events)
    }
    ...
    private func observationEvent(_ observation: ComputerUseObservation, step: Int?) -> ComputerUseTraceEvent {
        let app = observation.appName.isEmpty ? String(localized: "computer_use_planner.app.unknown", defaultValue: "Unknown app", bundle: .module, comment: "Fallback app name when target app cannot be identified") : observation.appName
    ...
    private func stateDelta(
        ...
        let summary: String
        if status == .changed {
            summary = String(format: String(localized: "computer_use_planner.state_delta.ui_state_changed", defaultValue: "Observed UI state changed after %@.", bundle: .module, comment: "Summary indicating UI state changed after a tool call"), "\(toolCall.summary)")
        } else if toolCall.tool == .typeText || toolCall.tool == .pasteText || toolCall.tool == .setValue {
            summary = String(format: String(localized: "computer_use_planner.state_delta.no_text_change_guidance", defaultValue: "%@ executed but no focused value, selected text, or visible AX text change was observed; refocus the editable target or use a different insertion primitive.", bundle: .module, comment: "Guidance when text insertion attempt causes no observed text change"), "\(toolCall.summary)")
        } else {
            summary = String(format: String(localized: "computer_use_planner.state_delta.no_relevant_change_guidance", defaultValue: "%@ executed but no relevant UI change was observed; choose a different strategy.", bundle: .module, comment: "Guidance when a tool call causes no relevant UI change"), "\(toolCall.summary)")
        }
        return ComputerUseStateDelta(
            status: status,
            summary: summary,
    ...
    private func verifiedOutcomeMessage(base: String, delta: ComputerUseStateDelta?) -> String {
        ...
        return String(format: String(localized: "computer_use_planner.verified_outcome.message", defaultValue: "%@. Verification: %@", bundle: .module, comment: "Outcome message with base result and verification summary"), "\(base)", "\(delta.summary)")
    }
    ...
    private func observationToolFeedback(
        ...
        guard observationSignature(before) == observationSignature(after) else {
            ...
            return (
                String(format: String(localized: "computer_use_planner.observation_feedback.captured_fresh_state", defaultValue: "%@. Captured fresh state; continue from the visible AX/screenshot context.", bundle: .module, comment: "Feedback message confirming fresh state capture and next guidance"), "\(base)"),
                nil
            )
        }
    ...
    private func repeatedUnchangedMessage(
        ...
        return String(format: String(localized: "computer_use_planner.feedback.repeated_unchanged_attempts", defaultValue: "CUA stopped repeated %@ after two unchanged attempts: no relevant UI change was observed. Choose a different strategy after running get_app_state.", bundle: .module, comment: "Feedback when repeated unchanged attempts are halted"), "\(toolCall.summary)")
    }
    ...
    private func resultStatusTitle(
        ...
        case .performSecondaryAction:
            return String(localized: "computer_use_planner.result_status.performed_action", defaultValue: "Performed action", bundle: .module, comment: "Generic result status for completed action")
        case .moveCursor:
            return "Moving cursor"
        case .typeText:
            return "Typed text"
        case .pasteText:
            return String(localized: "computer_use_planner.result_status.pasted_text", defaultValue: "Pasted text", bundle: .module, comment: "Result status for pasting text action")
        case .openNewBrowserTab:
            return "Opened new tab"
        case .navigateURL, .navigateActiveBrowserTab:
            return "Navigated"
        case .pressKey, .hotkey:
            return String(localized: "computer_use_planner.result_status.pressed_key", defaultValue: "Pressed key", bundle: .module, comment: "Result status shown after key press action completes")
        case .scroll:
            return String(localized: "computer_use_planner.result_status.scrolled", defaultValue: "Scrolled", bundle: .module, comment: "Result status shown after scroll action completes")
        case .setValue:
            return String(localized: "computer_use_planner.result_status.set_value", defaultValue: "Set value", bundle: .module, comment: "Result status shown after set-value action completes")
        case .drag:
            return String(localized: "computer_use_planner.result_status.dragged", defaultValue: "Dragged", bundle: .module, comment: "Result status shown after drag action completes")
        case .activateBrowserTab:
            return String(localized: "computer_use_planner.result_status.switched_tab", defaultValue: "Switched tab", bundle: .module, comment: "Result status shown after tab switch action completes")
        default:
            return nil
        }
    }
    ...
    private func statusTitle(for toolCall: ComputerUseToolCall) -> String {
        ...
        case .launchApp:
            ...
            return String(format: "Opening %@", "\(target?.isEmpty == false ? target! : String(localized: "computer_use_planner.status.target_app_fallback", defaultValue: "app", bundle: .module, comment: "Fallback target name used when no app name is available"))")
        case .click, .clickElement, .clickPoint:
            return "Clicking"
        case .performSecondaryAction:
            return String(localized: "computer_use_planner.status.performing_action", defaultValue: "Performing action", bundle: .module, comment: "Generic in-progress status shown while executing a planner action")
        case .moveCursor:
            return toolCall.label?.isEmpty == false ? String(format: "Moving to %@", "\(toolCall.label!)") : "Moving cursor"
        case .setValue:
            return String(localized: "computer_use_planner.status.setting_value", defaultValue: "Setting value", bundle: .module, comment: "In-progress status shown while setting a value")
        case .typeText:
            return "Typing"
        case .pasteText:
            return String(localized: "computer_use_planner.status.pressing_key", defaultValue: "Pasting", bundle: .module, comment: "In-progress status shown while pasting text")
        case .pressKey, .hotkey:
            return "Pressing key"
        case .scroll:
            return "Scrolling"
        case .drag:
            return String(localized: "computer_use_planner.status.opening_new_tab", defaultValue: "Dragging", bundle: .module, comment: "In-progress status shown while dragging")
        case .openNewBrowserTab:
            return "Opening new tab"
        case .navigateURL, .navigateActiveBrowserTab:
            return String(localized: "computer_use_planner.status.navigating-opening-new-tab-opening-new-tab", defaultValue: "Navigating", bundle: .module, comment: "In-progress status shown while navigating to a destination")
        case .activateBrowserTab:
            return String(localized: "computer_use_planner.status.switching_tab", defaultValue: "Switching tab", bundle: .module, comment: "Status shown while switching tabs")
        case .listApps, .listWindows, .listBrowserTabs, .pageGetText, .pageQueryDOM:
            return String(localized: "computer_use_planner.status.reading", defaultValue: "Reading", bundle: .module, comment: "Status shown while reading content")
        case .getAppState, .getWindowState:
            return String(localized: "computer_use_planner.status.observing", defaultValue: "Observing", bundle: .module, comment: "Status shown while observing UI state")
        case .finish:
            return "Done"
        case .fail:
            return "Failed"
    ...
    private func planWithRetry(
        ...
        while true {
            ...
            do {
                ...
                attempt += 1
                let message = String(format: String(localized: "computer_use_planner.message.transient_failure_retrying", defaultValue: "Planner request failed transiently: %@. Retrying once.", bundle: .module, comment: "Message shown when planner request transiently fails and retries once"), "\(error.localizedDescription)")
