...
struct DictationsView: View {
    ...
    private var groupedDictations: [(id: Date, header: String, records: [DictationRecord])] {
        ...
        for record in appState.dictationRows {
            ...
            if dayStart != currentDayStart {
                ...
                if dayStart == today {
                    currentHeader = String(localized: "dictations.section.today", defaultValue: "TODAY", bundle: .module, comment: "Section header title for today's dictations.")
                } else if dayStart == yesterday {
                    currentHeader = String(localized: "dictations.section.yesterday", defaultValue: "YESTERDAY", bundle: .module, comment: "Section header title for yesterday's dictations.")
                } else {
                                    ...
                                    ForEach(group.records) { record in
                                        ...
                                        .contextMenu {
                                            Button {
                                                ...
                                                Label(String(localized: "common.copy", defaultValue: "Copy", bundle: .module, comment: "Menu action to copy dictation content."), systemImage: "doc.on.doc")
                                            }
                                            if record.computerUseTrace != nil {
                                                Button {
                                                    controller.copyToClipboard(ComputerUseTraceFormatter.debugText(for: record))
                                                } label: {
                                                    Label(String(localized: "dictations.actions.copy_cua_trace", defaultValue: "Copy CUA Trace", bundle: .module, comment: "Menu action to copy CUA trace from dictation."), systemImage: "list.bullet.clipboard")
                                                }
                                            }
                                        }
                                    }
    ...
    private var emptyStateInstruction: String {
        if appState.dictationOriginFilter != .all
            ...
            return String(localized: "dictations.empty_state.instruction.adjust_filters", defaultValue: "Try another source, app, or time range", bundle: .module, comment: "Empty-state instruction to adjust dictation filters.")
        }
        return appState.config.resolvedOnboardingUseCase.includesVoiceNotes
            ? String(localized: "dictations.empty_state.instruction.record_first_note", defaultValue: "Click Record Voice Note to capture your first note", bundle: .module, comment: "Empty-state instruction for first voice note recording.")
            : String(format: String(localized: "dictations.empty_state.instruction.hold_hotkey_to_dictate", defaultValue: "Hold %@ to start dictating", bundle: .module, comment: "Empty-state instruction showing hotkey to start dictation."), "\(appState.config.dictationHotkey.label)")
    }

    private var emptyStateTitle: String {
        if let application = appState.dictationApplicationFilter {
            return String(format: String(localized: "dictations.empty_state.title.no_dictations_for_app", defaultValue: "No dictations for %@", bundle: .module, comment: "Empty-state title when no dictations exist for selected app."), "\(application.name)")
        }
        switch appState.dictationOriginFilter {
        case .all: return String(localized: "dictations.empty_state.title.no_dictations_yet", defaultValue: "No dictations yet", bundle: .module, comment: "Empty-state title when no dictations exist yet.")
        case .thisMac: return String(localized: "dictations.empty_state.title.no_dictations_from_this_mac", defaultValue: "No dictations from this Mac", bundle: .module, comment: "Empty-state title when no dictations from this Mac exist.")
        case .fromIPhone: return String(localized: "dictations.empty_state.title.no_dictations_from_iphone", defaultValue: "No dictations from iPhone", bundle: .module, comment: "Empty-state title when no dictations from iPhone exist.")
        }
    }
        ...
        return Button {
            ...
            HStack(spacing: 6) {
                Image(systemName: isRecording ? "stop.fill" : "mic.fill")
                    .font(.system(size: 12, weight: .semibold))
                Text(isRecording ? String(localized: "dictations.voice_note.stop", defaultValue: "Stop Voice Note", bundle: .module, comment: "Button title to stop recording a voice note.") : String(localized: "dictations.voice_note.record", defaultValue: "Record Voice Note", bundle: .module, comment: "Button title to start recording a voice note."))
                    .font(.system(size: 12, weight: .semibold))
            }
