...
actor SenseVoiceTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notLoaded:
                return String(localized: "sensevoice_backend.error.models_not_loaded", defaultValue: "SenseVoice models not loaded. Call loadModels() first.", comment: "Error when transcription is requested before SenseVoice models are loaded")
            }
        }
    }
    ...
    func loadModels(
        ...
        let loadedManager = try await ManagedASRModelDownloader.loadValidated(
            ...
            let preparing = ModelDownloadProgress.preparing(
                ...
                message: String(localized: "sensevoice_backend.status.loading_into_coreml", defaultValue: "Loading SenseVoice into Core ML...", comment: "Status text shown while loading SenseVoice model into Core ML")
            )
            progressSnapshot?(preparing)
            progress?(0.95, String(localized: "sensevoice_backend.status.loading", defaultValue: "Loading SenseVoice...", comment: "General status text shown while SenseVoice models are loading"))
        ...
        progress?(1.0, nil)
        progressSnapshot?(preparing.replacing(phase: .ready, message: String(localized: "sensevoice_backend.status.model_ready", defaultValue: "Model ready", comment: "Status text shown when SenseVoice model is ready")))
    ...
    private func warmupIfNeeded(progress: ((Double, String?) -> Void)?) async {
        ...
        progress?(0.98, String(localized: "sensevoice_backend.status.warming_up", defaultValue: "Warming up SenseVoice...", comment: "Status text shown while SenseVoice is warming up"))
