...
actor SenseVoiceTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notLoaded:
                return String(localized: "sensevoice_backend.error.models_not_loaded", defaultValue: "SenseVoice models not loaded. Call loadModels() first.", bundle: .module, comment: "Error shown when SenseVoice transcription is requested before model load")
            }
        }
    }
    ...
    func loadModels(
        ...
        let loadedManager = try await ManagedASRModelDownloader.loadValidated(
            ...
            progress?(0.95, String(localized: "sensevoice_backend.status.loading", defaultValue: "Loading SenseVoice...", bundle: .module, comment: "Status shown while loading SenseVoice resources"))
        ...
        progress?(1.0, nil)
        progressSnapshot?(preparing.replacing(phase: .ready, message: String(localized: "sensevoice_backend.status.model_ready", defaultValue: "Model ready", bundle: .module, comment: "Status shown when SenseVoice model is ready")))
    ...
    private func warmupIfNeeded(progress: ((Double, String?) -> Void)?) async {
        ...
        progress?(0.98, String(localized: "sensevoice_backend.status.warming_up", defaultValue: "Warming up SenseVoice...", bundle: .module, comment: "Status shown while SenseVoice backend performs warm-up"))
