            ...
            if displayIndex < total {
                ...
                VStack(spacing: MuesliTheme.spacing8) {
                    Text(String(format: String(
                        localized: "onboarding.permissions.progress-label",
                        defaultValue: "Permission %d of %d",
                        comment: ""
            ...
            if !isSelectedModelReadyForDictationTest {
                VStack(spacing: MuesliTheme.spacing8) {
                    if isModelPreparingAfterDownload {
                        ...
                        Text(modelDownloadStatus ?? String(format: String(
                            localized: "onboarding.model-setup.preparing-backend",
                        ...
                        ProgressView(value: modelDownloadProgress, total: 1.0)
                            .frame(width: 260)
                        Text(modelDownloadStatus ?? String(format: String(
                            localized: "onboarding.model-download.progress-percent-complete",
                            defaultValue: "%d%% complete",
                            comment: ""
                        ), Int((modelDownloadProgress * 100).rounded())))
                            .font(.system(size: 11, weight: .medium, design: .rounded))
                            .foregroundStyle(MuesliTheme.textTertiary)
                    } else {
                        ProgressView()
                ...
                VStack(spacing: MuesliTheme.spacing16) {
                    Text(dictationTestResult ?? String(
                        localized: "onboarding.live-transcription.placeholder",
                        defaultValue: "Your transcription will appear here...",
                        comment: ""
                    ))
                    ...
                    if isDictationTesting {
                        HStack(spacing: 8) {
                            ProgressView()
                                .controlSize(.small)
                            Text(String(format: String(
                                localized: "onboarding.push-to-talk.listening-release-hint",
                                defaultValue: "Listening... release %@ when done",
                                comment: ""
                        ...
                        HStack(spacing: 6) {
                            Image(systemName: "keyboard")
                                .font(.system(size: 14))
                            Text(String(format: String(
                                localized: "onboarding.push-to-talk.hold-to-start-hint",
                                defaultValue: "Hold %@ to start",
                                comment: ""
                            ), "\(selectedHotkey.label)"))
                                .font(MuesliTheme.body())
                        }
