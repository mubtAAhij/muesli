...
struct ShortcutHotkeyPolicy {
    static let conflictMessage = String(localized: "shortcut_hotkey_policy.conflict_message", defaultValue: "These shortcuts need different keys.", bundle: .module, comment: "Warning message shown when shortcuts conflict and require different key combinations.")
    static let commonGlobalShortcutWarning = String(localized: "shortcut_hotkey_policy.common_global_shortcut_warning", defaultValue: "This shortcut is commonly used by other apps. Muesli listens globally, so choose a less common combination if it conflicts with your workflow.", bundle: .module, comment: "Warning explaining that a global shortcut may conflict with other apps.")
    ...
    static func resolvedComputerUseHotkeyWhenEnabling(
        ...
        if hotkeysConflict(resolved, dictationHotkey) {
            ...
            notice = String(format: String(localized: "shortcut_hotkey_policy.resolution.computer_use_avoids_push_to_talk", defaultValue: "Computer Use Command moved to %@ to avoid matching Push to Talk.", bundle: .module, comment: "Resolution message when Computer Use shortcut is remapped to avoid Push to Talk conflict."), "\(resolved.label)")
        }
