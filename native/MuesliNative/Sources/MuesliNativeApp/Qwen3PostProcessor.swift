        ...
        do {
            ...
            if Qwen3PostProcessorOutputCleaner.shouldFallbackToInput(cleaned: cleaned, input: text) {
                ...
                throw NSError(domain: "Qwen3PostProcessor", code: 4, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "qwen3_post_processor.error.transcript_rejected_safety_checks", defaultValue: "Qwen3 GGUF output was rejected by transcript safety checks", bundle: .module, comment: "Error shown when Qwen3 post-processing output fails transcript safety checks."),
                ])
            } else {
                result = cleaned
            }
    ...
    private func loadBot() throws -> LLM {
        ...
        guard let loaded = LLM(
            ...
            throw NSError(domain: "Qwen3PostProcessor", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(format: String(localized: "qwen3_post_processor.error.failed_to_load_model_at_path", defaultValue: "Failed to load Qwen3 GGUF model at %@", bundle: .module, comment: "Error shown when loading Qwen3 model file fails, including file path."), "\(modelURL.path)"),
            ])
        }
        ...
        let task = Task<Qwen3PostProcessorManager, Error> {
            guard FileManager.default.fileExists(atPath: url.path) else {
                throw NSError(domain: "Qwen3PostProcessor", code: 1, userInfo: [
                    NSLocalizedDescriptionKey: String(format: String(localized: "qwen3_post_processor.error.model_not_found_download_from_models_tab", defaultValue: "Post-processor model not found at %@. Download it from the Models tab.", bundle: .module, comment: "Error shown when post-processor model file is missing, including path and recovery guidance."), "\(url.path)"),
                ])
            }
