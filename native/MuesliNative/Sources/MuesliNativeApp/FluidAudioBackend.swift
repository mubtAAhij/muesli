...
actor FluidAudioTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notLoaded:
                return String(localized: "fluid_audio_backend.error.models_not_loaded", defaultValue: "FluidAudio models not loaded. Call loadModels() first.", comment: "Error when FluidAudio transcription is requested before loading models")
            }
        }
    }
    ...
    func loadModels(
        ...
        let manager = try await ManagedASRModelDownloader.loadValidated(
            ...
            let preparing = ModelDownloadProgress.preparing(
                ...
                message: String(localized: "fluid_audio_backend.status.loading_parakeet_coreml", defaultValue: "Loading Parakeet into Core ML...", comment: "Status shown while loading Parakeet model into Core ML")
            )
        ...
        progress?(1, nil)
        progressSnapshot?(preparing.replacing(phase: .ready, message: String(localized: "fluid_audio_backend.status.model_ready", defaultValue: "Model ready", comment: "Status shown when FluidAudio model is ready")))
