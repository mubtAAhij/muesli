...
actor FluidAudioTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notLoaded:
                return String(localized: "fluid_audio_backend.error.models_not_loaded", defaultValue: "FluidAudio models not loaded. Call loadModels() first.", comment: "Error shown when transcription is attempted before models are loaded")
            }
        }
    }
    ...
    func loadModels(
        ...
        let manager = try await ManagedASRModelDownloader.loadValidated(
            ...
            let preparing = ModelDownloadProgress.preparing(
                ...
                message: String(localized: "fluid_audio_backend.status.loading_parakeet_coreml", defaultValue: "Loading Parakeet into Core ML...", comment: "Progress status while preparing Parakeet model in Core ML")
            )
        ...
        progress?(1, nil)
        progressSnapshot?(preparing.replacing(phase: .ready, message: String(localized: "fluid_audio_backend.status.model_ready", defaultValue: "Model ready", comment: "Progress status shown when model loading is complete")))
