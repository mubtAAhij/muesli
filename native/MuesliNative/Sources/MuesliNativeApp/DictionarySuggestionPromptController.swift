...
final class DictionarySuggestionPromptController: NSObject {
    ...
    func show(
        ...
        dismissButton.toolTip = String(localized: "dictionary_suggestion_prompt.dismiss", defaultValue: "Dismiss", comment: "Button label to dismiss dictionary suggestion prompt")
...
private final class DictionarySuggestionPromptView: NSView {
    ...
    private func setup() {
        ...
        let title = label(String(localized: "dictionary_suggestion_prompt.add_correction_title", defaultValue: "Add correction?", comment: "Prompt title asking whether to add dictionary correction"), font: .systemFont(ofSize: 13, weight: .semibold), color: .white)
        ...
        let add = button(title: String(localized: "dictionary_suggestion_prompt.action.add", defaultValue: "Add", comment: "Action button label to add dictionary correction"), action: #selector(addTapped), isPrimary: true)
        add.frame = NSRect(x: addX, y: buttonY, width: buttonWidth, height: buttonHeight)
        cardView.addSubview(add)

        let ignore = button(title: String(localized: "dictionary_suggestion_prompt.action.ignore", defaultValue: "Ignore", comment: "Action button label to ignore dictionary suggestion"), action: #selector(ignoreTapped), isPrimary: false)
