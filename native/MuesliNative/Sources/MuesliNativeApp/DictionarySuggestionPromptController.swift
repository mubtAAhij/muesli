...
private final class DictionarySuggestionPromptView: NSView {
    ...
    private func setup() {
        ...
        let title = label(String(
            localized: "dictionary-suggestion-prompt.add-correction.title",
            defaultValue: "Add correction?",
            comment: ""
        ...
        let detail = label(
            String(format: String(
                localized: "dictionary-suggestion-prompt.add-correction.message",
                defaultValue: "\"%@\" -> \"%@\"",
                comment: ""
        ...
        let add = button(title: String(
            localized: "dictionary-suggestion-prompt.add-correction.confirm",
            defaultValue: "Add",
            comment: ""
        ), action: #selector(addTapped), isPrimary: true)
        add.frame = NSRect(x: addX, y: buttonY, width: buttonWidth, height: buttonHeight)
        cardView.addSubview(add)

        let ignore = button(title: String(
            localized: "dictionary-suggestion-prompt.add-correction.cancel",
            defaultValue: "Ignore",
            comment: ""
        ), action: #selector(ignoreTapped), isPrimary: false)
