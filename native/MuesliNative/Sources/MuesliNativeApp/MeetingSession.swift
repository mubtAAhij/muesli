    ...
    private func setupStreamingPartialsIfAvailable() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                await mic.connect()
                let system = MeetingStreamingPartialSession(engine: engines.system, label: String(localized: "meeting_session.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label for non-local participants in meeting session transcript grouping."))
                system.onPartialUpdate = { [weak self] text in self?.onPartialTranscript?(String(localized: "meeting_session.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label for non-local participants in meeting session transcript grouping."), text) }
    ...
    private func rotateSystemChunkOnQueue() {
        ...
        if registered {
            ...
            Task { [weak self] in
                ...
                guard !resolvedSegments.isEmpty else { return }
                self.onChunkTranscribed?(resolvedSegments, String(localized: "meeting_session.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label for non-local participants in meeting session transcript output."))
            }
        } else {
            task.cancel()
        }
