    ...
    private func setupStreamingPartialsIfAvailable() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                await mic.connect()
                let system = MeetingStreamingPartialSession(engine: engines.system, label: String(localized: "meeting_session.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label used for grouped non-user speakers in meeting session transcript display."))
                system.onPartialUpdate = { [weak self] text in self?.onPartialTranscript?(String(localized: "meeting_session.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label used for grouped non-user speakers in meeting session transcript display."), text) }
    ...
    private func rotateSystemChunkOnQueue() {
        ...
        if registered {
            ...
            Task { [weak self] in
                ...
                guard !resolvedSegments.isEmpty else { return }
                self.onChunkTranscribed?(resolvedSegments, String(localized: "meeting_session.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label used for grouped non-user speakers in meeting session transcript display."))
            }
        } else {
            task.cancel()
        }
