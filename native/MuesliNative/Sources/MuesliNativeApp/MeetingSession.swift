    ...
    private func setupStreamingPartialsIfAvailable() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                await mic.connect()
                let system = MeetingStreamingPartialSession(engine: engines.system, label: String(localized: "meeting_session.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in meeting transcript grouping"))
                system.onPartialUpdate = { [weak self] text in self?.onPartialTranscript?(String(localized: "meeting_session.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in meeting transcript grouping"), text) }
    ...
    private func rotateSystemChunkOnQueue() {
        ...
        if registered {
            ...
            Task { [weak self] in
                ...
                guard !resolvedSegments.isEmpty else { return }
                self.onChunkTranscribed?(resolvedSegments, String(localized: "meeting_session.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in meeting transcript grouping"))
            }
        } else {
            task.cancel()
        }
