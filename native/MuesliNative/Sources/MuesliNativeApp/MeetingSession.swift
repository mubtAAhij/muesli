    ...
    private func setupStreamingPartialsIfAvailable() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                await mic.connect()
                let system = MeetingStreamingPartialSession(engine: engines.system, label: String(localized: "meeting_session.speaker.others", defaultValue: "Others", comment: "Speaker label for participants other than current user"))
                system.onPartialUpdate = { [weak self] text in self?.onPartialTranscript?(String(localized: "meeting_session.speaker.others", defaultValue: "Others", comment: "Speaker label for participants other than current user"), text) }
    ...
    private func rotateSystemChunkOnQueue() {
        ...
        if registered {
            ...
            Task { [weak self] in
                ...
                guard !resolvedSegments.isEmpty else { return }
                self.onChunkTranscribed?(resolvedSegments, String(localized: "meeting_session.speaker.others", defaultValue: "Others", comment: "Speaker label for participants other than current user"))
            }
        } else {
            task.cancel()
        }
