...
struct MeetingParticipantsView: View {
    ...
    private var peopleDescription: String {
        participants.count == 1 ? String(localized: "meeting_participants.people_description.one", defaultValue: "1 person", comment: "") : String(format: String(localized: "meeting_participants.people_description.many", defaultValue: "%d people", comment: ""), participants.count)
    }
    ...
    var body: some View {
        Button {
            ...
            HStack(spacing: 6) {
                ...
                if let firstParticipantName {
                    ...
                    Text(String(localized: "meeting_participants.title.add_people", defaultValue: "Add people", comment: ""))
                }
            }
        ...
        .help(participants.isEmpty ? String(localized: "meeting_participants.help.add_people", defaultValue: "Add people to this meeting", comment: "") : String(format: String(localized: "meeting_participants.help.show_people_description", defaultValue: "Show %@", comment: ""), "\(peopleDescription)"))
        .accessibilityLabel(
            participants.isEmpty ? String(localized: "meeting_participants.accessibility.add_people_to_meeting", defaultValue: "Add people to this meeting", comment: "") : String(format: String(localized: "meeting_participants.accessibility.people_description_in_meeting", defaultValue: "%@ in this meeting", comment: ""), "\(peopleDescription)")
        )
        ...
        .alert(String(localized: "meeting_participants.alert.update_failed.title", defaultValue: "Couldn't Update People", comment: ""), isPresented: errorBinding) {
            Button(String(localized: "common.button.ok", defaultValue: "OK", comment: ""), role: .cancel) {
                errorMessage = nil
            }
        } message: {
            Text(errorMessage ?? String(localized: "meeting_participants.alert.update_failed.message", defaultValue: "The meeting's people could not be updated.", comment: ""))
        }
    }
    ...
    private var peoplePopover: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing12) {
            HStack(spacing: MuesliTheme.spacing8) {
                Text(String(localized: "meeting_participants.popover.title", defaultValue: "People", comment: ""))
            ...
            if participants.isEmpty {
                Text(String(localized: "meeting_participants.popover.empty_state", defaultValue: "No one has been added yet.", comment: ""))
    ...
    private var addPersonMenu: some View {
        Menu {
            Button {
                ...
                Label(String(localized: "meeting_participants.menu.choose_from_contacts", defaultValue: "Choose from Contacts…", comment: ""), systemImage: "person.crop.circle.badge.plus")
            }

            Button {
                createNewContact()
            } label: {
                Label(String(localized: "meeting_participants.menu.create_new_contact", defaultValue: "Create New Contact…", comment: ""), systemImage: "person.badge.plus")
            }
        } label: {
            HStack(spacing: 5) {
                Image(systemName: "person.badge.plus")
                Text(String(localized: "meeting_participants.menu.add_person", defaultValue: "Add person", comment: ""))
        ...
        .fixedSize()
        .help(String(localized: "meeting_participants.menu.help.choose_or_create_contact", defaultValue: "Choose or create an Apple contact", comment: ""))
    }
    ...
    private func participantListRow(_ participant: MeetingParticipant) -> some View {
        ...
        return HStack(spacing: MuesliTheme.spacing8) {
            ...
            .buttonStyle(.plain)
            .help(String(format: String(localized: "meeting_participants.menu.remove_person", defaultValue: "Remove %@", comment: ""), "\(displayName)"))
        }
    }
