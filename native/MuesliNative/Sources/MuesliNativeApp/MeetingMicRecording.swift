            ...
            DispatchWorkItem { [weak self] in
                self?.failPendingHandoff(
                    ...
                    error: NSError(domain: "MeetingMicrophoneRoute", code: 1, userInfo: [
                        NSLocalizedDescriptionKey: String(localized: "meeting_mic_recording.error.selected_microphone_no_audio", defaultValue: "The selected microphone did not produce audio.", comment: "Error when chosen microphone records no audio")
                    ])
                )
            }
        )
