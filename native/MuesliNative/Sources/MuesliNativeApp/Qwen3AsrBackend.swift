...
actor Qwen3AsrTranscriber {
    ...
    enum TranscriberError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notLoaded:
                return String(localized: "qwen3_asr_backend.error.models_not_loaded", defaultValue: "Qwen3 ASR models not loaded. Call loadModels() first.", comment: "")
            }
        }
    }
    ...
    func loadModels(
        ...
        let mgr = try await ManagedASRModelDownloader.loadValidated(
            ...
            let preparing = ModelDownloadProgress.preparing(
                ...
                message: String(localized: "qwen3_asr_backend.status.loading_into_coreml", defaultValue: "Loading Qwen3 ASR into Core ML...", comment: "")
            )
        ...
        progress?(1, nil)
        progressSnapshot?(preparing.replacing(phase: .ready, message: String(localized: "qwen3_asr_backend.status.model_ready", defaultValue: "Model ready", comment: "")))
