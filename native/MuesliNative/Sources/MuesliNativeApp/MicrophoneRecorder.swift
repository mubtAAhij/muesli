...
final class MicrophoneRecorder: NSObject, AVAudioRecorderDelegate, @unchecked Sendable {
        ...
        if recorder != nil {
            ...
            guard !isRecording else {
                throw NSError(domain: "MicrophoneRecorder", code: 4, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.cannot_change_input_while_recording", defaultValue: "Cannot change microphone input while recording", bundle: .module, comment: "Error shown when attempting to switch microphone input while recording is active"),
                ])
            }
    ...
    func start() throws -> UUID {
        ...
        guard let recorder else {
            throw NSError(domain: "MicrophoneRecorder", code: 1, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.no_audio_recorder_available", defaultValue: "No audio recorder available", bundle: .module, comment: "Error shown when no audio recorder instance is available"),
            ])
        }
        resetCaptureState()
        guard recorder.record() else {
            throw NSError(domain: "MicrophoneRecorder", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.failed_to_start", defaultValue: "Microphone recorder failed to start", bundle: .module, comment: "Error shown when microphone recorder fails to start recording"),
            ])
        }
        isRecording = true
    ...
    private func notifyRecordingFailed(from failedRecorder: AVAudioRecorder, error: Error?) {
        ...
        let resolvedError = error ?? NSError(domain: "MicrophoneRecorder", code: 5, userInfo: [
            NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.recording_stopped_unexpectedly", defaultValue: "Microphone recording stopped unexpectedly", bundle: .module, comment: "Error shown when microphone recording stops unexpectedly"),
        ])
        lock.unlock()
    ...
    init(preferredDeviceID: AudioObjectID) throws {
        ...
        guard Self.setDefaultInputDeviceID(preferredDeviceID) else {
            throw NSError(domain: "MicrophoneRecorder", code: 3, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.could_not_select_preferred_microphone", defaultValue: "Could not select preferred microphone", bundle: .module, comment: "Error shown when preferred microphone cannot be selected"),
            ])
        }
        didApply = true
