        ...
        if recorder != nil {
            ...
            guard !isRecording else {
                throw NSError(domain: "MicrophoneRecorder", code: 4, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.cannot_change_input_while_recording", defaultValue: "Cannot change microphone input while recording", comment: "Error when trying to switch microphone input during active recording"),
                ])
            }
        ...
        do {
            ...
            guard nextRecorder.prepareToRecord() else {
                throw NSError(domain: "MicrophoneRecorder", code: 6, userInfo: [
                    NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.failed_to_prepare", defaultValue: "Microphone recorder failed to prepare", comment: "Error when microphone recorder preparation fails"),
                ])
            }
            preparedURL = fileURL
    ...
    func start() throws -> UUID {
        ...
        guard recorder.record() else {
            throw NSError(domain: "MicrophoneRecorder", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.failed_to_start", defaultValue: "Microphone recorder failed to start", comment: "Error when microphone recorder start operation fails"),
            ])
        }
        isRecording = true
    ...
    init(preferredDeviceID: AudioObjectID) throws {
        ...
        guard Self.setDefaultInputDeviceID(preferredDeviceID) else {
            throw NSError(domain: "MicrophoneRecorder", code: 3, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "microphone_recorder.error.could_not_select_preferred_microphone", defaultValue: "Could not select preferred microphone", comment: "Error when preferred microphone cannot be selected"),
            ])
        }
        didApply = true
