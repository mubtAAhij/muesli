...
enum LiveTranscriptCopyContent {
    static func text(transcript: String, partialYou: String, partialOthers: String) -> String {
        ...
        if !others.isEmpty {
            sections.append(String(format: String(localized: "live_transcript.export.others_line", defaultValue: "Others: %@", bundle: .module, comment: "Export line prefix for transcript content spoken by others."), "\(others)"))
        }
        let you = partialYou.trimmingCharacters(in: .whitespacesAndNewlines)
        if !you.isEmpty {
            sections.append(String(format: String(localized: "live_transcript.export.you_line", defaultValue: "You: %@", bundle: .module, comment: "Export line prefix for transcript content spoken by current user."), "\(you)"))
        }
...
struct LiveTranscriptBubble: View {
    ...
    private var copyButton: some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "live_transcript.copy_message.help", defaultValue: "Copy message", bundle: .module, comment: "Context menu/help label for copying a transcript message."))
    ...
    var body: some View {
        LazyVStack(alignment: .leading, spacing: 6) {
            if messages.isEmpty && trimmedPartialYou.isEmpty && trimmedPartialOthers.isEmpty {
                Text(String(localized: "live_transcript.status.waiting_for_speech", defaultValue: "Waiting for speech…", bundle: .module, comment: "Placeholder status while no live speech is detected yet."))
        ...
        ZStack(alignment: .topTrailing) {
            ...
            Button(action: copyTranscript) {
                HStack(spacing: 6) {
                    ...
                    Text(didCopy ? String(localized: "live_transcript.copy_button.copied", defaultValue: "Copied", bundle: .module, comment: "Copy button state text after message content is copied.") : String(localized: "live_transcript.copy_button.copy", defaultValue: "Copy", bundle: .module, comment: "Default copy button text for transcript message row."))
