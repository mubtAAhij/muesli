...
enum LiveTranscriptCopyContent {
    static func text(transcript: String, partialYou: String, partialOthers: String) -> String {
        ...
        if !others.isEmpty {
            sections.append(String(format: String(localized: "live_transcript.export.others_line", defaultValue: "Others: %@", comment: "Exported transcript line prefix for non-user speaker text"), "\(others)"))
        }
        let you = partialYou.trimmingCharacters(in: .whitespacesAndNewlines)
        if !you.isEmpty {
            sections.append(String(format: String(localized: "live_transcript.export.you_line", defaultValue: "You: %@", comment: "Exported transcript line prefix for current user speaker text"), "\(you)"))
        }
...
struct LiveTranscriptBubble: View {
    ...
    private var copyButton: some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "live_transcript.copy_message.help", defaultValue: "Copy message", comment: "Help text for copying a transcript message"))
    ...
    var body: some View {
        LazyVStack(alignment: .leading, spacing: 6) {
            if messages.isEmpty && trimmedPartialYou.isEmpty && trimmedPartialOthers.isEmpty {
                Text(String(localized: "live_transcript.status.waiting_for_speech", defaultValue: "Waiting for speech…", comment: "Status text shown when waiting for speech input"))
                ...
                if !trimmedPartialOthers.isEmpty {
                    LiveTranscriptBubble(
                        speaker: String(localized: "live_transcript.speaker.others", defaultValue: "Others", comment: "Speaker label for non-user participants in live transcript"),
                        timestamp: nil,
        ...
        ZStack(alignment: .topTrailing) {
            ...
            Button(action: copyTranscript) {
                HStack(spacing: 6) {
                    ...
                    Text(didCopy ? String(localized: "live_transcript.copy_button.copied", defaultValue: "Copied", comment: "Button title shown after transcript text has been copied") : String(localized: "live_transcript.copy_button.copy", defaultValue: "Copy", comment: "Button title for copying transcript text"))
