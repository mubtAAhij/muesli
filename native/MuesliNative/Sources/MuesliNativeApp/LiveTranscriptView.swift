...
enum LiveTranscriptCopyContent {
    static func text(transcript: String, partialYou: String, partialOthers: String) -> String {
        ...
        if !others.isEmpty {
            sections.append(String(format: String(localized: "live_transcript.export.others_line", defaultValue: "Others: %@", bundle: .module, comment: "Export line for transcript text attributed to other speakers."), "\(others)"))
        }
        let you = partialYou.trimmingCharacters(in: .whitespacesAndNewlines)
        if !you.isEmpty {
            sections.append(String(format: String(localized: "live_transcript.export.you_line", defaultValue: "You: %@", bundle: .module, comment: "Export line for transcript text attributed to current user."), "\(you)"))
        }
...
struct LiveTranscriptBubble: View {
    ...
    private var copyButton: some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "live_transcript.copy_message.help", defaultValue: "Copy message", bundle: .module, comment: "Help text for action that copies a transcript message."))
    ...
    var body: some View {
        LazyVStack(alignment: .leading, spacing: 6) {
            if messages.isEmpty && trimmedPartialYou.isEmpty && trimmedPartialOthers.isEmpty {
                Text(String(localized: "live_transcript.status.waiting_for_speech", defaultValue: "Waiting for speech…", bundle: .module, comment: "Status text shown while waiting for speech input."))
                ...
                if !trimmedPartialOthers.isEmpty {
                    LiveTranscriptBubble(
                        speaker: String(localized: "live_transcript.speaker.others", defaultValue: "Others", bundle: .module, comment: "Speaker label representing non-user speakers in live transcript."),
                        timestamp: nil,
        ...
        ZStack(alignment: .topTrailing) {
            ...
            Button(action: copyTranscript) {
                HStack(spacing: 6) {
                    ...
                    Text(didCopy ? String(localized: "live_transcript.copy_button.copied", defaultValue: "Copied", bundle: .module, comment: "Copy button label shown after transcript text has been copied.") : String(localized: "live_transcript.copy_button.copy", defaultValue: "Copy", bundle: .module, comment: "Copy button label for transcript text before copy action is completed."))
