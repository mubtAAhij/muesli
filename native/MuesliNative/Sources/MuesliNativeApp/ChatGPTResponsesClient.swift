...
enum ChatGPTResponsesError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .backendFailed(statusCode, message):
            return String(format: String(localized: "chatgpt_responses_client.error.http_status", defaultValue: "ChatGPT failed with status %d. %@", bundle: .module, comment: "Error shown when ChatGPT request fails with HTTP status code and message."), statusCode, "\(message)")
        }
    }
}
    ...
    static func respond(
        ...
        guard httpStatus == 200 else {
            ...
            let message = extractErrorMessage(from: errorData)
                ...
                ?? String(localized: "chatgpt_responses_client.error.unknown_message", defaultValue: "(unknown)", bundle: .module, comment: "Fallback placeholder when ChatGPT error message is unavailable.")
    ...
    static func decodeStreamPayload(_ jsonString: String, httpStatus: Int) throws -> [String: Any]? {
        ...
        guard
            ...
            throw ChatGPTResponsesError.backendFailed(
                ...
                message: String(localized: "chatgpt_responses_client.error.malformed_stream_payload", defaultValue: "Malformed ChatGPT stream payload.", bundle: .module, comment: "Error shown when ChatGPT streaming payload cannot be parsed.")
            )
        }
        return json
    }
