...
enum MeetingFollowUpPolicy {
    static let titlePrefix = String(localized: "meeting_follow_up_policy.title_prefix", defaultValue: "Follow-up:", comment: "Prefix for generated follow-up meeting title") + " "
    ...
    static func followUpTitle(from predecessorTitle: String) -> String {
        ...
        guard !base.isEmpty else { return String(localized: "meeting_follow_up_policy.follow_up_title", defaultValue: "Follow-up meeting", comment: "Default title for follow-up meeting note") }
    ...
    static func carriedContext(fromPredecessorNotes notes: String) -> String? {
        ...
        return String(trimmed.prefix(maxCarriedNotesLength)) + "\n" + String(localized: "meeting_follow_up_policy.carried_context.truncated_marker", defaultValue: "[…previous notes truncated]", comment: "Marker appended when previous notes are truncated")
    }
}
