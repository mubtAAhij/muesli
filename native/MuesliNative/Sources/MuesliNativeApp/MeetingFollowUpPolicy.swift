...
enum MeetingFollowUpPolicy {
    static let titlePrefix = String(localized: "meeting_follow_up_policy.title_prefix", defaultValue: "Follow-up: %@", bundle: .module, comment: "Prefix format prepended to a follow-up meeting title.")
    ...
    static func followUpTitle(from predecessorTitle: String) -> String {
        ...
        guard !base.isEmpty else { return String(localized: "meeting_follow_up_policy.follow_up_title", defaultValue: "Follow-up meeting", bundle: .module, comment: "Fallback title for a follow-up meeting when no source title is available.") }
    ...
    static func carriedContext(fromPredecessorNotes notes: String) -> String? {
        ...
        return String(trimmed.prefix(maxCarriedNotesLength)) + "\n" + String(localized: "meeting_follow_up_policy.carried_context.truncated_marker", defaultValue: "[…previous notes truncated]", bundle: .module, comment: "Marker inserted when carried meeting notes are truncated.")
    }
}
