...
enum MeetingFollowUpPolicy {
    static let titlePrefix = String(localized: "meeting_follow_up_policy.title_prefix", defaultValue: "Follow-up:", bundle: .module, comment: "Prefix used when constructing follow-up meeting titles") + " "
    ...
    static func followUpTitle(from predecessorTitle: String) -> String {
        ...
        guard !base.isEmpty else { return String(localized: "meeting_follow_up_policy.follow_up_title", defaultValue: "Follow-up meeting", bundle: .module, comment: "Default title for follow-up meeting when no source title is available") }
    ...
    static func carriedContext(fromPredecessorNotes notes: String) -> String? {
        ...
        return String(trimmed.prefix(maxCarriedNotesLength)) + "\n" + String(localized: "meeting_follow_up_policy.carried_context.truncated_marker", defaultValue: "[…previous notes truncated]", bundle: .module, comment: "Marker inserted when carried meeting context is truncated")
    }
}
