...
struct LLMBackendOption: Equatable, Identifiable {
    ...
    static let customLLM = LLMBackendOption(backend: "custom_llm", label: String(localized: "llm_backend_option.custom_llm.title", defaultValue: "Custom LLM", bundle: Bundle.module, comment: "Display name for the custom LLM backend option."))
...
struct TranscriptCleanupBackendOption: Equatable, Identifiable {
    ...
    static let local = TranscriptCleanupBackendOption(
        backend: "local",
        label: String(localized: "transcript_cleanup_backend_option.local_model.title", defaultValue: "Local Model", bundle: Bundle.module, comment: "Display name for the local model transcript cleanup backend option."),
        llmBackend: nil
    )
