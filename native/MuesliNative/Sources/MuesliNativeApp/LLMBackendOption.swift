...
struct LLMBackendOption: Equatable, Identifiable {
    ...
    static let customLLM = LLMBackendOption(backend: "custom_llm", label: String(localized: "llm_backend_option.custom_llm.title", defaultValue: "Custom LLM", comment: "Display name for custom LLM backend option"))
...
struct TranscriptCleanupBackendOption: Equatable, Identifiable {
    ...
    static let local = TranscriptCleanupBackendOption(
        backend: "local",
        label: String(localized: "transcript_cleanup_backend_option.local_model.title", defaultValue: "Local Model", comment: "Display name for local transcript cleanup model option"),
        llmBackend: nil
    )
