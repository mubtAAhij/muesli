    ...
    private func createFileState() throws -> State {
        ...
        guard let fileHandle = FileHandle(forWritingAtPath: fileURL.path) else {
            throw NSError(
                ...
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "pcm_chunk_recorder.error.could_not_open_file_for_writing", defaultValue: "Could not open chunk recorder file for writing.", bundle: .module, comment: "Error when chunk recorder output file cannot be opened for writing.")]
            )
        }
