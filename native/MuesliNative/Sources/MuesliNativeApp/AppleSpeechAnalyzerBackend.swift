...
struct AppleSpeechLanguageOption: Identifiable, Hashable, Sendable {
    ...
    static var system: AppleSpeechLanguageOption {
        ...
        return AppleSpeechLanguageOption(
            id: systemIdentifier,
            label: String(format: String(localized: "apple_speech.language_option.system_language", defaultValue: "System Language - %@", bundle: .module, comment: "Language option label showing current system language name."), "\(localeName)")
        )
    }
...
enum AppleSpeechAnalyzerError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable:
            return String(localized: "apple_speech.error.requires_supported_os_and_hardware", defaultValue: "Apple Speech requires macOS 26 and compatible Apple hardware.", bundle: .module, comment: "Error shown when Apple Speech is unavailable due to OS or hardware requirements.")
        case .unsupportedLocale(let identifier):
            return String(format: String(localized: "apple_speech.error.locale_not_supported", defaultValue: "Apple Speech does not support the %@ locale on this Mac.", bundle: .module, comment: "Error shown when requested locale is unsupported for Apple Speech."), "\(identifier)")
        case .assetUnavailable(let identifier):
            return String(format: String(localized: "apple_speech.error.model_unavailable", defaultValue: "The Apple Speech model for %@ is unavailable.", bundle: .module, comment: "Error shown when Apple Speech model for locale is unavailable."), "\(identifier)")
        case .reservationUnavailable(let maximum):
            return String(format: String(localized: "apple_speech.error.language_reservation_limit_reached", defaultValue: "Apple Speech cannot reserve another language on this Mac (limit: %d).", bundle: .module, comment: "Error shown when Apple Speech language reservation limit has been reached."), maximum)
        case .emptyTranscript:
            return String(localized: "apple_speech.error.no_transcript_produced", defaultValue: "Apple Speech completed without producing a transcript.", bundle: .module, comment: "Error shown when Apple Speech finishes with no transcript output.")
        }
    }
}
...
actor AppleSpeechAnalyzerTranscriber {
    ...
    func prepare(
        ...
        callbacks.progress?(1, String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Status shown when Apple Speech backend is ready."))
    ...
    private func performPrepare(
        ...
        callbacks.progress?(0.05, String(format: String(localized: "apple_speech.status.preparing_for_locale", defaultValue: "Preparing Apple Speech for %@...", bundle: .module, comment: "Status shown while preparing Apple Speech for a specific locale."), "\(locale.localizedString(forIdentifier: locale.identifier) ?? locale.identifier)"))
        callbacks.progressSnapshot?(ModelDownloadProgress.preparing(
            modelID: Self.modelID,
            message: String(localized: "apple_speech.status.preparing_generic", defaultValue: "Preparing Apple Speech...", bundle: .module, comment: "Generic status shown while Apple Speech is preparing.")
        ))
        ...
        if let request = try await AssetInventory.assetInstallationRequest(supporting: [transcriber]) {
            ...
            let progressTask = Task { [callbacks] in
                while !Task.isCancelled && !progressBox.progress.isFinished {
                    ...
                    callbacks.progress?(mappedFraction, String(localized: "apple_speech.status.downloading", defaultValue: "Downloading Apple Speech...", bundle: .module, comment: "Status shown while Apple Speech resources are downloading."))
    ...
    private func downloadSnapshot(fraction: Double) -> ModelDownloadProgress {
        ...
        return ModelDownloadProgress(
            ...
            retryCount: 0,
            message: String(localized: "apple_speech.status.downloading", defaultValue: "Downloading Apple Speech...", bundle: .module, comment: "Status shown while Apple Speech resources are downloading.")
        )
    }
    ...
    private func readySnapshot() -> ModelDownloadProgress {
        ModelDownloadProgress.preparing(
            modelID: Self.modelID,
            message: String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Status shown when Apple Speech backend is ready.")
        ).replacing(phase: .ready, message: String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Status shown when Apple Speech backend is ready."))
    }
}
