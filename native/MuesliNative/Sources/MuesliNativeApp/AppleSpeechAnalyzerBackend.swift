...
enum AppleSpeechAnalyzerError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable:
            return String(localized: "apple_speech.error.requires_supported_os_and_hardware", defaultValue: "Apple Speech requires macOS 26 and compatible Apple hardware.", bundle: .module, comment: "Error shown when Apple Speech prerequisites are not met.")
        case .unsupportedLocale(let identifier):
            return String(format: String(localized: "apple_speech.error.locale_not_supported", defaultValue: "Apple Speech does not support the %@ locale on this Mac.", bundle: .module, comment: "Error shown when selected locale is unsupported on current Mac."), "\(identifier)")
        case .assetUnavailable(let identifier):
            return String(format: String(localized: "apple_speech.error.model_unavailable", defaultValue: "The Apple Speech model for %@ is unavailable.", bundle: .module, comment: "Error shown when Apple Speech model for a locale is unavailable."), "\(identifier)")
        case .reservationUnavailable(let maximum):
            return String(format: String(localized: "apple_speech.error.language_reservation_limit_reached", defaultValue: "Apple Speech cannot reserve another language on this Mac (limit: %d).", bundle: .module, comment: "Error shown when Apple Speech language reservation limit is reached."), maximum)
        case .emptyTranscript:
            return String(localized: "apple_speech.error.no_transcript_produced", defaultValue: "Apple Speech completed without producing a transcript.", bundle: .module, comment: "Error shown when analysis completes but transcript is empty.")
        }
    }
}
...
actor AppleSpeechAnalyzerTranscriber {
    ...
    func prepare(
        ...
        callbacks.progress?(1, String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Status indicating Apple Speech backend is ready."))
    ...
    private func performPrepare(
        ...
        callbacks.progressSnapshot?(ModelDownloadProgress.preparing(
            ...
            message: String(localized: "apple_speech.status.preparing_generic", defaultValue: "Preparing Apple Speech...", bundle: .module, comment: "Generic status shown while preparing Apple Speech backend.")
        ))
    ...
    private func readySnapshot() -> ModelDownloadProgress {
        ModelDownloadProgress.preparing(
            modelID: Self.modelID,
            message: String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Status indicating Apple Speech backend is ready.")
        ).replacing(phase: .ready, message: String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Status indicating Apple Speech backend is ready."))
    }
}
