...
enum AppleSpeechAnalyzerError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable:
            return String(localized: "apple_speech.error.requires_supported_os_and_hardware", defaultValue: "Apple Speech requires macOS 26 and compatible Apple hardware.", bundle: .module, comment: "Error when Apple Speech is unavailable due to OS or hardware requirements.")
        case .unsupportedLocale(let identifier):
            return String(format: String(localized: "apple_speech.error.locale_not_supported", defaultValue: "Apple Speech does not support the %@ locale on this Mac.", bundle: .module, comment: "Error when requested locale is unsupported on the current Mac."), "\(identifier)")
        case .assetUnavailable(let identifier):
            return String(format: String(localized: "apple_speech.error.model_unavailable", defaultValue: "The Apple Speech model for %@ is unavailable.", bundle: .module, comment: "Error when Apple Speech model for locale cannot be loaded."), "\(identifier)")
        case .reservationUnavailable(let maximum):
            return String(format: String(localized: "apple_speech.error.language_reservation_limit_reached", defaultValue: "Apple Speech cannot reserve another language on this Mac (limit: %@).", bundle: .module, comment: "Error when language reservation limit has been reached."), "\(maximum)")
        case .emptyTranscript:
            return String(localized: "apple_speech.error.no_transcript_produced", defaultValue: "Apple Speech completed without producing a transcript.", bundle: .module, comment: "Error when analysis completes but no transcript text is returned.")
        }
    }
}
...
actor AppleSpeechAnalyzerTranscriber {
    ...
    private func performPrepare(
        ...
        callbacks.progress?(0.05, String(format: String(localized: "apple_speech.status.preparing_for_locale", defaultValue: "Preparing Apple Speech for %@...", bundle: .module, comment: "Status while preparing Apple Speech resources for a specific locale."), "\(locale.localizedString(forIdentifier: locale.identifier) ?? locale.identifier)"))
        callbacks.progressSnapshot?(ModelDownloadProgress.preparing(
            modelID: Self.modelID,
            message: String(localized: "apple_speech.status.preparing_generic", defaultValue: "Preparing Apple Speech...", bundle: .module, comment: "Generic status while Apple Speech backend is preparing.")
        ))
        ...
        if let request = try await AssetInventory.assetInstallationRequest(supporting: [transcriber]) {
            ...
            let progressTask = Task { [callbacks] in
                while !Task.isCancelled && !progressBox.progress.isFinished {
                    ...
                    callbacks.progress?(mappedFraction, String(localized: "apple_speech.status.downloading", defaultValue: "Downloading Apple Speech...", bundle: .module, comment: "Status while Apple Speech assets are being downloaded."))
    ...
    private func downloadSnapshot(fraction: Double) -> ModelDownloadProgress {
        ...
        return ModelDownloadProgress(
            ...
            retryCount: 0,
            message: String(localized: "apple_speech.status.downloading", defaultValue: "Downloading Apple Speech...", bundle: .module, comment: "Repeated download status for Apple Speech assets.")
        )
    }
    ...
    private func readySnapshot() -> ModelDownloadProgress {
        ModelDownloadProgress.preparing(
            modelID: Self.modelID,
            message: String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Repeated ready status for Apple Speech backend.")
        ).replacing(phase: .ready, message: String(localized: "apple_speech.status.ready", defaultValue: "Apple Speech ready", bundle: .module, comment: "Repeated ready status for Apple Speech backend."))
    }
}
