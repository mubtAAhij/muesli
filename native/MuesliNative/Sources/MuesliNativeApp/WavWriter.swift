...
enum WavReader {
    ...
    static func readFloatMonoWAV(from url: URL) throws -> WavData {
        ...
        guard data.count >= 12,
            ...
            throw NSError(domain: "WavReader", code: 1, userInfo: [NSLocalizedDescriptionKey: String(format: String(localized: "wav_reader.error.invalid_wav_file", defaultValue: "Invalid WAV file: %@", bundle: .module, comment: "Error when a WAV file is invalid; includes file path."), "\(url.path)")])
        }
        ...
        guard audioFormat == 1,
            ...
            throw NSError(domain: "WavReader", code: 2, userInfo: [NSLocalizedDescriptionKey: String(format: String(localized: "wav_reader.error.expected_16bit_pcm_wav", defaultValue: "Expected 16-bit PCM WAV: %@", bundle: .module, comment: "Error when WAV file is not 16-bit PCM; includes file path."), "\(url.path)")])
        }
