...
struct SidebarView: View {
    ...
    private var pendingUpdateCTA: UpdateCTA? {
        ...
        case .available:
            return UpdateCTA(
                label: String(localized: "sidebar.update.cta_title", defaultValue: "Update", comment: ""),
                icon: "arrow.down",
                foreground: updateCTAForeground,
                accessibilityLabel: String(localized: "sidebar.update.available", defaultValue: "Update available", comment: ""),
                tooltip: String(localized: "sidebar.update.instructions", defaultValue: "Open About for update instructions", comment: "")
            )
        case .downloaded:
            return UpdateCTA(
                label: String(localized: "sidebar.update.ready_title", defaultValue: "Ready", comment: ""),
                icon: "arrow.clockwise",
                foreground: updateCTAForeground,
                accessibilityLabel: String(localized: "sidebar.update.ready_message", defaultValue: "Update ready to install", comment: ""),
                tooltip: String(localized: "sidebar.update.instructions", defaultValue: "Open About for update instructions", comment: "")
            )
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
            ...
            sidebarItem(tab: .timeline, icon: "clock.fill", label: String(localized: "sidebar.timeline", defaultValue: "Timeline", comment: ""))
            sidebarItem(tab: .dictations, icon: "mic.fill", label: String(localized: "sidebar.dictations", defaultValue: "Dictations", comment: ""))
            meetingsSection
            sidebarItem(tab: .dictionary, icon: "character.book.closed", label: String(localized: "sidebar.dictionary", defaultValue: "Dictionary", comment: ""))
            sidebarItem(tab: .models, icon: "square.and.arrow.down", label: String(localized: "sidebar.models", defaultValue: "Models", comment: ""))
            sidebarItem(tab: .shortcuts, icon: "keyboard", label: String(localized: "sidebar.shortcuts", defaultValue: "Shortcuts", comment: ""))

            Spacer()

            modelPreparationStatus
            spreadTheWordSection
            sidebarItem(tab: .settings, icon: "gearshape", label: String(localized: "sidebar.settings", defaultValue: "Settings", comment: ""))
            sidebarItem(tab: .about, icon: "info.circle", label: String(localized: "sidebar.about", defaultValue: "About", comment: ""), updateCTA: pendingUpdateCTA)
            darkModeToggle
        ...
        .alert(
            String(format: String(localized: "sidebar.folder.delete.confirmation", defaultValue: "Delete \"%@\"?", comment: ""), "\(folderToDelete?.name ?? "")"),
            isPresented: $showDeleteConfirmation
        ) {
            Button(String(localized: "common.cancel", defaultValue: "Cancel", comment: ""), role: .cancel) {
                folderToDelete = nil
            }
            Button(String(localized: "sidebar.meetings.delete", defaultValue: "Delete", comment: ""), role: .destructive) {
            ...
            if directCount > 0 {
                ...
                Text(String(localized: "sidebar.folder.delete.warning", defaultValue: "This folder will be permanently removed. Subfolders will be kept.", comment: ""))
            }
        }
    }
    ...
    private var sidebarHeader: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
            ...
            if !userName.isEmpty {
                Text(String(format: String(localized: "sidebar.greeting.hi_user", defaultValue: "Hi, %@", comment: ""), "\(userName)"))
    ...
    private var searchBar: some View {
        HStack(spacing: MuesliTheme.spacing8) {
            Image(systemName: "magnifyingglass")
                .font(.system(size: 12))
                .foregroundStyle(MuesliTheme.textTertiary)
            TextField(String(localized: "sidebar.search.placeholder", defaultValue: "Search...", comment: ""), text: searchTextBinding)
        ...
        VStack(alignment: .leading, spacing: 2) {
            ...
            HStack(spacing: MuesliTheme.spacing12) {
                Button {
                    ...
                    HStack(spacing: MuesliTheme.spacing12) {
                        ...
                        Text(String(localized: "sidebar.meetings.title", defaultValue: "Meetings", comment: ""))
                ...
                .buttonStyle(.plain)
                .help(String(localized: "sidebar.meetings.new_folder.help", defaultValue: "New Meeting Folder", comment: ""))
            }
            ...
            if meetingsExpanded {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    meetingFilterRow(
                        icon: "tray.2",
                        label: String(localized: "sidebar.meetings.filter.all", defaultValue: "All Meetings", comment: ""),
                    ...
                    ForEach(folderTree.visibleFolders) { folder in
                        ...
                        if renamingFolderID == folder.id {
                            ...
                            .contextMenu {
                                Button(String(localized: "sidebar.folder.new_subfolder", defaultValue: "New Subfolder", comment: "")) {
                                    createNewSubfolder(parentID: folder.id)
                                }
                                Button(String(localized: "sidebar.meetings.rename", defaultValue: "Rename", comment: "")) {
                                    renamingFolderID = folder.id
                                    renamingFolderName = folder.name
                                }
                                if folder.parentID != nil {
                                    Button(String(localized: "sidebar.folder.move_to_top_level", defaultValue: "Move to Top Level", comment: "")) {
    ...
    private var spreadTheWordSection: some View {
        ...
        if wordMilestone != nil {
            VStack(alignment: .leading, spacing: 2) {
                Text(String(localized: "sidebar.spread_the_word.title", defaultValue: "Spread the Word", comment: ""))
                    .font(.system(size: 11, weight: .semibold, design: .rounded))
                    .foregroundStyle(MuesliTheme.textTertiary)
                    .padding(.horizontal, sidebarRowHorizontalPadding)
                    .padding(.bottom, 2)
                ...
                socialShareRow(
                    ...
                    label: String(localized: "sidebar.spread_the_word.tweet", defaultValue: "Tweet about Muesli", comment: ""),
                ...
                socialShareRow(
                    ...
                    label: String(localized: "sidebar.spread_the_word.linkedin", defaultValue: "Post on LinkedIn", comment: ""),
    ...
    private func folderRenameField(folder: MeetingFolder) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            Image(systemName: "folder")
                ...
                .foregroundStyle(MuesliTheme.accent)
                .frame(width: sidebarIconColumnWidth)
            TextField(String(localized: "sidebar.folder.rename.placeholder", defaultValue: "Folder name", comment: ""), text: $renamingFolderName)
                .font(MuesliTheme.callout())
                .textFieldStyle(.plain)
                .onSubmit {
    ...
    private func createNewFolder() {
        if let id = controller.createFolder(name: String(localized: "sidebar.folder.new_default_name", defaultValue: "New Folder", comment: "")) {
            withAnimation(.easeInOut(duration: 0.15)) {
                meetingsExpanded = true
            }
            renamingFolderID = id
            renamingFolderName = String(localized: "sidebar.folder.new_default_name", defaultValue: "New Folder", comment: "")
            controller.showMeetingsHome(folderID: id)
        }
    }

    private func createNewSubfolder(parentID: Int64) {
        if let id = controller.createSubfolder(name: String(localized: "sidebar.folder.new_default_name", defaultValue: "New Folder", comment: ""), parentID: parentID) {
            withAnimation(.easeInOut(duration: 0.15)) {
                meetingsExpanded = true
