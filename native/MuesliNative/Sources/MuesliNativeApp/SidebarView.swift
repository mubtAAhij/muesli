...
struct SidebarView: View {
    ...
    var body: some View {
        ...
        .alert(
            String(format: String(
                localized: "sidebar.folder.delete.confirmation-title",
                defaultValue: "Delete \"%@\"?",
                comment: ""
            ), "\(folderToDelete?.name ?? "")"),
            ...
            if directCount > 0 {
                Text(String(format: String(
                    localized: "sidebar.folder.delete.move-to-unfiled.message",
                    defaultValue: "%d meetings in this folder will be moved to Unfiled. Subfolders will be kept.",
                    comment: "Confirmation message explaining meetings moved to Unfiled when deleting a folder"
                ), directCount))
            } else {
    ...
    private var sidebarHeader: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
            ...
            if !userName.isEmpty {
                Text(String(format: String(
                    localized: "sidebar.greeting.user",
                    defaultValue: "Hi, %@",
                    comment: ""
                ), "\(userName)"))
