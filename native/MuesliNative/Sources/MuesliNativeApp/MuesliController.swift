...
enum DictationBackendReadiness: Equatable {
    ...
    func blockingMessage(backendLabel: String) -> String? {
        switch self {
        case .preparing:
            return String(format: String(localized: "status.backend_warmup.message", defaultValue: "Warming up %@...", bundle: .module, comment: "Status message while backend is warming up"), "\(backendLabel)")
        case .ready:
            return nil
        case .failed:
            return String(format: String(localized: "status.backend_unavailable.message", defaultValue: "%@ unavailable", bundle: .module, comment: "Status message when backend is unavailable"), "\(backendLabel)")
        }
    }
}
...
enum MeetingResummarizationPolicy {
    static func plan(for meeting: MeetingRecord) -> MeetingResummarizationPlan {
        ...
        let promptTitle = trimmed.isEmpty ? String(localized: "prompt.meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Meeting title label") : trimmed
...
enum MeetingSummaryPersistenceError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .failedToSaveSummary(let underlying):
            ...
            if detail.isEmpty {
                return String(localized: "error.meeting_notes.save_failed.message", defaultValue: "The updated meeting notes could not be saved.", bundle: .module, comment: "Error message when updated meeting notes cannot be saved")
            }
            return String(format: String(localized: "error.meeting_notes.save_failed_with_detail.message", defaultValue: "The updated meeting notes could not be saved. %@", bundle: .module, comment: "Error message when updated meeting notes cannot be saved with additional detail"), "\(detail)")
        }
    }
}
...
enum MeetingTemplateSelectionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .templateNoLongerExists:
            return String(localized: "error.meeting_template.missing.message", defaultValue: "That template no longer exists. Choose another template and try again.", bundle: .module, comment: "Error message when selected meeting template no longer exists")
        }
    }
}
...
enum MeetingRetranscriptionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .recordingUnavailable:
            return String(localized: "error.retranscription.recording_missing_on_disk.message", defaultValue: "The saved meeting recording is no longer available on disk.", bundle: .module, comment: "Error message when saved recording is missing from disk during retranscription")
        case .noDownloadedTranscriptionModel:
            return String(localized: "error.retranscription.model_required.message", defaultValue: "Download a transcription model before re-transcribing this meeting.", bundle: .module, comment: "Error message prompting user to download transcription model before retranscription")
        case .emptyTranscript:
            return String(localized: "error.retranscription.no_speech_detected.message", defaultValue: "Re-transcription finished, but no speech was detected in the saved recording.", bundle: .module, comment: "Error message when retranscription finds no speech in recording")
        case .failedToSave(let underlying):
            return String(format: String(localized: "error.retranscription.save_failed_with_detail.message", defaultValue: "The re-transcribed meeting could not be saved. %@", bundle: .module, comment: "Error message when retranscribed meeting cannot be saved with underlying error detail"), "\(underlying.localizedDescription)")
        }
    }
}
...
enum MeetingLifecycleError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .failedToSaveRecording(let underlying):
            return String(format: String(localized: "error.meeting_recording.save_failed_with_detail.message", defaultValue: "The meeting finished transcribing, but the recording could not be saved. %@", bundle: .module, comment: "Error message when meeting recording cannot be saved with underlying error detail"), "\(underlying.localizedDescription)")
        case .failedToDeleteRecording(let underlying):
            return String(format: String(localized: "error.meeting_recording.delete_failed_with_detail.message", defaultValue: "The saved meeting recording could not be deleted, so the meeting was left in place. %@", bundle: .module, comment: "Error message when meeting recording cannot be deleted with underlying error detail"), "\(underlying.localizedDescription)")
        case .failedToDeleteMeeting(let underlying):
            return String(format: String(localized: "error.delete_meeting.failed_with_detail.message", defaultValue: "The meeting could not be deleted. %@", bundle: .module, comment: "Error message when meeting deletion fails with underlying error detail"), "\(underlying.localizedDescription)")
        }
    }
}
...
public final class MuesliController: NSObject {
    ...
    func truncate(_ text: String, limit: Int) -> String {
        ...
        return String(compact.prefix(limit - 3)).trimmingCharacters(in: .whitespacesAndNewlines) + String(localized: "common.ellipsis", defaultValue: "...", bundle: .module, comment: "Common ellipsis text value")
    }
        ...
        iCloudSubscriptionTask = Task { [weak self] in
            do {
                ...
                await MainActor.run {
                    ...
                    self.appState.iCloudSyncStatus = String(format: String(localized: "sync.icloud.required_message", defaultValue: "Sync needs iCloud: %@", bundle: .module, comment: "Status text indicating iCloud sync requirement with message detail"), "\(message)")
    ...
    private func startICloudSync() {
        ...
        guard config.iCloudSyncEnabled else {
            if userInitiated {
                appState.iCloudSyncStatus = String(localized: "sync.icloud.turn_on_first", defaultValue: "Turn on iCloud sync first.", bundle: .module, comment: "Instructional message to enable iCloud sync first")
            }
        ...
        guard iCloudSyncTask == nil else {
            ...
            appState.iCloudBridgeMessage = nil
            if userInitiated {
                appState.iCloudSyncStatus = String(localized: "sync.already_in_progress", defaultValue: "Sync already in progress.", bundle: .module, comment: "Status message when sync is already running")
            }
        ...
        enableICloudPersistentSync()
        appState.isICloudSyncInProgress = true
        appState.iCloudSyncStatus = String(localized: "sync.icloud.syncing_private", defaultValue: "Syncing with private iCloud...", bundle: .module, comment: "Status message while syncing with private iCloud")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        ...
        iCloudSyncTask = Task { [weak self] in
            do {
                ...
                await MainActor.run {
                    ...
                    let remoteDeviceName = MuesliBridgeDeviceIdentity.remoteDeviceDisplayName ?? String(localized: "device.iphone.default_name", defaultValue: "iPhone", bundle: .module, comment: "Default device name shown when remote device name is unavailable")
                    self.appState.iCloudSyncStatus = result.downloaded.total > 0
                        ? String(format: String(localized: "sync.icloud.synced_with_device", defaultValue: "Synced with %@.", bundle: .module, comment: "Status message showing the synced remote device name"), "\(remoteDeviceName)")
                        : String(localized: "sync.icloud.all_text_up_to_date", defaultValue: "All text is up to date.", bundle: .module, comment: "Status message when all iCloud text is up to date")
                ...
                await MainActor.run {
                    ...
                    self.bridgeDiscoveryFollowUpPending = false
                    let message = error.localizedDescription
                    self.appState.iCloudSyncStatus = String(format: String(localized: "sync.failed_with_reason", defaultValue: "Sync failed: %@", bundle: .module, comment: "Error status message when sync fails with reason"), "\(message)")
                    if MuesliICloudSyncEngine.isICloudAccountAvailabilityError(error) {
                        self.appState.iCloudBridgeState = .needsICloud
                    } else {
                        self.appState.iCloudBridgeState = .error
    ...
    private func disableICloudSyncRuntimeState() {
        ...
        resetBridgeDiscoveryRuntimeState()
        appState.iCloudSyncStatus = String(localized: "sync.icloud.turning_off", defaultValue: "Turning off iCloud sync...", bundle: .module, comment: "Status message while disabling iCloud sync")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        Task { [weak self] in
            ...
            self.appState.iCloudSyncStatus = String(localized: "sync.icloud.off", defaultValue: "iCloud sync is off.", bundle: .module, comment: "Status message when iCloud sync is disabled")
    ...
    private func disableICloudSyncForUnavailableEntitlement() {
        ...
        resetICloudSubscriptionState()
        resetBridgeDiscoveryRuntimeState()
        appState.iCloudSyncStatus = String(localized: "sync.icloud.stopping_unavailable", defaultValue: "Stopping unavailable iCloud sync...", bundle: .module, comment: "Status message while stopping unavailable iCloud sync")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        Task { [weak self] in
            await cancellationTask?.value
            guard let self,
                  self.iCloudSyncGeneration == generation,
                  self.ckSyncEngine == nil else { return }
            self.appState.iCloudSyncStatus = String(localized: "sync.icloud.unavailable_local_build", defaultValue: "iCloud sync is unavailable in this local-only build.", bundle: .module, comment: "Status message when iCloud sync is unavailable in local-only build")
            self.appState.iCloudBridgeState = .notConfigured
        }
    }
    ...
    private func formatICloudSyncCounts(_ counts: ICloudSyncKindCounts) -> String {
        ...
        if counts.dictations > 0 {
            parts.append("\(counts.dictations) \(counts.dictations == 1 ? String(localized: "sync.icloud.count.item.dictation.singular", defaultValue: "dictation", bundle: .module, comment: "Singular item label for dictation sync count") : String(localized: "sync.icloud.count.item.dictation.plural", defaultValue: "dictations", bundle: .module, comment: "Plural item label for dictation sync count"))")
        }
        if counts.meetings > 0 {
            parts.append("\(counts.meetings) \(counts.meetings == 1 ? "meeting" : String(localized: "sync.icloud.count.item.meeting.plural", defaultValue: "meetings", bundle: .module, comment: "Plural item label for meeting sync count"))")
        }
    ...
    func selectBackend(_ option: BackendOption) {
        ...
        Task { [weak self] in
            ...
            if needsWarmup {
                await MainActor.run {
                    self.indicator.showLoading(String(localized: "status.warming_up", defaultValue: "Warming up...", bundle: .module, comment: "Status text shown while system is warming up"))
                }
            }
            let ppOption = self.runtimePostProcessorOption()
            await self.configureTranscriptCleanupForRuntime(option: ppOption)
    ...
    func selectMeetingTranscriptionBackend(_ option: BackendOption, requireDownloaded: Bool = true) {
        guard option.supportsMeetingTranscription else {
            presentErrorAlert(
                title: String(localized: "error.meeting_transcription.model_unavailable", defaultValue: "Meeting model unavailable", bundle: .module, comment: "Error title when selected meeting transcription model is unavailable"),
                message: String(format: String(localized: "error.meeting_transcription.option_not_supported.message", defaultValue: "%@ is optimized for dictation and cannot be used for meeting transcription.", bundle: .module, comment: "Error message when selected option cannot be used for meeting transcription"), "\(option.label)")
            )
            normalizeMeetingTranscriptionSelectionForAvailability()
            return
        }
        guard !requireDownloaded || option.isDownloaded else {
            presentErrorAlert(
                title: String(localized: "error.meeting_transcription.model_unavailable", defaultValue: "Meeting model unavailable", bundle: .module, comment: "Error title when selected meeting transcription model is unavailable"),
                message: String(format: String(localized: "error.meeting_transcription.download_required.message", defaultValue: "Download %@ before using it for meeting transcription.", bundle: .module, comment: "Error message prompting download before using option for meeting transcription"), "\(option.label)")
            )
            normalizeMeetingTranscriptionSelectionForAvailability()
            return
        }
    ...
    func selectPostProcessorBackend(_ option: TranscriptCleanupBackendOption) {
        guard option.isCompatible(with: selectedBackend) else {
            presentErrorAlert(
                title: String(localized: "error.cleanup_model.unavailable.title", defaultValue: "Cleanup model unavailable", bundle: .module, comment: "Error title when selected cleanup model is unavailable"),
                message: String(localized: "error.cleanup_model.gemma4_same_backend.message", defaultValue: "Gemma 4 cannot clean up a transcription produced by the same Gemma 4 backend.", bundle: .module, comment: "Error message when Gemma 4 cleanup uses same backend as transcription")
            )
            return
        }
    ...
    private func showMeetingStartingNowNotification(
        ...
        meetingNotification.show(
            title: String(localized: "notifications.meeting_starting_now.title", defaultValue: "Meeting starting now", bundle: .module, comment: "Notification title when meeting starts now"),
            subtitle: title,
    ...
    func continueModelPreparationAfterOnboarding(
        ...
        updateModelPreparationStatus(
            title: String(format: String(localized: "model_preparation.preparing_backend", defaultValue: "Preparing %@", bundle: .module, comment: "Status message while preparing backend"), "\(backend.label)"),
            detail: initialStatus ?? String(format: String(localized: "model_preparation.preparing_backend_ellipsis", defaultValue: "Preparing %@...", bundle: .module, comment: "Status message while preparing backend with ellipsis"), "\(backend.label)"),
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.updateModelPreparationStatus(
                        title: String(format: String(localized: "model_preparation.backend_ready", defaultValue: "%@ ready", bundle: .module, comment: "Status message when backend is ready"), "\(backend.label)"),
                        detail: String(localized: "model_preparation.ready_for_transcription", defaultValue: "Ready for transcription", bundle: .module, comment: "Status message when system is ready for transcription"),
                        progress: 1.0,
                        isPreparing: false,
                        isComplete: true
                    )
                ...
                await MainActor.run {
                    self.onboardingModelPreparationTask = nil
                    self.updateModelPreparationStatus(
                        title: backend.isDownloaded ? String(localized: "model_preparation.paused", defaultValue: "Model setup paused", bundle: .module, comment: "Status message when model setup is paused") : String(localized: "model_preparation.download_paused", defaultValue: "Download paused", bundle: .module, comment: "Status message when model download is paused"),
    ...
    private func applyModelPreparationProgress(_ progress: Double, status: String?, backend: BackendOption) {
        ...
        if isPreparing {
            updateModelPreparationStatus(
                ...
                detail: String(format: String(localized: "model_preparation.status.optimizing_backend_for_mac", defaultValue: "Optimizing %@ for this Mac...", bundle: .module, comment: "Status message while optimizing backend for current Mac"), "\(backend.label)"),
                progress: nil,
                isPreparing: true,
                isComplete: false
            )
    ...
    private func modelPreparationFailureMessage(for backend: BackendOption) -> String {
        backend.isDownloaded
            ? String(localized: "model_preparation.failure.setup_failed_restart_or_retry", defaultValue: "Model setup failed. Restart Muesli or retry from Models.", bundle: .module, comment: "Error message when model setup fails and user should restart or retry")
            : String(localized: "model_preparation.failure.download_failed_check_connection", defaultValue: "Download failed. Check your connection and retry.", bundle: .module, comment: "Error message when model download fails and user should retry")
    }
    ...
    private func presentStandardUpdateCheck() {
        guard let updaterController else {
            appState.sparkleUpdateStatus = .disabled(message: String(localized: "updates.disabled_for_build", defaultValue: "Update checks are disabled for this build.", bundle: .module, comment: "Status message when update checks are disabled for current build"))
            return
        }
            ...
            do {
                ...
                do {
                    formattedNotes = try await MeetingSummaryClient.summarize(
                        ...
                        meetingTitle: meeting.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Generic meeting title label") : meeting.title,
                        config: self.config,
                        template: templateSnapshot,
                        existingNotes: self.notesContextForResummary(meeting),
                        manualNotesToRetain: meeting.manualNotes
    ...
    static func stripManualNotesSection(from notes: String) -> String {
        let markers = [
            "\n\n" + String(localized: "notes.section.written_notes.h3_spaced", defaultValue: "### Written notes", bundle: .module, comment: "Markdown heading block for written notes section with surrounding spacing") + "\n\n",
            "\n" + String(localized: "notes.section.written_notes.h3_leading_newline", defaultValue: "### Written notes", bundle: .module, comment: "Markdown heading block for written notes section with leading newline") + "\n\n",
            String(localized: "notes.section.written_notes.h3", defaultValue: "### Written notes", bundle: .module, comment: "Markdown heading block for written notes section") + "\n\n",
            "\n\n" + String(localized: "notes.section.manual_notes.h2_spaced", defaultValue: "## Manual Notes", bundle: .module, comment: "Markdown heading block for manual notes section with surrounding spacing") + "\n\n",
            "\n" + String(localized: "notes.section.manual_notes.h2_leading_newline", defaultValue: "## Manual Notes", bundle: .module, comment: "Markdown heading block for manual notes section with leading newline") + "\n\n",
            String(localized: "notes.section.manual_notes.h2", defaultValue: "## Manual Notes", bundle: .module, comment: "Markdown heading block for manual notes section") + "\n\n"
        ]
    ...
    func deleteMeeting(id: Int64) {
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.delete_meeting.failed.title", defaultValue: "Couldn't Delete Meeting", bundle: .module, comment: "Alert title when deleting meeting fails"), message: error.localizedDescription)
            return
        } catch {
            presentErrorAlert(
                title: String(localized: "error.delete_meeting.failed.title", defaultValue: "Couldn't Delete Meeting", bundle: .module, comment: "Alert title when deleting meeting fails"),
    ...
    func clearMeetingHistory() {
        guard !isMeetingRecording(), !isStartingMeetingRecording, backgroundMeetingProcessingCount == 0 else {
            presentErrorAlert(
                ...
                message: String(localized: "error.clear_meeting_history.active_meeting.message", defaultValue: "A meeting is recording or still being processed. Please wait before clearing saved meetings.", bundle: .module, comment: "Error message when meeting history cannot be cleared due to active meeting")
            )
            return
        }
    ...
    func shouldTerminateApplication() -> Bool {
        ...
        case .none:
            return true
        case .starting:
            messageText = String(localized: "meeting_recording.starting_message", defaultValue: "Meeting recording is starting", bundle: .module, comment: "Termination warning title when meeting recording is starting")
            informativeText = String(localized: "termination.meeting_recording_starting.warning", defaultValue: "Quitting now will cancel the meeting recording before it has been saved.", bundle: .module, comment: "Termination warning message when recording has not been saved yet")
        case .recording:
            messageText = String(localized: "termination.meeting_recording_in_progress.title", defaultValue: "Meeting recording in progress", bundle: .module, comment: "Termination warning title when meeting recording is in progress")
            informativeText = String(localized: "termination.meeting_recording_in_progress.warning", defaultValue: "Quitting now will stop the meeting recording and the current transcript may be lost. Stop the recording first if you want Muesli to save notes.", bundle: .module, comment: "Termination warning message when meeting recording is in progress")
        case .processing:
            messageText = String(localized: "termination.meeting_transcription_in_progress.title", defaultValue: "Meeting transcription in progress", bundle: .module, comment: "Termination warning title when meeting transcription is in progress")
            informativeText = String(localized: "termination.meeting_transcription_in_progress.warning", defaultValue: "Quitting now will interrupt transcription and the meeting notes may not be saved.", bundle: .module, comment: "Termination warning message when meeting transcription is in progress")
        }
    ...
    func resumeMeetingRecording() {
        ...
        statusBarController?.setStatus(String(format: String(localized: "meeting.status.with_title", defaultValue: "Meeting: %@", bundle: .module, comment: "Status text showing current meeting title"), "\(activeMeetingDisplayTitle())"))
        statusBarController?.refresh()
        syncAppState()
    }
    ...
    func startMeetingRecordingFromEntryPoint(
        title: String = String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Generic meeting title label"),
    ...
    func startMeetingRecording(
        ...
        guard let meetingBackend = normalizeMeetingTranscriptionSelectionForAvailability() else {
            presentErrorAlert(
                title: String(localized: "error.meeting_start.failed.title", defaultValue: "Meeting failed to start", bundle: .module, comment: "Error title when meeting start fails"),
                message: String(localized: "error.meeting_start.model_required.message", defaultValue: "Download a transcription model before recording a meeting.", bundle: .module, comment: "Error message prompting download of transcription model before recording meeting")
            )
            return false
        }
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.meeting_start.failed.title", defaultValue: "Meeting failed to start", bundle: .module, comment: "Error title when meeting start fails"), message: error.localizedDescription)
            return false
        }
        armMeetingAutoStop(
        ...
        updateMeetingStartStatus(String(localized: "meeting.start_status.transcription_starting_soon", defaultValue: "Meeting transcription will start shortly.", bundle: .module, comment: "Status message when meeting transcription is about to begin"))
        indicator.setState(.preparing, config: config)
        beginMeetingActivity(reason: String(localized: "meeting.activity.recording_and_transcribing", defaultValue: "Recording and transcribing a meeting", bundle: .module, comment: "Activity description while recording and transcribing a meeting"))
        ...
        meetingStartTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if self.meetingStartMeetingID == meetingID {
                    ...
                    self.meetingMonitor.refreshState()
                    self.statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", bundle: .module, comment: "Status text when system is idle"))
                    self.statusBarController?.refresh()
                    self.setState(.idle)
    ...
    func startQuickNoteMeeting() {
        startMeetingRecordingFromEntryPoint(title: String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Generic meeting title label"))
    }
    ...
    func resumeFinishedMeeting(meetingID: Int64) {
        ...
        guard let meetingBackend = normalizeMeetingTranscriptionSelectionForAvailability() else {
            presentErrorAlert(
                title: String(localized: "error.resume_meeting.failed.title", defaultValue: "Resume failed", bundle: .module, comment: "Error title when resuming meeting fails"),
                message: String(localized: "error.resume_meeting.model_required.message", defaultValue: "Download a transcription model before recording.", bundle: .module, comment: "Error message prompting model download before resuming recording")
            )
            return
        }
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.resume_meeting.failed.title", defaultValue: "Resume failed", bundle: .module, comment: "Error title when resuming meeting fails"), message: error.localizedDescription)
            return
        }
    ...
    func importAudioFile() {
        ...
        guard normalizeMeetingTranscriptionSelectionForAvailability() != nil else {
            presentErrorAlert(
                ...
                message: String(localized: "error.import_audio.model_required.message", defaultValue: "Download a transcription model before importing audio files.", bundle: .module, comment: "Error message prompting model download before importing audio")
            )
            return
        }
    ...
    private func importAudioFile(from sourceURL: URL, sessionID: UUID) async {
        ...
        let title = filename.isEmpty ? String(localized: "import.recording.title", defaultValue: "Imported Recording", bundle: .module, comment: "Title for an imported recording item") : filename
        ...
        self.beginMeetingActivity(reason: String(localized: "import.audio.activity.importing_for_transcription", defaultValue: "Importing audio file for transcription", bundle: .module, comment: "Activity description while importing audio for transcription"))
    ...
    private func startMeetingRecordingWithSystemAudioRecovery(
        ...
        statusBarController?.setStatus(String(localized: "meeting.start_status.transcription_starting_soon", defaultValue: "Meeting transcription will start shortly.", bundle: .module, comment: "Status message when meeting transcription is about to begin"))
        statusBarController?.refresh()
        try Task.checkCancellation()
        try await transcriptionCoordinator.preloadRequired(
            backend: backend,
        ...
        while true {
            ...
            do {
                ...
                meetingMonitor.suppressWhileActive()
                meetingMonitor.refreshState()
                statusBarController?.setStatus(String(format: String(localized: "meeting.status.with_title", defaultValue: "Meeting: %@", bundle: .module, comment: "Status text showing current meeting title"), "\(title)"))
                ...
                try Task.checkCancellation()
                try checkMeetingStartStillCurrent(meetingID)
                updateMeetingStartStatus(String(localized: "meeting.start_status.requesting_system_audio_permission", defaultValue: "Requesting system audio permission...", bundle: .module, comment: "Status message while requesting system audio permission during meeting start"))
                statusBarController?.setStatus(String(localized: "meeting.start_status.requesting_system_audio_permission", defaultValue: "Requesting system audio permission...", bundle: .module, comment: "Status message while requesting system audio permission during meeting start"))
                statusBarController?.refresh()
                let granted = await CoreAudioSystemRecorder.requestSystemAudioAccess()
                try Task.checkCancellation()
                try checkMeetingStartStillCurrent(meetingID)
                if granted {
                    updateMeetingStartStatus(String(localized: "meeting.start_status.retrying", defaultValue: "Retrying meeting start...", bundle: .module, comment: "Status message while retrying meeting start"))
                    statusBarController?.setStatus(String(localized: "meeting.start_status.retrying", defaultValue: "Retrying meeting start...", bundle: .module, comment: "Status message while retrying meeting start"))
                    statusBarController?.refresh()
                    continue
                }
                throw error
    ...
    private final class MeetingDiscardButtonTitleUpdater: NSObject {
        ...
        @MainActor @objc func manualNotesCheckboxChanged(_ sender: NSButton) {
            discardButton?.title = sender.state == .on ? String(localized: "common.discard", defaultValue: "Discard", bundle: .module, comment: "Common discard action label") : String(localized: "meeting.discard_recording.action", defaultValue: "Discard Recording", bundle: .module, comment: "Action label to discard ongoing meeting recording")
        }
    }
    ...
    @objc func discardMeetingWithConfirmation() {
        ...
        alert.messageText = String(localized: "meeting.discard_recording.confirmation_title", defaultValue: "Discard recording?", bundle: .module, comment: "Confirmation dialog title for discarding recording")
        alert.alertStyle = .warning
        var manualNotesCheckbox: NSButton?
        if hasManualNotes {
            alert.informativeText = String(localized: "meeting.discard_recording.message_with_notes_choice", defaultValue: "This will stop the meeting. Choose whether to delete the written notes too.", bundle: .module, comment: "Confirmation message when discarding recording with notes choice")
            let accessory = Self.makeDiscardMeetingAccessoryView()
            manualNotesCheckbox = accessory.manualNotesCheckbox
            alert.accessoryView = accessory.view
            alert.addButton(withTitle: String(localized: "meeting.discard_recording.action", defaultValue: "Discard Recording", bundle: .module, comment: "Action label to discard ongoing meeting recording"))
            alert.addButton(withTitle: String(localized: "common.cancel", defaultValue: "Cancel", bundle: .module, comment: "Common cancel action label"))
            ...
            alert.addButton(withTitle: String(localized: "common.discard", defaultValue: "Discard", bundle: .module, comment: "Common discard action label"))
            alert.addButton(withTitle: String(localized: "common.cancel", defaultValue: "Cancel", bundle: .module, comment: "Common cancel action label"))
            alert.buttons.first?.hasDestructiveAction = true
        }
    ...
    private static func makeDiscardMeetingAccessoryView() -> MeetingDiscardAccessory {
        let label = NSTextField(labelWithString: String(localized: "meeting.discard_recording.will_delete_label", defaultValue: "Will delete:", bundle: .module, comment: "Label introducing list of items that will be deleted when discarding recording"))
        label.font = .systemFont(ofSize: NSFont.systemFontSize)
        label.textColor = .secondaryLabelColor

        let recordingCheckbox = NSButton(checkboxWithTitle: String(localized: "meeting.discard_recording.item.recording_audio", defaultValue: "Recording audio", bundle: .module, comment: "List item label for recording audio deletion"), target: nil, action: nil)
        recordingCheckbox.state = .on
        recordingCheckbox.isEnabled = false

        let notesCheckbox = NSButton(checkboxWithTitle: String(localized: "meeting.discard_recording.item.manual_notes", defaultValue: "Manual notes", bundle: .module, comment: "List item label for manual notes deletion"), target: nil, action: nil)
    ...
    func stopMeetingRecording() {
        ...
        Task { [weak self] in
            guard let self else { return }
            var meetingTitle = String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Generic meeting title label")
            ...
            do {
                ...
                await MainActor.run {
                    self.setMeetingProcessingStatus(String(localized: "meeting.processing.finalizing", defaultValue: "Finalizing", bundle: .module, comment: "Status message while finalizing meeting processing"))
                }
                ...
                if let recordingSaveError = persistenceResult.recordingSaveError {
                    await MainActor.run {
                        ...
                        self.presentErrorAlert(title: String(localized: "meeting.recording.title", defaultValue: "Meeting Recording", bundle: .module, comment: "Title label for meeting recording section"), message: recordingSaveError.localizedDescription)
                    }
                }
            } catch {
                ...
                await MainActor.run {
                    self.presentErrorAlert(title: String(localized: "meeting.recording.title", defaultValue: "Meeting Recording", bundle: .module, comment: "Title label for meeting recording section"), message: message)
                }
            }
            await MainActor.run {
    ...
    func revealMeetingRecordingInFinder(path: String) {
        ...
        guard FileManager.default.fileExists(atPath: url.path) else {
            presentErrorAlert(
                title: String(localized: "error.meeting_recording.not_found.title", defaultValue: "Recording Not Found", bundle: .module, comment: "Error title when saved meeting recording cannot be found"),
                message: String(localized: "error.meeting_recording.not_found.message", defaultValue: "The saved meeting recording is no longer available on disk.", bundle: .module, comment: "Error message when saved meeting recording is missing from disk")
            )
            return
        }
    ...
    private func activeMeetingDisplayTitle() -> String {
        guard let activeMeetingID,
              ...
              !title.isEmpty else {
            return String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Generic meeting title label")
        }
        return title
    }
    ...
    private func promptToSaveMeetingRecording(for title: String) async -> Bool {
        ...
        alert.informativeText = String(format: String(localized: "meeting.recording.save_prompt.message", defaultValue: "Keep a merged audio file for \"%@\" so you can inspect it later in Finder.", bundle: .module, comment: "Prompt message explaining why to save meeting recording with meeting title"), "\(title)")
        alert.alertStyle = .informational
        alert.addButton(withTitle: String(localized: "meeting.recording.save_prompt.action.save_recording", defaultValue: "Save Recording", bundle: .module, comment: "Action button title to save meeting recording"))
        alert.addButton(withTitle: String(localized: "meeting.recording.save_prompt.action.dont_save", defaultValue: "Don't Save", bundle: .module, comment: "Action button title to not save meeting recording"))
    ...
    private func presentMeetingStartFailureAlert(error: Error) {
        ...
        if isSystemAudioError {
            alert.messageText = String(localized: "meeting.start_failure.system_audio_capture_failed.title", defaultValue: "System audio capture failed", bundle: .module, comment: "Error title when system audio capture cannot start")
            alert.informativeText = String(format: String(localized: "meeting.start_failure.system_audio_capture_failed.message", defaultValue: "Could not start system audio recording. Open System Settings > Privacy & Security > Screen & System Audio Recording and enable %@ under \"System Audio Recording Only\".\n\nError: %@", bundle: .module, comment: "Error message guiding user to enable system audio recording permission with app name and error detail"), "\(AppIdentity.displayName)", "\(error.localizedDescription)")
            alert.addButton(withTitle: String(localized: "alert.meeting_start_failure.open_system_settings", defaultValue: "Open System Settings", bundle: .module, comment: "Alert action to open System Settings after meeting start failure"))
            alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK", bundle: .module, comment: "Common confirmation button label"))
        } else {
            alert.messageText = String(localized: "meeting.start_failure.generic.title", defaultValue: "Meeting failed to start", bundle: .module, comment: "Generic error title when meeting cannot start")
    ...
    private func setState(_ state: DictationState) {
        ...
        let status: String
        switch state {
        case .idle: status = String(localized: "status.idle", defaultValue: "Idle", bundle: .module, comment: "Status text when system is idle")
        case .preparing: status = String(localized: "meeting.state.preparing", defaultValue: "Preparing", bundle: .module, comment: "Meeting state label for preparing state")
        case .recording: status = String(localized: "meeting.state.recording", defaultValue: "Recording", bundle: .module, comment: "Meeting state label for recording state")
        case .transcribing: status = String(localized: "meeting.state.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Meeting state label for transcribing state")
        }
    ...
    private func showMeetingCompletionNotification(_ notification: PendingMeetingCompletionNotification) {
        meetingNotification.show(
            title: String(localized: "meeting.notification.transcription_complete.title", defaultValue: "Transcription complete", bundle: .module, comment: "Notification title when meeting transcription finishes"),
    ...
    private func presentMeetingSignalLossPromptIfNeeded() {
        ...
        let didShow = meetingNotification.show(
            promptID: promptID,
            title: String(localized: "meeting.signal_lost.title", defaultValue: "Meeting signal lost", bundle: .module, comment: "Alert title when meeting signal is lost"),
            subtitle: String(localized: "meeting.signal_lost.message", defaultValue: "Still recording. Stop if the meeting ended.", bundle: .module, comment: "Alert message shown when meeting signal is lost while recording continues"),
    ...
    private func presentMeetingDetection(_ candidate: MeetingCandidate) {
        ...
        let didShow = meetingNotification.show(
            ...
            title: String(localized: "meeting.detected.title", defaultValue: "Meeting detected", bundle: .module, comment: "Notification title when a meeting is detected"),
            subtitle: title,
    ...
    private func presentMeetingProcessingStage(_ stage: MeetingProcessingStage) {
        ...
        case .transcribingAudio:
            setMeetingProcessingStatus(String(localized: "meeting.processing.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Processing step label while transcribing meeting"))
        case .cleaningAudio:
            setMeetingProcessingStatus(String(localized: "meeting.processing.cleaning", defaultValue: "Cleaning", bundle: .module, comment: "Processing step label while cleaning meeting transcript"))
        case .generatingTitle:
            setMeetingProcessingStatus(String(localized: "meeting.processing.titling", defaultValue: "Titling", bundle: .module, comment: "Processing step label while generating meeting title"))
        case .summarizingNotes:
            setMeetingProcessingStatus(String(localized: "meeting.processing.summarizing", defaultValue: "Summarizing", bundle: .module, comment: "Processing step label while summarizing meeting"))
        }
    }
    ...
    private func finishComputerUseAudioStop(wavURL: URL?, startedAt: Date) {
        ...
        indicator.setTranscribingTitle(String(localized: "computer_use.transcribing_title.parsing_command", defaultValue: "Parsing command", bundle: .module, comment: "Computer use status label while parsing command"), config: config)
        ...
        let task = Task { [weak self] in
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.computerUseCommandTaskID = nil
                    self.setState(.idle)
                    self.indicator.showWarning(String(localized: "computer_use.command_failed.warning", defaultValue: "CUA command failed", bundle: .module, comment: "Warning text when computer use command fails"), icon: "!")
                    self.meetingMonitor.resumeAfterCooldown()
                    self.meetingMonitor.refreshState()
                }
            }
    ...
    private func computerUseFloatingStatusLabel(for status: String) -> String? {
        ...
        if status == "Retrying planner" {
            return String(localized: "computer_use.floating_status.retrying", defaultValue: "Retrying", bundle: .module, comment: "Floating indicator status while retrying action")
        }
        return status
    }
    ...
    private func presentComputerUseRuntimeResult(_ result: ComputerUsePlannerRuntimeResult) {
        ...
        case .done:
            message = result.message.hasPrefix("Done") ? result.message : String(format: String(localized: "computer_use.runtime_result.done_with_message", defaultValue: "Done: %@", bundle: .module, comment: "Computer use runtime result status with completion message"), "\(result.message)")
            floatingMessage = "Done"
            icon = ""
        case .timedOut:
            message = result.message
            floatingMessage = String(localized: "computer_use.runtime_result.timed_out", defaultValue: "Timed out", bundle: .module, comment: "Computer use runtime result status for timeout")
            icon = "!"
        case .needsConfirmation:
            message = result.message.hasPrefix("Confirm") ? result.message : String(format: String(localized: "computer_use.runtime_result.confirm_with_message", defaultValue: "Confirm: %@", bundle: .module, comment: "Computer use runtime result status requesting confirmation with message"), "\(result.message)")
        ...
        case .cancelled:
            message = result.message
            floatingMessage = String(localized: "computer_use.runtime_result.cancelled", defaultValue: "Cancelled", bundle: .module, comment: "Computer use runtime result status for cancellation")
            icon = ""
        }
    ...
    private func handleComputerUseAudioSessionEvent(_ event: DictationAudioSessionEvent) {
        ...
        case .noAudioTimeout(let sessionID, _):
            guard activeComputerUseAudioSessionID == sessionID else { break }
            statusBarController?.setStatus(String(localized: "dictation.mic_waiting_for_speech", defaultValue: "Mic waiting for speech", bundle: .module, comment: "Dictation microphone status when waiting for speech input"))
    ...
    public func startMeetingRecordingForShortcuts() -> Bool {
        startMeetingRecordingForShortcuts(
            title: String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Generic meeting title label")
        )
    }
    ...
    private func userFacingDictationTestError(_ error: Error) -> String {
        ...
        if nsError.domain == "MuesliTranscriptionRuntime" {
            switch nsError.code {
            case 1:
                return String(localized: "dictation.test_error.nemotron_requires_macos15", defaultValue: "Nemotron requires macOS 15 or later. Choose another model to test dictation.", bundle: .module, comment: "Dictation test error when Nemotron model requires newer macOS version")
            case 2:
                return String(localized: "dictation.test_error.qwen3_requires_macos15", defaultValue: "Qwen3 ASR requires macOS 15 or later. Choose another model to test dictation.", bundle: .module, comment: "Dictation test error when Qwen3 ASR model requires newer macOS version")
            case 4:
                return String(localized: "dictation.test_error.cohere_requires_macos15", defaultValue: "Cohere Transcribe requires macOS 15 or later. Choose another model to test dictation.", bundle: .module, comment: "Dictation test error when Cohere Transcribe model requires newer macOS version")
            default:
                return String(localized: "dictation.test_error.model_unavailable", defaultValue: "The selected model is not available. Choose another model and try again.", bundle: .module, comment: "Dictation test error when selected model is unavailable")
            }
        }
        ...
        if lowercasedMessage.contains("not loaded") || lowercasedMessage.contains("loadmodels") {
            return String(localized: "dictation.test_error.model_not_ready_retry", defaultValue: "The model was not ready yet. We are preparing it again, then try once more.", bundle: .module, comment: "Dictation test error when model is not ready and will be prepared again")
        }
        if lowercasedMessage.contains("network") || lowercasedMessage.contains("internet") || lowercasedMessage.contains("timed out") {
            return String(localized: "dictation.test_error.download_incomplete", defaultValue: "The model could not finish downloading. Check your connection and retry.", bundle: .module, comment: "Dictation test error when model download does not complete")
        }
        if lowercasedMessage.contains("permission") || lowercasedMessage.contains("microphone") {
            return String(localized: "dictation.test_error.microphone_permission_denied", defaultValue: "Muesli could not access the microphone. Check Microphone permission and try again.", bundle: .module, comment: "Dictation test error when microphone permission is denied")
        }
        return String(localized: "dictation.test_error.could_not_start", defaultValue: "Dictation could not start. Try again in a moment.", bundle: .module, comment: "Dictation test error when dictation cannot start")
    }
    ...
    private func checkMaraudersMapActivation(_ text: String) {
        ...
        indicator.showWarning(String(localized: "marauders_map.activation_title", defaultValue: "Mischief Managed", bundle: .module, comment: "Title shown when Marauder's Map mode is activated"), icon: "\u{26A1}", duration: 3.0)
        startMaraudersMapMonitoring()
    }
    ...
    private func handleUpcomingMeeting(_ event: UpcomingMeetingEvent) {
        ...
        if minutesUntil > 0 {
            timeLabel = String(format: String(localized: "meeting.upcoming.time.starts_in_minutes", defaultValue: "starts in %d min", bundle: .module, comment: "Upcoming meeting relative start time in minutes"), minutesUntil)
        } else if minutesUntil == 0 {
            timeLabel = String(localized: "meeting.upcoming.time.starting_now", defaultValue: "starting now", bundle: .module, comment: "Relative upcoming meeting time label when meeting starts now")
        } else {
            timeLabel = String(format: String(localized: "meeting.upcoming.time.started_minutes_ago", defaultValue: "started %d min ago", bundle: .module, comment: "Relative upcoming meeting time label when meeting started minutes ago"), abs(minutesUntil))
        }

        let title = event.title
        let notificationTitle = minutesUntil <= 0 ? String(localized: "meeting.upcoming.notification_title.starting_now", defaultValue: "Meeting starting now", bundle: .module, comment: "Upcoming meeting notification title when meeting starts now") : String(localized: "meeting.upcoming.notification_title.default", defaultValue: "Upcoming meeting", bundle: .module, comment: "Default upcoming meeting notification title")
        meetingNotification.show(
    ...
    private func showMeetingEndNotification(title: String) {
        ...
        meetingNotification.show(
            title: String(localized: "meeting.end_notification.title", defaultValue: "Meeting ended", bundle: .module, comment: "Notification title shown when meeting scheduled time ends"),
            subtitle: String(format: String(localized: "meeting.end_notification.body.scheduled_time_over", defaultValue: "%@ · scheduled time is over", bundle: .module, comment: "Meeting end notification body with meeting title"), "\(title)"),
            actionLabel: String(localized: "meeting.end_notification.action.stop_recording", defaultValue: "Stop Recording", bundle: .module, comment: "Meeting end notification action to stop recording"),
            dismissAfter: 45,
            onStartRecording: { [weak self] in
                self?.stopMeetingRecording()
            },
