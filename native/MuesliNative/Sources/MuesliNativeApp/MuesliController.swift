...
enum DictationBackendReadiness: Equatable {
    ...
    func blockingMessage(backendLabel: String) -> String? {
        switch self {
        case .preparing:
            return String(format: String(localized: "status.backend_warmup.message", defaultValue: "Warming up %@...", bundle: .module, comment: "Status message shown while backend is warming up"), "\(backendLabel)")
        case .ready:
            return nil
        case .failed:
            return String(format: String(localized: "status.backend_unavailable.message", defaultValue: "%@ unavailable", bundle: .module, comment: "Status message shown when backend is unavailable"), "\(backendLabel)")
        }
    }
}
...
enum MeetingResummarizationPolicy {
    static func plan(for meeting: MeetingRecord) -> MeetingResummarizationPlan {
        ...
        let promptTitle = trimmed.isEmpty ? String(localized: "prompt.meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Prompt title for meeting context") : trimmed
...
enum MeetingSummaryPersistenceError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .failedToSaveSummary(let underlying):
            ...
            if detail.isEmpty {
                return String(localized: "error.meeting_notes.save_failed.message", defaultValue: "The updated meeting notes could not be saved.", bundle: .module, comment: "Error message shown when updated meeting notes fail to save")
            }
            return String(format: String(localized: "error.meeting_notes.save_failed_with_detail.message", defaultValue: "The updated meeting notes could not be saved. %@", bundle: .module, comment: "Error message shown when updated meeting notes fail to save with detail"), "\(detail)")
        }
    }
}
...
enum MeetingTemplateSelectionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .templateNoLongerExists:
            return String(localized: "error.meeting_template.missing.message", defaultValue: "That template no longer exists. Choose another template and try again.", bundle: .module, comment: "Error message shown when selected meeting template no longer exists")
        }
    }
}
...
enum MeetingRetranscriptionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .controllerUnavailable:
            return String(localized: "error.retranscription.controller_unavailable.message", defaultValue: "Meeting re-transcription could not continue because Muesli is no longer available.", bundle: .module, comment: "Error message shown when retranscription cannot continue because controller is unavailable")
        case .recordingUnavailable:
            return String(localized: "error.retranscription.recording_missing_on_disk.message", defaultValue: "The saved meeting recording is no longer available on disk.", bundle: .module, comment: "Error message shown when saved meeting recording file is missing")
        case .noDownloadedTranscriptionModel:
            return String(localized: "error.retranscription.model_required.message", defaultValue: "Download a transcription model before re-transcribing this meeting.", bundle: .module, comment: "Error message shown when retranscription requires a downloaded model")
        case .emptyTranscript:
            return String(localized: "error.retranscription.no_speech_detected.message", defaultValue: "Re-transcription finished, but no speech was detected in the saved recording.", bundle: .module, comment: "Error message shown when retranscription detects no speech")
        case .failedToSave(let underlying):
            return String(format: String(localized: "error.retranscription.save_failed_with_detail.message", defaultValue: "The re-transcribed meeting could not be saved. %@", bundle: .module, comment: "Error message shown when retranscribed meeting cannot be saved with underlying error detail"), "\(underlying.localizedDescription)")
        }
    }
}
...
enum MeetingLifecycleError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .failedToSaveRecording(let underlying):
            return String(format: String(localized: "error.meeting_recording.save_failed_with_detail.message", defaultValue: "The meeting finished transcribing, but the recording could not be saved. %@", bundle: .module, comment: "Error message shown when meeting recording fails to save after transcription completes"), "\(underlying.localizedDescription)")
        case .failedToDeleteRecording(let underlying):
            return String(format: String(localized: "error.meeting_recording.delete_failed_with_detail.message", defaultValue: "The saved meeting recording could not be deleted, so the meeting was left in place. %@", bundle: .module, comment: "Error message shown when deleting saved meeting recording fails"), "\(underlying.localizedDescription)")
        case .failedToDeleteMeeting(let underlying):
            return String(format: String(localized: "error.delete_meeting.failed_with_detail.message", defaultValue: "The meeting could not be deleted. %@", bundle: .module, comment: "Error message shown when deleting a meeting fails with detail"), "\(underlying.localizedDescription)")
        }
    }
}
...
public final class MuesliController: NSObject {
    ...
    func truncate(_ text: String, limit: Int) -> String {
        ...
        return String(compact.prefix(limit - 3)).trimmingCharacters(in: .whitespacesAndNewlines) + String(localized: "common.ellipsis", defaultValue: "...", bundle: .module, comment: "Common ellipsis placeholder/status text")
    }
    ...
    func refreshUI() {
        statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", bundle: .module, comment: "Status text shown when system is idle"))
    ...
    func enableIPhoneBridgeSync() {
        ...
        appState.iCloudSyncStatus = String(localized: "sync.icloud.checking", defaultValue: "Checking iCloud...", bundle: .module, comment: "Status text shown while checking iCloud sync state")
        ...
        iCloudSubscriptionTask = Task { [weak self] in
            do {
                ...
                await MainActor.run {
                    ...
                    self.appState.iCloudSyncStatus = String(localized: "sync.icloud.setup_private", defaultValue: "Setting up private iCloud sync...", bundle: .module, comment: "Status text shown while setting up private iCloud sync")
                ...
                await MainActor.run {
                    ...
                    self.appState.iCloudSyncStatus = String(format: String(localized: "sync.icloud.required_message", defaultValue: "Sync needs iCloud: %@", bundle: .module, comment: "Status text shown when iCloud requirement message is present"), "\(message)")
    ...
    private func startICloudSync() {
        ...
        guard config.iCloudSyncEnabled else {
            if userInitiated {
                appState.iCloudSyncStatus = String(localized: "sync.icloud.turn_on_first", defaultValue: "Turn on iCloud sync first.", bundle: .module, comment: "Instruction shown when user must enable iCloud sync before continuing")
            }
        ...
        guard iCloudSyncTask == nil else {
            ...
            appState.iCloudBridgeMessage = nil
            if userInitiated {
                appState.iCloudSyncStatus = String(localized: "sync.already_in_progress", defaultValue: "Sync already in progress.", bundle: .module, comment: "Status message shown when sync request is ignored because syncing is already running")
            }
        ...
        enableICloudPersistentSync()
        appState.isICloudSyncInProgress = true
        appState.iCloudSyncStatus = String(localized: "sync.icloud.syncing_private", defaultValue: "Syncing with private iCloud...", bundle: .module, comment: "Status message shown while syncing with private iCloud")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        ...
        iCloudSyncTask = Task { [weak self] in
            do {
                ...
                await MainActor.run {
                    ...
                    let remoteDeviceName = MuesliBridgeDeviceIdentity.remoteDeviceDisplayName ?? String(localized: "device.iphone.default_name", defaultValue: "iPhone", bundle: .module, comment: "Default remote device name used when no synced device name is available")
                    self.appState.iCloudSyncStatus = result.downloaded.total > 0
                        ? String(format: String(localized: "sync.icloud.synced_with_device", defaultValue: "Synced with %@.", bundle: .module, comment: "Status message showing the remote device used for iCloud sync"), "\(remoteDeviceName)")
                        : String(localized: "sync.icloud.all_text_up_to_date", defaultValue: "All text is up to date.", bundle: .module, comment: "Status message indicating iCloud text content is fully synced")
                ...
                await MainActor.run {
                    ...
                    self.bridgeDiscoveryFollowUpPending = false
                    let message = error.localizedDescription
                    self.appState.iCloudSyncStatus = String(format: String(localized: "sync.failed_with_reason", defaultValue: "Sync failed: %@", bundle: .module, comment: "Status message showing sync failure reason"), "\(message)")
                    if MuesliICloudSyncEngine.isICloudAccountAvailabilityError(error) {
                        self.appState.iCloudBridgeState = .needsICloud
                    } else {
                        self.appState.iCloudBridgeState = .error
    ...
    private func disableICloudSyncRuntimeState() {
        ...
        resetBridgeDiscoveryRuntimeState()
        appState.iCloudSyncStatus = String(localized: "sync.icloud.turning_off", defaultValue: "Turning off iCloud sync...", bundle: .module, comment: "Status message shown while disabling iCloud sync")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        Task { [weak self] in
            ...
            self.appState.iCloudSyncStatus = String(localized: "sync.icloud.off", defaultValue: "iCloud sync is off.", bundle: .module, comment: "Status message shown when iCloud sync is disabled")
    ...
    private func disableICloudSyncForUnavailableEntitlement() {
        ...
        resetICloudSubscriptionState()
        resetBridgeDiscoveryRuntimeState()
        appState.iCloudSyncStatus = String(localized: "sync.icloud.stopping_unavailable", defaultValue: "Stopping unavailable iCloud sync...", bundle: .module, comment: "Status message shown while stopping iCloud sync because it is unavailable")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        Task { [weak self] in
            await cancellationTask?.value
            guard let self,
                  self.iCloudSyncGeneration == generation,
                  self.ckSyncEngine == nil else { return }
            self.appState.iCloudSyncStatus = String(localized: "sync.icloud.unavailable_local_build", defaultValue: "iCloud sync is unavailable in this local-only build.", bundle: .module, comment: "Status message explaining iCloud sync is unavailable in local-only build")
            self.appState.iCloudBridgeState = .notConfigured
        }
    }
    ...
    private func formatICloudSyncSummary(_ result: ICloudSyncResult) -> String {
        String(format: String(localized: "sync.icloud.summary.up_down", defaultValue: "%@ up, %@ down", bundle: .module, comment: "Sync summary showing uploaded and downloaded item counts"), "\(formatICloudSyncCounts(result.uploaded))", "\(formatICloudSyncCounts(result.downloaded))")
    }
    ...
    private func formatICloudSyncCounts(_ counts: ICloudSyncKindCounts) -> String {
        ...
        if counts.dictations > 0 {
            parts.append("\(counts.dictations) \(counts.dictations == 1 ? String(localized: "sync.icloud.count.item.dictation.singular", defaultValue: "dictation", bundle: .module, comment: "Singular noun used for dictation count in iCloud sync summary") : String(localized: "sync.icloud.count.item.dictation.plural", defaultValue: "dictations", bundle: .module, comment: "Plural noun used for dictation count in iCloud sync summary"))")
        }
        if counts.meetings > 0 {
            parts.append("\(counts.meetings) \(counts.meetings == 1 ? "meeting" : String(localized: "sync.icloud.count.item.meeting.plural", defaultValue: "meetings", bundle: .module, comment: "Plural noun used for meeting count in iCloud sync summary"))")
        }
    ...
    func selectBackend(_ option: BackendOption) {
        ...
        Task { [weak self] in
            ...
            if needsWarmup {
                await MainActor.run {
                    self.indicator.showLoading(String(localized: "status.warming_up", defaultValue: "Warming up...", bundle: .module, comment: "General status text shown while system components are warming up"))
                }
            }
            let ppOption = self.runtimePostProcessorOption()
            await self.configureTranscriptCleanupForRuntime(option: ppOption)
    ...
    func selectMeetingTranscriptionBackend(_ option: BackendOption, requireDownloaded: Bool = true) {
        guard option.supportsMeetingTranscription else {
            presentErrorAlert(
                title: String(localized: "error.meeting_transcription.model_unavailable", defaultValue: "Meeting model unavailable", bundle: .module, comment: "Error title shown when selected meeting transcription model is unavailable"),
                message: String(format: String(localized: "error.meeting_transcription.option_not_supported.message", defaultValue: "%@ is optimized for dictation and cannot be used for meeting transcription.", bundle: .module, comment: "Error message shown when a dictation-optimized model is chosen for meeting transcription"), "\(option.label)")
            )
            normalizeMeetingTranscriptionSelectionForAvailability()
            return
        }
        guard !requireDownloaded || option.isDownloaded else {
            presentErrorAlert(
                title: String(localized: "error.meeting_transcription.model_unavailable", defaultValue: "Meeting model unavailable", bundle: .module, comment: "Error title shown when meeting transcription model cannot be used"),
                message: String(format: String(localized: "error.meeting_transcription.download_required.message", defaultValue: "Download %@ before using it for meeting transcription.", bundle: .module, comment: "Error message shown when selected meeting model must be downloaded first"), "\(option.label)")
            )
            normalizeMeetingTranscriptionSelectionForAvailability()
            return
        }
    ...
    func selectPostProcessorBackend(_ option: TranscriptCleanupBackendOption) {
        guard option.isCompatible(with: selectedBackend) else {
            presentErrorAlert(
                title: String(localized: "error.cleanup_model.unavailable.title", defaultValue: "Cleanup model unavailable", bundle: .module, comment: "Error title shown when selected cleanup model cannot be used"),
                message: String(localized: "error.cleanup_model.gemma4_same_backend.message", defaultValue: "Gemma 4 cannot clean up a transcription produced by the same Gemma 4 backend.", bundle: .module, comment: "Error message shown when Gemma 4 cleanup is attempted on Gemma 4-generated transcription")
            )
            return
        }
    ...
    func refreshGoogleCalendarList() async {
        ...
        do {
            ...
            appState.googleCalendarListLoadState = .failed(String(format: String(localized: "calendar.google.token_refresh_failed", defaultValue: "Token refresh failed: %@", bundle: .module, comment: "Error status shown when Google Calendar token refresh fails"), "\(message)"))
        } catch {
    ...
    private func showMeetingStartingNowNotification(
        ...
        meetingNotification.show(
            title: String(localized: "notifications.meeting_starting_now.title", defaultValue: "Meeting starting now", bundle: .module, comment: "Notification title shown when a meeting is starting immediately"),
            subtitle: title,
    ...
    func continueModelPreparationAfterOnboarding(
        ...
        updateModelPreparationStatus(
            title: String(format: String(localized: "model_preparation.preparing_backend", defaultValue: "Preparing %@", bundle: .module, comment: "Model preparation status shown while backend setup begins"), "\(backend.label)"),
            detail: initialStatus ?? String(format: String(localized: "model_preparation.preparing_backend_ellipsis", defaultValue: "Preparing %@...", bundle: .module, comment: "Model preparation status with ellipsis while backend setup is ongoing"), "\(backend.label)"),
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.updateModelPreparationStatus(
                        title: String(format: String(localized: "model_preparation.backend_ready", defaultValue: "%@ ready", bundle: .module, comment: "Status shown when a backend finishes preparation"), "\(backend.label)"),
                        detail: String(localized: "model_preparation.ready_for_transcription", defaultValue: "Ready for transcription", bundle: .module, comment: "Status shown when model preparation is complete for transcription"),
                        progress: 1.0,
                        isPreparing: false,
                        isComplete: true
                    )
                ...
                await MainActor.run {
                    self.onboardingModelPreparationTask = nil
                    self.updateModelPreparationStatus(
                        title: backend.isDownloaded ? String(localized: "model_preparation.paused", defaultValue: "Model setup paused", bundle: .module, comment: "Status shown when model setup is paused") : String(localized: "model_preparation.download_paused", defaultValue: "Download paused", bundle: .module, comment: "Status shown when model download is paused"),
    ...
    func downloadModelForOnboarding(
        ...
        progress(
            ...
            wasDownloaded ? String(format: String(localized: "onboarding.model_download.warming_up_backend", defaultValue: "Warming up %@...", bundle: .module, comment: "Onboarding status shown while warming up selected backend"), "\(backend.label)") : String(format: String(localized: "onboarding.model_download.downloading_backend", defaultValue: "Downloading %@...", bundle: .module, comment: "Onboarding status shown while downloading selected backend"), "\(backend.label)")
        )
        try await transcriptionCoordinator.preloadRequired(
            backend: backend,
            ...
            progress: { value, status in
                ...
                if status?.localizedCaseInsensitiveContains("download") == true {
                    progress(value, "\(status ?? String(format: String(localized: "onboarding.model_download.downloading_backend", defaultValue: "Downloading %@...", bundle: .module, comment: "Onboarding status shown while downloading selected backend"), "\(backend.label)"))")
                } else if value >= 0.9 {
                    progress(value, status ?? String(format: String(localized: "onboarding.model_download.warming_up_backend", defaultValue: "Warming up %@...", bundle: .module, comment: "Onboarding status shown while warming up selected backend"), "\(backend.label)"))
                } else {
                    progress(value, status ?? String(format: String(localized: "onboarding.model_download.preparing_backend_ellipsis", defaultValue: "Preparing %@...", bundle: .module, comment: "Onboarding status shown while preparing selected backend with ellipsis"), "\(backend.label)"))
                }
            },
        ...
        progress(1.0, String(format: String(localized: "onboarding.model_download.backend_ready", defaultValue: "%@ ready", bundle: .module, comment: "Onboarding status shown when selected backend is ready"), "\(backend.label)"))
    }

    private func applyModelPreparationProgress(_ progress: Double, status: String?, backend: BackendOption) {
        let detail = status ?? String(format: String(localized: "onboarding.model_download.preparing_backend_ellipsis", defaultValue: "Preparing %@...", bundle: .module, comment: "Onboarding status shown while preparing selected backend with ellipsis"), "\(backend.label)")
        ...
        if isPreparing {
            updateModelPreparationStatus(
                title: String(format: String(localized: "onboarding.model_download.preparing_backend", defaultValue: "Preparing %@", bundle: .module, comment: "Onboarding status shown while preparing selected backend"), "\(backend.label)"),
                detail: String(format: String(localized: "model_preparation.status.optimizing_backend_for_mac", defaultValue: "Optimizing %@ for this Mac...", bundle: .module, comment: "Status shown while optimizing selected backend for the current Mac"), "\(backend.label)"),
                progress: nil,
                isPreparing: true,
                isComplete: false
            )
    ...
    private func modelPreparationFailureMessage(for backend: BackendOption) -> String {
        backend.isDownloaded
            ? String(localized: "model_preparation.failure.setup_failed_restart_or_retry", defaultValue: "Model setup failed. Restart Muesli or retry from Models.", bundle: .module, comment: "Model preparation failure message instructing user to restart app or retry from Models")
            : String(localized: "model_preparation.failure.download_failed_check_connection", defaultValue: "Download failed. Check your connection and retry.", bundle: .module, comment: "Model download failure message instructing user to check network and retry")
    }
    ...
    private func presentStandardUpdateCheck() {
        guard let updaterController else {
            appState.sparkleUpdateStatus = .disabled(message: String(localized: "updates.disabled_for_build", defaultValue: "Update checks are disabled for this build.", bundle: .module, comment: "Status text shown when update checks are disabled in current build"))
            return
        }
            ...
            do {
                ...
                do {
                    formattedNotes = try await MeetingSummaryClient.summarize(
                        ...
                        meetingTitle: meeting.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Default meeting title label") : meeting.title,
                        config: self.config,
                        template: templateSnapshot,
                        existingNotes: self.notesContextForResummary(meeting),
                        manualNotesToRetain: meeting.manualNotes
    ...
    static func stripManualNotesSection(from notes: String) -> String {
        let markers = [
            "\n\n" + String(localized: "notes.section.written_notes.h3_spaced", defaultValue: "### Written notes", bundle: .module, comment: "Markdown section header block for written notes with surrounding spacing") + "\n\n",
            "\n" + String(localized: "notes.section.written_notes.h3_leading_newline", defaultValue: "### Written notes", bundle: .module, comment: "Markdown section header block for written notes with leading newline") + "\n\n",
            String(localized: "notes.section.written_notes.h3", defaultValue: "### Written notes", bundle: .module, comment: "Markdown section header block for written notes") + "\n\n",
            "\n\n" + String(localized: "notes.section.manual_notes.h2_spaced", defaultValue: "## Manual Notes", bundle: .module, comment: "Markdown section header block for manual notes with surrounding spacing") + "\n\n",
            "\n" + String(localized: "notes.section.manual_notes.h2_leading_newline", defaultValue: "## Manual Notes", bundle: .module, comment: "Markdown section header block for manual notes with leading newline") + "\n\n",
            String(localized: "notes.section.manual_notes.h2", defaultValue: "## Manual Notes", bundle: .module, comment: "Markdown section header block for manual notes") + "\n\n"
        ]
    ...
    func deleteMeeting(id: Int64) {
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.delete_meeting.failed.title", defaultValue: "Couldn't Delete Meeting", bundle: .module, comment: "Error alert title shown when deleting a meeting fails"), message: error.localizedDescription)
            return
        } catch {
            presentErrorAlert(
                title: String(localized: "error.delete_meeting.failed.title", defaultValue: "Couldn't Delete Meeting", bundle: .module, comment: "Error alert title shown when deleting a meeting fails"),
    ...
    func clearMeetingHistory() {
        guard !isMeetingRecording(), !isStartingMeetingRecording, backgroundMeetingProcessingCount == 0 else {
            presentErrorAlert(
                title: String(localized: "error.clear_meeting_history.failed.title", defaultValue: "Couldn't Clear Meeting History", bundle: .module, comment: "Error alert title shown when clearing meeting history fails"),
                message: String(localized: "error.clear_meeting_history.active_meeting.message", defaultValue: "A meeting is recording or still being processed. Please wait before clearing saved meetings.", bundle: .module, comment: "Error message shown when clearing meeting history is blocked by active or processing meeting")
            )
            return
        }
    ...
    func shouldTerminateApplication() -> Bool {
        ...
        case .none:
            return true
        case .starting:
            messageText = String(localized: "meeting_recording.starting_message", defaultValue: "Meeting recording is starting", bundle: .module, comment: "Termination warning title shown while meeting recording startup is in progress")
            informativeText = String(localized: "termination.meeting_recording_starting.warning", defaultValue: "Quitting now will cancel the meeting recording before it has been saved.", bundle: .module, comment: "Termination warning message shown during meeting recording startup")
        case .recording:
            messageText = String(localized: "termination.meeting_recording_in_progress.title", defaultValue: "Meeting recording in progress", bundle: .module, comment: "Termination warning title shown while meeting recording is active")
            informativeText = String(localized: "termination.meeting_recording_in_progress.warning", defaultValue: "Quitting now will stop the meeting recording and the current transcript may be lost. Stop the recording first if you want Muesli to save notes.", bundle: .module, comment: "Termination warning message shown while meeting recording is in progress")
        case .processing:
            messageText = String(localized: "termination.meeting_transcription_in_progress.title", defaultValue: "Meeting transcription in progress", bundle: .module, comment: "Termination warning title shown while meeting transcription is in progress")
            informativeText = String(localized: "termination.meeting_transcription_in_progress.warning", defaultValue: "Quitting now will interrupt transcription and the meeting notes may not be saved.", bundle: .module, comment: "Termination warning message shown while meeting transcription is in progress")
        }
        ...
        alert.addButton(withTitle: String(localized: "termination.action.keep_muesli_running", defaultValue: "Keep Muesli Running", bundle: .module, comment: "Button label to cancel app termination and keep Muesli running"))
        alert.addButton(withTitle: String(localized: "termination.action.quit_anyway", defaultValue: "Quit Anyway", bundle: .module, comment: "Button label to force quit despite active meeting work"))
    ...
    func pauseMeetingRecording() {
        ...
        statusBarController?.setStatus(String(localized: "meeting.status.paused", defaultValue: "Meeting paused", bundle: .module, comment: "Status text shown when meeting recording is paused"))
        statusBarController?.refresh()
        syncAppState()
    }
    ...
    func resumeMeetingRecording() {
        ...
        statusBarController?.setStatus(String(format: String(localized: "meeting.status.with_title", defaultValue: "Meeting: %@", bundle: .module, comment: "Status text showing active meeting with title"), "\(activeMeetingDisplayTitle())"))
        statusBarController?.refresh()
        syncAppState()
    }
    ...
    func startMeetingRecordingFromEntryPoint(
        title: String = String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Default meeting title when no custom title is available"),
    ...
    func startMeetingRecording(
        ...
        guard let meetingBackend = normalizeMeetingTranscriptionSelectionForAvailability() else {
            presentErrorAlert(
                title: String(localized: "error.meeting_start.failed.title", defaultValue: "Meeting failed to start", bundle: .module, comment: "Error title shown when meeting recording cannot start"),
                message: String(localized: "error.meeting_start.model_required.message", defaultValue: "Download a transcription model before recording a meeting.", bundle: .module, comment: "Error message shown when no transcription model is available to start a meeting")
            )
            return false
        }
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.meeting_start.failed.title", defaultValue: "Meeting failed to start", bundle: .module, comment: "Error title shown when meeting recording cannot start"), message: error.localizedDescription)
            return false
        }
        armMeetingAutoStop(
        ...
        updateMeetingStartStatus(String(localized: "meeting.start_status.transcription_starting_soon", defaultValue: "Meeting transcription will start shortly.", bundle: .module, comment: "Status message shown immediately after meeting start before transcription begins"))
        indicator.setState(.preparing, config: config)
        beginMeetingActivity(reason: String(localized: "meeting.activity.recording_and_transcribing", defaultValue: "Recording and transcribing a meeting", bundle: .module, comment: "Activity description shown while actively recording and transcribing a meeting"))
        ...
        meetingStartTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if self.meetingStartMeetingID == meetingID {
                    ...
                    self.meetingMonitor.refreshState()
                    self.statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", bundle: .module, comment: "Status text shown when no active recording or processing is running"))
                    self.statusBarController?.refresh()
                    self.setState(.idle)
    ...
    func startQuickNoteMeeting() {
        startMeetingRecordingFromEntryPoint(title: String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Default meeting title when no custom title is available"))
    }
    ...
    func resumeFinishedMeeting(meetingID: Int64) {
        ...
        guard let meetingBackend = normalizeMeetingTranscriptionSelectionForAvailability() else {
            presentErrorAlert(
                title: String(localized: "error.resume_meeting.failed.title", defaultValue: "Resume failed", bundle: .module, comment: "Error title shown when resuming meeting recording fails"),
                message: String(localized: "error.resume_meeting.model_required.message", defaultValue: "Download a transcription model before recording.", bundle: .module, comment: "Error message shown when no transcription model is available to resume recording")
            )
            return
        }
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.resume_meeting.failed.title", defaultValue: "Resume failed", bundle: .module, comment: "Error title shown when resuming meeting recording fails"), message: error.localizedDescription)
            return
        }
        ...
        syncDictationRecorderWarmup(intent: .idlePrewarm(.meetingStateChanged))
        meetingStartMeetingID = meetingID
        updateMeetingStartStatus(String(localized: "meeting.resume.status.resuming_recording", defaultValue: "Resuming meeting recording…", bundle: .module, comment: "Status message shown while meeting recording is resuming"))
        indicator.setState(.preparing, config: config)
        beginMeetingActivity(reason: String(localized: "meeting.activity.recording_and_transcribing", defaultValue: "Recording and transcribing a meeting", bundle: .module, comment: "Activity text shown while recording and transcribing an active meeting"))
        meetingMonitor.suppressWhileActive()
        meetingMonitor.refreshState()
        updateMeetingNotificationVisibility()
    ...
    func importAudioFile() {
        ...
        guard normalizeMeetingTranscriptionSelectionForAvailability() != nil else {
            presentErrorAlert(
                title: String(localized: "error.import_audio.failed.title", defaultValue: "Import Failed", bundle: .module, comment: "Error title shown when audio import fails"),
                message: String(localized: "error.import_audio.model_required.message", defaultValue: "Download a transcription model before importing audio files.", bundle: .module, comment: "Error message shown when importing audio requires a downloaded transcription model")
            )
            return
        }
    ...
    func importAudioFileFromURL(_ url: URL) {
        ...
        guard AudioFileImportController.isSupportedFileURL(url) else {
            presentErrorAlert(
                title: String(localized: "error.import_audio.failed.title", defaultValue: "Import Failed", bundle: .module, comment: "Error title shown when audio import fails"),
                message: String(localized: "error.import_audio.unsupported_format.message", defaultValue: "This audio file format is not supported.", bundle: .module, comment: "Error message shown when imported audio file format is unsupported")
            )
            return
        }
        guard normalizeMeetingTranscriptionSelectionForAvailability() != nil else {
    ...
    private func importAudioFile(from sourceURL: URL, sessionID: UUID) async {
        ...
        let title = filename.isEmpty ? String(localized: "import.recording.title", defaultValue: "Imported Recording", bundle: .module, comment: "Default title assigned to a newly imported recording") : filename

        self.updateImportProgressStatus(String(localized: "import.audio.progress.importing_file", defaultValue: "Importing audio file...", bundle: .module, comment: "Progress status shown while importing an audio file"), sessionID: sessionID)
        self.beginMeetingActivity(reason: String(localized: "import.audio.activity.importing_for_transcription", defaultValue: "Importing audio file for transcription", bundle: .module, comment: "Activity text shown while importing an audio file for transcription"))
    ...
    func cancelMeetingPreparation() {
        ...
        statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", bundle: .module, comment: "Status text shown when no active recording or processing is running"))
        statusBarController?.refresh()
        setState(.idle)
        endMeetingActivity()
        disarmMeetingAutoStop()
    ...
    private func startMeetingRecordingWithSystemAudioRecovery(
        ...
        statusBarController?.setStatus(String(localized: "meeting.start_status.transcription_starting_soon", defaultValue: "Meeting transcription will start shortly.", bundle: .module, comment: "Status text shown after meeting start before transcription begins"))
        statusBarController?.refresh()
        try Task.checkCancellation()
        try await transcriptionCoordinator.preloadRequired(
            backend: backend,
        ...
        while true {
            ...
            do {
                ...
                meetingMonitor.suppressWhileActive()
                meetingMonitor.refreshState()
                statusBarController?.setStatus(String(format: String(localized: "meeting.status.with_title", defaultValue: "Meeting: %@", bundle: .module, comment: "Status text showing active meeting with its title"), "\(title)"))
                ...
                try Task.checkCancellation()
                try checkMeetingStartStillCurrent(meetingID)
                updateMeetingStartStatus(String(localized: "meeting.start_status.requesting_system_audio_permission", defaultValue: "Requesting system audio permission...", bundle: .module, comment: "Meeting start status shown while requesting system audio permission"))
                statusBarController?.setStatus(String(localized: "meeting.start_status.requesting_system_audio_permission", defaultValue: "Requesting system audio permission...", bundle: .module, comment: "Meeting start status shown while requesting system audio permission"))
                statusBarController?.refresh()
                let granted = await CoreAudioSystemRecorder.requestSystemAudioAccess()
                try Task.checkCancellation()
                try checkMeetingStartStillCurrent(meetingID)
                if granted {
                    updateMeetingStartStatus(String(localized: "meeting.start_status.retrying", defaultValue: "Retrying meeting start...", bundle: .module, comment: "Meeting start status shown while retrying failed start attempt"))
                    statusBarController?.setStatus(String(localized: "meeting.start_status.retrying", defaultValue: "Retrying meeting start...", bundle: .module, comment: "Meeting start status shown while retrying failed start attempt"))
                    statusBarController?.refresh()
                    continue
                }
                throw error
    ...
    private final class MeetingDiscardButtonTitleUpdater: NSObject {
        ...
        @MainActor @objc func manualNotesCheckboxChanged(_ sender: NSButton) {
            discardButton?.title = sender.state == .on ? String(localized: "common.discard", defaultValue: "Discard", bundle: .module, comment: "Common destructive action label used to discard content") : String(localized: "meeting.discard_recording.action", defaultValue: "Discard Recording", bundle: .module, comment: "Action label to discard current meeting recording")
        }
    }
    ...
    @objc func discardMeetingWithConfirmation() {
        ...
        alert.messageText = String(localized: "meeting.discard_recording.confirmation_title", defaultValue: "Discard recording?", bundle: .module, comment: "Confirmation dialog title for discarding current meeting recording")
        alert.alertStyle = .warning
        var manualNotesCheckbox: NSButton?
        if hasManualNotes {
            alert.informativeText = String(localized: "meeting.discard_recording.message_with_notes_choice", defaultValue: "This will stop the meeting. Choose whether to delete the written notes too.", bundle: .module, comment: "Confirmation dialog message describing discard recording behavior with notes choice")
            let accessory = Self.makeDiscardMeetingAccessoryView()
            manualNotesCheckbox = accessory.manualNotesCheckbox
            alert.accessoryView = accessory.view
            alert.addButton(withTitle: String(localized: "meeting.discard_recording.action", defaultValue: "Discard Recording", bundle: .module, comment: "Action label to discard current meeting recording"))
            alert.addButton(withTitle: String(localized: "common.cancel", defaultValue: "Cancel", bundle: .module, comment: "Common action label to cancel an in-progress dialog or operation"))
            ...
            alert.informativeText = String(localized: "meeting.discard_recording.destructive_warning", defaultValue: "This will stop the meeting recording and delete all captured audio. This cannot be undone.", bundle: .module, comment: "Destructive warning text shown before discarding recording and captured audio")
            alert.addButton(withTitle: String(localized: "common.discard", defaultValue: "Discard", bundle: .module, comment: "Common destructive action label used to discard content"))
            alert.addButton(withTitle: String(localized: "common.cancel", defaultValue: "Cancel", bundle: .module, comment: "Common action label to cancel an in-progress dialog or operation"))
            alert.buttons.first?.hasDestructiveAction = true
        }
    ...
    private static func makeDiscardMeetingAccessoryView() -> MeetingDiscardAccessory {
        let label = NSTextField(labelWithString: String(localized: "meeting.discard_recording.will_delete_label", defaultValue: "Will delete:", bundle: .module, comment: "Prefix label introducing list of items that will be deleted when discarding recording"))
        label.font = .systemFont(ofSize: NSFont.systemFontSize)
        label.textColor = .secondaryLabelColor

        let recordingCheckbox = NSButton(checkboxWithTitle: String(localized: "meeting.discard_recording.item.recording_audio", defaultValue: "Recording audio", bundle: .module, comment: "Checklist item describing recording audio data that will be deleted"), target: nil, action: nil)
        recordingCheckbox.state = .on
        recordingCheckbox.isEnabled = false

        let notesCheckbox = NSButton(checkboxWithTitle: String(localized: "meeting.discard_recording.item.manual_notes", defaultValue: "Manual notes", bundle: .module, comment: "Checklist item describing manual notes that may be deleted"), target: nil, action: nil)
    ...
    func stopMeetingRecording() {
        ...
        Task { [weak self] in
            guard let self else { return }
            var meetingTitle = String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Default meeting title when no custom title is available")
            ...
            do {
                ...
                await MainActor.run {
                    self.setMeetingProcessingStatus(String(localized: "meeting.processing.finalizing", defaultValue: "Finalizing", bundle: .module, comment: "Status text shown while meeting processing is finalizing"))
                }
                ...
                if let recordingSaveError = persistenceResult.recordingSaveError {
                    await MainActor.run {
                        ...
                        self.presentErrorAlert(title: String(localized: "meeting.recording.title", defaultValue: "Meeting Recording", bundle: .module, comment: "Title shown for active meeting recording session"), message: recordingSaveError.localizedDescription)
                    }
                }
            } catch {
                ...
                await MainActor.run {
                    self.presentErrorAlert(title: String(localized: "meeting.recording.title", defaultValue: "Meeting Recording", bundle: .module, comment: "Title shown for active meeting recording session"), message: message)
                }
            }
            await MainActor.run {
                ...
                if !self.isMeetingRecording()
                    ...
                    self.statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", bundle: .module, comment: "Status text shown when meeting activity is idle"))
                    self.statusBarController?.refresh()
    ...
    func revealMeetingRecordingInFinder(path: String) {
        ...
        guard FileManager.default.fileExists(atPath: url.path) else {
            presentErrorAlert(
                title: String(localized: "error.meeting_recording.not_found.title", defaultValue: "Recording Not Found", bundle: .module, comment: "Error title shown when expected saved meeting recording file is missing"),
                message: String(localized: "error.meeting_recording.not_found.message", defaultValue: "The saved meeting recording is no longer available on disk.", bundle: .module, comment: "Error message shown when saved meeting recording file cannot be found on disk")
            )
            return
        }
    ...
    private func activeMeetingDisplayTitle() -> String {
        guard let activeMeetingID,
              ...
              !title.isEmpty else {
            return String(localized: "meeting.title", defaultValue: "Meeting", bundle: .module, comment: "Default title used for meeting context when no custom title is available")
        }
        return title
    }
    ...
    private func promptToSaveMeetingRecording(for title: String) async -> Bool {
        NSApp.activate(ignoringOtherApps: true)
        let alert = NSAlert()
        alert.messageText = String(localized: "meeting.recording.save_prompt.title", defaultValue: "Save meeting recording?", bundle: .module, comment: "Confirmation prompt title asking whether to keep merged meeting audio file")
        alert.informativeText = String(format: String(localized: "meeting.recording.save_prompt.message", defaultValue: "Keep a merged audio file for \"%@\" so you can inspect it later in Finder.", bundle: .module, comment: "Confirmation prompt message describing saved merged meeting audio file"), "\(title)")
        alert.alertStyle = .informational
        alert.addButton(withTitle: String(localized: "meeting.recording.save_prompt.action.save_recording", defaultValue: "Save Recording", bundle: .module, comment: "Confirmation prompt action to save merged meeting recording file"))
        alert.addButton(withTitle: String(localized: "meeting.recording.save_prompt.action.dont_save", defaultValue: "Don't Save", bundle: .module, comment: "Confirmation prompt action to discard merged meeting recording file"))
    ...
    private func presentMeetingStartFailureAlert(error: Error) {
        ...
        if isSystemAudioError {
            alert.messageText = String(localized: "meeting.start_failure.system_audio_capture_failed.title", defaultValue: "System audio capture failed", bundle: .module, comment: "Alert title shown when system audio recording cannot start for meeting")
            alert.informativeText = String(format: String(localized: "meeting.start_failure.system_audio_capture_failed.message", defaultValue: "Could not start system audio recording. Open System Settings > Privacy & Security > Screen & System Audio Recording and enable %@ under \"System Audio Recording Only\".\n\nError: %@", bundle: .module, comment: "Alert message with instructions and underlying error when system audio capture fails at meeting start"), "\(AppIdentity.displayName)", "\(error.localizedDescription)")
            alert.addButton(withTitle: String(localized: "alert.meeting_start_failure.open_system_settings", defaultValue: "Open System Settings", bundle: .module, comment: "Alert button label to open System Settings for fixing system audio permissions"))
            alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK", bundle: .module, comment: "Common acknowledgment button label in alerts"))
        } else {
            alert.messageText = String(localized: "meeting.start_failure.generic.title", defaultValue: "Meeting failed to start", bundle: .module, comment: "Generic alert title shown when meeting start fails")
    ...
    private func setState(_ state: DictationState) {
        ...
        let status: String
        switch state {
        case .idle: status = String(localized: "status.idle", defaultValue: "Idle", bundle: .module, comment: "Meeting state label shown when no active activity is in progress")
        case .preparing: status = String(localized: "meeting.state.preparing", defaultValue: "Preparing", bundle: .module, comment: "Meeting state label shown while meeting session is preparing")
        case .recording: status = String(localized: "meeting.state.recording", defaultValue: "Recording", bundle: .module, comment: "Meeting state label shown while meeting session is actively recording")
        case .transcribing: status = String(localized: "meeting.state.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Meeting state label shown while meeting session is transcribing")
        }
    ...
    private func blockDictationForMeetingActivityIfNeeded() -> Bool {
        guard isStartingMeetingRecording else { return false }
        let status = meetingStartStatus ?? String(localized: "meeting.status.preparing", defaultValue: "Preparing meeting...", bundle: .module, comment: "Status text shown while meeting startup preparation is in progress")
        indicator.showLoading(status)
        statusBarController?.setStatus(status)
        statusBarController?.refresh()
        return true
    ...
    private func showMeetingCompletionNotification(_ notification: PendingMeetingCompletionNotification) {
        meetingNotification.show(
            title: String(localized: "meeting.notification.transcription_complete.title", defaultValue: "Transcription complete", bundle: .module, comment: "Notification title shown when meeting transcription has finished"),
            subtitle: notification.title,
            actionLabel: String(localized: "meeting.notification.view_notes.action", defaultValue: "View Notes", bundle: .module, comment: "Notification action label to open generated meeting notes"),
            onStartRecording: { [weak self] in
                guard let self else { return }
    ...
    private func presentMeetingSignalLossPromptIfNeeded() {
        ...
        let didShow = meetingNotification.show(
            promptID: promptID,
            title: String(localized: "meeting.signal_lost.title", defaultValue: "Meeting signal lost", bundle: .module, comment: "Alert title shown when meeting signal is no longer detected"),
            subtitle: String(localized: "meeting.signal_lost.message", defaultValue: "Still recording. Stop if the meeting ended.", bundle: .module, comment: "Alert message shown after meeting signal loss while recording continues"),
            actionLabel: String(localized: "meeting.signal_lost.stop_recording_action", defaultValue: "Stop Recording", bundle: .module, comment: "Action label to stop recording when meeting signal is lost"),
            dismissAfter: 30,
    ...
    private func presentMeetingDetection(_ candidate: MeetingCandidate) {
        ...
        let didShow = meetingNotification.show(
            ...
            title: String(localized: "meeting.detected.title", defaultValue: "Meeting detected", bundle: .module, comment: "Notification title shown when a meeting is auto-detected"),
            subtitle: title,
    ...
    private func presentMeetingProcessingStage(_ stage: MeetingProcessingStage) {
        ...
        case .transcribingAudio:
            setMeetingProcessingStatus(String(localized: "meeting.processing.transcribing", defaultValue: "Transcribing", bundle: .module, comment: "Meeting processing stage label shown while transcription runs"))
        case .cleaningAudio:
            setMeetingProcessingStatus(String(localized: "meeting.processing.cleaning", defaultValue: "Cleaning", bundle: .module, comment: "Meeting processing stage label shown during transcript cleanup"))
        case .generatingTitle:
            setMeetingProcessingStatus(String(localized: "meeting.processing.titling", defaultValue: "Titling", bundle: .module, comment: "Meeting processing stage label shown while generating title"))
        case .summarizingNotes:
            setMeetingProcessingStatus(String(localized: "meeting.processing.summarizing", defaultValue: "Summarizing", bundle: .module, comment: "Meeting processing stage label shown while generating summary"))
        }
    }
    ...
    private func finishComputerUseAudioStop(wavURL: URL?, startedAt: Date) {
        ...
        indicator.setTranscribingTitle(String(localized: "computer_use.transcribing_title.parsing_command", defaultValue: "Parsing command", bundle: .module, comment: "Computer Use status title shown while parsing spoken command"), config: config)
        ...
        let task = Task { [weak self] in
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.computerUseCommandTaskID = nil
                    self.setState(.idle)
                    self.indicator.showWarning(String(localized: "computer_use.command_failed.warning", defaultValue: "CUA command failed", bundle: .module, comment: "Warning text shown when a Computer Use Automation command fails"), icon: "!")
                    self.meetingMonitor.resumeAfterCooldown()
                    self.meetingMonitor.refreshState()
                }
            }
    ...
    private func computerUseFloatingStatusLabel(for status: String) -> String? {
        ...
        if status == "Retrying planner" {
            return String(localized: "computer_use.floating_status.retrying", defaultValue: "Retrying", bundle: .module, comment: "Floating Computer Use status shown when retrying the current step")
        }
        return status
    }
    ...
    private func presentComputerUseRuntimeResult(_ result: ComputerUsePlannerRuntimeResult) {
        ...
        case .done:
            message = result.message.hasPrefix("Done") ? result.message : String(format: String(localized: "computer_use.runtime_result.done_with_message", defaultValue: "Done: %@", bundle: .module, comment: "Runtime result status showing successful completion message"), "\(result.message)")
            floatingMessage = "Done"
            icon = ""
        case .timedOut:
            message = result.message
            floatingMessage = String(localized: "computer_use.runtime_result.timed_out", defaultValue: "Timed out", bundle: .module, comment: "Runtime result status shown when command execution times out")
            icon = "!"
        case .needsConfirmation:
            message = result.message.hasPrefix("Confirm") ? result.message : String(format: String(localized: "computer_use.runtime_result.confirm_with_message", defaultValue: "Confirm: %@", bundle: .module, comment: "Runtime result status requesting user confirmation with message"), "\(result.message)")
        ...
        case .cancelled:
            message = result.message
            floatingMessage = String(localized: "computer_use.runtime_result.cancelled", defaultValue: "Cancelled", bundle: .module, comment: "Runtime result status shown when command execution is cancelled")
            icon = ""
        }
    ...
    private func handleComputerUseAudioSessionEvent(_ event: DictationAudioSessionEvent) {
        ...
        case .noAudioTimeout(let sessionID, _):
            guard activeComputerUseAudioSessionID == sessionID else { break }
            statusBarController?.setStatus(String(localized: "dictation.mic_waiting_for_speech", defaultValue: "Mic waiting for speech", bundle: .module, comment: "Dictation status shown when microphone is active but no speech is detected yet"))
    ...
    @discardableResult
    public func startMeetingRecordingForShortcuts(title: String = String(localized: "meeting.title", defaultValue: "Meeting", comment: "Default title used for meeting context when no custom title is available")) -> Bool {
    ...
    private func userFacingDictationTestError(_ error: Error) -> String {
        ...
        if nsError.domain == "MuesliTranscriptionRuntime" {
            switch nsError.code {
            case 1:
                return String(localized: "dictation.test_error.nemotron_requires_macos15", defaultValue: "Nemotron requires macOS 15 or later. Choose another model to test dictation.", bundle: .module, comment: "Error message shown when Nemotron dictation test is unavailable on older macOS versions")
            case 2:
                return String(localized: "dictation.test_error.qwen3_requires_macos15", defaultValue: "Qwen3 ASR requires macOS 15 or later. Choose another model to test dictation.", bundle: .module, comment: "Error message shown when Qwen3 ASR dictation test is unavailable on older macOS versions")
            case 4:
                return String(localized: "dictation.test_error.cohere_requires_macos15", defaultValue: "Cohere Transcribe requires macOS 15 or later. Choose another model to test dictation.", bundle: .module, comment: "Error message shown when Cohere Transcribe dictation test is unavailable on older macOS versions")
            default:
                return String(localized: "dictation.test_error.model_unavailable", defaultValue: "The selected model is not available. Choose another model and try again.", bundle: .module, comment: "Error message shown when selected dictation model is unavailable")
            }
        }
        ...
        if lowercasedMessage.contains("not loaded") || lowercasedMessage.contains("loadmodels") {
            return String(localized: "dictation.test_error.model_not_ready_retry", defaultValue: "The model was not ready yet. We are preparing it again, then try once more.", bundle: .module, comment: "Error message shown when dictation model is not ready and preparation is retried")
        }
        if lowercasedMessage.contains("network") || lowercasedMessage.contains("internet") || lowercasedMessage.contains("timed out") {
            return String(localized: "dictation.test_error.download_incomplete", defaultValue: "The model could not finish downloading. Check your connection and retry.", bundle: .module, comment: "Error message shown when dictation model download does not complete")
        }
        if lowercasedMessage.contains("permission") || lowercasedMessage.contains("microphone") {
            return String(localized: "dictation.test_error.microphone_permission_denied", defaultValue: "Muesli could not access the microphone. Check Microphone permission and try again.", bundle: .module, comment: "Error message shown when microphone permission blocks dictation test")
        }
        return String(localized: "dictation.test_error.could_not_start", defaultValue: "Dictation could not start. Try again in a moment.", bundle: .module, comment: "Error message shown when dictation test fails to start")
    }
    ...
    private func checkMaraudersMapActivation(_ text: String) {
        ...
        indicator.showWarning(String(localized: "marauders_map.activation_title", defaultValue: "Mischief Managed", bundle: .module, comment: "Status title shown when Marauder’s Map mode is activated"), icon: "\u{26A1}", duration: 3.0)
        startMaraudersMapMonitoring()
    }
    ...
    private func handleUpcomingMeeting(_ event: UpcomingMeetingEvent) {
        ...
        if minutesUntil > 0 {
            timeLabel = String(format: String(localized: "meeting.upcoming.time.starts_in_minutes", defaultValue: "starts in %d min", bundle: .module, comment: "Upcoming meeting relative time text for minutes until start"), minutesUntil)
        } else if minutesUntil == 0 {
            timeLabel = String(localized: "meeting.upcoming.time.starting_now", defaultValue: "starting now", bundle: .module, comment: "Relative meeting time text shown when meeting is starting immediately")
        } else {
            timeLabel = String(format: String(localized: "meeting.upcoming.time.started_minutes_ago", defaultValue: "started %d min ago", bundle: .module, comment: "Relative meeting time text shown when meeting started minutes ago"), abs(minutesUntil))
        }

        let title = event.title
        let notificationTitle = minutesUntil <= 0 ? String(localized: "meeting.upcoming.notification_title.starting_now", defaultValue: "Meeting starting now", bundle: .module, comment: "Upcoming meeting notification title when meeting is starting now") : String(localized: "meeting.upcoming.notification_title.default", defaultValue: "Upcoming meeting", bundle: .module, comment: "Default upcoming meeting notification title")
        meetingNotification.show(
    ...
    private func showMeetingEndNotification(title: String) {
        ...
        meetingNotification.show(
            title: String(localized: "meeting.end_notification.title", defaultValue: "Meeting ended", bundle: .module, comment: "Meeting-end notification title"),
            subtitle: String(format: String(localized: "meeting.end_notification.body.scheduled_time_over", defaultValue: "%@ · scheduled time is over", bundle: .module, comment: "Meeting-end notification body including meeting title"), "\(title)"),
            actionLabel: String(localized: "meeting.end_notification.action.stop_recording", defaultValue: "Stop Recording", bundle: .module, comment: "Meeting-end notification action to stop ongoing recording"),
            dismissAfter: 45,
            onStartRecording: { [weak self] in
                self?.stopMeetingRecording()
            },
