...
enum DictationBackendReadiness: Equatable {
    ...
    func blockingMessage(backendLabel: String) -> String? {
        switch self {
        case .preparing:
            return String(format: String(localized: "status.backend_warmup.message", defaultValue: "Warming up %@...", comment: "Status message while selected backend is warming up"), "\(backendLabel)")
        case .ready:
            return nil
        case .failed:
            return String(format: String(localized: "status.backend_unavailable.message", defaultValue: "%@ unavailable", comment: "Status message when selected backend is unavailable"), "\(backendLabel)")
        }
    }
}
...
enum MeetingResummarizationPolicy {
    static func plan(for meeting: MeetingRecord) -> MeetingResummarizationPlan {
        ...
        let promptTitle = trimmed.isEmpty ? String(localized: "prompt.meeting.title", defaultValue: "Meeting", comment: "Generic meeting title label") : trimmed
...
enum MeetingSummaryPersistenceError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .failedToSaveSummary(let underlying):
            ...
            if detail.isEmpty {
                return String(localized: "error.meeting_notes.save_failed.message", defaultValue: "The updated meeting notes could not be saved.", comment: "Error message when saving updated meeting notes fails")
            }
            return String(format: String(localized: "error.meeting_notes.save_failed_with_detail.message", defaultValue: "The updated meeting notes could not be saved. %@", comment: "Error message when saving meeting notes fails with additional detail"), "\(detail)")
        }
    }
}
...
enum MeetingTemplateSelectionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .templateNoLongerExists:
            return String(localized: "error.meeting_template.missing.message", defaultValue: "That template no longer exists. Choose another template and try again.", comment: "Error message when selected meeting template is missing")
        }
    }
}
...
enum MeetingRetranscriptionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .controllerUnavailable:
            return String(localized: "error.retranscription.controller_unavailable.message", defaultValue: "Meeting re-transcription could not continue because Muesli is no longer available.", comment: "Error message when retranscription cannot continue because controller is unavailable")
        case .recordingUnavailable:
            return String(localized: "error.meeting_recording.not_found.message", defaultValue: "The saved meeting recording is no longer available on disk.", comment: "Error message when saved meeting recording file is missing")
        case .noDownloadedTranscriptionModel:
            return String(localized: "error.retranscription.model_required.message", defaultValue: "Download a transcription model before re-transcribing this meeting.", comment: "Error message when retranscription requires a downloaded model")
        case .emptyTranscript:
            return String(localized: "error.retranscription.no_speech_detected.message", defaultValue: "Re-transcription finished, but no speech was detected in the saved recording.", comment: "Error message when retranscription detects no speech in recording")
        case .failedToSave(let underlying):
            return String(format: String(localized: "error.retranscription.save_failed_with_detail.message", defaultValue: "The re-transcribed meeting could not be saved. %@", comment: "Error message when saving retranscribed meeting fails with detail"), "\(underlying.localizedDescription)")
        }
    }
}
...
enum MeetingLifecycleError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .failedToSaveRecording(let underlying):
            return String(format: String(localized: "error.meeting_recording.save_failed_with_detail.message", defaultValue: "The meeting finished transcribing, but the recording could not be saved. %@", comment: "Error message when recording save fails after transcription completes"), "\(underlying.localizedDescription)")
        case .failedToDeleteRecording(let underlying):
            return String(format: String(localized: "error.meeting_recording.delete_failed_with_detail.message", defaultValue: "The saved meeting recording could not be deleted, so the meeting was left in place. %@", comment: "Error message when deleting saved recording fails with detail"), "\(underlying.localizedDescription)")
        case .failedToDeleteMeeting(let underlying):
            return String(format: String(localized: "error.delete_meeting.failed_with_detail.message", defaultValue: "The meeting could not be deleted. %@", comment: "Error message when deleting meeting fails with detail"), "\(underlying.localizedDescription)")
        }
    }
}
...
public final class MuesliController: NSObject {
    ...
    func truncate(_ text: String, limit: Int) -> String {
        ...
        return String(compact.prefix(limit - 3)).trimmingCharacters(in: .whitespacesAndNewlines) + String(localized: "common.ellipsis", defaultValue: "...", comment: "Common ellipsis placeholder text")
    }
    ...
    func refreshUI() {
        statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", comment: "Status message when system is idle"))
    ...
    func enableIPhoneBridgeSync() {
        ...
        appState.iCloudSyncStatus = String(localized: "sync.icloud.checking", defaultValue: "Checking iCloud...", comment: "Status message while checking iCloud sync availability")
        ...
        iCloudSubscriptionTask = Task { [weak self] in
            do {
                ...
                await MainActor.run {
                    ...
                    self.appState.iCloudSyncStatus = String(localized: "sync.icloud.setup_private", defaultValue: "Setting up private iCloud sync...", comment: "Status message while setting up private iCloud sync")
                ...
                await MainActor.run {
                    ...
                    self.appState.iCloudSyncStatus = String(format: String(localized: "sync.icloud.required_message", defaultValue: "Sync needs iCloud: %@", comment: "Status message when sync requires iCloud with reason"), "\(message)")
    ...
    private func startICloudSync() {
        ...
        guard config.iCloudSyncEnabled else {
            if userInitiated {
                appState.iCloudSyncStatus = String(localized: "sync.icloud.turn_on_first", defaultValue: "Turn on iCloud sync first.", comment: "Prompt telling user to enable iCloud sync first")
            }
        ...
        guard iCloudSyncTask == nil else {
            ...
            appState.iCloudBridgeMessage = nil
            if userInitiated {
                appState.iCloudSyncStatus = String(localized: "sync.already_in_progress", defaultValue: "Sync already in progress.", comment: "Status message shown when a sync attempt starts while another sync is already running")
            }
        ...
        enableICloudPersistentSync()
        appState.isICloudSyncInProgress = true
        appState.iCloudSyncStatus = String(localized: "sync.icloud.syncing_private", defaultValue: "Syncing with private iCloud...", comment: "Status message while syncing with private iCloud")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        ...
        iCloudSyncTask = Task { [weak self] in
            do {
                ...
                await MainActor.run {
                    ...
                    let remoteDeviceName = MuesliBridgeDeviceIdentity.remoteDeviceDisplayName ?? String(localized: "device.iphone.default_name", defaultValue: "iPhone", comment: "Default device name label for iPhone")
                    self.appState.iCloudSyncStatus = result.downloaded.total > 0
                        ? String(format: String(localized: "sync.icloud.synced_with_device", defaultValue: "Synced with %@.", comment: "Status message indicating successful sync with a specific device"), "\(remoteDeviceName)")
                        : String(localized: "sync.icloud.all_text_up_to_date", defaultValue: "All text is up to date.", comment: "Status message indicating all synced text is current")
                ...
                await MainActor.run {
                    ...
                    self.bridgeDiscoveryFollowUpPending = false
                    let message = error.localizedDescription
                    self.appState.iCloudSyncStatus = String(format: String(localized: "sync.failed_with_reason", defaultValue: "Sync failed: %@", comment: "Error status message when sync fails with a reason"), "\(message)")
                    if MuesliICloudSyncEngine.isICloudAccountAvailabilityError(error) {
                        self.appState.iCloudBridgeState = .needsICloud
                    } else {
                        self.appState.iCloudBridgeState = .error
    ...
    private func disableICloudSyncRuntimeState() {
        ...
        resetBridgeDiscoveryRuntimeState()
        appState.iCloudSyncStatus = String(localized: "sync.icloud.turning_off", defaultValue: "Turning off iCloud sync...", comment: "Status message while turning off iCloud sync")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        Task { [weak self] in
            ...
            self.appState.iCloudSyncStatus = String(localized: "sync.icloud.off", defaultValue: "iCloud sync is off.", comment: "Status message when iCloud sync is disabled")
    ...
    private func disableICloudSyncForUnavailableEntitlement() {
        ...
        resetICloudSubscriptionState()
        resetBridgeDiscoveryRuntimeState()
        appState.iCloudSyncStatus = String(localized: "sync.icloud.stopping_unavailable", defaultValue: "Stopping unavailable iCloud sync...", comment: "Status message while stopping unavailable iCloud sync")
        appState.iCloudBridgeState = .syncing
        appState.iCloudBridgeMessage = nil
        Task { [weak self] in
            await cancellationTask?.value
            guard let self,
                  self.iCloudSyncGeneration == generation,
                  self.ckSyncEngine == nil else { return }
            self.appState.iCloudSyncStatus = String(localized: "sync.icloud.unavailable_local_build", defaultValue: "iCloud sync is unavailable in this local-only build.", comment: "Status message when iCloud sync is unavailable in local-only build")
            self.appState.iCloudBridgeState = .notConfigured
        }
    }
    ...
    private func formatICloudSyncSummary(_ result: ICloudSyncResult) -> String {
        String(format: String(localized: "sync.icloud.summary.up_down", defaultValue: "%@ up, %@ down", comment: "Sync summary message with uploaded and downloaded item counts"), "\(formatICloudSyncCounts(result.uploaded))", "\(formatICloudSyncCounts(result.downloaded))")
    }
    ...
    private func formatICloudSyncCounts(_ counts: ICloudSyncKindCounts) -> String {
        ...
        if counts.dictations > 0 {
            parts.append("\(counts.dictations) \(counts.dictations == 1 ? String(localized: "sync.icloud.count.item.dictation.singular", defaultValue: "dictation", comment: "Singular noun label for iCloud dictation sync count") : String(localized: "sync.icloud.count.item.dictation.plural", defaultValue: "dictations", comment: "Plural noun label for iCloud dictation sync count"))")
        }
        if counts.meetings > 0 {
            parts.append("\(counts.meetings) \(counts.meetings == 1 ? String(localized: "sync.icloud.count.item.meeting.singular", defaultValue: "meeting", comment: "Singular noun label for iCloud meeting sync count") : String(localized: "sync.icloud.count.item.meeting.plural", defaultValue: "meetings", comment: "Plural noun label for iCloud meeting sync count"))")
        }
    ...
    func selectBackend(_ option: BackendOption) {
        ...
        Task { [weak self] in
            ...
            if needsWarmup {
                await MainActor.run {
                    self.indicator.showLoading(String(localized: "status.warming_up", defaultValue: "Warming up...", comment: "Generic status message while system is warming up"))
                }
            }
            let ppOption = self.runtimePostProcessorOption()
            await self.configureTranscriptCleanupForRuntime(option: ppOption)
    ...
    func selectMeetingTranscriptionBackend(_ option: BackendOption, requireDownloaded: Bool = true) {
        guard option.supportsMeetingTranscription else {
            presentErrorAlert(
                title: String(localized: "error.meeting_transcription.model_unavailable.title", defaultValue: "Meeting model unavailable", comment: "Error title when selected model cannot be used for meeting transcription"),
                message: String(format: String(localized: "error.meeting_transcription.option_not_supported.message", defaultValue: "%@ is optimized for dictation and cannot be used for meeting transcription.", comment: "Error message when selected model supports dictation but not meeting transcription"), "\(option.label)")
            )
            normalizeMeetingTranscriptionSelectionForAvailability()
            return
        }
        guard !requireDownloaded || option.isDownloaded else {
            presentErrorAlert(
                title: String(localized: "error.meeting_transcription.model_unavailable.title", defaultValue: "Meeting model unavailable", comment: "Error title when meeting transcription model is unavailable"),
                message: String(format: String(localized: "error.meeting_transcription.download_required.message", defaultValue: "Download %@ before using it for meeting transcription.", comment: "Error message instructing user to download model before meeting transcription"), "\(option.label)")
            )
            normalizeMeetingTranscriptionSelectionForAvailability()
            return
        }
    ...
    func selectPostProcessorBackend(_ option: TranscriptCleanupBackendOption) {
        guard option.isCompatible(with: selectedBackend) else {
            presentErrorAlert(
                title: String(localized: "error.cleanup_model.unavailable.title", defaultValue: "Cleanup model unavailable", comment: "Error title shown when cleanup model cannot be used"),
                message: String(localized: "error.cleanup_model.gemma4_same_backend.message", defaultValue: "Gemma 4 cannot clean up a transcription produced by the same Gemma 4 backend.", comment: "Error message explaining Gemma 4 cleanup cannot use transcription from same backend")
            )
            return
        }
    ...
    func refreshGoogleCalendarList() async {
        ...
        do {
            ...
            appState.googleCalendarListLoadState = .failed(String(format: String(localized: "calendar.google.token_refresh_failed", defaultValue: "Token refresh failed: %@", comment: "Error message shown when Google Calendar token refresh fails with detail"), "\(message)"))
        } catch {
    ...
    private func showMeetingStartingNowNotification(
        ...
        meetingNotification.show(
            title: String(localized: "notifications.meeting_starting_now.title", defaultValue: "Meeting starting now", comment: "Notification title for meetings beginning immediately"),
            subtitle: title,
    ...
    func continueModelPreparationAfterOnboarding(
        ...
        updateModelPreparationStatus(
            title: String(format: String(localized: "model_preparation.preparing_backend", defaultValue: "Preparing %@", comment: "Status message while preparing selected backend model"), "\(backend.label)"),
            detail: initialStatus ?? String(format: String(localized: "model_preparation.preparing_backend_ellipsis", defaultValue: "Preparing %@...", comment: "Status message with ellipsis while preparing selected backend model"), "\(backend.label)"),
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.updateModelPreparationStatus(
                        title: String(format: String(localized: "model_preparation.backend_ready", defaultValue: "%@ ready", comment: "Status message when selected backend model is ready"), "\(backend.label)"),
                        detail: String(localized: "model_preparation.ready_for_transcription", defaultValue: "Ready for transcription", comment: "Status message indicating transcription is ready"),
                        progress: 1.0,
                        isPreparing: false,
                        isComplete: true
                    )
                ...
                await MainActor.run {
                    self.onboardingModelPreparationTask = nil
                    self.updateModelPreparationStatus(
                        title: backend.isDownloaded ? String(localized: "model_preparation.paused", defaultValue: "Model setup paused", comment: "Status message when model setup is paused") : String(localized: "model_preparation.download_paused", defaultValue: "Download paused", comment: "Status message when model download is paused"),
    ...
    func downloadModelForOnboarding(
        ...
        progress(
            ...
            wasDownloaded ? String(format: String(localized: "onboarding.model_download.warming_up_backend", defaultValue: "Warming up %@...", comment: "Onboarding status while warming up backend model"), "\(backend.label)") : String(format: String(localized: "onboarding.model_download.downloading_backend", defaultValue: "Downloading %@...", comment: "Onboarding status while downloading backend model"), "\(backend.label)")
        )
        try await transcriptionCoordinator.preloadRequired(
            backend: backend,
            ...
            progress: { value, status in
                ...
                if status?.localizedCaseInsensitiveContains("download") == true {
                    progress(value, "\(status ?? String(format: String(localized: "onboarding.model_download.downloading_backend", defaultValue: "Downloading %@...", comment: "Onboarding status while downloading backend model"), "\(backend.label)"))")
                } else if value >= 0.9 {
                    progress(value, status ?? String(format: String(localized: "onboarding.model_download.warming_up_backend", defaultValue: "Warming up %@...", comment: "Onboarding status while warming up backend model"), "\(backend.label)"))
                } else {
                    progress(value, status ?? String(format: String(localized: "onboarding.model_download.preparing_backend", defaultValue: "Preparing %@...", comment: "Onboarding status while preparing backend model"), "\(backend.label)"))
                }
            },
        ...
        progress(1.0, String(format: String(localized: "onboarding.model_download.backend_ready", defaultValue: "%@ ready", comment: "Onboarding status when backend model is ready"), "\(backend.label)"))
    }

    private func applyModelPreparationProgress(_ progress: Double, status: String?, backend: BackendOption) {
        let detail = status ?? String(format: String(localized: "model_preparation.detail.preparing_backend", defaultValue: "Preparing %@...", comment: "Detailed model preparation status while preparing backend"), "\(backend.label)")
        ...
        if isPreparing {
            updateModelPreparationStatus(
                title: String(format: String(localized: "model_preparation.status.preparing_backend", defaultValue: "Preparing %@", comment: "Model preparation status while preparing backend"), "\(backend.label)"),
                detail: String(format: String(localized: "model_preparation.status.optimizing_backend_for_mac", defaultValue: "Optimizing %@ for this Mac...", comment: "Model preparation status while optimizing backend for current Mac"), "\(backend.label)"),
                progress: nil,
                isPreparing: true,
                isComplete: false
            )
    ...
    private func modelPreparationFailureMessage(for backend: BackendOption) -> String {
        backend.isDownloaded
            ? String(localized: "model_preparation.failure.setup_failed_restart_or_retry", defaultValue: "Model setup failed. Restart Muesli or retry from Models.", comment: "Error message when model setup fails with recovery guidance")
            : String(localized: "model_preparation.failure.download_failed_check_connection", defaultValue: "Download failed. Check your connection and retry.", comment: "Error message when model download fails with network guidance")
    }
    ...
    private func presentStandardUpdateCheck() {
        guard let updaterController else {
            appState.sparkleUpdateStatus = .disabled(message: String(localized: "updates.disabled_for_build", defaultValue: "Update checks are disabled for this build.", comment: "Status message when update checking is disabled in current build"))
            return
        }
            ...
            do {
                ...
                do {
                    formattedNotes = try await MeetingSummaryClient.summarize(
                        ...
                        meetingTitle: meeting.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? String(localized: "meeting.title", defaultValue: "Meeting", comment: "") : meeting.title,
                        config: self.config,
                        template: templateSnapshot,
                        existingNotes: self.notesContextForResummary(meeting),
                        manualNotesToRetain: meeting.manualNotes
    ...
    static func stripManualNotesSection(from notes: String) -> String {
        let markers = [
            "\n\n" + String(localized: "notes.section.written_notes.h3_spaced", defaultValue: "### Written notes", comment: "Markdown heading block for written notes section with surrounding spacing") + "\n\n",
            "\n" + String(localized: "notes.section.written_notes.h3_leading_newline", defaultValue: "### Written notes", comment: "Markdown heading block for written notes section with leading newline") + "\n\n",
            String(localized: "notes.section.written_notes.h3", defaultValue: "### Written notes", comment: "Markdown heading block for written notes section") + "\n\n",
            "\n\n" + String(localized: "notes.section.manual_notes.h2_spaced", defaultValue: "## Manual Notes", comment: "Markdown heading block for manual notes section with surrounding spacing") + "\n\n",
            "\n" + String(localized: "notes.section.manual_notes.h2_leading_newline", defaultValue: "## Manual Notes", comment: "Markdown heading block for manual notes section with leading newline") + "\n\n",
            String(localized: "notes.section.manual_notes.h2", defaultValue: "## Manual Notes", comment: "Markdown heading block for manual notes section") + "\n\n"
        ]
    ...
    func deleteMeeting(id: Int64) {
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.delete_meeting.failed.title", defaultValue: "Couldn't Delete Meeting", comment: "Error alert title when deleting a meeting fails"), message: error.localizedDescription)
            return
        } catch {
            presentErrorAlert(
                title: String(localized: "error.delete_meeting.failed.title", defaultValue: "Couldn't Delete Meeting", comment: "Error alert title when deleting a meeting fails"),
    ...
    func clearMeetingHistory() {
        guard !isMeetingRecording(), !isStartingMeetingRecording, backgroundMeetingProcessingCount == 0 else {
            presentErrorAlert(
                title: String(localized: "error.clear_meeting_history.failed.title", defaultValue: "Couldn't Clear Meeting History", comment: "Error alert title when clearing meeting history fails"),
                message: String(localized: "error.clear_meeting_history.active_meeting.message", defaultValue: "A meeting is recording or still being processed. Please wait before clearing saved meetings.", comment: "Error message when meeting history cannot be cleared during active processing")
            )
            return
        }
    ...
    func shouldTerminateApplication() -> Bool {
        ...
        case .none:
            return true
        case .starting:
            messageText = String(localized: "meeting_recording.starting_message", defaultValue: "Meeting recording is starting", comment: "Termination warning title when meeting recording start is in progress")
            informativeText = String(localized: "termination.meeting_recording_starting.warning", defaultValue: "Quitting now will cancel the meeting recording before it has been saved.", comment: "Termination warning while meeting recording is starting")
        case .recording:
            messageText = String(localized: "termination.meeting_recording_in_progress.title", defaultValue: "Meeting recording in progress", comment: "Termination warning title when meeting recording is active")
            informativeText = String(localized: "termination.meeting_recording_in_progress.warning", defaultValue: "Quitting now will stop the meeting recording and the current transcript may be lost. Stop the recording first if you want Muesli to save notes.", comment: "Termination warning when quitting during active meeting recording")
        case .processing:
            messageText = String(localized: "termination.meeting_transcription_in_progress.title", defaultValue: "Meeting transcription in progress", comment: "Termination warning title when meeting transcription is currently running")
            informativeText = String(localized: "termination.meeting_transcription_in_progress.warning", defaultValue: "Quitting now will interrupt transcription and the meeting notes may not be saved.", comment: "Termination warning message when quitting during meeting transcription")
        }
        ...
        alert.addButton(withTitle: String(localized: "termination.action.keep_running", defaultValue: "Keep Muesli Running", comment: "Termination alert action to continue running the app"))
        alert.addButton(withTitle: String(localized: "termination.action.quit_anyway", defaultValue: "Quit Anyway", comment: "Termination alert action to quit despite active work"))
    ...
    func pauseMeetingRecording() {
        ...
        statusBarController?.setStatus(String(localized: "meeting.status.paused", defaultValue: "Meeting paused", comment: "Status message when active meeting recording is paused"))
        statusBarController?.refresh()
        syncAppState()
    }
    ...
    func resumeMeetingRecording() {
        ...
        statusBarController?.setStatus(String(format: String(localized: "meeting.status.with_title", defaultValue: "Meeting: %@", comment: "Status message showing current meeting with display title"), "\(activeMeetingDisplayTitle())"))
        statusBarController?.refresh()
        syncAppState()
    }
    ...
    func startMeetingRecordingFromEntryPoint(
        title: String = String(localized: "meeting.title", defaultValue: "Meeting", comment: ""),
    ...
    func startMeetingRecording(
        ...
        guard let meetingBackend = normalizeMeetingTranscriptionSelectionForAvailability() else {
            presentErrorAlert(
                title: String(localized: "error.meeting_start.failed.title", defaultValue: "Meeting failed to start", comment: "Error alert title when meeting recording cannot start"),
                message: String(localized: "error.meeting_start.model_required.message", defaultValue: "Download a transcription model before recording a meeting.", comment: "Error message when meeting start requires a downloaded transcription model")
            )
            return false
        }
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.meeting_start.failed.title", defaultValue: "Meeting failed to start", comment: "Error alert title when meeting recording cannot start"), message: error.localizedDescription)
            return false
        }
        armMeetingAutoStop(
        ...
        updateMeetingStartStatus(String(localized: "meeting.start_status.transcription_starting_soon", defaultValue: "Meeting transcription will start shortly.", comment: "Status message shown right after meeting recording starts"))
        indicator.setState(.preparing, config: config)
        beginMeetingActivity(reason: String(localized: "meeting.activity.recording_and_transcribing", defaultValue: "Recording and transcribing a meeting", comment: "Activity description while meeting is being recorded and transcribed"))
        ...
        meetingStartTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if self.meetingStartMeetingID == meetingID {
                    ...
                    self.meetingMonitor.refreshState()
                    self.statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", comment: "Status message when no active operation is running"))
                    self.statusBarController?.refresh()
                    self.setState(.idle)
    ...
    func startQuickNoteMeeting() {
        startMeetingRecordingFromEntryPoint(title: String(localized: "meeting.title", defaultValue: "Meeting", comment: ""))
    }
    ...
    func resumeFinishedMeeting(meetingID: Int64) {
        ...
        guard let meetingBackend = normalizeMeetingTranscriptionSelectionForAvailability() else {
            presentErrorAlert(
                title: String(localized: "error.resume_meeting.failed.title", defaultValue: "Resume failed", comment: "Error alert title when resuming meeting recording fails"),
                message: String(localized: "error.resume_meeting.model_required.message", defaultValue: "Download a transcription model before recording.", comment: "Error message when resuming meeting requires downloaded transcription model")
            )
            return
        }
        ...
        do {
            ...
            presentErrorAlert(title: String(localized: "error.resume_meeting.failed.title", defaultValue: "Resume failed", comment: "Error alert title when resuming meeting recording fails"), message: error.localizedDescription)
            return
        }
        ...
        syncDictationRecorderWarmup(intent: .idlePrewarm(.meetingStateChanged))
        meetingStartMeetingID = meetingID
        updateMeetingStartStatus(String(localized: "meeting.resume.status.resuming_recording", defaultValue: "Resuming meeting recording…", comment: "Status message while resuming meeting recording"))
        indicator.setState(.preparing, config: config)
        beginMeetingActivity(reason: String(localized: "meeting.activity.recording_and_transcribing", defaultValue: "Recording and transcribing a meeting", comment: "Activity description while meeting recording and transcription are active"))
        meetingMonitor.suppressWhileActive()
        meetingMonitor.refreshState()
        updateMeetingNotificationVisibility()
        ...
        meetingStartTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if self.meetingStartMeetingID == meetingID {
                    ...
                    self.meetingMonitor.resumeAfterCooldown()
                    self.meetingMonitor.refreshState()
                    self.statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", comment: "Status text when no active work is running"))
                    self.statusBarController?.refresh()
                    self.setState(.idle)
                    self.endMeetingActivity()
                    self.syncDictationRecorderWarmup(intent: .idlePrewarm(.meetingStateChanged))
    ...
    func importAudioFile() {
        ...
        guard normalizeMeetingTranscriptionSelectionForAvailability() != nil else {
            presentErrorAlert(
                title: String(localized: "error.import_audio.failed.title", defaultValue: "Import Failed", comment: "Error alert title when audio import fails"),
                message: String(localized: "error.import_audio.model_required.message", defaultValue: "Download a transcription model before importing audio files.", comment: "Error message when importing audio requires a downloaded transcription model")
            )
            return
        }
    ...
    func importAudioFileFromURL(_ url: URL) {
        ...
        guard AudioFileImportController.isSupportedFileURL(url) else {
            presentErrorAlert(
                title: String(localized: "error.import_audio.failed.title", defaultValue: "Import Failed", comment: "Error alert title when audio import fails"),
                message: String(localized: "error.import_audio.unsupported_format.message", defaultValue: "This audio file format is not supported.", comment: "Error message for unsupported imported audio format")
            )
            return
        }
        guard normalizeMeetingTranscriptionSelectionForAvailability() != nil else {
    ...
    private func importAudioFile(from sourceURL: URL, sessionID: UUID) async {
        ...
        let title = filename.isEmpty ? String(localized: "import.recording.title", defaultValue: "Imported Recording", comment: "Default title for imported audio recording meeting") : filename

        self.updateImportProgressStatus(String(localized: "import.audio.progress.importing_file", defaultValue: "Importing audio file...", comment: "Progress text while importing audio file"), sessionID: sessionID)
        self.beginMeetingActivity(reason: String(localized: "import.audio.activity.importing_for_transcription", defaultValue: "Importing audio file for transcription", comment: "Activity text while importing audio file for transcription"))
    ...
    func cancelMeetingPreparation() {
        ...
        statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", comment: "Status text when no active work is running"))
        statusBarController?.refresh()
        setState(.idle)
        endMeetingActivity()
        disarmMeetingAutoStop()
    ...
    private func startMeetingRecordingWithSystemAudioRecovery(
        ...
        statusBarController?.setStatus(String(localized: "meeting.start_status.transcription_starting_soon", defaultValue: "Meeting transcription will start shortly.", comment: "Status message shown after starting meeting before transcription begins"))
        statusBarController?.refresh()
        try Task.checkCancellation()
        try await transcriptionCoordinator.preloadRequired(
            backend: backend,
        ...
        while true {
            ...
            do {
                ...
                meetingMonitor.suppressWhileActive()
                meetingMonitor.refreshState()
                statusBarController?.setStatus(String(format: String(localized: "meeting.status.with_title", defaultValue: "Meeting: %@", comment: "Meeting status text including meeting title"), "\(title)"))
                ...
                try Task.checkCancellation()
                try checkMeetingStartStillCurrent(meetingID)
                updateMeetingStartStatus(String(localized: "meeting.start_status.requesting_system_audio_permission", defaultValue: "Requesting system audio permission...", comment: "Status message while requesting system audio permission before meeting start"))
                statusBarController?.setStatus(String(localized: "meeting.start_status.requesting_system_audio_permission", defaultValue: "Requesting system audio permission...", comment: "Status message while requesting system audio permission before meeting start"))
                statusBarController?.refresh()
                let granted = await CoreAudioSystemRecorder.requestSystemAudioAccess()
                try Task.checkCancellation()
                try checkMeetingStartStillCurrent(meetingID)
                if granted {
                    updateMeetingStartStatus(String(localized: "meeting.start_status.retrying_start", defaultValue: "Retrying meeting start...", comment: "Status message when retrying meeting startup"))
                    statusBarController?.setStatus(String(localized: "meeting.start_status.retrying_start", defaultValue: "Retrying meeting start...", comment: "Status message when retrying meeting startup"))
                    statusBarController?.refresh()
                    continue
                }
                throw error
    ...
    private final class MeetingDiscardButtonTitleUpdater: NSObject {
        ...
        @MainActor @objc func manualNotesCheckboxChanged(_ sender: NSButton) {
            discardButton?.title = sender.state == .on ? String(localized: "common.discard", defaultValue: "Discard", comment: "Common destructive action label for discarding changes or data") : String(localized: "meeting.discard_recording.action", defaultValue: "Discard Recording", comment: "Action label to discard current meeting recording")
        }
    }
    ...
    @objc func discardMeetingWithConfirmation() {
        ...
        alert.messageText = String(localized: "meeting.discard_recording.confirmation_title", defaultValue: "Discard recording?", comment: "Confirmation alert title when discarding a meeting recording")
        alert.alertStyle = .warning
        var manualNotesCheckbox: NSButton?
        if hasManualNotes {
            alert.informativeText = String(localized: "meeting.discard_recording.message_with_notes_choice", defaultValue: "This will stop the meeting. Choose whether to delete the written notes too.", comment: "Confirmation message when discarding recording with notes deletion choice")
            let accessory = Self.makeDiscardMeetingAccessoryView()
            manualNotesCheckbox = accessory.manualNotesCheckbox
            alert.accessoryView = accessory.view
            alert.addButton(withTitle: String(localized: "meeting.discard_recording.action", defaultValue: "Discard Recording", comment: "Action label to discard current meeting recording"))
            alert.addButton(withTitle: String(localized: "alert.discard_meeting.cancel", defaultValue: "Cancel", comment: "Cancel action label in discard meeting alert"))
            ...
            alert.informativeText = String(localized: "meeting.discard_recording.destructive_warning", defaultValue: "This will stop the meeting recording and delete all captured audio. This cannot be undone.", comment: "Destructive warning message when discarding meeting recording and audio")
            alert.addButton(withTitle: String(localized: "common.discard", defaultValue: "Discard", comment: "Common destructive action label for discarding changes or data"))
            alert.addButton(withTitle: String(localized: "alert.discard_meeting.cancel", defaultValue: "Cancel", comment: "Cancel action label in discard meeting alert"))
            alert.buttons.first?.hasDestructiveAction = true
        }
    ...
    private static func makeDiscardMeetingAccessoryView() -> MeetingDiscardAccessory {
        let label = NSTextField(labelWithString: String(localized: "meeting.discard_recording.will_delete_label", defaultValue: "Will delete:", comment: "Label introducing list of items that will be deleted when discarding recording"))
        label.font = .systemFont(ofSize: NSFont.systemFontSize)
        label.textColor = .secondaryLabelColor

        let recordingCheckbox = NSButton(checkboxWithTitle: String(localized: "meeting.discard_recording.item.recording_audio", defaultValue: "Recording audio", comment: "Item label for recording audio in discard confirmation list"), target: nil, action: nil)
        recordingCheckbox.state = .on
        recordingCheckbox.isEnabled = false

        let notesCheckbox = NSButton(checkboxWithTitle: String(localized: "meeting.discard_recording.item.manual_notes", defaultValue: "Manual notes", comment: "Item label for manual notes in discard confirmation list"), target: nil, action: nil)
    ...
    func stopMeetingRecording() {
        ...
        Task { [weak self] in
            guard let self else { return }
            var meetingTitle = String(localized: "meeting.title", defaultValue: "Meeting", comment: "")
            ...
            do {
                ...
                await MainActor.run {
                    self.setMeetingProcessingStatus(String(localized: "meeting.processing.finalizing", defaultValue: "Finalizing", comment: "Status label while finalizing meeting processing"))
                }
                ...
                if let recordingSaveError = persistenceResult.recordingSaveError {
                    await MainActor.run {
                        ...
                        self.presentErrorAlert(title: String(localized: "error.meeting_recording.title", defaultValue: "Meeting Recording", comment: "Error alert title for meeting recording related issues"), message: recordingSaveError.localizedDescription)
                    }
                }
            } catch {
                ...
                await MainActor.run {
                    self.presentErrorAlert(title: String(localized: "error.meeting_recording.title", defaultValue: "Meeting Recording", comment: "Error alert title for meeting recording related issues"), message: message)
                }
            }
            await MainActor.run {
                ...
                if !self.isMeetingRecording()
                    ...
                    self.statusBarController?.setStatus(String(localized: "status.idle", defaultValue: "Idle", comment: "Status text when no activity is running"))
                    self.statusBarController?.refresh()
    ...
    func revealMeetingRecordingInFinder(path: String) {
        ...
        guard FileManager.default.fileExists(atPath: url.path) else {
            presentErrorAlert(
                title: String(localized: "error.meeting_recording.not_found.title", defaultValue: "Recording Not Found", comment: "Error alert title when a saved meeting recording cannot be found"),
                message: String(localized: "error.meeting_recording.not_found.message", defaultValue: "The saved meeting recording is no longer available on disk.", comment: "Error message when saved meeting recording file is missing")
            )
            return
        }
    ...
    private func activeMeetingDisplayTitle() -> String {
        guard let activeMeetingID,
              ...
              !title.isEmpty else {
            return String(localized: "meeting.title", defaultValue: "Meeting", comment: "")
        }
        return title
    }
    ...
    private func promptToSaveMeetingRecording(for title: String) async -> Bool {
        NSApp.activate(ignoringOtherApps: true)
        let alert = NSAlert()
        alert.messageText = String(localized: "meeting.recording.save_prompt.title", defaultValue: "Save meeting recording?", comment: "Prompt title asking whether to save finished meeting recording")
        alert.informativeText = String(format: String(localized: "meeting.recording.save_prompt.message", defaultValue: "Keep a merged audio file for \"%@\" so you can inspect it later in Finder.", comment: "Prompt message explaining saved merged audio file for meeting title"), "\(title)")
        alert.alertStyle = .informational
        alert.addButton(withTitle: String(localized: "meeting.recording.save_prompt.save_button", defaultValue: "Save Recording", comment: "Button label to save merged meeting recording"))
        alert.addButton(withTitle: String(localized: "meeting.recording.save_prompt.dont_save_button", defaultValue: "Don't Save", comment: "Button label to discard merged meeting recording"))
    ...
    private func presentMeetingStartFailureAlert(error: Error) {
        ...
        if isSystemAudioError {
            alert.messageText = String(localized: "meeting.start_failure.system_audio_capture_failed.title", defaultValue: "System audio capture failed", comment: "Error alert title when meeting start fails due to system audio capture setup")
            alert.informativeText = String(format: String(localized: "meeting.start_failure.system_audio_capture_failed.message", defaultValue: "Could not start system audio recording. Open System Settings > Privacy & Security > Screen & System Audio Recording and enable %@ under \"System Audio Recording Only\".\n\nError: %@", comment: "Error message with remediation steps when system audio recording cannot start"), "\(AppIdentity.displayName)", "\(error.localizedDescription)")
            alert.addButton(withTitle: String(localized: "alert.meeting_start_failure.open_system_settings", defaultValue: "Open System Settings", comment: "Action button label to open System Settings from meeting start failure alert"))
            alert.addButton(withTitle: String(localized: "alert.meeting_start_failure.ok", defaultValue: "OK", comment: "Confirmation button label in meeting start failure alert"))
        } else {
            alert.messageText = String(localized: "meeting.start_failure.generic.title", defaultValue: "Meeting failed to start", comment: "Generic error alert title when meeting cannot start")
    ...
    private func presentErrorAlert(title: String, message: String) {
        ...
        alert.alertStyle = .warning
        alert.addButton(withTitle: String(localized: "alert.error.ok", defaultValue: "OK", comment: "Generic confirmation button label in error alerts"))
    ...
    private func setState(_ state: DictationState) {
        ...
        let status: String
        switch state {
        case .idle: status = String(localized: "status.idle", defaultValue: "Idle", comment: "Status text when no activity is running")
        case .preparing: status = String(localized: "meeting.state.preparing", defaultValue: "Preparing", comment: "Meeting state label while preparing resources")
        case .recording: status = String(localized: "meeting.state.recording", defaultValue: "Recording", comment: "Meeting state label while recording audio")
        case .transcribing: status = String(localized: "meeting.state.transcribing", defaultValue: "Transcribing", comment: "Meeting state label while transcribing audio")
        }
    ...
    private func blockDictationForMeetingActivityIfNeeded() -> Bool {
        guard isStartingMeetingRecording else { return false }
        let status = meetingStartStatus ?? String(localized: "meeting.status.preparing", defaultValue: "Preparing meeting...", comment: "Status message while meeting setup is in progress")
        indicator.showLoading(status)
        statusBarController?.setStatus(status)
        statusBarController?.refresh()
        return true
    ...
    private func showMeetingCompletionNotification(_ notification: PendingMeetingCompletionNotification) {
        meetingNotification.show(
            title: String(localized: "meeting.notification.transcription_complete.title", defaultValue: "Transcription complete", comment: "Notification title shown when meeting transcription finishes"),
            subtitle: notification.title,
            actionLabel: String(localized: "meeting.notification.view_notes.action", defaultValue: "View Notes", comment: "Notification action button to open meeting notes"),
            onStartRecording: { [weak self] in
                guard let self else { return }
    ...
    private func presentMeetingSignalLossPromptIfNeeded() {
        ...
        let didShow = meetingNotification.show(
            promptID: promptID,
            title: String(localized: "meeting.signal_lost.title", defaultValue: "Meeting signal lost", comment: "Warning title shown when meeting signal is no longer detected"),
            subtitle: String(localized: "meeting.signal_lost.message", defaultValue: "Still recording. Stop if the meeting ended.", comment: "Warning message shown when signal is lost but recording continues"),
            actionLabel: String(localized: "meeting.signal_lost.stop_recording_action", defaultValue: "Stop Recording", comment: "Action button label to stop recording after signal loss"),
            dismissAfter: 30,
    ...
    private func presentMeetingDetection(_ candidate: MeetingCandidate) {
        ...
        let didShow = meetingNotification.show(
            ...
            title: String(localized: "meeting.detected.title", defaultValue: "Meeting detected", comment: "Notification title shown when a meeting is detected automatically"),
            subtitle: title,
    ...
    private func presentMeetingProcessingStage(_ stage: MeetingProcessingStage) {
        ...
        case .transcribingAudio:
            setMeetingProcessingStatus(String(localized: "meeting.processing.transcribing", defaultValue: "Transcribing", comment: "Processing phase label while meeting audio is being transcribed"))
        case .cleaningAudio:
            setMeetingProcessingStatus(String(localized: "meeting.processing.cleaning", defaultValue: "Cleaning", comment: "Processing phase label while transcript cleanup is running"))
        case .generatingTitle:
            setMeetingProcessingStatus(String(localized: "meeting.processing.titling", defaultValue: "Titling", comment: "Processing phase label while meeting title is being generated"))
        case .summarizingNotes:
            setMeetingProcessingStatus(String(localized: "meeting.processing.summarizing", defaultValue: "Summarizing", comment: "Processing phase label while meeting summary is being generated"))
        }
    }
    ...
    private func finishComputerUseAudioStop(wavURL: URL?, startedAt: Date) {
        ...
        indicator.setTranscribingTitle(String(localized: "computer_use.transcribing_title.parsing_command", defaultValue: "Parsing command", comment: "Computer use transcription title while parsing spoken command"), config: config)
        ...
        let task = Task { [weak self] in
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.computerUseCommandTaskID = nil
                    self.setState(.idle)
                    self.indicator.showWarning(String(localized: "computer_use.command_failed.warning", defaultValue: "CUA command failed", comment: "Warning text when a computer use command execution fails"), icon: "!")
                    self.meetingMonitor.resumeAfterCooldown()
                    self.meetingMonitor.refreshState()
                }
            }
    ...
    private func computerUseFloatingStatusLabel(for status: String) -> String? {
        ...
        if status == "Retrying planner" {
            return String(localized: "computer_use.floating_status.retrying", defaultValue: "Retrying", comment: "Floating status text while retrying command execution")
        }
        return status
    }
    ...
    private func presentComputerUseRuntimeResult(_ result: ComputerUsePlannerRuntimeResult) {
        ...
        case .done:
            message = result.message.hasPrefix("Done") ? result.message : String(format: String(localized: "computer_use.runtime_result.done_with_message", defaultValue: "Done: %@", comment: "Runtime result status prefixing completion message"), "\(result.message)")
            floatingMessage = "Done"
            icon = ""
        case .timedOut:
            message = result.message
            floatingMessage = String(localized: "computer_use.runtime_result.timed_out", defaultValue: "Timed out", comment: "Runtime result status label when computer use action times out.")
            icon = "!"
        case .needsConfirmation:
            message = result.message.hasPrefix("Confirm") ? result.message : String(format: String(localized: "computer_use.runtime_result.confirm_with_message", defaultValue: "Confirm: %@", comment: "Runtime result status with confirmation message content."), "\(result.message)")
        ...
        case .cancelled:
            message = result.message
            floatingMessage = String(localized: "computer_use.runtime_result.cancelled", defaultValue: "Cancelled", comment: "Runtime result status label when computer use action was cancelled.")
            icon = ""
        }
    ...
    private func handleComputerUseAudioSessionEvent(_ event: DictationAudioSessionEvent) {
        ...
        case .noAudioTimeout(let sessionID, _):
            guard activeComputerUseAudioSessionID == sessionID else { break }
            statusBarController?.setStatus(String(localized: "computer_use.audio_status.mic_waiting_for_speech", defaultValue: "Mic waiting for speech", comment: "Computer use audio status when microphone is waiting for speech input."))
    ...
    private func handleDictationAudioSessionEvent(_ event: DictationAudioSessionEvent) {
        ...
        case .noAudioTimeout:
            statusBarController?.setStatus(String(localized: "dictation.audio_status.mic_waiting_for_speech", defaultValue: "Mic waiting for speech", comment: "Dictation audio status when microphone is waiting for speech input."))
        case .stopped(let eventSessionID, let wavURL):
    ...
    @discardableResult
    public func startMeetingRecordingForShortcuts(title: String = String(localized: "meeting.title", defaultValue: "Meeting", comment: "Meeting section title.")) -> Bool {
    ...
    private func userFacingDictationTestError(_ error: Error) -> String {
        ...
        if nsError.domain == "MuesliTranscriptionRuntime" {
            switch nsError.code {
            case 1:
                return String(localized: "dictation.test_error.nemotron_requires_macos15", defaultValue: "Nemotron requires macOS 15 or later. Choose another model to test dictation.", comment: "Dictation test error when Nemotron model requires a newer macOS version.")
            case 2:
                return String(localized: "dictation.test_error.qwen3_requires_macos15", defaultValue: "Qwen3 ASR requires macOS 15 or later. Choose another model to test dictation.", comment: "Dictation test error when Qwen3 ASR model requires a newer macOS version.")
            case 4:
                return String(localized: "dictation.test_error.cohere_requires_macos15", defaultValue: "Cohere Transcribe requires macOS 15 or later. Choose another model to test dictation.", comment: "Dictation test error when Cohere Transcribe model requires a newer macOS version.")
            default:
                return String(localized: "dictation.test_error.model_unavailable", defaultValue: "The selected model is not available. Choose another model and try again.", comment: "Dictation test error when selected model is unavailable.")
            }
        }
        ...
        if lowercasedMessage.contains("not loaded") || lowercasedMessage.contains("loadmodels") {
            return String(localized: "dictation.test_error.model_not_ready_retry", defaultValue: "The model was not ready yet. We are preparing it again, then try once more.", comment: "Dictation test error when model is not ready and preparation is retried.")
        }
        if lowercasedMessage.contains("network") || lowercasedMessage.contains("internet") || lowercasedMessage.contains("timed out") {
            return String(localized: "dictation.test_error.download_incomplete", defaultValue: "The model could not finish downloading. Check your connection and retry.", comment: "Dictation test error when model download is incomplete.")
        }
        if lowercasedMessage.contains("permission") || lowercasedMessage.contains("microphone") {
            return String(localized: "dictation.test_error.microphone_permission_denied", defaultValue: "Muesli could not access the microphone. Check Microphone permission and try again.", comment: "Dictation test error when microphone permission is denied.")
        }
        return String(localized: "dictation.test_error.could_not_start", defaultValue: "Dictation could not start. Try again in a moment.", comment: "Dictation test error when dictation startup fails.")
    }
    ...
    private func checkMaraudersMapActivation(_ text: String) {
        ...
        indicator.showWarning(String(localized: "marauders_map.activation_title", defaultValue: "Mischief Managed", comment: "Title shown when Marauders Map style feature is activated."), icon: "\u{26A1}", duration: 3.0)
        startMaraudersMapMonitoring()
    }
    ...
    private func handleUpcomingMeeting(_ event: UpcomingMeetingEvent) {
        ...
        if minutesUntil > 0 {
            timeLabel = String(format: String(localized: "meeting.upcoming.time.starts_in_minutes", defaultValue: "starts in %d min", comment: "Upcoming meeting relative time text showing minutes until start."), minutesUntil)
        } else if minutesUntil == 0 {
            timeLabel = String(localized: "meeting.upcoming.time.starting_now", defaultValue: "starting now", comment: "Upcoming meeting relative time text when meeting is starting immediately.")
        } else {
            timeLabel = String(format: String(localized: "meeting.upcoming.time.started_minutes_ago", defaultValue: "started %d min ago", comment: "Relative upcoming meeting time when meeting started minutes ago."), abs(minutesUntil))
        }

        let title = event.title
        let notificationTitle = minutesUntil <= 0 ? String(localized: "meeting.upcoming.notification_title.starting_now", defaultValue: "Meeting starting now", comment: "Upcoming meeting notification title when starting now.") : String(localized: "meeting.upcoming.notification_title.default", defaultValue: "Upcoming meeting", comment: "Default upcoming meeting notification title.")
        meetingNotification.show(
    ...
    private func showMeetingEndNotification(title: String) {
        ...
        meetingNotification.show(
            title: String(localized: "meeting.end_notification.title", defaultValue: "Meeting ended", comment: "Meeting end notification title."),
            subtitle: String(format: String(localized: "meeting.end_notification.body.scheduled_time_over", defaultValue: "%@ · scheduled time is over", comment: "Meeting end notification body including meeting title."), "\(title)"),
            actionLabel: String(localized: "meeting.end_notification.action.stop_recording", defaultValue: "Stop Recording", comment: "Meeting end notification action to stop recording."),
            dismissAfter: 45,
            onStartRecording: { [weak self] in
                self?.stopMeetingRecording()
            },
