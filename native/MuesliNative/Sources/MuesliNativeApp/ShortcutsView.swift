...
struct ShortcutsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
                Text(String(localized: "shortcuts.title", defaultValue: "Shortcuts", comment: "Title for shortcuts settings screen."))
                    .font(MuesliTheme.title1())
                    .foregroundStyle(MuesliTheme.textPrimary)

                Text(String(localized: "shortcuts.subtitle", defaultValue: "Choose your preferred shortcuts for dictation and computer use commands.", comment: "Subtitle describing shortcut customization purpose."))
        ...
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                    Text(String(localized: "shortcuts.dictation.push_to_talk.title", defaultValue: "Push to Talk", comment: "Section title for dictation push-to-talk shortcut."))
                        .font(MuesliTheme.headline())
                        .foregroundStyle(MuesliTheme.textPrimary)
                    Text(String(localized: "shortcuts.dictation.push_to_talk.description", defaultValue: "Hold to record, release to transcribe", comment: "Description for push-to-talk dictation behavior."))
    ...
    private var computerUseShortcutSection: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                    Text(String(localized: "shortcuts.computer_use_command.title", defaultValue: "Computer Use Command", comment: "Section title for computer use command shortcut."))
                        .font(MuesliTheme.headline())
                        .foregroundStyle(MuesliTheme.textPrimary)
                    Text(String(localized: "shortcuts.computer_use_command.description", defaultValue: "Hold to record a command, release to plan and run it", comment: "Description for computer use command recording shortcut."))
                        .font(MuesliTheme.caption())
                        .foregroundStyle(MuesliTheme.textSecondary)
                }
                Spacer()
    ...
    private var meetingRecordingShortcutSection: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                    Text(String(localized: "shortcuts.meeting_recording.title", defaultValue: "Meeting Recording", comment: "Section title for meeting recording shortcut."))
                        .font(MuesliTheme.headline())
                        .foregroundStyle(MuesliTheme.textPrimary)
                    Text(String(localized: "shortcuts.meeting_recording.description", defaultValue: "Toggle meeting recording on/off", comment: "Description for meeting recording toggle shortcut."))
                        .font(MuesliTheme.caption())
                        .foregroundStyle(MuesliTheme.textSecondary)
                }
                Spacer()
    ...
    private func thresholdInput(value: Int, onChange: @escaping (Int) -> Void) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            Text(String(localized: "shortcuts.threshold.hold_label", defaultValue: "Hold", comment: "Label for hold-threshold mode."))
                .font(MuesliTheme.caption())
                .foregroundStyle(MuesliTheme.textSecondary)
        ...
        .help(String(format: String(localized: "shortcuts.threshold.range_label", defaultValue: "Hold threshold: %@-%@ ms", comment: "Label showing configured hold-threshold bounds in milliseconds."), "\(HotkeyTriggerTiming.minThresholdMilliseconds)", "\(HotkeyTriggerTiming.maxThresholdMilliseconds)"))
    }
    ...
    private func changeButton(for target: ShortcutTarget) -> some View {
        Button {
            ...
            Text(recordingTarget == target ? recordingPrompt(for: target) : String(localized: "shortcuts.change_shortcut", defaultValue: "Change Shortcut", comment: "Button title to begin recording a new shortcut."))
                .font(MuesliTheme.body())
    ...
    private func recordingPrompt(for target: ShortcutTarget) -> String {
        switch target {
        case .meetingRecording:
            return String(localized: "shortcuts.recording_prompt.key_or_modifier", defaultValue: "Press a key or modifier...", comment: "Prompt shown while waiting for key or modifier input.")
        case .dictation, .computerUse:
            return String(localized: "shortcuts.recording_prompt.modifier_only", defaultValue: "Press a modifier key...", comment: "Prompt shown while waiting for modifier-only input.")
        }
    }
    ...
    private var doubleTapSection: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                    Text(String(localized: "shortcuts.hands_free_mode.title", defaultValue: "Hands-Free Mode", comment: "Section title for hands-free shortcut mode."))
                        .font(MuesliTheme.headline())
                        .foregroundStyle(MuesliTheme.textPrimary)
                    Text(String(localized: "shortcuts.hands_free_mode.description", defaultValue: "Double-tap dictation or CUA to start, tap again to stop", comment: "Description for hands-free shortcut activation behavior."))
                        .font(MuesliTheme.caption())
                        .foregroundStyle(MuesliTheme.textSecondary)
                }
                Spacer()
    ...
    private var resetButton: some View {
        Button {
            ...
            Text(String(localized: "shortcuts.reset_to_defaults", defaultValue: "Reset to Defaults", comment: "Button title to restore shortcut settings to defaults."))
                .font(MuesliTheme.body())
                .foregroundStyle(MuesliTheme.textSecondary)
        }
        .buttonStyle(.plain)
