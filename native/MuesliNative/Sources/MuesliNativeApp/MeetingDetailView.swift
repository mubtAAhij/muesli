...
private enum ManualNotesSaveStatus {
    ...
    var label: String {
        switch self {
        case .saved: return String(localized: "meeting_detail.save_state.saved", defaultValue: "Saved", bundle: .module, comment: "Meeting detail save state label when save is complete.")
        case .saving: return String(localized: "meeting_detail.save_state.saving", defaultValue: "Saving...", bundle: .module, comment: "Meeting detail save state label while saving in progress.")
        }
    }
}
...
struct MeetingDetailView: View {
    ...
    init(
        ...
        appState: AppState,
        onBack: (() -> Void)? = nil,
        backLabel: String = String(localized: "meeting_detail.navigation.back_to_meetings", defaultValue: "Back to Meetings", bundle: .module, comment: "Navigation button title to return to meetings list.")
    ) {
    ...
    var body: some View {
        Group {
            if let meeting {
                ...
                VStack(spacing: MuesliTheme.spacing12) {
                    ...
                    Text(String(localized: "meeting_detail.empty_state.choose_meeting_message", defaultValue: "Choose a meeting from the Meetings browser to open it here.", bundle: .module, comment: "Empty state message prompting selection of a meeting."))
        ...
        .alert(String(localized: "meeting_detail.alert.couldnt_save_summary", defaultValue: "Couldn't Save Summary", bundle: .module, comment: "Alert title when meeting summary save fails."), isPresented: summaryErrorBinding) {
            ...
            Text(summaryErrorMessage ?? String(localized: "meeting_detail.alert.couldnt_save_summary.message", defaultValue: "The updated meeting notes could not be saved.", bundle: .module, comment: "Alert message when meeting summary save fails."))
        }
        .alert(String(localized: "meeting_detail.alert.couldnt_retranscribe_meeting", defaultValue: "Couldn't Re-transcribe Meeting", bundle: .module, comment: "Alert title when retranscription fails."), isPresented: retranscriptionErrorBinding) {
            Button(String(localized: "meeting_detail.alert.ok.retranscribe", defaultValue: "OK", bundle: .module, comment: "Confirmation button title for retranscription failure alert."), role: .cancel) {
                retranscriptionErrorMessage = nil
            }
        } message: {
            Text(retranscriptionErrorMessage ?? String(localized: "meeting_detail.alert.couldnt_retranscribe_meeting.message", defaultValue: "The saved recording could not be re-transcribed.", bundle: .module, comment: "Alert message when retranscription of saved recording fails."))
        }
        .alert(String(localized: "meeting_detail.alert.resummarize_notes_title", defaultValue: "Re-summarize Notes?", bundle: .module, comment: "Alert title asking whether to regenerate notes."), isPresented: transcriptResummaryPromptBinding) {
            Button(String(localized: "meeting_detail.alert.resummarize_action", defaultValue: "Re-summarize", bundle: .module, comment: "Primary action button to regenerate notes.")) {
                resummarizeAfterTranscriptEdit()
            }
            Button(String(localized: "meeting_detail.alert.not_now", defaultValue: "Not Now", bundle: .module, comment: "Secondary action button to defer notes regeneration."), role: .cancel) {
        ...
        .alert(String(localized: "meeting_detail.alert.delete_meeting_title", defaultValue: "Delete Meeting", bundle: .module, comment: "Alert title for deleting a meeting."), isPresented: $showDeleteConfirmation) {
            Button(String(localized: "common.delete", defaultValue: "Delete", bundle: .module, comment: "Common delete action button label."), role: .destructive) {
                if let meeting {
                    controller.deleteMeeting(id: meeting.id)
                }
            }
            Button(String(localized: "meeting_detail.alert.cancel", defaultValue: "Cancel", bundle: .module, comment: "Cancel action button label in delete meeting alert."), role: .cancel) {}
        } message: {
            Text(String(localized: "meeting_detail.alert.delete_meeting_message", defaultValue: "Are you sure you want to delete this meeting? Saved notes, transcript, and any retained recording will be removed.", bundle: .module, comment: "Confirmation message describing data removal when deleting a meeting."))
        }
    }
            ...
            HStack(alignment: .top, spacing: MuesliTheme.spacing24) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing8) {
                    ...
                    HStack(spacing: MuesliTheme.spacing8) {
                        ...
                        metadataDivider
                        metadataItem(systemImage: "doc.text", text: String(format: String(localized: "meeting_detail.metadata.word_count", defaultValue: "%d words", bundle: .module, comment: "Meeting metadata label showing total word count."), meeting.wordCount))
    ...
    private var recordingModePicker: some View {
        Picker("", selection: $recordingMode) {
            Text(String(localized: "meeting_detail.recording_mode.live-notes", defaultValue: "Notes", bundle: .module, comment: "Recording mode label for notes tab in live meeting context.")).tag(RecordingContentMode.notes)
            Text(String(localized: "meeting_detail.recording_mode.live", defaultValue: "Live", bundle: .module, comment: "Recording mode label for live transcript tab.")).tag(RecordingContentMode.live)
        }
        .pickerStyle(.segmented)
        .tint(MuesliTheme.accent)
        .frame(width: 180)
    ...
    private func compactHeaderActions(for meeting: MeetingRecord, appliedTemplate: MeetingTemplateSnapshot) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            if isSummarizing {
                ProgressView()
                    .controlSize(.small)
                    .help(String(localized: "meeting_detail.header.summarizing_meeting_help", defaultValue: "Summarizing meeting", bundle: .module, comment: "Header status text while meeting summary is being generated."))
            }
        ...
        if meeting.savedRecordingPath != nil {
            if isRetranscribing {
                HStack(spacing: 6) {
                    ProgressView()
                        .controlSize(.small)
                    Text(String(localized: "meeting_detail.actions.retranscribing", defaultValue: "Re-transcribing...", bundle: .module, comment: "Action button label while retranscription is in progress."))
                        .font(.system(size: 11))
                        .foregroundStyle(MuesliTheme.textTertiary)
                }
                .padding(.horizontal, MuesliTheme.spacing8)
            } else {
                iconButton("arrow.clockwise", label: String(localized: "meeting_detail.actions.retranscribe", defaultValue: "Re-transcribe", bundle: .module, comment: "Action button label to start retranscribing meeting audio.")) {
    ...
    private func templateMenuItems(for meeting: MeetingRecord, appliedTemplate: MeetingTemplateSnapshot) -> some View {
            ...
            Section(String(localized: "meeting_detail.templates.built_in_section", defaultValue: "Built-in Templates", bundle: .module, comment: "Section title for built-in note templates.")) {
            ...
            if !controller.customMeetingTemplates().isEmpty {
                Section(String(localized: "meeting_detail.templates.custom_section", defaultValue: "Custom Templates", bundle: .module, comment: "Section title for user-created note templates.")) {
    ...
    private func manualNotesToolbar(for meeting: MeetingRecord) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            markdownToolbarButton(systemImage: "textformat.size", label: String(localized: "meeting_detail.notes_toolbar.heading", defaultValue: "Heading", bundle: .module, comment: "Notes toolbar button title for heading formatting.")) {
                manualEditorCommand = MarkdownEditorCommand(kind: .heading)
            }
            markdownToolbarButton(systemImage: "bold", label: String(localized: "meeting_detail.notes_toolbar.bold", defaultValue: "Bold", bundle: .module, comment: "Notes toolbar button title for bold formatting.")) {
                manualEditorCommand = MarkdownEditorCommand(kind: .bold)
            }
            markdownToolbarButton(systemImage: "list.bullet", label: String(localized: "meeting_detail.notes_toolbar.bullet", defaultValue: "Bullet", bundle: .module, comment: "Notes toolbar button title for bulleted list formatting.")) {
                manualEditorCommand = MarkdownEditorCommand(kind: .bullet)
            }
            markdownToolbarButton(systemImage: "checklist", label: String(localized: "meeting_detail.notes_toolbar.checkbox", defaultValue: "Checkbox", bundle: .module, comment: "Notes toolbar button title for checklist formatting.")) {
    ...
    private func statusChip(for meeting: MeetingRecord) -> some View {
        ...
        let label = isPreparing ? String(localized: "meeting_detail.preparation.preparing", defaultValue: "Preparing", bundle: .module, comment: "Preparation status label shown while meeting resources are initializing.") : isPaused ? String(localized: "meeting_detail.preparation.paused", defaultValue: "Paused", bundle: .module, comment: "Preparation status label shown when meeting preparation is paused.") : meeting.status.displayLabel
    ...
    private func exportMenu(for meeting: MeetingRecord) -> some View {
        ...
        let currentLabel = documentMode == .transcript ? String(localized: "meeting_detail.export.transcript", defaultValue: "Export Transcript", bundle: .module, comment: "Export menu item title for exporting transcript only.") : String(localized: "meeting_detail.export.notes", defaultValue: "Export Notes", bundle: .module, comment: "Export menu item title for exporting notes only.")
        Menu {
            Button {
            ...
            Button {
                ...
                Label(String(localized: "meeting_detail.export.full_meeting", defaultValue: "Export Full Meeting", bundle: .module, comment: "Export menu item title for exporting full meeting package."), systemImage: "doc.on.doc")
            }
        } label: {
            HStack(spacing: 6) {
                Image(systemName: "square.and.arrow.up")
                    .font(.system(size: 10, weight: .semibold))
                Text(String(localized: "meeting_detail.export.title", defaultValue: "Export", bundle: .module, comment: "Primary export button/menu title in meeting detail view."))
                    .font(.system(size: 12, weight: .semibold))
            }
            .foregroundStyle(MuesliTheme.textPrimary)
            .padding(.horizontal, MuesliTheme.spacing12)
    ...
    private func compactMoreActionsMenu(
        ...
        Menu {
            Menu {
                ...
                Label(
                    String(format: String(localized: "meeting_detail.template.current_selection", defaultValue: "Template: %@", bundle: .module, comment: "Label showing currently selected template name."), "\(labelForSelection(on: meeting, appliedTemplate: appliedTemplate))"),
            ...
            if meeting.savedRecordingPath != nil || controller.canDeleteMeeting(meeting) {
                ...
                if let savedRecordingPath = meeting.savedRecordingPath {
                    Button {
                        ...
                        Label(String(localized: "meeting_detail.menu.show_recording", defaultValue: "Show Recording", bundle: .module, comment: "Menu item title to reveal meeting recording."), systemImage: "folder")
                    }
                }
                ...
                if controller.canDeleteMeeting(meeting) {
                    Button(role: .destructive) {
                        ...
                        Label(String(localized: "meeting_detail.menu.delete_meeting", defaultValue: "Delete Meeting", bundle: .module, comment: "Menu item title to delete current meeting."), systemImage: "trash")
                    }
                }
            }
        } label: {
        ...
        .menuStyle(.borderlessButton)
        .fixedSize()
        .help(String(localized: "meeting_detail.menu.more_actions_help", defaultValue: "More actions", bundle: .module, comment: "Accessibility/help label for more actions menu button."))
    }
    ...
    private var deleteButton: some View {
        iconButton("trash", label: String(localized: "common.delete", defaultValue: "Delete", bundle: .module, comment: "Common delete action title.")) {
            showDeleteConfirmation = true
        }
    }
    ...
    private var meetingPreparationStatus: some View {
        HStack(spacing: 6) {
            ProgressView()
                ...
                .accessibilityLabel(String(localized: "meeting_detail.preparation.preparing_transcription_accessibility", defaultValue: "Preparing transcription", bundle: .module, comment: "Accessibility label while preparing meeting transcription."))
            Text(appState.meetingStartStatus ?? String(localized: "meeting_detail.preparation.starts_shortly_message", defaultValue: "Meeting transcription will start shortly.", bundle: .module, comment: "Preparation status message shown before transcription begins."))
                .font(.system(size: 12, weight: .semibold))
                .foregroundStyle(MuesliTheme.textSecondary)
                .lineLimit(1)
        }
    ...
    private var cancelMeetingPreparationButton: some View {
        ...
        .help(String(localized: "meeting_detail.preparation.cancel_help", defaultValue: "Cancel meeting preparation", bundle: .module, comment: "Accessibility/help label for cancel preparation action."))
    }
        ...
        HStack(spacing: 1) {
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "record.circle")
                        .font(.system(size: 10, weight: .semibold))
                    Text(String(localized: "meeting_detail.recording.resume_button", defaultValue: "Resume", bundle: .module, comment: "Secondary resume recording button title."))
                        .font(.system(size: 12, weight: .semibold))
                }
            ...
            Menu {
                ...
                Button {
                    ...
                    Label(String(localized: "meeting_detail.recording.start_follow_up", defaultValue: "Start a follow-up", bundle: .module, comment: "Button title to begin a follow-up meeting recording."), systemImage: "arrow.turn.down.right")
                }
            } label: {
            ...
            .fixedSize(horizontal: true, vertical: false)
            .help(String(localized: "meeting_detail.recording.resume_or_follow_up_help", defaultValue: "Resume recording, or start a follow-up meeting", bundle: .module, comment: "Accessibility/help text for resume or follow-up recording action."))
        }
        .fixedSize()
        .clipShape(RoundedRectangle(cornerRadius: MuesliTheme.cornerSmall))
        .overlay(
    ...
    private var discardRecordingButton: some View {
        iconButton("xmark", label: String(localized: "common.discard", defaultValue: "Discard", bundle: .module, comment: "Common discard action button title.")) {
    ...
    private var threadBreadcrumb: some View {
        if let threadContext {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(String(format: String(localized: "meeting_detail.thread.count_meetings", defaultValue: "Thread · %d meetings", bundle: .module, comment: "Thread context label showing total meeting count in thread."), threadContext.count))
                    .font(.system(size: 11, weight: .medium))
                    .foregroundStyle(MuesliTheme.textTertiary)
                ...
                default:
                    VStack(alignment: .leading, spacing: 3) {
                        Text(String(format: String(localized: "meeting_detail.thread.follow_ups_count", defaultValue: "Follow-ups (%d)", bundle: .module, comment: "Section header showing number of follow-up meetings."), threadContext.successors.count))
                            .font(.system(size: 11, weight: .medium))
                            .foregroundStyle(MuesliTheme.textTertiary)
                        ForEach(threadContext.successors) { successor in
                            threadLink(
                                icon: "arrow.turn.left.down",
                                text: String(format: String(localized: "meeting_detail.thread.successor_title_time", defaultValue: "%@ · %@", bundle: .module, comment: "Follow-up row label showing successor title and start time."), "\(successor.title)", "\(MeetingBrowserLogic.formatStartTime(successor.startTime))"),
                                targetID: successor.id
                            )
                        }
                    }
    ...
    private func threadLink(icon: String, text: String, targetID: Int64) -> some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "meeting_detail.thread_link.open_this_meeting_help", defaultValue: "Open this meeting", bundle: .module, comment: "Accessibility/help label for opening linked meeting."))
    }
    ...
    private func folderPill(for meeting: MeetingRecord) -> some View {
        ...
        Button {
            ...
            HStack(spacing: 5) {
                Image(systemName: hasFolder ? "folder.fill" : "folder.badge.plus")
                    .font(.system(size: 10))
                Text(currentFolder?.name ?? String(localized: "meeting_detail.folder_pill.add_to_folder", defaultValue: "Add to folder", bundle: .module, comment: "Folder pill label when no folder is assigned."))
                    .font(.system(size: 11, weight: .medium))
            }
        ...
        .popover(isPresented: $showFolderPopover, arrowEdge: .bottom) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                folderPopoverRow(icon: "folder.badge.plus", label: String(localized: "meeting_detail.folder_pill.new_folder_row", defaultValue: "New Folder...", bundle: .module, comment: "Menu row title to create a new folder.")) {
                    showFolderPopover = false
                    newFolderName = ""
        ...
        .alert(String(localized: "meeting_detail.folder_pill.new_folder_title", defaultValue: "New Folder", bundle: .module, comment: "Dialog title for creating a new folder."), isPresented: $showNewFolderPrompt) {
            TextField(String(localized: "meeting_detail.folder_pill.folder_name", defaultValue: "Folder name", bundle: .module, comment: "Prompt label for entering new folder name."), text: $newFolderName)
            Button(String(localized: "common.create", defaultValue: "Create", bundle: .module, comment: "Common create action button title.")) {
                let trimmed = newFolderName.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !trimmed.isEmpty else { return }
                controller.createFolderAndMoveMeeting(name: trimmed, meetingID: meeting.id)
            }
            Button(String(localized: "meeting_detail.folder_pill.cancel", defaultValue: "Cancel", bundle: .module, comment: "Cancel action button title in new folder dialog."), role: .cancel) {}
        } message: {
            Text(String(localized: "meeting_detail.folder_pill.create_folder_description", defaultValue: "Create a new folder and move this meeting into it.", bundle: .module, comment: "Description text in new folder creation dialog."))
        }
    }
        ...
        HStack(spacing: MuesliTheme.spacing8) {
            if hasApiKey {
                Image(systemName: "sparkles")
                    .foregroundStyle(MuesliTheme.accent)
                Text(String(format: String(localized: "meeting_detail.transcript_cta.use_primary_summary_action", defaultValue: "Use %@ to turn this raw transcript into AI meeting notes and a cleaned-up title.", bundle: .module, comment: "Transcript CTA text describing primary summary action."), "\(primarySummaryActionLabel)"))
                    .font(MuesliTheme.callout())
                    .foregroundStyle(MuesliTheme.textSecondary)
            } else {
                Image(systemName: "key.fill")
                    .foregroundStyle(MuesliTheme.accent)
                Text(String(localized: "meeting_detail.transcript_cta.add_api_key_message", defaultValue: "Add your API key in Settings to generate meeting notes", bundle: .module, comment: "Transcript CTA text prompting user to add API key in settings."))
                    .font(MuesliTheme.callout())
                    .foregroundStyle(MuesliTheme.textSecondary)
                Spacer()
                Button(String(localized: "common.open_settings", defaultValue: "Open Settings", bundle: .module, comment: "Common action button title to open app settings.")) {
    ...
    private var primarySummaryActionLabel: String {
        guard let meeting else { return String(localized: "meeting_detail.transcript_cta.re_summarize", defaultValue: "Re-summarize", bundle: .module, comment: "CTA button title to regenerate meeting summary.") }
    ...
    private var editButtonLabel: String {
        ...
        return documentMode == .transcript ? String(localized: "meeting_detail.edit_button.edit_transcript", defaultValue: "Edit Transcript", bundle: .module, comment: "Edit button title for transcript mode.") : String(localized: "meeting_detail.edit_button.edit_notes", defaultValue: "Edit Notes", bundle: .module, comment: "Edit button title for notes mode.")
    }
    ...
    private func formatDuration(_ seconds: Double) -> String {
        ...
        if rounded >= 3600 {
            return String(format: String(localized: "meeting_detail.duration.hours_minutes", defaultValue: "%dh %dm", bundle: .module, comment: "Duration label formatted as hours and minutes."), rounded / 3600, (rounded % 3600) / 60)
        }
        if rounded >= 60 {
            let m = rounded / 60
            let s = rounded % 60
            return s == 0 ? String(format: String(localized: "meeting_detail.duration.minutes_only", defaultValue: "%dm", bundle: .module, comment: "Duration label formatted as minutes only."), m) : String(format: String(localized: "meeting_detail.duration.minutes_seconds", defaultValue: "%dm %ds", bundle: .module, comment: "Duration label formatted as minutes and seconds."), m, s)
        }
        return String(format: String(localized: "meeting_detail.duration.seconds_only", defaultValue: "%ds", bundle: .module, comment: "Duration label formatted as seconds only."), rounded)
    }
}
...
private struct MarqueeTitleTextField: View {
    ...
    var body: some View {
        ZStack(alignment: .leading) {
            TextField(String(localized: "meeting_detail.title_editor.meeting_title.primary", defaultValue: "Meeting Title", bundle: .module, comment: "Primary title editor prompt for meeting title field."), text: $text)
                .font(titleFont)
                .foregroundStyle(MuesliTheme.textPrimary)
            ...
            Text(text.isEmpty ? String(localized: "meeting_detail.title_editor.meeting_title.secondary", defaultValue: "Meeting Title", bundle: .module, comment: "Secondary title editor prompt for meeting title field.") : text)
                .font(titleFont)
                .fontWeight(.bold)
                .foregroundStyle(MuesliTheme.textPrimary)
                .lineLimit(1)
        ...
        .overlay(
            Text(text.isEmpty ? String(localized: "meeting_detail.title_editor.meeting_title.tertiary", defaultValue: "Meeting Title", bundle: .module, comment: "Tertiary title editor prompt for meeting title field.") : text)
                .font(titleFont)
                .fontWeight(.bold)
                .lineLimit(1)
                .fixedSize(horizontal: true, vertical: false)
...
struct TranscriptChatMessage: Identifiable, Equatable {
    ...
    private static func isLikelySpeakerLabel(_ label: String) -> Bool {
        ...
        if label.localizedCaseInsensitiveCompare(String(localized: "meeting_detail.transcript.speaker_others", defaultValue: "Others", bundle: .module, comment: "Speaker label for non-user speakers in transcript.")) == .orderedSame { return true }
    ...
    var body: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: MuesliTheme.spacing8) {
                if messages.isEmpty {
                    Text(String(localized: "meeting_detail.transcript.no_transcript_available", defaultValue: "No transcript available", bundle: .module, comment: "Empty transcript message when no transcript content exists."))
