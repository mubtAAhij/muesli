...
private enum ManualNotesSaveStatus {
    ...
    var label: String {
        switch self {
        case .saved: return String(localized: "meeting_detail.save_state.saved", defaultValue: "Saved", bundle: Bundle.module, comment: "Save state label shown when meeting notes are saved")
        case .saving: return String(localized: "meeting_detail.save_state.saving", defaultValue: "Saving...", bundle: Bundle.module, comment: "Save state label shown while meeting notes are being saved")
        }
    }
}
...
struct MeetingDetailView: View {
    ...
    init(
        ...
        appState: AppState,
        onBack: (() -> Void)? = nil,
        backLabel: String = String(localized: "meeting_detail.navigation.back_to_meetings", defaultValue: "Back to Meetings", bundle: Bundle.module, comment: "Navigation action to return to meetings list")
    ) {
    ...
    var body: some View {
        Group {
            if let meeting {
                ...
                VStack(spacing: MuesliTheme.spacing12) {
                    Text(String(localized: "meeting_detail.empty_state.no_meeting_selected", defaultValue: "No meeting selected", bundle: Bundle.module, comment: "Empty state title when no meeting is selected"))
                        .font(MuesliTheme.title3())
                        .foregroundStyle(MuesliTheme.textSecondary)
                    Text(String(localized: "meeting_detail.empty_state.choose_meeting_message", defaultValue: "Choose a meeting from the Meetings browser to open it here.", bundle: Bundle.module, comment: "Empty state message guiding user to select a meeting"))
        ...
        .alert(String(localized: "meeting_detail.alert.couldnt_save_summary", defaultValue: "Couldn't Save Summary", bundle: Bundle.module, comment: "Alert title when summary save fails"), isPresented: summaryErrorBinding) {
            Button(String(localized: "meeting_detail.alert.ok.save_summary", defaultValue: "OK", bundle: Bundle.module, comment: "Confirmation button for save summary failure alert"), role: .cancel) {
                summaryErrorMessage = nil
            }
        } message: {
            Text(summaryErrorMessage ?? String(localized: "meeting_detail.alert.couldnt_save_summary.message", defaultValue: "The updated meeting notes could not be saved.", bundle: Bundle.module, comment: "Alert message when updated meeting notes fail to save"))
        }
        .alert(String(localized: "meeting_detail.alert.couldnt_retranscribe_meeting", defaultValue: "Couldn't Re-transcribe Meeting", bundle: Bundle.module, comment: "Alert title when meeting retranscription fails"), isPresented: retranscriptionErrorBinding) {
            Button(String(localized: "meeting_detail.alert.ok.retranscribe", defaultValue: "OK", bundle: Bundle.module, comment: "Confirmation button for retranscription failure alert"), role: .cancel) {
                retranscriptionErrorMessage = nil
            }
        } message: {
            Text(retranscriptionErrorMessage ?? String(localized: "meeting_detail.alert.couldnt_retranscribe_meeting.message", defaultValue: "The saved recording could not be re-transcribed.", bundle: Bundle.module, comment: "Alert message when saved recording retranscription fails"))
        }
        .alert(String(localized: "meeting_detail.alert.resummarize_notes_title", defaultValue: "Re-summarize Notes?", bundle: Bundle.module, comment: "Alert title asking whether to re-summarize notes"), isPresented: transcriptResummaryPromptBinding) {
            Button(String(localized: "meeting_detail.alert.resummarize_action", defaultValue: "Re-summarize", bundle: Bundle.module, comment: "Destructive alert action to re-summarize notes")) {
                resummarizeAfterTranscriptEdit()
            }
            Button(String(localized: "meeting_detail.alert.not_now", defaultValue: "Not Now", bundle: Bundle.module, comment: "Alert cancel action to defer re-summarization"), role: .cancel) {
                transcriptResummaryPromptMeetingID = nil
            }
        } message: {
            Text(String(localized: "meeting_detail.alert.resummarize_notes_message", defaultValue: "Your transcript edits may change the generated notes. Re-summarize now to update them from the edited transcript.", bundle: Bundle.module, comment: "Alert message explaining why re-summarization is recommended"))
        }
        .alert(String(localized: "meeting_detail.alert.delete_meeting_title", defaultValue: "Delete Meeting", bundle: Bundle.module, comment: "Alert title when confirming meeting deletion"), isPresented: $showDeleteConfirmation) {
            Button(String(localized: "common.delete", defaultValue: "Delete", bundle: Bundle.module, comment: "Destructive delete action label"), role: .destructive) {
                if let meeting {
                    controller.deleteMeeting(id: meeting.id)
                }
            }
            Button(String(localized: "meeting_detail.alert.cancel", defaultValue: "Cancel", bundle: Bundle.module, comment: "Cancel action for delete meeting alert"), role: .cancel) {}
        } message: {
            Text(String(localized: "meeting_detail.alert.delete_meeting_message", defaultValue: "Are you sure you want to delete this meeting? Saved notes, transcript, and any retained recording will be removed.", bundle: Bundle.module, comment: "Confirmation message describing effects of deleting a meeting"))
        }
    }
            ...
            HStack(alignment: .top, spacing: MuesliTheme.spacing24) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing8) {
                    ...
                    HStack(spacing: MuesliTheme.spacing8) {
                        ...
                        metadataDivider
                        metadataItem(systemImage: "doc.text", text: String(format: String(localized: "meeting_detail.metadata.word_count", defaultValue: "%d words", bundle: Bundle.module, comment: "Metadata label showing total word count for the meeting transcript"), meeting.wordCount))
    ...
    private var documentModePicker: some View {
        Picker("", selection: $documentMode) {
            Text(String(localized: "meeting_detail.document_mode.notes", defaultValue: "Notes", bundle: Bundle.module, comment: "Document mode option label for notes view")).tag(MeetingDocumentMode.notes)
            Text(String(localized: "meeting_detail.document_mode.transcript", defaultValue: "Transcript", bundle: Bundle.module, comment: "Document mode option label for transcript view")).tag(MeetingDocumentMode.transcript)
        }
    ...
    private var recordingModePicker: some View {
        Picker("", selection: $recordingMode) {
            Text(String(localized: "meeting_detail.recording_mode.live-notes", defaultValue: "Notes", bundle: Bundle.module, comment: "Recording mode option label for notes")).tag(RecordingContentMode.notes)
            Text(String(localized: "meeting_detail.recording_mode.live", defaultValue: "Live", bundle: Bundle.module, comment: "Recording mode option label for live content")).tag(RecordingContentMode.live)
        }
        .pickerStyle(.segmented)
        .tint(MuesliTheme.accent)
        .frame(width: 180)
    ...
    private func compactHeaderActions(for meeting: MeetingRecord, appliedTemplate: MeetingTemplateSnapshot) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            if isSummarizing {
                ProgressView()
                    .controlSize(.small)
                    .help(String(localized: "meeting_detail.header.summarizing_meeting_help", defaultValue: "Summarizing meeting", bundle: Bundle.module, comment: "Header status shown while meeting notes are being summarized"))
            }
        ...
        if meeting.savedRecordingPath != nil {
            if isRetranscribing {
                HStack(spacing: 6) {
                    ProgressView()
                        .controlSize(.small)
                    Text(String(localized: "meeting_detail.actions.retranscribing", defaultValue: "Re-transcribing...", bundle: Bundle.module, comment: "Action label shown while retranscription is running"))
                        .font(.system(size: 11))
                        .foregroundStyle(MuesliTheme.textTertiary)
                }
                .padding(.horizontal, MuesliTheme.spacing8)
            } else {
                iconButton("arrow.clockwise", label: String(localized: "meeting_detail.actions.retranscribe", defaultValue: "Re-transcribe", bundle: Bundle.module, comment: "Action label to re-transcribe meeting audio")) {
    ...
    private func templateMenuItems(for meeting: MeetingRecord, appliedTemplate: MeetingTemplateSnapshot) -> some View {
            ...
            Section(String(localized: "meeting_detail.templates.built_in_section", defaultValue: "Built-in Templates", bundle: Bundle.module, comment: "Section header for built-in summary templates")) {
            ...
            if !controller.customMeetingTemplates().isEmpty {
                Section(String(localized: "meeting_detail.templates.custom_section", defaultValue: "Custom Templates", bundle: Bundle.module, comment: "Section header for user-defined summary templates")) {
            ...
            Button(String(localized: "meeting_detail.template_menu.manage_templates", defaultValue: "Manage Templates…", bundle: Bundle.module, comment: "Menu item to open template management")) {
    ...
    private func manualNotesToolbar(for meeting: MeetingRecord) -> some View {
        HStack(spacing: MuesliTheme.spacing8) {
            ...
            markdownToolbarButton(systemImage: "textformat.size", label: String(localized: "meeting_detail.notes_toolbar.heading", defaultValue: "Heading", bundle: Bundle.module, comment: "Notes toolbar button label for heading formatting")) {
                manualEditorCommand = MarkdownEditorCommand(kind: .heading)
            }
            markdownToolbarButton(systemImage: "bold", label: String(localized: "meeting_detail.notes_toolbar.bold", defaultValue: "Bold", bundle: Bundle.module, comment: "Notes toolbar button label for bold formatting")) {
                manualEditorCommand = MarkdownEditorCommand(kind: .bold)
            }
            markdownToolbarButton(systemImage: "list.bullet", label: String(localized: "meeting_detail.notes_toolbar.bullet", defaultValue: "Bullet", bundle: Bundle.module, comment: "Notes toolbar button label for bullet list formatting")) {
                manualEditorCommand = MarkdownEditorCommand(kind: .bullet)
            }
            markdownToolbarButton(systemImage: "checklist", label: String(localized: "meeting_detail.notes_toolbar.checkbox", defaultValue: "Checkbox", bundle: Bundle.module, comment: "Notes toolbar button label for checkbox list formatting")) {
    ...
    private func statusChip(for meeting: MeetingRecord) -> some View {
        ...
        let label = isPreparing ? String(localized: "meeting_detail.preparation.preparing", defaultValue: "Preparing", bundle: Bundle.module, comment: "Preparation status label when meeting content is preparing") : isPaused ? String(localized: "meeting_detail.preparation.paused", defaultValue: "Paused", bundle: Bundle.module, comment: "Preparation status label when meeting processing is paused") : meeting.status.displayLabel
    ...
    private func exportMenu(for meeting: MeetingRecord) -> some View {
        ...
        let currentLabel = documentMode == .transcript ? String(localized: "meeting_detail.export.transcript", defaultValue: "Export Transcript", bundle: Bundle.module, comment: "Export action title for transcript content") : String(localized: "meeting_detail.export.notes", defaultValue: "Export Notes", bundle: Bundle.module, comment: "Export action title for notes content")
        Menu {
            Button {
            ...
            Button {
                ...
                Label(String(localized: "meeting_detail.export.full_meeting", defaultValue: "Export Full Meeting", bundle: Bundle.module, comment: "Export action title for complete meeting package"), systemImage: "doc.on.doc")
            }
        } label: {
            HStack(spacing: 6) {
                Image(systemName: "square.and.arrow.up")
                    .font(.system(size: 10, weight: .semibold))
                Text(String(localized: "meeting_detail.export.title", defaultValue: "Export", bundle: Bundle.module, comment: "Generic export button title"))
                    .font(.system(size: 12, weight: .semibold))
            }
            .foregroundStyle(MuesliTheme.textPrimary)
            .padding(.horizontal, MuesliTheme.spacing12)
    ...
    private func compactMoreActionsMenu(
        ...
        Menu {
            Menu {
                ...
                Label(
                    String(format: String(localized: "meeting_detail.template.current_selection", defaultValue: "Template: %@", bundle: Bundle.module, comment: "Label showing currently selected summary template"), "\(labelForSelection(on: meeting, appliedTemplate: appliedTemplate))"),
            ...
            if meeting.savedRecordingPath != nil || controller.canDeleteMeeting(meeting) {
                ...
                if let savedRecordingPath = meeting.savedRecordingPath {
                    Button {
                        ...
                        Label(String(localized: "meeting_detail.menu.show_recording", defaultValue: "Show Recording", bundle: Bundle.module, comment: "Menu item to reveal meeting recording section"), systemImage: "folder")
                    }
                }
                ...
                if controller.canDeleteMeeting(meeting) {
                    Button(role: .destructive) {
                        ...
                        Label(String(localized: "meeting_detail.menu.delete_meeting", defaultValue: "Delete Meeting", bundle: Bundle.module, comment: "Menu item to delete the current meeting"), systemImage: "trash")
                    }
                }
            }
        } label: {
        ...
        .menuStyle(.borderlessButton)
        .fixedSize()
        .help(String(localized: "meeting_detail.menu.more_actions_help", defaultValue: "More actions", bundle: Bundle.module, comment: "Accessibility/help label for more actions menu"))
    }
    ...
    private var deleteButton: some View {
        iconButton("trash", label: String(localized: "common.delete", defaultValue: "Delete", bundle: Bundle.module, comment: "Common destructive action label")) {
            showDeleteConfirmation = true
        }
    }
    ...
    private var meetingPreparationStatus: some View {
        HStack(spacing: 6) {
            ProgressView()
                ...
                .accessibilityLabel(String(localized: "meeting_detail.preparation.preparing_transcription_accessibility", defaultValue: "Preparing transcription", bundle: Bundle.module, comment: "Accessibility label while transcription preparation is in progress"))
            Text(appState.meetingStartStatus ?? String(localized: "meeting_detail.preparation.starts_shortly_message", defaultValue: "Meeting transcription will start shortly.", bundle: Bundle.module, comment: "Message indicating transcription will begin soon"))
                .font(.system(size: 12, weight: .semibold))
                .foregroundStyle(MuesliTheme.textSecondary)
                .lineLimit(1)
        }
    ...
    private var cancelMeetingPreparationButton: some View {
        iconButton("xmark", label: String(localized: "common.cancel", defaultValue: "Cancel", bundle: Bundle.module, comment: "Common cancel action label")) {
            controller.cancelMeetingPreparation()
        }
        .help(String(localized: "meeting_detail.preparation.cancel_help", defaultValue: "Cancel meeting preparation", bundle: Bundle.module, comment: "Accessibility/help label for cancelling preparation"))
    }
    ...
    private var pauseResumeRecordingButton: some View {
        ...
        return Button {
            ...
            HStack(spacing: 6) {
                Image(systemName: isPaused ? "play.fill" : "pause.fill")
                    .font(.system(size: 10, weight: .semibold))
                Text(isPaused ? String(localized: "meeting_detail.recording.resume", defaultValue: "Resume", bundle: Bundle.module, comment: "Recording control label to resume") : String(localized: "meeting_detail.recording.pause", defaultValue: "Pause", bundle: Bundle.module, comment: "Recording control label to pause"))
                    .font(.system(size: 12, weight: .semibold))
            }
        ...
        .help(isPaused ? String(localized: "meeting_detail.recording.resume_help.primary", defaultValue: "Resume recording", bundle: Bundle.module, comment: "Primary accessibility/help label for resuming recording") : String(localized: "meeting_detail.recording.pause_help", defaultValue: "Pause recording", bundle: Bundle.module, comment: "Accessibility/help label for pausing recording"))
    }
    ...
    private func resumeRecordingButton(for meeting: MeetingRecord) -> some View {
        HStack(spacing: 1) {
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "record.circle")
                        .font(.system(size: 10, weight: .semibold))
                    Text(String(localized: "meeting_detail.recording.resume_button", defaultValue: "Resume", bundle: Bundle.module, comment: "Secondary resume button label"))
                        .font(.system(size: 12, weight: .semibold))
                }
            ...
            .buttonStyle(.plain)
            .help(String(localized: "meeting_detail.recording.resume_help.secondary", defaultValue: "Resume recording", bundle: Bundle.module, comment: "Secondary accessibility/help label for resuming recording"))

            Menu {
                Button {
                    controller.resumeFinishedMeeting(meetingID: meeting.id)
                } label: {
                    Label(String(localized: "meeting_detail.recording.resume_help.tertiary", defaultValue: "Resume recording", bundle: Bundle.module, comment: "Tertiary accessibility/help label for resuming recording"), systemImage: "record.circle")
                }
                Button {
                    controller.startFollowUpMeeting(fromMeetingID: meeting.id)
                } label: {
                    Label(String(localized: "meeting_detail.recording.start_follow_up", defaultValue: "Start a follow-up", bundle: Bundle.module, comment: "Action label to start a follow-up meeting"), systemImage: "arrow.turn.down.right")
                }
            } label: {
            ...
            .fixedSize(horizontal: true, vertical: false)
            .help(String(localized: "meeting_detail.recording.resume_or_follow_up_help", defaultValue: "Resume recording, or start a follow-up meeting", bundle: Bundle.module, comment: "Accessibility/help text for resume or follow-up actions"))
        }
        .fixedSize()
        .clipShape(RoundedRectangle(cornerRadius: MuesliTheme.cornerSmall))
        .overlay(
    ...
    private var stopRecordingButton: some View {
        Button {
            ...
            HStack(spacing: 6) {
                Image(systemName: "stop.fill")
                    .font(.system(size: 10, weight: .semibold))
                Text(String(localized: "meeting_detail.recording.stop", defaultValue: "Stop", bundle: Bundle.module, comment: "Recording control label to stop"))
                    .font(.system(size: 12, weight: .semibold))
            }
        ...
        .buttonStyle(.plain)
        .disabled(!appState.isMeetingRecording)
        .help(String(localized: "meeting_detail.recording.stop_help", defaultValue: "Stop recording", bundle: Bundle.module, comment: "Accessibility/help label for stopping recording"))
    }

    private var discardRecordingButton: some View {
        iconButton("xmark", label: String(localized: "common.discard", defaultValue: "Discard", bundle: Bundle.module, comment: "Common discard action label")) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if let predecessor = threadContext.predecessor {
                    threadLink(
                        ...
                        text: String(format: String(localized: "meeting_detail.thread.follow_up_to", defaultValue: "Follow-up to: %@ · %@", bundle: Bundle.module, comment: "Thread context label showing predecessor meeting title and start time"), "\(predecessor.title)", "\(MeetingBrowserLogic.formatStartTime(predecessor.startTime))"),
                        targetID: predecessor.id
                    )
                }
                Text(String(format: String(localized: "meeting_detail.thread.count_meetings", defaultValue: "Thread · %d meetings", bundle: Bundle.module, comment: "Thread context label showing number of meetings in thread"), threadContext.count))
                    .font(.system(size: 11, weight: .medium))
                    .foregroundStyle(MuesliTheme.textTertiary)
                ...
                case 1:
                    if let successor = threadContext.successors.first {
                        threadLink(
                            ...
                            text: String(format: String(localized: "meeting_detail.thread.followed_by", defaultValue: "Followed by: %@ · %@", bundle: Bundle.module, comment: "Thread context label showing successor meeting title and start time"), "\(successor.title)", "\(MeetingBrowserLogic.formatStartTime(successor.startTime))"),
                            targetID: successor.id
                        )
                    }
                default:
                    VStack(alignment: .leading, spacing: 3) {
                        Text(String(format: String(localized: "meeting_detail.thread.follow_ups_count", defaultValue: "Follow-ups (%d)", bundle: Bundle.module, comment: "Section label showing number of follow-up meetings"), threadContext.successors.count))
                            .font(.system(size: 11, weight: .medium))
                            .foregroundStyle(MuesliTheme.textTertiary)
                        ForEach(threadContext.successors) { successor in
                            threadLink(
                                icon: "arrow.turn.left.down",
                                text: String(format: String(localized: "meeting_detail.thread.successor_title_time", defaultValue: "%@ · %@", bundle: Bundle.module, comment: "Thread row label showing successor meeting title and start time"), "\(successor.title)", "\(MeetingBrowserLogic.formatStartTime(successor.startTime))"),
                                targetID: successor.id
                            )
                        }
                    }
    ...
    private func threadLink(icon: String, text: String, targetID: Int64) -> some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "meeting_detail.thread_link.open_this_meeting_help", defaultValue: "Open this meeting", bundle: Bundle.module, comment: "Accessibility/help label for opening a meeting from thread link"))
    }
    ...
    private func folderPill(for meeting: MeetingRecord) -> some View {
        ...
        Button {
            ...
            HStack(spacing: 5) {
                Image(systemName: hasFolder ? "folder.fill" : "folder.badge.plus")
                    .font(.system(size: 10))
                Text(currentFolder?.name ?? String(localized: "meeting_detail.folder_pill.add_to_folder", defaultValue: "Add to folder", bundle: Bundle.module, comment: "Action label to add current meeting to a folder"))
                    .font(.system(size: 11, weight: .medium))
            }
        ...
        .buttonStyle(.plain)
        .help(hasFolder ? String(localized: "meeting_detail.folder_pill.change_folder_help", defaultValue: "Change folder", bundle: Bundle.module, comment: "Accessibility/help label for changing meeting folder") : String(localized: "meeting_detail.folder_pill.add_to_folder.help", defaultValue: "Add to folder", bundle: Bundle.module, comment: "Accessibility/help label for adding meeting to folder"))
        .popover(isPresented: $showFolderPopover, arrowEdge: .bottom) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                folderPopoverRow(icon: "folder.badge.plus", label: String(localized: "meeting_detail.folder_pill.new_folder_row", defaultValue: "New Folder...", bundle: Bundle.module, comment: "Menu row label to create a new folder")) {
                    showFolderPopover = false
                    newFolderName = ""
        ...
        .alert(String(localized: "meeting_detail.folder_pill.new_folder_title", defaultValue: "New Folder", bundle: Bundle.module, comment: "Dialog title for creating a new folder"), isPresented: $showNewFolderPrompt) {
            TextField(String(localized: "meeting_detail.folder_pill.folder_name", defaultValue: "Folder name", bundle: Bundle.module, comment: "Prompt label for entering a new folder name"), text: $newFolderName)
            Button(String(localized: "common.create", defaultValue: "Create", bundle: Bundle.module, comment: "Common create action label")) {
                let trimmed = newFolderName.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !trimmed.isEmpty else { return }
                controller.createFolderAndMoveMeeting(name: trimmed, meetingID: meeting.id)
            }
            Button(String(localized: "meeting_detail.folder_pill.cancel", defaultValue: "Cancel", bundle: Bundle.module, comment: "Cancel action label for new folder dialog"), role: .cancel) {}
        } message: {
            Text(String(localized: "meeting_detail.folder_pill.create_folder_description", defaultValue: "Create a new folder and move this meeting into it.", bundle: Bundle.module, comment: "Description text for new folder creation dialog"))
        }
    }
        ...
        HStack(spacing: MuesliTheme.spacing8) {
            if hasApiKey {
                Image(systemName: "sparkles")
                    .foregroundStyle(MuesliTheme.accent)
                Text(String(format: String(localized: "meeting_detail.transcript_cta.use_primary_summary_action", defaultValue: "Use %@ to turn this raw transcript into AI meeting notes and a cleaned-up title.", bundle: Bundle.module, comment: "CTA explaining primary summary action on raw transcript"), "\(primarySummaryActionLabel)"))
                    .font(MuesliTheme.callout())
                    .foregroundStyle(MuesliTheme.textSecondary)
            } else {
                Image(systemName: "key.fill")
                    .foregroundStyle(MuesliTheme.accent)
                Text(String(localized: "meeting_detail.transcript_cta.add_api_key_message", defaultValue: "Add your API key in Settings to generate meeting notes", bundle: Bundle.module, comment: "CTA message instructing user to add API key for meeting notes"))
                    .font(MuesliTheme.callout())
                    .foregroundStyle(MuesliTheme.textSecondary)
                Spacer()
                Button(String(localized: "common.open_settings", defaultValue: "Open Settings", bundle: Bundle.module, comment: "Common action label to open Settings")) {
    ...
    private var primarySummaryActionLabel: String {
        guard let meeting else { return String(localized: "meeting_detail.transcript_cta.re_summarize", defaultValue: "Re-summarize", bundle: Bundle.module, comment: "Action label to regenerate meeting summary") }
        return primarySummaryActionLabel(for: meeting)
    }

    private var copyButtonLabel: String {
        String(localized: "common.copy", defaultValue: "Copy", bundle: Bundle.module, comment: "Common copy action label")
    }

    private var editButtonLabel: String {
        if isEditingNotes || isEditingTranscript {
            return String(localized: "common.done", defaultValue: "Done", bundle: Bundle.module, comment: "Common done action label")
        }
        return documentMode == .transcript ? String(localized: "meeting_detail.edit_button.edit_transcript", defaultValue: "Edit Transcript", bundle: Bundle.module, comment: "Edit mode toggle label for transcript") : String(localized: "meeting_detail.edit_button.edit_notes", defaultValue: "Edit Notes", bundle: Bundle.module, comment: "Edit mode toggle label for notes")
    }

    private func primarySummaryActionLabel(for meeting: MeetingRecord) -> String {
        hasPendingTemplateChange(for: meeting) ? String(localized: "meeting_detail.summary.primary_action.apply_template", defaultValue: "Apply Template", bundle: Bundle.module, comment: "Primary summary action label to apply template") : String(localized: "meeting_detail.summary.primary_action.re_summarize", defaultValue: "Re-summarize", bundle: Bundle.module, comment: "Primary summary action label to regenerate summary")
    }
    ...
    private func formatDuration(_ seconds: Double) -> String {
        ...
        if rounded >= 3600 {
            return String(format: String(localized: "meeting_detail.duration.hours_minutes", defaultValue: "%dh %dm", bundle: Bundle.module, comment: "Duration label in hours and minutes"), rounded / 3600, (rounded % 3600) / 60)
        }
        if rounded >= 60 {
            let m = rounded / 60
            let s = rounded % 60
            return s == 0 ? String(format: String(localized: "meeting_detail.duration.minutes_only", defaultValue: "%dm", bundle: Bundle.module, comment: "Duration label in minutes only"), m) : String(format: String(localized: "meeting_detail.duration.minutes_seconds", defaultValue: "%dm %ds", bundle: Bundle.module, comment: "Duration label in minutes and seconds"), m, s)
        }
        return String(format: String(localized: "meeting_detail.duration.seconds_only", defaultValue: "%ds", bundle: Bundle.module, comment: "Duration label in seconds only"), rounded)
    }
}
...
private struct MarqueeTitleTextField: View {
    ...
    var body: some View {
        ZStack(alignment: .leading) {
            TextField(String(localized: "meeting_detail.title_editor.meeting_title.primary", defaultValue: "Meeting Title", bundle: Bundle.module, comment: "Primary placeholder/title label for meeting title editor"), text: $text)
                .font(titleFont)
                .foregroundStyle(MuesliTheme.textPrimary)
            ...
            Text(text.isEmpty ? String(localized: "meeting_detail.title_editor.meeting_title.secondary", defaultValue: "Meeting Title", bundle: Bundle.module, comment: "Secondary placeholder/title label for meeting title editor") : text)
                .font(titleFont)
                .fontWeight(.bold)
                .foregroundStyle(MuesliTheme.textPrimary)
                .lineLimit(1)
        ...
        .overlay(
            Text(text.isEmpty ? String(localized: "meeting_detail.title_editor.meeting_title.tertiary", defaultValue: "Meeting Title", bundle: Bundle.module, comment: "Tertiary placeholder/title label for meeting title editor") : text)
                .font(titleFont)
                .fontWeight(.bold)
                .lineLimit(1)
                .fixedSize(horizontal: true, vertical: false)
...
struct TranscriptChatMessage: Identifiable, Equatable {
    ...
    private static func isLikelySpeakerLabel(_ label: String) -> Bool {
        ...
        if label.localizedCaseInsensitiveCompare(String(localized: "meeting_detail.transcript.speaker_others", defaultValue: "Others", bundle: Bundle.module, comment: "Speaker label for other participants in transcript filter")) == .orderedSame { return true }
    ...
    var body: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: MuesliTheme.spacing8) {
                if messages.isEmpty {
                    Text(String(localized: "meeting_detail.transcript.no_transcript_available", defaultValue: "No transcript available", bundle: Bundle.module, comment: "Empty state label when transcript is unavailable"))
