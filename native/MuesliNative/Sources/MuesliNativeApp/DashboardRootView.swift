...
struct DashboardRootView: View {
    ...
    var body: some View {
        ...
        .alert(
            appState.contributionMilestonePrompt?.title ?? String(localized: "dashboard_root_view.title.muesli_milestone", defaultValue: "Muesli milestone", bundle: .module, comment: "Title shown on milestone support/share prompt in dashboard"),
            isPresented: Binding(
            ...
            if appState.contributionMilestonePrompt?.showGitHubStar == true {
                Button(String(localized: "dashboard_root_view.button.star_on_github", defaultValue: "Star on GitHub", bundle: .module, comment: "Button title for starring project on GitHub")) {
                    controller.openContributionMilestoneAction(.githubStar)
                }
            }
            if appState.contributionMilestonePrompt?.showBuyMeCoffee == true {
                Button(String(localized: "dashboard_root_view.button.buy_me_a_coffee", defaultValue: "Buy Me a Coffee", bundle: .module, comment: "Button title for opening Buy Me a Coffee support link")) {
                    controller.openContributionMilestoneAction(.buyMeCoffee)
                }
            }
            if appState.contributionMilestonePrompt?.showTweetAboutMuesli == true {
                Button(String(localized: "dashboard_root_view.button.tweet_about_muesli", defaultValue: "Tweet about Muesli", bundle: .module, comment: "Button title for sharing Muesli on X/Twitter")) {
            ...
            if appState.contributionMilestonePrompt?.showPostOnLinkedIn == true {
                Button(String(localized: "dashboard_root_view.button.post_on_linkedin", defaultValue: "Post about Muesli on LinkedIn", bundle: .module, comment: "Button title for posting about Muesli on LinkedIn")) {
                    controller.openContributionMilestoneAction(.postOnLinkedIn)
                }
            }
            Button(String(localized: "dashboard_root_view.button.later", defaultValue: "Later", bundle: .module, comment: "Button title to postpone milestone prompt action"), role: .cancel) {
        ...
        if appState.isSearchActive,
           case .document(let id) = appState.meetingsNavigationState {
            MeetingDetailView(
                ...
                backLabel: String(localized: "dashboard_root_view.back_to_search", defaultValue: "Back to Search", bundle: .module, comment: "Navigation button label to return from detail view to search view")
            )
            .id(id)
            ...
            MeetingDetailView(
                ...
                backLabel: String(localized: "dashboard_root_view.back_to_timeline", defaultValue: "Back to Timeline", bundle: .module, comment: "Navigation button label to return from detail view to timeline view")
            )
            .id(id)
