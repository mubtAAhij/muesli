...
struct DashboardRootView: View {
    ...
    var body: some View {
        ...
        .alert(
            appState.contributionMilestonePrompt?.title ?? String(localized: "dashboard_root_view.title.muesli_milestone", defaultValue: "Muesli milestone", comment: "Title for milestone prompt in dashboard root view"),
            isPresented: Binding(
            ...
            if appState.contributionMilestonePrompt?.showGitHubStar == true {
                Button(String(localized: "dashboard_root_view.button.star_on_github", defaultValue: "Star on GitHub", comment: "Button title to open GitHub star action from milestone prompt")) {
                    controller.openContributionMilestoneAction(.githubStar)
                }
            }
            if appState.contributionMilestonePrompt?.showBuyMeCoffee == true {
                Button(String(localized: "dashboard_root_view.button.buy_me_a_coffee", defaultValue: "Buy Me a Coffee", comment: "Button title to open Buy Me a Coffee action from milestone prompt")) {
                    controller.openContributionMilestoneAction(.buyMeCoffee)
                }
            }
            if appState.contributionMilestonePrompt?.showTweetAboutMuesli == true {
                Button(String(localized: "dashboard_root_view.button.tweet_about_muesli", defaultValue: "Tweet about Muesli", comment: "Button title to share about Muesli on X/Twitter")) {
            ...
            if appState.contributionMilestonePrompt?.showPostOnLinkedIn == true {
                Button(String(localized: "dashboard_root_view.button.post_on_linkedin", defaultValue: "Post about Muesli on LinkedIn", comment: "Button title to share about Muesli on LinkedIn")) {
                    controller.openContributionMilestoneAction(.postOnLinkedIn)
                }
            }
            Button(String(localized: "dashboard_root_view.button.later", defaultValue: "Later", comment: "Button title to dismiss milestone prompt for later"), role: .cancel) {
        ...
        if appState.isSearchActive,
           case .document(let id) = appState.meetingsNavigationState {
            MeetingDetailView(
                ...
                backLabel: String(localized: "dashboard_root_view.back_to_search", defaultValue: "Back to Search", comment: "Navigation button title to return to search view")
            )
            .id(id)
            ...
            MeetingDetailView(
                ...
                backLabel: String(localized: "dashboard_root_view.back_to_timeline", defaultValue: "Back to Timeline", comment: "Navigation button title to return to timeline view")
            )
            .id(id)
