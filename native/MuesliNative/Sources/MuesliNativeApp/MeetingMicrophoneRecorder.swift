    ...
    func start() throws {
        ...
        guard let recorder, recorder.record() else {
            ...
            throw NSError(domain: "MeetingMicrophoneRecorder", code: 1, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "meeting_microphone_recorder.error.start_failed", defaultValue: "Could not start full-session meeting microphone recording", bundle: .module, comment: "Error message when full-session meeting microphone recording cannot start."),
            ])
        }
    }
