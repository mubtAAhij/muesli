    ...
    func start() async throws {
        ...
        guard let file = FileHandle(forWritingAtPath: url.path) else {
            throw NSError(domain: "SystemAudio", code: 1, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "system_audio_recorder.error.could_not_open_output_file", defaultValue: "Could not open output file", comment: ""),
            ])
        }
            ...
            try await withThrowingTaskGroup(of: Void.self) { group in
                ...
                group.addTask {
                    ...
                    throw NSError(domain: "SystemAudio", code: 3, userInfo: [
                        NSLocalizedDescriptionKey: String(localized: "system_audio_recorder.error.start_timeout", defaultValue: "Timed out while starting system audio capture", comment: ""),
                    ])
                }

                guard let _ = try await group.next() else {
                    throw NSError(domain: "SystemAudio", code: 4, userInfo: [
                        NSLocalizedDescriptionKey: String(localized: "system_audio_recorder.error.startup_ended_unexpectedly", defaultValue: "System audio startup ended unexpectedly", comment: ""),
                    ])
                }
                group.cancelAll()
            }
    ...
    private func startStream() async throws {
        ...
        guard let display = content.displays.first else {
            throw NSError(domain: "SystemAudio", code: 2, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "system_audio_recorder.error.no_display_for_scstream", defaultValue: "No display found for SCStream", comment: ""),
            ])
        }
