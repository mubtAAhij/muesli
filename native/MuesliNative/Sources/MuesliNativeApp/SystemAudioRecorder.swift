            ...
            try await withThrowingTaskGroup(of: Void.self) { group in
                ...
                guard let _ = try await group.next() else {
                    throw NSError(domain: "SystemAudio", code: 4, userInfo: [
                        NSLocalizedDescriptionKey: String(localized: "system_audio_recorder.error.startup_ended_unexpectedly", defaultValue: "System audio startup ended unexpectedly", bundle: .module, comment: "Error shown when system audio startup terminates unexpectedly."),
                    ])
                }
                group.cancelAll()
            }
