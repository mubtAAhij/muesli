...
enum AudioFileImportController {
    ...
    static func selectFile() async -> URL? {
        await withCheckedContinuation { continuation in
            DispatchQueue.main.async {
                ...
                panel.title = String(localized: "audio.import.dialog.title", defaultValue: "Import Audio File for Transcription", bundle: .module, comment: "Open panel title for importing an audio file.")
    ...
    enum ImportError: Error, Equatable, LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .conversionFailed(let detail):
                return String(format: String(localized: "audio.import.error.convert_failed_detail", defaultValue: "Could not convert the audio file. %@", bundle: .module, comment: "Error shown when audio conversion fails with detail."), "\(detail)")
            case .noAudioTracks:
            ...
            case .readError(let detail):
                return String(format: String(localized: "audio.import.error.read_failed_detail", defaultValue: "Could not read the audio file. %@", bundle: .module, comment: "Error shown when reading audio file fails with detail."), "\(detail)")
            }
        }
    }
    ...
    static func importAudioFile(
        ...
        progress(String(localized: "audio.import.progress.converting", defaultValue: "Converting audio file...", bundle: .module, comment: "Progress status shown during audio conversion."))
        ...
        progress(String(localized: "audio.import.progress.loading_model", defaultValue: "Loading transcription model...", bundle: .module, comment: "Progress status shown while loading transcription model."))
        ...
        if let vadManager = await transcriptionCoordinator.getVadManager() {
            do {
                ...
                if !hasSpeech {
                    throw ImportError.readError(String(localized: "audio.import.error.no_speech_detected", defaultValue: "No speech detected in the selected audio file.", bundle: .module, comment: "Error shown when no speech is detected in imported audio."))
                }
        ...
        progress(String(localized: "audio.import.progress.transcribing", defaultValue: "Transcribing audio...", bundle: .module, comment: "Progress status shown while transcribing audio."))
        ...
        guard !rawTranscript.isEmpty else {
            throw ImportError.readError(String(localized: "audio.import.error.no_speech_transcribed", defaultValue: "No speech was transcribed from the selected audio file.", bundle: .module, comment: "Error shown when transcription yields no speech text."))
        }
        ...
        if let diarizerManager = await transcriptionCoordinator.getDiarizerManager(),
           diarizerManager.isAvailable {
            progress(String(localized: "audio.import.progress.identifying_speakers", defaultValue: "Identifying speakers...", bundle: .module, comment: "Progress status shown while identifying speakers."))
            do {
        ...
        progress(String(localized: "audio.import.progress.generating_title", defaultValue: "Generating title...", bundle: .module, comment: "Progress status shown while generating meeting title."))
        ...
        progress(String(localized: "audio.import.progress.generating_summary", defaultValue: "Generating summary...", bundle: .module, comment: "Progress status shown while generating summary."))
        ...
        progress(String(localized: "audio.import.progress.saving", defaultValue: "Saving...", bundle: .module, comment: "Progress status shown while saving imported transcription."))
        let now = Date()
    ...
    private static func decodeSamplesWithAssetReader(sourceURL: URL) async throws -> [Float] {
        ...
        guard reader.canAdd(output) else {
            throw ImportError.conversionFailed(String(localized: "audio.import.error.could_not_read_samples", defaultValue: "Could not read audio samples from the selected file.", bundle: .module, comment: "Error shown when audio samples cannot be read from selected file."))
        }
        reader.add(output)

        guard reader.startReading() else {
            throw ImportError.readError(reader.error?.localizedDescription ?? String(localized: "audio.import.error.unknown_read_error", defaultValue: "Unknown read error", bundle: .module, comment: "Fallback error shown for unknown audio read failure."))
        }
        ...
        guard reader.status == .completed else {
            throw ImportError.readError(reader.error?.localizedDescription ?? String(localized: "audio.import.error.read_not_complete", defaultValue: "Read did not complete", bundle: .module, comment: "Error shown when audio read operation does not complete."))
        }
        return samples
    }
