...
enum AudioFileImportController {
    ...
    static func selectFile() async -> URL? {
        await withCheckedContinuation { continuation in
            DispatchQueue.main.async {
                ...
                panel.title = String(localized: "audio.import.dialog.title", defaultValue: "Import Audio File for Transcription", comment: "")
                panel.message = String(localized: "audio.import.dialog.prompt", defaultValue: "Choose an audio file (m4a, mp4, wav, mp3)", comment: "")
    ...
    enum ImportError: Error, Equatable, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .unsupportedFormat:
                return String(localized: "audio.import.error.unsupported_format", defaultValue: "This audio file format is not supported.", comment: "")
            case .conversionFailed(let detail):
                return String(format: String(localized: "audio.import.error.convert_failed_detail", defaultValue: "Could not convert the audio file. %@", comment: ""), "\(detail)")
            case .noAudioTracks:
                return String(localized: "audio.import.error.no_audio_tracks", defaultValue: "The selected file does not contain any audio tracks.", comment: "")
            case .readError(let detail):
                return String(format: String(localized: "audio.import.error.read_failed_detail", defaultValue: "Could not read the audio file. %@", comment: ""), "\(detail)")
            }
        }
    }
    ...
    static func convertToWAV(sourceURL: URL) async throws -> (wavURL: URL, duration: TimeInterval) {
        ...
        guard resolvedDuration > 0, resolvedDuration.isFinite else {
            ...
            throw ImportError.readError(String(localized: "audio.import.error.invalid_duration", defaultValue: "Invalid audio duration.", comment: ""))
        }
    ...
    static func importAudioFile(
        ...
        progress(String(localized: "audio.import.progress.converting", defaultValue: "Converting audio file...", comment: ""))
        ...
        progress(String(localized: "audio.import.progress.loading_model", defaultValue: "Loading transcription model...", comment: ""))
        ...
        if let vadManager = await transcriptionCoordinator.getVadManager() {
            do {
                ...
                if !hasSpeech {
                    throw ImportError.readError(String(localized: "audio.import.error.no_speech_detected", defaultValue: "No speech detected in the selected audio file.", comment: ""))
                }
        ...
        progress(String(localized: "audio.import.progress.transcribing", defaultValue: "Transcribing audio...", comment: ""))
        ...
        guard !rawTranscript.isEmpty else {
            throw ImportError.readError(String(localized: "audio.import.error.no_speech_transcribed", defaultValue: "No speech was transcribed from the selected audio file.", comment: ""))
        }
        ...
        if let diarizerManager = await transcriptionCoordinator.getDiarizerManager(),
           diarizerManager.isAvailable {
            progress(String(localized: "audio.import.progress.identifying_speakers", defaultValue: "Identifying speakers...", comment: ""))
            do {
        ...
        progress(String(localized: "audio.import.progress.generating_title", defaultValue: "Generating title...", comment: ""))
        ...
        progress(String(localized: "audio.import.progress.generating_summary", defaultValue: "Generating summary...", comment: ""))
        ...
        progress(String(localized: "audio.import.progress.saving", defaultValue: "Saving...", comment: ""))
        let now = Date()
    ...
    private static func decodeSamplesWithAssetReader(sourceURL: URL) async throws -> [Float] {
        ...
        guard reader.canAdd(output) else {
            throw ImportError.conversionFailed(String(localized: "audio.import.error.could_not_read_samples", defaultValue: "Could not read audio samples from the selected file.", comment: ""))
        }
        reader.add(output)

        guard reader.startReading() else {
            throw ImportError.readError(reader.error?.localizedDescription ?? String(localized: "audio.import.error.unknown_read_error", defaultValue: "Unknown read error", comment: ""))
        }
        ...
        guard reader.status == .completed else {
            throw ImportError.readError(reader.error?.localizedDescription ?? String(localized: "audio.import.error.read_not_complete", defaultValue: "Read did not complete", comment: ""))
        }
        return samples
    }
