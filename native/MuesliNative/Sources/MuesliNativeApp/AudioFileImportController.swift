...
enum AudioFileImportController {
    ...
    static func selectFile() async -> URL? {
        await withCheckedContinuation { continuation in
            DispatchQueue.main.async {
                ...
                panel.title = String(localized: "audio.import.dialog.title", defaultValue: "Import Audio File for Transcription", bundle: .module, comment: "Open panel title for importing an audio file to transcribe.")
                panel.message = String(localized: "audio.import.dialog.prompt", defaultValue: "Choose an audio file (m4a, mp4, wav, mp3)", bundle: .module, comment: "Open panel prompt describing supported audio formats.")
    ...
    enum ImportError: Error, Equatable, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .unsupportedFormat:
                return String(localized: "audio.import.error.unsupported_format", defaultValue: "This audio file format is not supported.", bundle: .module, comment: "Error shown for unsupported audio file type.")
            case .conversionFailed(let detail):
                return String(format: String(localized: "audio.import.error.convert_failed_detail", defaultValue: "Could not convert the audio file. %@", bundle: .module, comment: "Error shown when audio conversion fails with detailed reason."), "\(detail)")
            case .noAudioTracks:
            ...
            case .readError(let detail):
                return String(format: String(localized: "audio.import.error.read_failed_detail", defaultValue: "Could not read the audio file. %@", bundle: .module, comment: "Error shown when audio file read fails with detailed reason."), "\(detail)")
            }
        }
    }
    ...
    static func importAudioFile(
        ...
        progress(String(localized: "audio.import.progress.converting", defaultValue: "Converting audio file...", bundle: .module, comment: "Progress status while converting imported audio."))
        ...
        progress(String(localized: "audio.import.progress.loading_model", defaultValue: "Loading transcription model...", bundle: .module, comment: "Progress status while loading transcription model."))
        ...
        if let vadManager = await transcriptionCoordinator.getVadManager() {
            do {
                ...
                if !hasSpeech {
                    throw ImportError.readError(String(localized: "audio.import.error.no_speech_detected", defaultValue: "No speech detected in the selected audio file.", bundle: .module, comment: "Error shown when speech activity is not detected in selected audio."))
                }
        ...
        progress(String(localized: "audio.import.progress.transcribing", defaultValue: "Transcribing audio...", bundle: .module, comment: "Progress status while transcribing audio."))
        ...
        guard !rawTranscript.isEmpty else {
            throw ImportError.readError(String(localized: "audio.import.error.no_speech_transcribed", defaultValue: "No speech was transcribed from the selected audio file.", bundle: .module, comment: "Error shown when transcription returns no spoken content."))
        }
        ...
        if let diarizerManager = await transcriptionCoordinator.getDiarizerManager(),
           diarizerManager.isAvailable {
            progress(String(localized: "audio.import.progress.identifying_speakers", defaultValue: "Identifying speakers...", bundle: .module, comment: "Progress status while identifying speakers."))
            do {
        ...
        progress(String(localized: "audio.import.progress.generating_title", defaultValue: "Generating title...", bundle: .module, comment: "Progress status while generating note title."))
        ...
        progress(String(localized: "audio.import.progress.generating_summary", defaultValue: "Generating summary...", bundle: .module, comment: "Progress status while generating summary."))
        ...
        progress(String(localized: "audio.import.progress.saving", defaultValue: "Saving...", bundle: .module, comment: "Progress status while saving imported transcription result."))
        let now = Date()
    ...
    private static func compatibleWAVInfo(sourceURL: URL) throws -> CompatibleWAVInfo? {
        ...
        guard duration > 0, duration.isFinite else {
            throw ImportError.readError(String(localized: "audio.import.error.invalid_audio_duration", defaultValue: "Invalid audio duration.", bundle: .module, comment: "Repeated invalid audio duration error."))
        }
    ...
    private static func decodeSamplesWithAssetReader(sourceURL: URL) async throws -> [Float] {
        ...
        guard reader.canAdd(output) else {
            throw ImportError.conversionFailed(String(localized: "audio.import.error.could_not_read_samples", defaultValue: "Could not read audio samples from the selected file.", bundle: .module, comment: "Error shown when sample extraction from selected file fails."))
        }
        reader.add(output)

        guard reader.startReading() else {
            throw ImportError.readError(reader.error?.localizedDescription ?? String(localized: "audio.import.error.unknown_read_error", defaultValue: "Unknown read error", bundle: .module, comment: "Fallback error text for unknown audio read failures."))
        }
        ...
        guard reader.status == .completed else {
            throw ImportError.readError(reader.error?.localizedDescription ?? String(localized: "audio.import.error.read_not_complete", defaultValue: "Read did not complete", bundle: .module, comment: "Error shown when asynchronous read does not complete successfully."))
        }
        return samples
    }
