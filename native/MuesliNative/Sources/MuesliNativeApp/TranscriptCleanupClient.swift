...
enum TranscriptCleanupError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .emptyResponse(backend):
            return String(format: String(localized: "transcript_cleanup.error.empty_response", defaultValue: "%@ returned an empty transcript cleanup response.", bundle: .module, comment: "Error shown when selected cleanup backend returns no transcript content"), "\(backend)")
...
enum TranscriptCleanupClient {
    ...
    static func clean(
        ...
        guard let llmBackend = backend.llmBackend else {
            throw TranscriptCleanupError.missingConfiguration(String(localized: "transcript_cleanup.error.local_cleanup_configuration", defaultValue: "Local cleanup is handled by Qwen3PostProcessor.", bundle: .module, comment: "Message indicating local transcript cleanup path is handled by Qwen3 post-processor"))
        }
        ...
        case .lmStudio:
            guard let requestURL = MeetingSummaryClient.resolveLMStudioURL(config: cleanupConfig(config, model: model)) else {
                throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.invalid_lm_studio_url", defaultValue: "Invalid LM Studio URL: %@", bundle: .module, comment: "Error shown when LM Studio endpoint URL is invalid"), "\(config.lmStudioURL)"))
            }
            raw = try await cleanWithChatCompletions(
                backend: "LM Studio",
        ...
        case .customLLM:
            ...
            guard let requestURL = resolveConfiguredCustomLLMURL(config: config, format: format) else {
                throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.invalid_custom_url", defaultValue: "Invalid custom URL: %@", bundle: .module, comment: "Error shown when custom transcript cleanup backend URL is invalid"), "\(config.customLLMURL)"))
            }
            switch format {
            case .openAI:
                raw = try await cleanWithChatCompletions(
        ...
        default:
            throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.unsupported_backend", defaultValue: "Unsupported transcript cleanup backend: %@", bundle: .module, comment: "Error shown when configured transcript cleanup backend is unsupported"), "\(backend.label)"))
        }
    ...
    private static func cleanWithOpenAI(
        ...
        guard !apiKey.isEmpty else {
            throw TranscriptCleanupError.missingConfiguration(String(localized: "transcript_cleanup.error.openai_key_missing", defaultValue: "OpenAI API key is not configured.", bundle: .module, comment: "Error shown when OpenAI key is required but missing for cleanup backend"))
        }
        var body: [String: Any] = [
            "model": model,
    ...
    private static func cleanWithOllama(
        ...
        guard let baseURL else {
            throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.invalid_ollama_url", defaultValue: "Invalid Ollama URL: %@", bundle: .module, comment: "Error shown when Ollama endpoint URL is invalid"), "\(config.ollamaURL)"))
        }
    ...
    private static func validateHTTPResponse(_ response: URLResponse, data: Data, backend: String) throws {
        ...
        guard (200..<300).contains(http.statusCode) else {
            let message = extractErrorMessage(from: data)
                ...
                ?? String(format: String(localized: "transcript_cleanup.error.http_status", defaultValue: "HTTP %d", bundle: .module, comment: "HTTP status prefix used in transcript cleanup backend error handling"), http.statusCode)
            throw TranscriptCleanupError.backendFailed(String(format: String(localized: "transcript_cleanup.error.backend_failed", defaultValue: "%@ cleanup failed. %@", bundle: .module, comment: "Error shown when transcript cleanup backend request fails with message"), "\(backend)", "\(message)"))
        }
    }
