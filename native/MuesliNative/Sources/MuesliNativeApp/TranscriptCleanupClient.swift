...
enum TranscriptCleanupError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .missingConfiguration(message):
            return message
        case .rejectedOutput:
            return String(localized: "transcript_cleanup.error.safety_rejected", defaultValue: "Transcript cleanup output was rejected by safety checks.", comment: "Error when transcript cleanup result fails safety validation")
        case let .emptyResponse(backend):
            return String(format: String(localized: "transcript_cleanup.error.empty_response", defaultValue: "%@ returned an empty transcript cleanup response.", comment: "Error when transcript cleanup backend returns an empty response"), "\(backend)")
...
enum TranscriptCleanupClient {
    ...
    static func clean(
        ...
        guard let llmBackend = backend.llmBackend else {
            throw TranscriptCleanupError.missingConfiguration(String(localized: "transcript_cleanup.error.local_cleanup_configuration", defaultValue: "Local cleanup is handled by Qwen3PostProcessor.", comment: "Error message indicating local cleanup backend is configured incorrectly"))
        }
        ...
        case .lmStudio:
            guard let requestURL = MeetingSummaryClient.resolveLMStudioURL(config: cleanupConfig(config, model: model)) else {
                throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.invalid_lm_studio_url", defaultValue: "Invalid LM Studio URL: %@", comment: "Error when configured LM Studio URL is invalid"), "\(config.lmStudioURL)"))
            }
            raw = try await cleanWithChatCompletions(
                backend: "LM Studio",
        ...
        case .customLLM:
            ...
            guard let requestURL = resolveConfiguredCustomLLMURL(config: config, format: format) else {
                throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.invalid_custom_url", defaultValue: "Invalid custom URL: %@", comment: "Error when configured custom cleanup URL is invalid"), "\(config.customLLMURL)"))
            }
            switch format {
            case .openAI:
                raw = try await cleanWithChatCompletions(
        ...
        default:
            throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.unsupported_backend", defaultValue: "Unsupported transcript cleanup backend: %@", comment: "Error when selected transcript cleanup backend is not supported"), "\(backend.label)"))
        }
    ...
    private static func cleanWithOpenAI(
        ...
        guard !apiKey.isEmpty else {
            throw TranscriptCleanupError.missingConfiguration(String(localized: "transcript_cleanup.error.openai_key_missing", defaultValue: "OpenAI API key is not configured.", comment: "Error when OpenAI transcript cleanup backend is selected without API key"))
        }
        var body: [String: Any] = [
            "model": model,
    ...
    private static func cleanWithOllama(
        ...
        guard let baseURL else {
            throw TranscriptCleanupError.missingConfiguration(String(format: String(localized: "transcript_cleanup.error.invalid_ollama_url", defaultValue: "Invalid Ollama URL: %@", comment: "Error when configured Ollama URL is invalid"), "\(config.ollamaURL)"))
        }
    ...
    private static func validateHTTPResponse(_ response: URLResponse, data: Data, backend: String) throws {
        ...
        guard (200..<300).contains(http.statusCode) else {
            let message = extractErrorMessage(from: data)
                ...
                ?? String(format: String(localized: "transcript_cleanup.error.http_status", defaultValue: "HTTP %d", comment: "HTTP status prefix included in transcript cleanup backend error reporting"), http.statusCode)
            throw TranscriptCleanupError.backendFailed(String(format: String(localized: "transcript_cleanup.error.backend_failed", defaultValue: "%@ cleanup failed. %@", comment: "Error when transcript cleanup backend request fails with message"), "\(backend)", "\(message)"))
        }
    }
