...
struct ContributionMilestonePrompt: Equatable, Identifiable {
    ...
    var title: String {
        switch kind {
        case .dictationWords:
            return String(format: String(localized: "contribution.milestone.prompt.words.title", defaultValue: "You crossed %@ words!", bundle: .module, comment: "Milestone prompt title for reached dictated word count."), "\(ContributionSocialShare.formatCount(count))")
        case .meetings:
            return String(format: String(localized: "contribution.milestone.prompt.meetings.title", defaultValue: "You captured %@ meetings!", bundle: .module, comment: "Milestone prompt title for reached captured meeting count."), "\(ContributionSocialShare.formatCount(count))")
        }
    }
...
enum ContributionSocialShare {
    ...
    static func message(wordCount: Int) -> String {
        String(format: String(localized: "contribution.social_share.words.message", defaultValue: "I've dictated %@ words with Muesli. It's fast, open source, on-device, and free to use. Try it: %@", bundle: .module, comment: "Social share message for total dictated words milestone."), "\(formatCount(wordCount))", "\(muesliURL.absoluteString)")
    }
