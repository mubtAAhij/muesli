...
struct ContributionMilestonePrompt: Equatable, Identifiable {
    ...
    var title: String {
        ...
        case .meetings:
            return String(format: String(localized: "contribution.milestone.prompt.meetings.title", defaultValue: "You captured %@ meetings!", bundle: .module, comment: "Milestone prompt title when user reaches a meetings-count milestone"), "\(ContributionSocialShare.formatCount(count))")
        }
    }
...
enum ContributionSocialShare {
    ...
    static func message(wordCount: Int) -> String {
        String(format: String(localized: "contribution.social_share.words.message", defaultValue: "I've dictated %@ words with Muesli. It's fast, open source, on-device, and free to use. Try it: %@", bundle: .module, comment: "Share text for posting word-count milestone with app link"), "\(formatCount(wordCount))", "\(muesliURL.absoluteString)")
    }
