...
struct ContributionMilestonePrompt: Equatable, Identifiable {
    ...
    var title: String {
        switch kind {
        case .dictationWords:
            return String(format: String(localized: "contribution.milestone.prompt.words.title", defaultValue: "You crossed %@ words!", comment: "Milestone prompt title shown when user crosses a word-count milestone"), "\(ContributionSocialShare.formatCount(count))")
        case .meetings:
            return String(format: String(localized: "contribution.milestone.prompt.meetings.title", defaultValue: "You captured %@ meetings!", comment: "Milestone prompt title shown when user crosses a meetings-count milestone"), "\(ContributionSocialShare.formatCount(count))")
        }
    }
...
enum ContributionSocialShare {
    ...
    static func message(wordCount: Int) -> String {
        String(format: String(localized: "contribution.social_share.words.message", defaultValue: "I've dictated %@ words with Muesli. It's fast, open source, on-device, and free to use. Try it: %@", comment: "Share text for word-count milestone post"), "\(formatCount(wordCount))", "\(muesliURL.absoluteString)")
    }
