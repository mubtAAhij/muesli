...
public enum HuggingFaceModelManifestError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidRepository(let repository):
            return String(format: String(localized: "huggingface_manifest.error.invalid_repository", defaultValue: "Invalid Hugging Face repository: %@", comment: "Error when the provided Hugging Face repository identifier is invalid."), "\(repository)")
        case .invalidResponse(let path):
            return String(format: String(localized: "huggingface_manifest.error.invalid_model_metadata", defaultValue: "Hugging Face returned invalid model metadata for %@", comment: "Error when Hugging Face returns malformed metadata for a model path."), "\(path)")
        case .invalidHTTPStatus(let status, let path):
            return String(format: String(localized: "huggingface_manifest.error.http_status_listing_path", defaultValue: "Hugging Face returned HTTP %d while listing %@", comment: "Error when Hugging Face listing request returns a non-success HTTP status."), status, "\(path)")
        case .emptySelection(let path):
            return String(format: String(localized: "huggingface_manifest.error.no_downloadable_files", defaultValue: "No downloadable files were found for %@", comment: "Error when no downloadable files are available for the requested Hugging Face path."), "\(path)")
        case .conflictingDestination(let path):
            return String(format: String(localized: "huggingface_manifest.error.multiple_files_map_to_path", defaultValue: "More than one Hugging Face file maps to %@", comment: "Error when multiple Hugging Face files match the same target path."), "\(path)")
        }
    }
}
