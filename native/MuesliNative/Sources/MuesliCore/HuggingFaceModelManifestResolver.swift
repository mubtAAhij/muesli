...
public enum HuggingFaceModelManifestError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidRepository(let repository):
            return String(format: String(localized: "huggingface_manifest.error.invalid_repository", defaultValue: "Invalid Hugging Face repository: %@", comment: "Error shown when configured Hugging Face repository identifier is invalid."), "\(repository)")
        case .invalidResponse(let path):
            return String(format: String(localized: "huggingface_manifest.error.invalid_model_metadata", defaultValue: "Hugging Face returned invalid model metadata for %@", comment: "Error shown when Hugging Face metadata response is invalid for a model path."), "\(path)")
        case .invalidHTTPStatus(let status, let path):
            return String(format: String(localized: "huggingface_manifest.error.http_status_listing_path", defaultValue: "Hugging Face returned HTTP %d while listing %@", comment: "Error shown when Hugging Face returns an HTTP status while listing files at a model path."), status, "\(path)")
        case .emptySelection(let path):
            return String(format: String(localized: "huggingface_manifest.error.no_downloadable_files", defaultValue: "No downloadable files were found for %@", comment: "Error shown when no downloadable files are found for the requested model path."), "\(path)")
        case .conflictingDestination(let path):
            return String(format: String(localized: "huggingface_manifest.error.multiple_files_map_to_path", defaultValue: "More than one Hugging Face file maps to %@", comment: "Error shown when multiple Hugging Face files resolve to the same target path."), "\(path)")
        }
    }
}
