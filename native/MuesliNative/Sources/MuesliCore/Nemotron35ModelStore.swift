...
public enum Nemotron35ModelStoreError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .invalidURL(let value):
            return String(format: String(localized: "nemotron35.error.invalid_model_url", defaultValue: "Invalid Nemotron model URL: %@", comment: "Error shown when Nemotron model URL value is invalid"), "\(value)")
        ...
        case .httpError(let code, let path):
            return String(format: String(localized: "nemotron35.error.http_downloading_model_file", defaultValue: "HTTP %d downloading Nemotron model file %@", comment: "Error shown when downloading Nemotron model file fails with HTTP status"), code, "\(path)")
        case .retriesExhausted(let path, let underlying):
            return String(format: String(localized: "nemotron35.error.download_failed_after_retries", defaultValue: "Failed to download Nemotron model file %@ after retries: %@", comment: "Error shown when Nemotron model file download fails after retry attempts"), "\(path)", "\(underlying.localizedDescription)")
        }
    }
}
...
public enum Nemotron35ModelStore {
    ...
    public static func ensureDownloaded(
        ...
        progress?(1.0, String(localized: "nemotron35.download.status.model_ready", defaultValue: "Nemotron 3.5 model ready", comment: "Status shown when Nemotron 3.5 model download and setup is complete"))
        return directory
    }
