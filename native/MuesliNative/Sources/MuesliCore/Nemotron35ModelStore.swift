...
public enum Nemotron35ModelStoreError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .invalidURL(let value):
            return String(format: String(localized: "nemotron35.error.invalid_model_url", defaultValue: "Invalid Nemotron model URL: %@", comment: "Error when Nemotron model URL is malformed or invalid"), "\(value)")
        case .invalidResponse(let value):
            return String(format: String(localized: "nemotron35.error.invalid_model_response", defaultValue: "Invalid Nemotron model response: %@", comment: "Error when Nemotron model metadata response is invalid"), "\(value)")
        case .httpError(let code, let path):
            return String(format: String(localized: "nemotron35.error.http_downloading_model_file", defaultValue: "HTTP %d downloading Nemotron model file %@", comment: "Error when HTTP status indicates failure while downloading model file"), code, "\(path)")
        case .retriesExhausted(let path, let underlying):
            return String(format: String(localized: "nemotron35.error.download_failed_after_retries", defaultValue: "Failed to download Nemotron model file %@ after retries: %@", comment: "Error when model download fails after retry attempts"), "\(path)", "\(underlying.localizedDescription)")
        }
    }
}
...
public enum Nemotron35ModelStore {
    ...
    public static func ensureDownloaded(
        ...
        progress?(0.0, String(localized: "nemotron35.download.progress.downloading_model", defaultValue: "Downloading Nemotron 3.5 model...", comment: "Progress status while downloading Nemotron 3.5 model files"))
        ...
        try await ModelDownloadCoordinator.shared.download(manifest, to: directory) { snapshot in
            progress?(snapshot.fractionCompleted ?? 0, String(localized: "nemotron35.download.progress.downloading_model", defaultValue: "Downloading Nemotron 3.5 model...", comment: "Progress status while downloading Nemotron 3.5 model files"))
        ...
        progress?(1.0, String(localized: "nemotron35.download.status.model_ready", defaultValue: "Nemotron 3.5 model ready", comment: "Status shown when Nemotron 3.5 model is fully downloaded and ready"))
        return directory
    }
