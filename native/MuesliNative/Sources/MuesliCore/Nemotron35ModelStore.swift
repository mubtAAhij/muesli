...
public enum Nemotron35ModelStoreError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .invalidURL(let value):
            return String(format: String(localized: "nemotron35.error.invalid_model_url", defaultValue: "Invalid Nemotron model URL: %@", comment: "Error when Nemotron model URL is invalid"), "\(value)")
        case .invalidResponse(let value):
            return String(format: String(localized: "nemotron35.error.invalid_model_response", defaultValue: "Invalid Nemotron model response: %@", comment: "Error when Nemotron model response payload is invalid"), "\(value)")
        case .httpError(let code, let path):
            return String(format: String(localized: "nemotron35.error.http_downloading_model_file", defaultValue: "HTTP %d downloading Nemotron model file %@", comment: "Error when HTTP status indicates failure downloading Nemotron model file"), code, "\(path)")
