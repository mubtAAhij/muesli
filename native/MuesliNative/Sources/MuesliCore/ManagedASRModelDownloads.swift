...
public enum ManagedASRModelDownloader {
    ...
    private static func performDownload(
        ...
        scalarProgress.call(0.01, String(localized: "asr.download.finding_model_files", defaultValue: "Finding model files...", comment: "Status shown while scanning for ASR model files"))
        progressSnapshot?(ModelDownloadProgress.preparing(
            modelID: plan.modelID,
            message: String(localized: "asr.download.finding_model_files", defaultValue: "Finding model files...", comment: "Status shown while scanning for ASR model files")
        ))
