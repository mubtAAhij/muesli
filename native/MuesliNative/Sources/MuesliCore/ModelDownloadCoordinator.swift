...
public enum ModelDownloadError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .emptyManifest(let modelID):
            return String(format: String(localized: "model_download.error.no_files_specified", defaultValue: "No files were specified for model download %@", comment: "Error when model download request has no files."), "\(modelID)")
        case .invalidRelativePath(let path):
            return String(format: String(localized: "model_download.error.invalid_relative_path", defaultValue: "The model contains an invalid relative path: %@", comment: "Error when model package contains invalid relative path."), "\(path)")
        case .invalidHTTPStatus(let status, let path):
            return "HTTP " + String(status) + " while downloading " + path
        case .stalled(let path):
            return String(localized: "model_download.error.download_stalled_receiving", defaultValue: "Download stalled while receiving", comment: "Prefix for stalled download error while receiving a file.") + " " + path
        case .invalidContentLength(let path, let expected, let actual):
            return String(localized: "model_download.error.invalid_reported_size_prefix", defaultValue: "The server reported an invalid size for", comment: "Prefix for server invalid file size error.") + " " + path + " (expected " + String(expected) + " bytes, reported " + String(actual) + ")"
        case .sizeMismatch(let path, let expected, let actual):
            return String(localized: "model_download.status.downloaded_prefix_primary", defaultValue: "Downloaded", comment: "Prefix for downloaded bytes progress text.") + " " + path + " is incomplete (expected " + String(expected) + " bytes, received " + String(actual) + ")"
        case .checksumMismatch(let path):
            return String(localized: "model_download.status.downloaded_prefix_secondary", defaultValue: "Downloaded", comment: "Prefix for secondary downloaded progress text.") + " " + path + " failed integrity validation"
        ...
        case .diskSpaceUnavailable(let path):
            return String(localized: "model_download.error.could_not_determine_available_disk_space_for", defaultValue: "Could not determine available disk space for", comment: "Prefix for disk space check failure error.") + " " + path
        case .retriesExhausted(let path, let detail):
            return String(localized: "model_download.error.could_not_download", defaultValue: "Could not download", comment: "Prefix for generic file download failure error.") + " " + path + " after three attempts: " + detail
        case .conflictingInFlightDownload(let modelID):
            return String(localized: "model_download.error.another_download_for_model", defaultValue: "Another download for model", comment: "Prefix for duplicate model download attempt error.") + " " + modelID + " is already active in this location"
        }
    }
}
...
public actor ModelDownloadCoordinator {
    ...
    private func performDownload(
        ...
        emit(manifest, jobKey: jobKey, phase: .downloading, message: String(localized: "model_download.status.starting_download", defaultValue: "Starting download", comment: "Status text shown when model download starts."))
        ...
        try? fm.removeItem(at: stateURL(for: directory))
        emit(manifest, jobKey: jobKey, phase: .ready, message: String(localized: "model_download.status.model_ready", defaultValue: "Model ready", comment: "Status text shown when model download is complete."))
    }
    ...
    private func downloadFile(
        ...
        throw ModelDownloadError.retriesExhausted(file.relativePath, lastError?.localizedDescription ?? String(localized: "model_download.error.unknown_error", defaultValue: "unknown error", comment: "Fallback error text when no specific error is available."))
    }
