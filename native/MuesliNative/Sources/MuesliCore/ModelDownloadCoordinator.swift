...
public enum ModelDownloadError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .emptyManifest(let modelID):
            return String(format: String(localized: "model_download.error.no_files_specified", defaultValue: "No files were specified for model download %@", comment: "Error when no files are specified for a model download request"), "\(modelID)")
        case .invalidRelativePath(let path):
            return String(format: String(localized: "model_download.error.invalid_relative_path", defaultValue: "The model contains an invalid relative path: %@", comment: "Error when model manifest contains an invalid relative path"), "\(path)")
        ...
        case .invalidContentLength(let path, let expected, let actual):
            return String(localized: "model_download.error.invalid_reported_size_prefix", defaultValue: "The server reported an invalid size for", comment: "Prefix for error when server reports invalid file size") + " " + path + " (expected " + String(expected) + " bytes, reported " + String(actual) + ")"
        ...
        case .diskSpaceUnavailable(let path):
            return String(localized: "model_download.error.could_not_determine_available_disk_space_for", defaultValue: "Could not determine available disk space for", comment: "Prefix for error when available disk space cannot be determined") + " " + path
        case .retriesExhausted(let path, let detail):
            return String(localized: "model_download.error.could_not_download", defaultValue: "Could not download", comment: "Prefix for error when a file could not be downloaded") + " " + path + " after three attempts: " + detail
        case .conflictingInFlightDownload(let modelID):
            return String(localized: "model_download.error.another_download_for_model", defaultValue: "Another download for model", comment: "Prefix for error when another download is already active for a model") + " " + modelID + " is already active in this location"
        }
    }
}
...
public actor ModelDownloadCoordinator {
    ...
    private func performDownload(
        ...
        emit(manifest, jobKey: jobKey, phase: .downloading, message: String(localized: "model_download.status.starting_download", defaultValue: "Starting download", comment: "Status text shown when model download is starting"))
        ...
        try? fm.removeItem(at: stateURL(for: directory))
        emit(manifest, jobKey: jobKey, phase: .ready, message: String(localized: "model_download.status.model_ready", defaultValue: "Model ready", comment: "Status text shown when model download and preparation are complete"))
    }
    ...
    private func downloadFile(
        ...
        throw ModelDownloadError.retriesExhausted(file.relativePath, lastError?.localizedDescription ?? String(localized: "model_download.error.unknown_error", defaultValue: "unknown error", comment: "Fallback error text for unknown model download failures"))
    }
