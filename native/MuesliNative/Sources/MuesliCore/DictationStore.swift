...
public enum DictationStoreError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .dictationNotFound(let id):
            return String(format: String(localized: "errors.dictation.no_longer_exists", defaultValue: "Dictation %@ no longer exists.", comment: "Error shown when a requested dictation record cannot be found."), "\(id)")
        case .meetingNotFound(let id):
            return String(format: String(localized: "errors.meeting.no_longer_exists", defaultValue: "Meeting %@ no longer exists.", comment: "Error shown when a requested meeting record cannot be found."), "\(id)")
        case .invalidParticipantIdentifier:
            return String(localized: "errors.meeting.participant_not_identified", defaultValue: "That meeting participant could not be identified.", comment: "Error shown when meeting participant identity resolution fails.")
        }
    }
}
    ...
    public func recoverLiveMeetingFromTranscriptCheckpoints(id: Int64) throws -> Bool {
        ...
        let formattedNotes = String(
            format: String(
                ...
                comment: "Fallback notes content when recovering a non-finalized meeting from transcript checkpoints."
            ),
            transcript
        )
    ...
    private func resumedRecoveryNotes(priorNotes: String, combinedTranscript: String) -> String {
        let recoveryNotes = String(
            format: String(
                ...
                comment: "Fallback notes content when recovering a resumed meeting from transcript checkpoints."
            ),
            combinedTranscript
        )
