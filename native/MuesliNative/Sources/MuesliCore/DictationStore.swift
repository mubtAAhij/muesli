...
public enum DictationStoreError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .dictationNotFound(let id):
            return String(format: String(localized: "errors.dictation.no_longer_exists", defaultValue: "Dictation %@ no longer exists.", comment: "Error shown when referenced dictation record no longer exists"), "\(id)")
        case .meetingNotFound(let id):
            return String(format: String(localized: "errors.meeting.no_longer_exists", defaultValue: "Meeting %@ no longer exists.", comment: "Error shown when referenced meeting record no longer exists"), "\(id)")
        case .invalidParticipantIdentifier:
            return String(localized: "errors.meeting.participant_not_identified", defaultValue: "That meeting participant could not be identified.", comment: "Error shown when a meeting participant identity cannot be resolved")
        }
    }
}
...
public final class DictationStore {
    ...
    public func recoverLiveMeetingFromTranscriptCheckpoints(id: Int64) throws -> Bool {
        ...
        let manualNotes = try manualNotesForMeeting(id: id, db: db)
        let formattedNotes = String(format: String(localized: "meetings.fallback.raw_transcript_notes", defaultValue: "## Raw Transcript\n\n        Recovered from live transcript checkpoints after the meeting did not finalize normally. This fallback may be incomplete and may not include final diarization or reconciliation.\n\n        %@", comment: "Fallback notes body when recovering transcript after an unfinished meeting"), transcript)
    ...
    private func combinedResumeRecoveryTranscript(prior: String, new: String) -> String {
        ...
        return prior + "\n\n" + String(localized: "meetings.transcript.resumed_separator", defaultValue: "— Resumed —", comment: "Separator inserted into meeting transcript when session is resumed") + "\n\n" + new
    }

    private func resumedRecoveryNotes(priorNotes: String, combinedTranscript: String) -> String {
        let recoveryNotes = String(format: String(localized: "meetings.recovery.notes.raw_transcript_resumed", defaultValue: "## Raw Transcript\n\n        Recovered from live transcript checkpoints after a resumed meeting did not finalize normally. This fallback may be incomplete and may not include final diarization or reconciliation.\n\n        %@", comment: "Fallback notes body when recovering transcript for an unfinished resumed meeting"), combinedTranscript)
