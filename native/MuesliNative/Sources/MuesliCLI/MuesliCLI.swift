...
func ensureDatabaseAvailable(_ context: CLIContext, command: String) throws {
    guard context.store.databaseExists else {
        throw CLIError.databaseUnavailable(
            String(localized: "cli.error.database_missing_at_resolved_path", defaultValue: "No Muesli database exists at the resolved path.", comment: "CLI error message when no database is found at resolved path."),
...
struct MuesliCLI: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: String(localized: "cli.configuration.abstract", defaultValue: "Agent-friendly CLI for local Muesli meetings and dictations.", comment: "Root command abstract for Muesli CLI."),
    ...
    static func specPayload() -> CommandSpecPayload {
        CommandSpecPayload(commands: [
            .init(name: "spec", usage: "muesli-cli spec", summary: String(localized: "cli.spec.description", defaultValue: "Dump the command tree and CLI schema metadata.", comment: "Description for spec subcommand."), examples: ["muesli-cli spec"]),
            .init(name: "info", usage: "muesli-cli info [--db-path <path>] [--support-dir <dir>]", summary: String(localized: "cli.info.description", defaultValue: "Show resolved support and database paths.", comment: "Description for info subcommand."), examples: ["muesli-cli info", "muesli-cli info --support-dir ~/Library/Application\\ Support/Muesli"]),
            .init(name: "transcribe", usage: "muesli-cli transcribe <file> [--format text|json|markdown] [--model parakeet-v3|parakeet-v2|parakeet-eou-320ms|sensevoice|qwen3-asr|nemotron35|whisper-tiny|whisper-tiny-english|whisper-small|whisper-small-english|whisper-medium-english|whisper-large-turbo] [--summarize] [--save-meeting] [--title <title>] [--output <path>] [--dictionary <path>]", summary: String(localized: "cli.transcribe.description", defaultValue: "Transcribe a local mp3, mp4, m4a, or wav file with Muesli's bundled local ASR models.", comment: "Description for transcribe subcommand."), examples: ["muesli-cli transcribe call.mp3", "muesli-cli transcribe call.m4a --format json", "muesli-cli transcribe call.wav --model nemotron35 --dictionary dictionary.json"]),
            .init(name: "meetings list", usage: "muesli-cli meetings list [--limit <n>] [--folder-id <id>]", summary: String(localized: "cli.meetings.list.description", defaultValue: "List recent meetings.", comment: "Description for meetings list subcommand."), examples: ["muesli-cli meetings list --limit 5", "muesli-cli meetings list --folder-id 2"]),
            .init(name: "meetings get", usage: "muesli-cli meetings get <id>", summary: String(localized: "cli.meetings.get.description", defaultValue: "Return a full meeting record.", comment: "Description for meetings get subcommand."), examples: ["muesli-cli meetings get 42"]),
            .init(name: "meetings update-notes", usage: "muesli-cli meetings update-notes <id> (--stdin | --file <path>)", summary: String(localized: "cli.meetings.update_notes.description", defaultValue: "Replace stored meeting notes only.", comment: "Description for meetings update-notes subcommand."), examples: ["muesli-cli meetings update-notes 42 --file notes.md", "cat notes.md | muesli-cli meetings update-notes 42 --stdin"]),
            .init(name: "dictations list", usage: "muesli-cli dictations list [--limit <n>]", summary: String(localized: "cli.dictations.list.description", defaultValue: "List recent dictations.", comment: "Description for dictations list subcommand."), examples: ["muesli-cli dictations list --limit 10"]),
            .init(name: "dictations get", usage: "muesli-cli dictations get <id>", summary: String(localized: "cli.dictations.get.description", defaultValue: "Return a full dictation record.", comment: "Description for dictations get subcommand."), examples: ["muesli-cli dictations get 7"]),
        ])
    }
}
...
struct MeetingsListCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        if !context.store.databaseExists {
            emitSuccess(command: "muesli-cli meetings list", data: [MeetingListRow](), dbPath: context.databaseURL, warnings: [String(localized: "cli.info.database_missing_at_resolved_path", defaultValue: "No Muesli database exists at the resolved path.", comment: "Info command error when database is missing at resolved path.")])
            return
        }
...
struct MeetingsGetCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        guard let meeting = try context.store.meeting(id: id) else {
            throw CLIError.notFound(String(format: String(localized: "cli.meetings.get.error.not_found_by_id", defaultValue: "No meeting exists with id %@.", comment: "Error message when a meeting id is not found."), "\(id)"), fix: "Run `muesli-cli meetings list` to find a valid ID.")
        }
...
struct MeetingsUpdateNotesCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        guard try context.store.meeting(id: id) != nil else {
            throw CLIError.notFound(String(format: String(localized: "cli.meetings.update_notes.error.not_found_by_id", defaultValue: "No meeting exists with id %@.", comment: "Error when update-notes target meeting id is not found."), "\(id)"), fix: "Run `muesli-cli meetings list` to find a valid ID.")
        }
        ...
        if stdin {
            ...
            throw CLIError.invalidInput(String(localized: "cli.meetings.update_notes.error.no_notes_source", defaultValue: "No notes source was provided.", comment: "Error when no notes input source is provided to update-notes command."), fix: "Use --stdin or --file <path>.")
        }

        let trimmed = notes.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else {
            throw CLIError.invalidInput(String(localized: "cli.meetings.update_notes.error.empty_notes", defaultValue: "Meeting notes cannot be empty.", comment: "Error when provided meeting notes content is empty."), fix: String(localized: "cli.meetings.update_notes.error.empty_notes_guidance", defaultValue: "Provide a non-empty markdown or plain text notes body.", comment: "Guidance shown when notes content is empty."))
        }
        ...
        guard let updated = try context.store.meeting(id: id) else {
            throw CLIError.databaseError(String(localized: "cli.error.meeting_updated_but_not_reloaded", defaultValue: "The meeting was updated but could not be reloaded.", comment: "Error when update succeeds but refreshed meeting record cannot be loaded."))
        }
...
struct DictationsListCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        if !context.store.databaseExists {
            emitSuccess(command: "muesli-cli dictations list", data: [DictationListRow](), dbPath: context.databaseURL, warnings: [String(localized: "cli.dictations.list.database_missing_at_resolved_path", defaultValue: "No Muesli database exists at the resolved path.", comment: "Error when dictations list command cannot find database at resolved path.")])
            return
        }
...
struct DictationsGetCommand: ParsableCommand {
    ...
    @OptionGroup var global: GlobalOptions
    @Argument(help: ArgumentHelp(String(localized: "cli.dictations.get.argument.id", defaultValue: "Dictation ID", comment: "Argument help text for dictations get id parameter."))) var id: Int64
    ...
    func run() throws {
        ...
        guard let dictation = try context.store.dictation(id: id) else {
            throw CLIError.notFound(String(format: String(localized: "cli.dictations.get.error.not_found", defaultValue: "No dictation exists with id %@.", comment: "Error when requested dictation id is not found."), "\(id)"), fix: "Run `muesli-cli dictations list` to find a valid ID.")
        }
