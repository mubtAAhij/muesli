...
struct MuesliAudioFilePreparer: AudioPreparing {
    ...
    func prepareAudio(sourceURL: URL) async throws -> PreparedAudioFile {
        ...
        guard resolvedDuration > 0, resolvedDuration.isFinite else {
            try? FileManager.default.removeItem(at: decoded.wavURL)
            throw PreparationError.readError(String(localized: "transcribe_command.error.invalid_audio_duration", defaultValue: "Invalid audio duration.", comment: ""))
        }
    ...
    private func load(model: TranscribeModel, progress: @escaping (String) -> Void) async throws {
        ...
        let manager = try await ManagedASRModelDownloader.loadValidated(
            ...
            progress: { fraction, message in
                progress(message ?? String(format: String(localized: "transcribe_command.status.model_download_progress", defaultValue: "model %d%%", comment: ""), Int((fraction * 100).rounded())))
            }
        ) { modelDirectory in
            progress(String(localized: "transcribe_command.status.preparing_model", defaultValue: "preparing model", comment: ""))
        ...
        asrManager = manager
        loadedModel = model
        progress(String(localized: "transcribe_command.status.model_ready", defaultValue: "model ready", comment: ""))
    }
}
...
actor SenseVoiceCLITranscriber: AudioTranscribing {
    ...
    func transcribe(wavURL: URL, model: TranscribeModel, progress: @escaping (String) -> Void) async throws -> HeadlessTranscription {
        ...
        guard let manager else {
            throw CLIError.invalidInput("SenseVoice model was not loaded.", fix: String(localized: "transcribe_command.status.run_again_after_model_download", defaultValue: "Run the command again after the model finishes downloading.", comment: ""))
        }
        let start = CFAbsoluteTimeGetCurrent()
        let text = try await manager.transcribe(audioURL: wavURL)
        progress(String(format: String(localized: "transcribe_command.status.transcription_complete_duration", defaultValue: "transcription complete in %.2fs", comment: ""), String(format: "%.2f", CFAbsoluteTimeGetCurrent() - start)))
...
actor Qwen3AsrCLITranscriber: AudioTranscribing {
    ...
    private func transcribeOnSupportedOS(wavURL: URL, progress: @escaping (String) -> Void) async throws -> HeadlessTranscription {
        ...
        guard let manager else {
            throw CLIError.invalidInput("Qwen3 ASR model was not loaded.", fix: String(localized: "transcribe_command.status.run_again_after_model_download", defaultValue: "Run the command again after the model finishes downloading.", comment: ""))
        }
        let start = CFAbsoluteTimeGetCurrent()
    ...
    private func loadedManager(progress: @escaping (String) -> Void) async throws -> StreamingNemotronMultilingualAsrManager {
        ...
        let modelDirectory = try await Nemotron35ModelStore.ensureDownloaded { fraction, message in
            if let message {
                ...
                progress(String(format: String(localized: "transcribe_command.status.model_download_progress", defaultValue: "model %d%%", comment: ""), Int((fraction * 100).rounded())))
            }
        }
